...
pub(crate) enum ModelCallPublicItemReservationError {
...
pub(crate) struct ModelCallPublicItemReservationRequest {
    kind: ModelCallPublicItemReservationKind,
    response: oneshot::Sender<Result<ResponseItemAuthority, ModelCallPublicItemReservationError>>,
}

impl ModelCallPublicItemReservationRequest {
    pub(crate) fn kind(&self) -> &ModelCallPublicItemReservationKind {
        &self.kind
    }
...
pub(crate) enum ModelCallPublicItemReservationKind {
    Message,
    FunctionCall {
        call_index: u32,
        call_id: String,
...
#[derive(Clone)]
pub(crate) struct ModelCallPublicItemAllocator {
...
impl ModelCallPublicItemAllocator {
    pub(crate) async fn reserve(
        ...
        self.reserve_kind(ModelCallPublicItemReservationKind::Message)
            .await
    }

    pub(crate) async fn reserve_function_call(
        &self,
        call_index: u32,
        call_id: impl Into<String>,
        tool_name: impl Into<String>,
    ) -> Result<ResponseItemAuthority, ModelCallPublicItemReservationError> {
        self.reserve_kind(ModelCallPublicItemReservationKind::FunctionCall {
            call_index,
...
pub(crate) fn model_call_public_item_reservation_channel() -> (
...
pub struct WorkerRuntimeServices {
    model_call_public_item_allocator: Option<ModelCallPublicItemAllocator>,
}

impl WorkerRuntimeServices {
    pub(crate) fn with_model_call_public_item_allocator(
        mut self,
        allocator: ModelCallPublicItemAllocator,
    ) -> Self {
        self.model_call_public_item_allocator = Some(allocator);
        self
    }

    pub(crate) fn model_call_public_item_allocator(&self) -> Option<&ModelCallPublicItemAllocator> {
        self.model_call_public_item_allocator.as_ref()
    }
}
...
impl TaskExecutionRequest {
    ...
    pub fn from_model_tool_claim(claim: &ModelToolTaskClaim) -> Result<Self, &'static str> {
        let identity = claim.identity();
        let action = identity.action();
        validate_model_tool_action_binding(action)?;
        if !claim.arguments().is_object()
            || serde_jcs::to_vec(claim.arguments()).is_err()
            || !crate::schema::compile_schema_2020(action.input_schema())
                .map_err(|_| WORKER_MODEL_TOOL_CLAIM_INVALID)?
                .is_valid(claim.arguments())
        {
        ...
        // configuration.
        PlanType::literal(claim.arguments().clone())
            .map_err(|_| WORKER_MODEL_TOOL_CLAIM_INVALID)?;
        let arguments = descriptor_value_from_model_tool_json(claim.arguments())?;
        let node_id = model_tool_node_id(identity.tool_task_id())?;
        ...
        let worker_version = VersionTag::new(action.action_version())
            .map_err(|_| WORKER_MODEL_TOOL_CLAIM_INVALID)?;
        Ok(Self {
            task_id: identity.tool_task_id().clone(),
            run_id: claim.run_id().clone(),
            activation_id: claim.parent_activation_id().clone(),
            node_id,
            effect_id: identity.effect_id().clone(),
            origin: TaskExecutionOrigin::ModelTool {
                parent_activation_id: claim.parent_activation_id().clone(),
                model_call_no: claim.model_call_no(),
                call_index: identity.call_index(),
                tool_task_id: identity.tool_task_id().clone(),
            },
...
fn validate_model_tool_action_binding(
    action: &super::repository::FrozenModelToolAction,
) -> Result<(), &'static str> {
    let binding = action
        .deployment_binding()
        .as_object()
...
mod tests {
    ...
    fn model_tool_claim(arguments: serde_json::Value) -> ModelToolTaskClaim {
        let effect_policy = WorkerEffectPolicy::frozen(
            crate::engine::WorkerEffectClass::ReadOnly,
    ...
    fn model_tool_claim_projects_one_stable_closed_action_request_without_stringifying_json() {
        let claim = model_tool_claim(json!({
            "query": "$literal-not-a-binding",
            "options": {"limit": 3, "flags": [true, null]},
        }));
        let first = TaskExecutionRequest::from_model_tool_claim(&claim).unwrap();
        let replay = TaskExecutionRequest::from_model_tool_claim(&claim).unwrap();
        assert_eq!(first, replay);
        assert_eq!(first.task_id(), claim.identity().tool_task_id());
        assert_eq!(first.effect_id(), claim.identity().effect_id());
        assert_eq!(first.run_id(), claim.run_id());
        assert_eq!(first.activation_id(), claim.parent_activation_id());
        ...
        assert_eq!(
            first.effect_policy(),
            claim.identity().action().effect_policy()
        );
        assert_eq!(
            first.deployment_binding(),
            claim.identity().action().deployment_binding()
        );
        ...
        assert!(first.is_model_tool_action_request());

        let mut with_extra_field = first.clone();
        with_extra_field
            .public_configuration
            .insert("model_call_no".to_owned(), DescriptorValue::Integer(1));
        assert!(!with_extra_field.is_model_tool_action_request());
    }

    #[test]
    fn model_tool_claim_rejects_values_outside_canonical_plan_json() {
        let claim = model_tool_claim(json!({"unsafe_integer": (1_u64 << 53) + 1}));
        assert_eq!(
            TaskExecutionRequest::from_model_tool_claim(&claim),
