...
use crate::{
    ...
    runtime::{
        stop_pair, ExecutionControl, LiveResponseBroker, LiveResponseItemIdentity,
        LiveResponsePayload, LiveResponsePublication, LiveResponseSeal, LiveResponseSealStatus,
        ResponseContentPart, ResponseItemStatus, ResponseOutputItem, ResponseRole, RunError,
        RunErrorKind, StopReason, WorkflowToolPublicProjection,
    },
};
...
struct FrozenLlmTool {
    ...
    validator: crate::schema::JsonSchemaValidator,
...
fn frozen_llm_tool_contract(
    ...
    for (linked, configured_name) in binding.tools.into_iter().zip(configured_tools) {
        ...
        .map_err(|_| invariant(LLM_BINDING_INVALID))?;
        let validator = crate::schema::compile_schema_2020(&linked.input_schema)
            .map_err(|_| invariant(LLM_BINDING_INVALID))?;
        crate::schema::compile_schema_2020(&linked.output_schema)
            .map_err(|_| invariant(LLM_BINDING_INVALID))?;
...
fn validate_frozen_action_binding(
    action: &crate::resources::actions::RegisteredAction,
    request: &TaskExecutionRequest,
) -> Result<(), WorkerFailure> {
...
fn frozen_action_binding(action: &crate::resources::actions::RegisteredAction) -> Value {
    let descriptor = action.descriptor();
    let identity = action.identity();
    serde_json::json!({
...
mod tests {
    ...
    use crate::{
        ...
        engine::{
            ...
            RunId, WorkerCancellation, WorkerEffectPolicy,
        },
        resources::{
        ...
        runtime::{
            InMemoryLiveResponseBroker, LiveResponseDelivery, ResponseStreamEventType, RunError,
        },
    };
    ...
    fn lookup_contract() -> FrozenLlmToolContract {
        ...
        FrozenLlmToolContract {
            tools: vec![FrozenLlmTool {
                validator: crate::schema::compile_schema_2020(&schema).unwrap(),
                definition: ChatToolDefinition {
                    name: "lookup".to_owned(),
                    description: None,
                    input_schema: schema,
    ...
    async fn collect_publication_types(
        subscriber: &mut Box<dyn crate::runtime::LiveResponseSubscriber>,
