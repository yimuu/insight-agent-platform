...
async fn upsert_occurrence_value(
    ...
    let stored_occurrence: crate::engine::LogicalOccurrence = serde_json::from_str(
...
async fn observed_subflow_outcome_sqlite(
    ...
    match lifecycle {
        "succeeded" => {
            let contracts =
                serde_json::from_str::<Vec<crate::engine::scheduler::TaskOutputContract>>(
                    output_contracts,
                )
                .map_err(|_| RepositoryError::invalid_data())?;
...
async fn settle_subflow_sqlite(
    ...
    .map_err(RepositoryError::storage)?
    .ok_or_else(RepositoryError::invalid_data)?;
    let stored_occurrence = serde_json::from_str::<crate::engine::LogicalOccurrence>(
        &row.try_get::<String, _>("occurrence_key")
            .map_err(|_| RepositoryError::invalid_data())?,
    )
    .map_err(|_| RepositoryError::invalid_data())?;
    ...
    match action.intent().action() {
        ...
        SchedulerAction::CompleteRun {
            ...
            settle_dynamic_scope(
                transaction,
                run_id,
                &crate::engine::ScopeInstanceId::root(),
                false,
            )
            .await?;
        ...
        SchedulerAction::OpenFork { admission } => {
            ...
            let (fork_scope, fork_node) =
                activation_identity(transaction, run_id, &group.fork_activation_id).await?;
            ...
            .bind(join_mode)
            .bind(i64::try_from(group.members.len()).map_err(|_| RepositoryError::invalid_data())?)
            .bind(i64::try_from(group.members.len()).map_err(|_| RepositoryError::invalid_data())?)
            .execute(&mut **transaction)
            .await
            .map_err(RepositoryError::storage)?;
...
async fn claim_tasks_sqlite(
    ...
    for candidate in candidates {
        ...
        let mode = if prior_state == "published" {
            SchedulerTaskClaimMode::Acknowledge
        } else if prior_state == "claimed" {
    ...
    if let Some(completion) = completion.as_ref() {
        ...
        for row in occurrence_values {
            ...
            let submitted_ref = submitted_refs
                ...
                .ok_or_else(RepositoryError::invalid_data)?;
            let occurrence: crate::engine::LogicalOccurrence = serde_json::from_str(
                &row.try_get::<String, _>("occurrence_key")
                    .map_err(|_| RepositoryError::invalid_data())?,
            )
            .map_err(|_| RepositoryError::invalid_data())?;
