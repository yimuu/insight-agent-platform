...
use crate::engine::{
    ...
    PublicEventId, PublicEventKind, PublicEventPayload, PublicFailureKind, RunId, RunLifecycle,
};

use super::{
    common::{decode_public_projection_decision, StoredPublicProjectionDecision},
...
impl PublicEventOutboxRepository for PostgresDurableRepository {
    async fn claim_public_events(
        ...
        for row in rows {
            ...
            let claim = PublicEventClaim::new(
    ...
    async fn load_next_public_event(
        ...
        let after_seq = after
            .map(|position| super::common::i64_from_u64(position.causation_seq()))
            .transpose()?;
    ...
    match publish_state {
        ...
        "published" => Ok(OrderedPublicEventRead::Event(Box::new(
            PublishedPublicEvent {
                ...
                public_event_id: complete.public_event_id,
                terminal: complete.terminal,
                safe_envelope: complete
                    .safe_envelope
                    .ok_or_else(RepositoryError::invalid_data)?,
...
fn position_from_public_envelope(
    ...
    PublicEventPosition::new(
        envelope.run_id().clone(),
        envelope.seq().get(),
        super::common::public_event_ordinal(envelope.kind()),
...
fn decode_postgres_complete_public_row(
    ...
    let position = PublicEventPosition::new(
        run_id.clone(),
        super::common::u64_from_i64(decision_seq)?,
...
mod tests {
    ...
    use super::{
        decode_safe_envelope, validate_claim_request, validate_prune_limit,
        validate_retention_seconds, OrderedPublicEventRead, PublicEventClaim,
        PublicEventOutboxRepository, MAX_CLAIM_BATCH, MAX_CLAIM_SECONDS,
    ...
    fn committed_test_execution(
        ...
        let event_id = super::super::common::event_id(&transition);
    ...
    fn operation_public_projection_accepts_the_three_closed_positive_mappings() {
        ...
        for (label, execution_payload, public_payload) in cases {
            ...
            let public_id =
                super::super::common::public_event_id(&run_id, &transition, public_payload.kind());
            let public = super::super::common::durable_public_event_envelope(
                &run_id,
                &public_id,
    ...
    async fn postgres_publish_commits_state_before_notifying_durable_id_when_available() {
        ...
        let (causation_event_id, event_kind, is_terminal, claim_expires_at, safe_envelope) =
            ...
            .await
            .unwrap();
        let retention_claim = PublicEventClaim::new(
            retention_run_id.clone(),
