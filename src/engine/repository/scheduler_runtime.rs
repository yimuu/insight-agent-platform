...
use crate::{
    engine::{
        ...
        SchedulerTaskKind, TaskExecutionRequest, TransitionOutcome, ValueRef, WorkerArtifactStore,
...
use super::{
    ...
    ModelToolTaskOutcome, ModelToolTaskTransitionOutcome, RepositoryError, SchedulerCommitReceipt,
    ...
    StageArtifactCommand, VerifyArtifactCommand, REPOSITORY_DATA_INVALID,
};
    ...
    O: SchedulerRetrievalLiveObserver + ?Sized,
{
    ...
    let public_item_model_call_no = model_call
        .as_ref()
        .map(crate::engine::worker::ModelCallAuthority::model_call_no);
    ...
    let model_checkpoint = match worker_result.as_ref() {
        Some(Ok(result)) => match (result.model_call(), result.model_tool_call_batch()) {
            ...
            (None, Some(_)) => return Err(RepositoryError::invalid_data()),
            (None, None) => None,
        },
    ...
    let pending_outcome = match worker_result {
        ...
        Some(Ok(result)) if result.model_tool_call_batch().is_some() => {
            let model_call_no = result
                ...
                .map(crate::engine::worker::ModelToolCallBatch::model_call_no)
                .ok_or_else(RepositoryError::invalid_data)?;
            if deadline.is_elapsed() {
    ...
    let request = match TaskExecutionRequest::from_model_tool_claim(&claim) {
        ...
        Err(_) => {
            let outcome = model_tool_invariant_failure(
                crate::engine::WORKER_MODEL_TOOL_CLAIM_INVALID,
                EffectEvidence::Started,
            )?;
...
fn model_tool_outcome_from_worker_success(
    ...
    let validator = crate::schema::compile_schema_2020(claim.identity().action().output_schema())
        .map_err(|_| RepositoryError::invalid_data())?;
...
fn worker_result_effect_evidence(
    result: &Option<Result<crate::engine::worker::TaskExecutionResult, WorkerFailure>>,
) -> EffectEvidence {
    match result {
...
fn publish_committed_retrieval_observation(
    ...
    let Some(retrieval) = success
        ...
        .and_then(crate::engine::RetrievalCompletion::public)
    else {
        return;
    };
...
mod tests {
    ...
    use crate::engine::{
        ...
        ActivationId, AttemptNo, EffectId, EffectIdempotency, LeafTaskExecutor, LeaseEpoch,
        ModelCallAuthority, ModelCallCompletion, ModelContinuationTurn, ModelFinishReason,
        ModelTokenUsage, ModelToolCall, ModelToolCallBatch, ModelToolResult, NodeId,
        PlannedSchedulerAction, RunId, RuntimeValue, SchedulerCheckpointId, SchedulerFacts,
        SchedulerIntent, SchedulerTaskId, SchedulerTaskKind, TaskAdmissionClass,
        TaskExecutionResult, TransitionKey, WorkerCancellation, WorkerEffectClass,
        WorkerEffectPolicy,
    };
    use crate::runtime::ResponseStreamEvent;
        ...
        ) -> Result<ModelToolTaskTransitionOutcome<ModelToolTaskCommitReceipt>, RepositoryError>
        {
            ...
            let (disposition, next_available_at, continuation_status) = match outcome {
                ModelToolTaskOutcome::Succeeded { .. } => (
                    ...
                    None,
                    super::super::ModelToolContinuationStatus::ReadyContinue,
                ),
                    ...
                    if *retryable && self.retry_failure =>
                {
                    (
                        ...
                        super::super::ModelToolContinuationStatus::WaitingTools,
                    )
                }
                ModelToolTaskOutcome::Failed { .. } => (
                    ModelToolTaskDisposition::Failed,
                    None,
                    super::super::ModelToolContinuationStatus::ReadyFailed,
                ),
                ModelToolTaskOutcome::Cancelled { .. } => (
    ...
    fn model_tool_worker_claim_with_policies(
        ...
        let identity = crate::engine::repository::deterministic_tool_identity(
            &run_id,
    ...
    fn parent_llm_claim(timeout_ms: u64) -> SchedulerTaskClaim {
        ...
        let envelope = super::super::DurableTaskExecutionRequest::new(
            request,
            AttemptNo::FIRST,
            LeaseEpoch::FIRST,
    ...
    impl DurableRepository for MockParentContinuationRepository {
        ...
        async fn publish_builtin_versioned_plan(
            &self,
            _plan: &VersionedPlan,
        ) -> Result<super::super::PublicationHead, RepositoryError> {
            unexpected_parent_repository_call()
        }
    ...
    impl ArtifactDurableRepository for MockParentContinuationRepository {
        ...
        async fn get_retained_artifact(
            &self,
            _run_id: &RunId,
            _artifact_id: &crate::engine::ArtifactId,
    ...
    async fn provider_tool_checkpoint_crash_recovers_without_replaying_provider() {
        ...
        let (initial_registry, initial_provider_calls, _) =
            parent_llm_registry(Ok(parent_tool_batch_result(1)));
        let crash = crate::engine::repository::FailOnceSchedulerCrash::new(
