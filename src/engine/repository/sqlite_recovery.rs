...
use super::{
    ...
    MigrationMappingCompatibility, RecoveryDurableRepository, RecoveryEventReceipt,
...
async fn ensure_redrive_effect_safety_sqlite(
    tx: &mut Transaction<'_, Sqlite>,
    source_run_id: &RunId,
) -> Result<(), RepositoryError> {
    let effects = sqlx::query(
        "SELECT lifecycle,effect_evidence,effect_idempotency FROM node_activations
         WHERE run_id=? AND execution_kind='worker' ORDER BY activation_id",
    )
...
async fn insert_redrive_effect_roots_sqlite(
    tx: &mut Transaction<'_, Sqlite>,
    source_run_id: &RunId,
    target_run_id: &RunId,
    ...
    sqlx::query(
        "INSERT INTO recovery_effect_roots (
         ...
         SELECT ?,a.run_id,a.run_id,a.activation_id,a.effect_id,?,0,CURRENT_TIMESTAMP
         FROM node_activations a
         WHERE a.run_id=? AND a.execution_kind='worker' AND a.lifecycle<>'succeeded'",
    )
...
async fn authoritative_scheduler_checkpoint(
    ...
    let fact_payload: Value = serde_json::from_str(
        &row.try_get::<String, _>("fact_payload")
            .map_err(|_| RepositoryError::invalid_data())?,
    )
    .map_err(|_| RepositoryError::invalid_data())?;
    ...
    let Some(encoded_ledger_batch) = row
        .try_get::<Option<String>, _>("projection_ledger_batch")
        .map_err(|_| RepositoryError::invalid_data())?
    else {
        return Ok(false);
    };
...
async fn checkpoint_projection_version_sqlite(
    tx: &mut Transaction<'_, Sqlite>,
    run_id: &RunId,
...
async fn revision_plan_sqlite(
    ...
    let value = sqlx::query_scalar::<_, String>(
        "SELECT r.canonical_plan FROM workflow_definition_revisions r
         ...
         WHERE d.definition_id=? AND d.definition_revision_id=?
           AND d.deployment_revision_id=? AND d.plan_hash=? AND d.binding_hash=?",
    )
...
async fn fork_contract_compatible_sqlite(
    tx: &mut Transaction<'_, Sqlite>,
    source: &SourceRun,
    ...
    let Some(source_plan) = revision_plan_sqlite(tx, &source_revision).await? else {
        return Ok(false);
    };
    let Some(target_plan) = revision_plan_sqlite(tx, target_revision).await? else {
        return Ok(false);
    };
    let input =
...
async fn derive_reuse_candidates_sqlite(
    tx: &mut Transaction<'_, Sqlite>,
    source: &SourceRun,
    target_run_id: &RunId,
    target_revision: &RecoveryRevisionSpec,
    ...
    let ceiling = if let Some(hash) = source_checkpoint_hash {
        ...
        let Some(version) = checkpoint_projection_version_sqlite(tx, &source.run_id, hash).await?
        else {
            return Ok(None);
        };
        version
    ...
    let rows = sqlx::query(
        "SELECT a.activation_id,a.node_id,a.stable_activation_key,a.effect_id,
                a.effect_evidence,a.output_payload_id,a.output_artifact_id,a.output_value_hash,
                t.task_id,t.task_envelope,t.created_by_transition_key
         FROM node_activations a
         JOIN task_outbox t
    ...
    for row in rows {
        ...
        let dispatch_hash = sqlx::query_scalar::<_, String>(
            "SELECT content_hash FROM scheduler_checkpoints
             WHERE run_id=? AND checkpoint_kind='planned_action' AND transition_key=?
               AND scheduler_projection_version<=?",
        )
        .bind(source.run_id.as_str())
        .bind(
        ...
        let token = sqlx::query(
            "SELECT source_port_id,current_scope_instance_id,provenance_frames
             FROM control_tokens
             WHERE run_id=? AND source_activation_id=?
        ...
        let frames: Vec<ControlFrame> = serde_json::from_str(
            &token
...
impl RecoveryDurableRepository for SqliteDurableRepository {
    async fn latest_authoritative_scheduler_checkpoint(
        ...
        // Select as of the request's optimistic source version. This remains
        // deterministic when a source advances after a successful Fork and
        // the client retries the exact same request identity. `fork_run` still
        // rejects a stale first attempt and replays only an exact prior intent.
        source.projection_version = expected_source_projection_version;
        let rows = sqlx::query(
            "SELECT content_hash FROM scheduler_checkpoints
             WHERE run_id=? AND checkpoint_kind IN ('planned_action','task_started','task_completed','task_retry_scheduled')
               AND scheduler_projection_version<=?
    ...
    async fn authoritative_scheduler_checkpoint_by_id(
        ...
        let hash = sqlx::query_scalar::<_, String>(
            "SELECT content_hash FROM scheduler_checkpoints
             WHERE run_id=? AND checkpoint_id=? AND scheduler_projection_version<=?",
        )
        .bind(source_run_id.as_str())
    ...
    async fn derive_compatible_reuse_candidates(
        ...
        let candidates = derive_reuse_candidates_sqlite(
            &mut tx,
            &source,
            target_run_id,
            target_revision,
    ...
    async fn redrive_run(
        ...
        ensure_redrive_effect_safety_sqlite(&mut tx, &source.run_id).await?;
    ...
    async fn fork_run(
        ...
        if source.projection_version != command.expected_source_projection_version()
            || !fork_contract_compatible_sqlite(
                &mut tx,
                &source,
    ...
    async fn load_pending_migration_intent(
        ...
        let row = sqlx::query(
            "SELECT i.*,e.projection_version_after AS source_projection_version_after,
             ...
             WHERE i.run_id=? AND i.intent_transition_key=? AND i.intent_state='pending'",
        )
        .bind(source_run_id.as_str())
        ...
        let mappings: Vec<MigrationMappingCompatibility> =
            serde_json::from_value(json_from_sqlite(&row, "mapping_contracts")?)
                .map_err(|_| RepositoryError::invalid_data())?;
        let candidates: Vec<CreateReuseCandidateCommand> =
            serde_json::from_value(json_from_sqlite(&row, "reuse_candidates")?)
                .map_err(|_| RepositoryError::invalid_data())?;
        ...
        let command = BeginMigrationCommand::new(
            ...
            json_from_sqlite(&row, "target_input")?,
            mappings,
            candidates,
        )?
    ...
    async fn begin_migration(
        ...
        let updated = sqlx::query(
            "UPDATE workflow_runs SET termination_intent_reason='migrated',
                    ...
                    projection_version=?,scheduler_lease_epoch=scheduler_lease_epoch+1,
                    scheduler_lease_owner=NULL,scheduler_fencing_token=NULL,
                    scheduler_lease_expires_at=NULL,scheduler_heartbeat_at=NULL,
                    updated_at=CURRENT_TIMESTAMP
             WHERE run_id=? AND projection_version=? AND lifecycle IN ('active','waiting')
        ...
        .await?;
        let (target_input, target_input_hash) = canonical_value(command.target_input())?;
        let mappings = canonical_json(
        ...
        sqlx::query(
            "INSERT INTO run_migration_intents (
             ...
             ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'pending',NULL,0,CURRENT_TIMESTAMP,NULL)",
        )
        .bind(source.run_id.as_str())
    ...
    async fn finalize_migration(
        ...
        let intent = sqlx::query(
            "SELECT * FROM run_migration_intents WHERE run_id=? AND target_run_id=?
               AND intent_state='pending' AND intent_transition_key=?",
        )
        .bind(source.run_id.as_str())
        .bind(command.target_run_id().as_str())
        ...
        let target_input = json_from_sqlite(&intent, "target_input")?;
        let mappings: Vec<MigrationMappingCompatibility> =
            serde_json::from_value(json_from_sqlite(&intent, "mapping_contracts")?)
                .map_err(|_| RepositoryError::invalid_data())?;
        let candidates: Vec<CreateReuseCandidateCommand> =
            serde_json::from_value(json_from_sqlite(&intent, "reuse_candidates")?)
                .map_err(|_| RepositoryError::invalid_data())?;
        let target_timeout_ms = u64_from_i64(
            intent
        ...
        sqlx::query(
            "UPDATE run_migration_intents SET intent_state='completed',final_transition_key=?,
                    projection_version=projection_version+1,completed_at=CURRENT_TIMESTAMP
             WHERE run_id=? AND intent_state='pending'",
        )
        .bind(transition_key.as_str())
        .bind(source.run_id.as_str())
        .execute(&mut *tx)
...
async fn load_source(
    ...
    row.map(|row| {
        ...
        let encoded = row
            .try_get::<Option<String>, _>("input_inline_value")
            .map_err(|_| RepositoryError::invalid_data())?
            .ok_or_else(RepositoryError::invalid_data)?;
        let value =
            serde_json::from_str::<Value>(&encoded).map_err(|_| RepositoryError::invalid_data())?;
...
async fn revision_exists(
    ...
    Ok(sqlx::query_scalar::<_, i64>(
        "SELECT 1 FROM deployment_revisions WHERE definition_id=?
           AND definition_revision_id=? AND deployment_revision_id=?
           AND plan_hash=? AND binding_hash=?",
    )
    .bind(revision.definition_id())
    .bind(revision.revision().definition_revision_id().as_str())
    .bind(revision.revision().deployment_revision_id().as_str())
...
async fn migration_ready(
    ...
    let unsettled = sqlx::query_scalar::<_, i64>(
        "SELECT COALESCE(SUM(admitted_children-settled_children),0)
         FROM scope_instances WHERE run_id=?",
    )
    .bind(source.run_id.as_str())
    .fetch_one(&mut **tx)
    .await
...
/// An unsettled wait is live control state, not reusable output.  Migrate may
/// only invalidate and rebuild it when the source node has an explicit mapping
/// and both sides of that mapping opt in to rebuilding every registered wait
/// kind.  This is checked before the migration intent mutates the source Run.
...
async fn create_target_run(
    ...
    .bind(i64_from_u64(target_timeout_ms)?)
    .fetch_one(&mut **tx)
    .await
    .map_err(RepositoryError::storage)?;
    let run_deadline_at = super::sqlite::parse_run_timestamp(&deadline)?;
    let (input_payload_id, input_hash) = insert_target_run(
        tx,
        source,
        target_run_id,
        target_revision,
    ...
    if kind == RunLineageKind::Redrive {
        insert_redrive_effect_roots_sqlite(
            tx,
            &source.run_id,
            target_run_id,
            &target_transition_key,
...
async fn insert_target_run(
    ...
    sqlx::query(
        "INSERT INTO workflow_runs (
         ...
         ) VALUES (?,?,?,?,?,?,?,'detached','created','open',NULL,NULL,NULL,?,NULL,NULL,NULL,NULL,NULL,NULL,
                   ?,?,?,NULL,1,0,0,NULL,NULL,NULL,NULL,CURRENT_TIMESTAMP,NULL,CURRENT_TIMESTAMP,NULL,?)",
    )
    .bind(target_run_id.as_str())
    ...
    .bind(run_deadline_at.to_rfc3339())
    .execute(&mut **tx)
    .await
    .map_err(RepositoryError::storage)?;
    ...
    sqlx::query(
        "INSERT INTO scope_instances (
         ...
         ) VALUES (?,'scope_root',NULL,'root',NULL,'root',1,'active','open',0,0,0,
    ...
    .await
    .map_err(RepositoryError::storage)?;
    Ok((input_payload_id, stored_hash))
}
        ...
        (None, Some(artifact)) => {
            let row = sqlx::query(
                "SELECT a.content_hash,a.artifact_state FROM artifacts a
                 ...
                 WHERE a.run_id=? AND a.artifact_id=?
...
async fn terminalize_source(
    ...
    sqlx::query(
        "INSERT INTO public_event_outbox (
         ...
         ) VALUES (?,?,?,?,1,?,1,'pending',?,CURRENT_TIMESTAMP,NULL,NULL,NULL,0,NULL,NULL,
    ...
    let updated = sqlx::query(
        "UPDATE workflow_runs SET lifecycle='cancelled',admission_state='closed',
                ...
                termination_intent_transition_key=COALESCE(termination_intent_transition_key,?),
                termination_intent_at=COALESCE(termination_intent_at,CURRENT_TIMESTAMP),
                error_code=?,terminal_event_id=?,terminal_public_event_id=?,replacement_run_id=?,
                projection_version=?,scheduler_lease_owner=NULL,scheduler_fencing_token=NULL,
                scheduler_lease_expires_at=NULL,scheduler_heartbeat_at=NULL,
    ...
    sqlx::query(
        "UPDATE timers SET timer_state='cancelled',projection_version=projection_version+1,
                fired_at=CURRENT_TIMESTAMP WHERE run_id=? AND timer_state='scheduled'",
    )
    .bind(source.run_id.as_str())
    .execute(&mut **tx)
    .await
    ...
    for signal_id in pending_signals {
        ...
        sqlx::query(
            "UPDATE signals_inbox SET signal_state='rejected',consumed_by_transition_key=?,
                    consumed_event_id=?,terminal_at=CURRENT_TIMESTAMP,
                    projection_version=projection_version+1
    ...
    sqlx::query(
        "UPDATE task_outbox SET task_state='dead',claimed_by=NULL,claim_token=NULL,
                claim_expires_at=NULL,last_error_code=?,projection_version=projection_version+1
         WHERE run_id=? AND task_state IN ('pending','claimed')",
    )
    .bind(public_error_code)
    .bind(source.run_id.as_str())
    .execute(&mut **tx)
...
async fn replay_result<T: DeserializeOwned>(
    ...
    let row = sqlx::query(
        "SELECT intent_hash,primary_event_run_id,primary_event_id,result_json
         ...
         WHERE authority_run_id=? AND transition_key=?",
    )
...
fn json_from_sqlite(row: &sqlx::sqlite::SqliteRow, column: &str) -> Result<Value, RepositoryError> {
    serde_json::from_str(
...
mod tests {
    ...
    async fn assert_recovery_deadline(
        ...
        let row = sqlx::query_as::<_, (String, i64, i64)>(
            "SELECT r.lineage_kind,
                    julianday(r.deadline_at) > julianday('now'),
        ...
        .await
        .unwrap();
        assert_eq!(row, (expected_lineage_kind.to_owned(), 1, 1));
    }

    async fn activate(repository: &SqliteDurableRepository, run_id: &RunId) {
        repository
    ...
    async fn pause(repository: &SqliteDurableRepository, run_id: &RunId, version: u64) {
        repository
            .commit_run_transition(
    ...
    async fn fail(repository: &SqliteDurableRepository, run_id: &RunId) {
        if repository
            .load_run(run_id)
            .await
            .unwrap()
    ...
    async fn install_scheduler_checkpoint(
        ...
        let intent_hash =
            ContentHash::from_bytes(format!("sqlite/recovery/checkpoint/{label}").as_bytes());
        let checkpoint_digest =
            ContentHash::from_bytes(format!("sqlite/recovery/checkpoint-id/{label}").as_bytes());
        ...
        .bind(canonical_json(&fact_payload).unwrap())
        .execute(&mut *tx)
        .await
        .unwrap();
    ...
    async fn valid_candidate(
        ...
        sqlx::query(
            "INSERT INTO artifacts (
             ...
             ) VALUES (?,?,?,24,'application/json','object://recovery/source',
        ...
        let effect_id: String = sqlx::query_scalar(
            "SELECT effect_id FROM node_activations WHERE run_id=? AND activation_id=?",
        )
        .bind(source.as_str())
        .bind(activation_id.as_str())
        .fetch_one(repository.test_pool())
        ...
        sqlx::query(
            "UPDATE node_activations SET lifecycle='succeeded',effect_evidence='committed',
                    output_artifact_id=?,output_value_hash=?,projection_version=projection_version+1,
                    updated_at=CURRENT_TIMESTAMP,terminal_at=CURRENT_TIMESTAMP
    ...
    #[tokio::test]
    async fn sqlite_run_and_recovery_payload_reads_recompute_inline_authority() {
        let repository = SqliteDurableRepository::in_memory().await.unwrap();
        let version = plan("payload_authority_sqlite");

        let completed = install_run(
            &repository,
            &version,
            "run_payload_authority_sqlite_completed",
        ...
        repository
            .commit_run_transition(
                key("payload_authority_sqlite.completing"),
                RunTransitionCommand::nonterminal(
                    completed.clone(),
                    1,
                    RunLifecycle::Active,
        ...
        assert!(matches!(
            repository
                .commit_run_transition(
                    key("payload_authority_sqlite.complete"),
        ...
        let (output_payload_id, output_hash): (String, String) = sqlx::query_as(
            "SELECT output_payload_id,output_value_hash FROM workflow_runs WHERE run_id=?",
        )
        ...
        let source = install_run(
            &repository,
            &version,
            "run_payload_authority_sqlite_source",
        ...
        let input_payload_id: String =
            sqlx::query_scalar("SELECT input_payload_id FROM workflow_runs WHERE run_id=?")
                .bind(source.as_str())
                .fetch_one(repository.test_pool())
                .await
                .unwrap();
        sqlx::query("UPDATE payloads SET inline_value=? WHERE run_id=? AND payload_id=?")
            .bind(serde_jcs::to_string(&json!({"report": "tampered"})).unwrap())
            .bind(source.as_str())
            .bind(input_payload_id)
            .execute(repository.test_pool())
            .await
            .unwrap();
        let target = RunId::new("run_payload_authority_sqlite_target").unwrap();
        let error = repository
            .redrive_run(
                key("payload_authority_sqlite.redrive"),
    ...
    #[tokio::test]
    async fn sqlite_redrive_and_fork_revision_contracts_are_durable() {
        let repository = SqliteDurableRepository::in_memory().await.unwrap();
        ...
        sqlx::query(
            "UPDATE artifacts SET artifact_state='referenced',referenced_at=CURRENT_TIMESTAMP
             WHERE run_id=? AND artifact_id='artifact_recovery_source'",
        )
        .bind(source.as_str())
        .execute(repository.test_pool())
        .await
        ...
        .unwrap();
        let redrive_outcome = repository
            .redrive_run(redrive_key.clone(), redrive.clone())
            .await
            .unwrap();
        assert!(
            matches!(redrive_outcome, TransitionOutcome::Committed { .. }),
            "unexpected redrive outcome: {redrive_outcome:?}"
        );
        ...
        let candidate_count: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM run_reuse_candidates WHERE run_id=?")
                .bind(redrive_target.as_str())
                .fetch_one(repository.test_pool())
                .await
                .unwrap();
        ...
        let target_activations: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM node_activations WHERE run_id=?")
                .bind(redrive_target.as_str())
                .fetch_one(repository.test_pool())
                .await
                .unwrap();
        ...
        let effect_roots: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM recovery_effect_roots WHERE run_id=?")
                .bind(redrive_target.as_str())
                .fetch_one(repository.test_pool())
                .await
                .unwrap();
        ...
        let artifact_roots: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM recovery_artifact_roots WHERE run_id=?")
                .bind(redrive_target.as_str())
                .fetch_one(repository.test_pool())
                .await
                .unwrap();
        ...
        let foreign_row = sqlx::query(
            "SELECT checkpoint_id,checkpoint_kind,transition_key,intent_hash,event_id,
             ...
             FROM scheduler_checkpoints WHERE run_id=? AND content_hash=?",
        )
        ...
        let tampered_checkpoint = scheduler_checkpoint_content_hash(
            ...
            u32::try_from(foreign_row.get::<i64, _>("checkpoint_schema_version")).unwrap(),
            u64_from_i64(foreign_row.get("scheduler_projection_version")).unwrap(),
            &tampered_payload,
        )
        .unwrap();
        sqlx::query(
            "UPDATE scheduler_checkpoints SET content_hash=?,fact_payload=?
             WHERE run_id=? AND content_hash=?",
        )
        .bind(tampered_checkpoint.as_str())
        .bind(canonical_json(&tampered_payload).unwrap())
        .bind(foreign_source.as_str())
        .bind(foreign_checkpoint.as_str())
        .execute(repository.test_pool())
        .await
        ...
        let fork_row = sqlx::query(
            "SELECT lineage_kind,source_checkpoint_hash FROM run_recovery_lineage WHERE run_id=?",
        )
    ...
    #[tokio::test]
    async fn sqlite_migration_unsettled_wait_requires_mapped_dual_rebuild_before_intent() {
        let repository = SqliteDurableRepository::in_memory().await.unwrap();
        ...
        sqlx::query(
            "INSERT INTO scheduler_wait_registrations (
             ...
             ) VALUES (?,?,?,'wait_v1',?,'resume','signal_migration_wait',NULL,NULL,NULL,
                       NULL,NULL,NULL,0,CURRENT_TIMESTAMP,NULL)",
        )
        .bind(source.as_str())
        .bind("wait_migration")
        .bind(activation_id.as_str())
        .bind(canonical_json(&serde_json::json!({"segments": []})).unwrap())
        .execute(repository.test_pool())
        .await
        .unwrap();
        // The readiness gate is independent from the wait-rebuild gate.  Make
        // this fixture drained so the latter is the only reason to reject.
        sqlx::query(
            "UPDATE scope_instances SET settled_children=admitted_children
             WHERE run_id=? AND scope_instance_id='root'",
        )
        .bind(source.as_str())
        .execute(repository.test_pool())
        .await
        ...
        assert_eq!(
            sqlx::query_scalar::<_, i64>(
                "SELECT COUNT(*) FROM run_migration_intents WHERE run_id=?",
            )
            .bind(source.as_str())
            .fetch_one(repository.test_pool())
            .await
    ...
    #[test]
    fn sqlite_recovery_rederives_map_loop_and_subflow_non_root_scopes() {
        use super::super::recovery_repository::dynamic_scope_test_contract::{
            assert_loop_iteration_scope_rederivation, assert_map_item_scope_rederivation,
            assert_subflow_invocation_scope_rederivation,
        };

        assert_map_item_scope_rederivation();
        assert_loop_iteration_scope_rederivation();
        assert_subflow_invocation_scope_rederivation();
    }

    #[tokio::test]
    async fn sqlite_fresh_migration_is_fenced_by_the_durable_target_head() {
        let repository = SqliteDurableRepository::in_memory().await.unwrap();
        ...
        let intent_count: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM run_migration_intents WHERE run_id=?")
                .bind(source.as_str())
                .fetch_one(repository.test_pool())
                .await
                .unwrap();
    ...
    async fn sqlite_migration_is_ready_fenced_signal_safe_and_atomic() {
        ...
        let source_row = sqlx::query(
            "SELECT lifecycle,error_code,replacement_run_id FROM workflow_runs WHERE run_id=?",
        )
        .bind(source.as_str())
        .fetch_one(repository.test_pool())
        .await
        ...
        let roots: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM recovery_revision_roots WHERE run_id=?")
                .bind(target.as_str())
                .fetch_one(repository.test_pool())
                .await
                .unwrap();
        ...
        let atomic_source = sqlx::query(
            "SELECT lifecycle,termination_intent_reason FROM workflow_runs WHERE run_id=?",
        )
    ...
    #[tokio::test]
    async fn sqlite_continue_as_new_increments_generation_atomically() {
        let repository = SqliteDurableRepository::in_memory().await.unwrap();
        ...
        let target_row = sqlx::query(
            "SELECT generation,lineage_kind,parent_run_id FROM workflow_runs WHERE run_id=?",
        )
        .bind(target.as_str())
        .fetch_one(repository.test_pool())
        .await
