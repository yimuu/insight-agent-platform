//! Public `response-stream/v1` protocol and its transient live-delivery plane.
//!
...
//!
//! The in-memory broker is the single-process adapter for that boundary. It
//! is live-only, bounded, and never treats response observations as execution
//! facts. Production multi-runtime deployments must use another implementation
//! of [`LiveResponseBroker`] backed by a shared transient transport.
...
use crate::engine::{ActivationId, ArtifactRef, AttemptNo, RunId};
...
impl RunQueue {
    ...
    pub(crate) fn publish(
        ...
        match source {
            ...
            LiveResponseSourceIdentity::WorkflowObservation(identity) => {
                self.publish_workflow_observation(&mut state, publication, identity, local_sequence)
            }
        }
    }
    ...
    fn publish_workflow_observation(
        ...
        let run_size_exhausted =
            state.size_exhausted || run_observed_bytes > self.byte_limits.max_run_bytes;
        let must_drop =
            state.body.len() >= self.body_capacity || source_size_exhausted || run_size_exhausted;
    ...
    pub(crate) fn discard_workflow_observation(
        &self,
        identity: LiveWorkflowObservationIdentity,
        local_sequence: u64,
    ) -> LiveResponsePublishOutcome {
    ...
    pub(crate) fn seal(&self, seal: LiveResponseSeal) -> LiveResponsePublishOutcome {
    ...
    pub(crate) fn discard_seal_with_gap(
        &self,
        identity: LiveResponseItemIdentity,
    ) -> LiveResponsePublishOutcome {
        if identity.run_id() != &self.run_id {
    ...
    pub(crate) fn accept_gap(&self, gap: LiveResponseGap) -> LiveResponsePublishOutcome {
    ...
    pub(crate) async fn recv(&self) -> Result<LiveResponseDelivery, LiveResponseBrokerError> {
        loop {
    ...
    pub(crate) fn close(&self) -> LiveResponseCloseOutcome {
        let mut state = lock(&self.state);
        if state.closed {
...
mod tests {
    use super::*;
    use crate::engine::{ArtifactId, ContentHash};
    use serde_json::json;
    ...
    async fn frame_item_and_run_byte_limits_drop_with_explicit_gaps() {
        ...
        let frame = delta(frame_item.clone(), 0, "oversize frame");
        let frame_bytes = frame.public_wire_bytes();
        ...
        let first_bytes = first.public_wire_bytes();
        let second_bytes = second.public_wire_bytes();
    ...
    async fn runs_are_isolated_and_late_publication_after_seal_is_rejected() {
        ...
        assert_eq!(
            broker.publish(delta(second.clone(), 0, "only b")),
            LiveResponsePublishOutcome::Enqueued
        );
