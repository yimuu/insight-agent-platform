...
const profile = __ENV.PROFILE || "phase0-full";
...
const runTimeout = Number.parseFloat(__ENV.RUN_TIMEOUT_SECONDS || "30");
...
// Keep this payload byte-for-byte stable. Phase 0 compares full-runtime write
// amplification, so per-iteration fixture variation would invalidate the
// payload/structural split.
const FIXED_INPUT = Object.freeze({
  text: "phase0 full WAL baseline fixture",
});

const accepted = new Counter("phase0_full_run_accepted");
const terminalObserved = new Counter("phase0_full_run_terminal_observed");
const succeeded = new Counter("phase0_full_run_succeeded");
const failed = new Counter("phase0_full_run_failed");
const interrupted = new Counter("phase0_full_run_interrupted");
const rejected = new Counter("phase0_full_run_rejected");
const polls = new Counter("phase0_full_run_poll_requests");
const acceptedClosure = new Rate("phase0_full_run_accepted_closure");
const scheduledSuccess = new Rate("phase0_full_run_scheduled_success");
const lifecycle = new Trend("phase0_full_run_lifecycle_duration", true);
const createDuration = new Trend("phase0_full_run_create_duration", true);
const arrivalsScheduled = new Counter("phase0_full_arrivals_scheduled");
const arrivalLateness = new Trend("phase0_full_arrival_lateness", true);
const arrivalsLate = new Counter("phase0_full_arrivals_late");
...
export default function () {
  ...
  const requestId =
    `phase0-full-${profile}-${arrivalOrdinal}-${startedAt}`;
  const create = http.post(
    `${baseUrl}/v1/agents/${agentId}/runs`,
    JSON.stringify(FIXED_INPUT),
    {
      headers: {
        "content-type": "application/json",
        "x-request-id": requestId,
      },
      tags: { endpoint: "create_phase0_full_run" },
      timeout: "10s",
    },
  );
  ...
  check(create, {"full run admitted": () => admitted});
  if (!admitted) {
    rejected.add(1);
    return;
  }
  ...
  while (Date.now() < deadline) {
    const response = http.get(`${baseUrl}/v1/runs/${runId}`, {
      tags: { endpoint: "get_phase0_full_run" },
      timeout: "10s",
    });
    polls.add(1);
    if (response.status === 200) {
  ...
  scheduledSuccess.add(false);
  lifecycle.add(Date.now() - startedAt);
  check(null, {"accepted full run reached a terminal view": () => false});
}
