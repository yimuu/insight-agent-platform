...
function Artifacts({ client, report }: { client: PlatformClient | null; report: (notice: Notice | null) => void }) {
  ...
  return <div className="nested-panel"><SearchForm label="Artifact ID" placeholder="art_…" value={id} onChange={setId} onSubmit={load} busy={busy} />{artifact && <><dl className="metrics"><Metric label="Artifact ID" value={artifact.artifact_id} mono /><Metric label="State" value={artifact.state} /><Metric label="Purpose" value={artifact.purpose} /><Metric label="Size" value={`${artifact.expected_size_bytes} bytes`} /></dl><button className="button button--primary" disabled={artifact.state !== 'ready'} onClick={download}>Controlled download</button></>}</div>
}
