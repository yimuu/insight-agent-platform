-- DSL durable execution kernel (PostgreSQL production authority).
...
CREATE FUNCTION durable_reject_published_revision_mutation()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE TRIGGER trg_published_revision_immutable
    BEFORE UPDATE OR DELETE ON workflow_definition_revisions
    FOR EACH ROW EXECUTE FUNCTION durable_reject_published_revision_mutation();

CREATE FUNCTION durable_reject_deployment_revision_update()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE TRIGGER trg_deployment_revision_immutable
    ...
    FOR EACH ROW EXECUTE FUNCTION durable_reject_deployment_revision_update();
...
CREATE INDEX idx_runs_scheduler_lease
...
CREATE INDEX idx_activation_live_fence
...
ALTER TABLE node_activations
    ADD CONSTRAINT fk_activation_current_attempt
    FOREIGN KEY (
        run_id,
        activation_id,
        current_attempt_no,
...
ALTER TABLE node_activations
    ADD CONSTRAINT fk_activation_winning_attempt
...
CREATE INDEX idx_execution_events_activation
...
ALTER TABLE workflow_runs
    ADD CONSTRAINT fk_run_terminal_public_event
...
ALTER TABLE node_activations
    ADD CONSTRAINT fk_activation_retry_timer
...
CREATE INDEX idx_control_tokens_available
...
ALTER TABLE control_tokens
    ADD CONSTRAINT fk_token_fork_leg
