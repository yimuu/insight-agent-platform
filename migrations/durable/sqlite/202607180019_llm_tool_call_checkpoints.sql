-- The model checkpoint and scheduler-owned tool queue share one authority.
-- Checkpoint inserts are immutable and body-safe; activation later fills all
-- execution fields atomically from the parent deployment binding.
...
CREATE TABLE model_tool_call_batches (
    ...
    parent_task_projection_version INTEGER CHECK (
...
CREATE TABLE model_tool_calls (
    ...
    arguments TEXT NOT NULL CHECK (
        json_valid(arguments) AND json_type(arguments) = 'object' AND length(arguments) <= 262144
    ),

    tool_task_id TEXT UNIQUE CHECK (
        tool_task_id IS NULL OR (length(tool_task_id) = 69 AND tool_task_id LIKE 'task_%')
    ),
    effect_id TEXT UNIQUE CHECK (
        effect_id IS NULL OR (length(effect_id) = 71 AND effect_id LIKE 'effect_%')
    ),
    action_id TEXT CHECK (action_id IS NULL OR (action_id <> '' AND length(action_id) <= 128)),
    action_version TEXT CHECK (
        action_version IS NULL OR (action_version <> '' AND length(action_version) <= 64)
    ),
    action_descriptor_hash TEXT CHECK (
        action_descriptor_hash IS NULL OR length(action_descriptor_hash) = 64
    ),
    action_input_schema TEXT CHECK (
        action_input_schema IS NULL OR json_valid(action_input_schema)
    ),
    action_output_schema TEXT CHECK (
        action_output_schema IS NULL OR json_valid(action_output_schema)
    ),
    action_effect_policy TEXT CHECK (
        action_effect_policy IS NULL OR (json_valid(action_effect_policy)
            AND json_type(action_effect_policy) = 'object')
    ),
    action_deployment_binding TEXT CHECK (
        action_deployment_binding IS NULL OR (json_valid(action_deployment_binding)
            AND json_type(action_deployment_binding) = 'object')
    ),
    effective_public_policy TEXT CHECK (
        effective_public_policy IS NULL OR (json_valid(effective_public_policy)
            AND json_type(effective_public_policy) = 'object')
    ),
    response_item_id TEXT CHECK (
        response_item_id IS NULL OR (response_item_id <> '' AND length(response_item_id) <= 256)
    ),
    ...
    failure_code TEXT CHECK (
        failure_code IS NULL OR (failure_code <> '' AND length(failure_code) <= 128)
    ),
    ...
    last_outcome_hash TEXT CHECK (
        last_outcome_hash IS NULL OR (length(last_outcome_hash) = 71
    ...
    last_outcome_lease_epoch INTEGER CHECK (
    ...
    last_failure_code TEXT CHECK (
        last_failure_code IS NULL OR (last_failure_code <> '' AND length(last_failure_code) <= 128)
    ),
    last_failure_retryable INTEGER CHECK (
        last_failure_retryable IS NULL OR last_failure_retryable IN (0,1)
    ),
