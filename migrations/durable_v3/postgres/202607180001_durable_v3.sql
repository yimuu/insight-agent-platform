-- DSL v3 durable execution kernel (PostgreSQL production authority).
-- Forward-only: this migration intentionally has no dependency on the formal-v2 tables.
...
CREATE FUNCTION durable_v3_reject_published_revision_mutation()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE TRIGGER trg_v3_published_revision_immutable
    BEFORE UPDATE OR DELETE ON workflow_definition_revisions
    FOR EACH ROW EXECUTE FUNCTION durable_v3_reject_published_revision_mutation();

CREATE FUNCTION durable_v3_reject_deployment_revision_update()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE TRIGGER trg_v3_deployment_revision_immutable
    ...
    FOR EACH ROW EXECUTE FUNCTION durable_v3_reject_deployment_revision_update();
...
CREATE TABLE workflow_runs (
    ...
    termination_intent_at TIMESTAMPTZ,
    ...
    scheduler_lease_epoch BIGINT NOT NULL DEFAULT 0 CHECK (scheduler_lease_epoch >= 0),
    ...
    CHECK (
        ...
        (lifecycle = 'succeeded'
            ...
            AND ((output_payload_id IS NOT NULL)::integer
                + (output_artifact_id IS NOT NULL)::integer) = 1
            AND error_code IS NULL)
        OR
...
CREATE INDEX idx_v3_runs_scheduler_lease
    ON workflow_runs(scheduler_lease_expires_at, run_id)
    WHERE scheduler_lease_owner IS NOT NULL;
...
CREATE TABLE scope_instances (
    ...
    admitted_children BIGINT NOT NULL DEFAULT 0 CHECK (admitted_children >= 0),
    settled_children BIGINT NOT NULL DEFAULT 0 CHECK (
...
CREATE UNIQUE INDEX uq_v3_root_scope_per_run
    ON scope_instances(run_id) WHERE is_root;
...
CREATE TABLE node_activations (
    ...
    last_lease_epoch BIGINT CHECK (last_lease_epoch IS NULL OR last_lease_epoch >= 1),
    current_attempt_no INTEGER CHECK (current_attempt_no IS NULL OR current_attempt_no >= 1),
    current_lease_epoch BIGINT CHECK (current_lease_epoch IS NULL OR current_lease_epoch >= 1),
...
CREATE INDEX idx_v3_activation_live_fence
    ON node_activations(run_id, activation_id, current_attempt_no, current_lease_epoch)
    WHERE current_attempt_no IS NOT NULL;

CREATE TABLE node_attempts (
    run_id TEXT NOT NULL,
    activation_id TEXT NOT NULL,
    attempt_no INTEGER NOT NULL CHECK (attempt_no >= 1),
    lease_epoch BIGINT NOT NULL CHECK (lease_epoch >= attempt_no),
...
ALTER TABLE node_activations
    ADD CONSTRAINT fk_v3_activation_current_attempt
    FOREIGN KEY (
        run_id,
        activation_id,
        current_attempt_no,
...
ALTER TABLE node_activations
    ADD CONSTRAINT fk_v3_activation_winning_attempt
...
CREATE TABLE execution_events (
    ...
    projection_version_after BIGINT NOT NULL CHECK (projection_version_after >= 0),
...
CREATE INDEX idx_v3_execution_events_activation
    ON execution_events(run_id, activation_id, seq) WHERE activation_id IS NOT NULL;
...
CREATE TABLE projection_checkpoints (
    ...
    checkpoint_schema_version INTEGER NOT NULL CHECK (checkpoint_schema_version >= 1),
    subject_projection_version BIGINT NOT NULL CHECK (subject_projection_version >= 0),
...
CREATE TABLE scheduler_checkpoints (
    ...
    event_id TEXT NOT NULL,
    checkpoint_schema_version INTEGER NOT NULL CHECK (checkpoint_schema_version >= 1),
    scheduler_projection_version BIGINT NOT NULL CHECK (scheduler_projection_version >= 1),
...
CREATE INDEX idx_v3_task_outbox_dispatch
    ON task_outbox(available_at, run_id, task_id)
    WHERE task_state = 'pending';
CREATE INDEX idx_v3_task_outbox_reclaim
    ON task_outbox(claim_expires_at, run_id, task_id)
...
CREATE UNIQUE INDEX uq_v3_public_terminal_per_run
    ON public_event_outbox(run_id) WHERE is_terminal;
...
ALTER TABLE workflow_runs
    ADD CONSTRAINT fk_v3_run_terminal_public_event
...
CREATE TABLE timers (
    ...
    expected_lease_epoch BIGINT CHECK (expected_lease_epoch IS NULL OR expected_lease_epoch >= 1),
...
ALTER TABLE node_activations
    ADD CONSTRAINT fk_v3_activation_retry_timer
...
CREATE TABLE scheduler_wait_registrations (
    ...
    signal_id TEXT,
    timer_id TEXT,
    due_at_ms BIGINT CHECK (due_at_ms IS NULL OR due_at_ms >= 0),
    payload_type JSONB,
...
CREATE INDEX idx_v3_control_tokens_available
    ON control_tokens(run_id, current_scope_instance_id, current_port_id, token_id)
...
ALTER TABLE control_tokens
    ADD CONSTRAINT fk_v3_token_fork_leg
...
CREATE TABLE join_arrivals (
    ...
    CHECK (
        ...
        (value_hash IS NOT NULL
            AND ((value_payload_id IS NOT NULL)::integer
                + (value_artifact_id IS NOT NULL)::integer) = 1)
    )
);
...
-- PostgreSQL repositories allocate one sequence with an atomic
-- UPDATE workflow_runs SET next_event_seq = next_event_seq + 1 ...
-- RETURNING next_event_seq - 1 AS seq,
-- then append the event, projection mutations, and outbox rows in that same transaction.
