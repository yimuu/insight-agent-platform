...
CREATE FUNCTION durable_v3_reject_definition_public_metadata_mutation()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE TRIGGER trg_v3_definition_public_metadata_immutable
    BEFORE UPDATE OR DELETE ON workflow_definition_public_metadata
    FOR EACH ROW EXECUTE FUNCTION durable_v3_reject_definition_public_metadata_mutation();
...
CREATE FUNCTION durable_v3_validate_publication_head()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE TRIGGER trg_v3_publication_head_agent_matches_definition
    BEFORE INSERT OR UPDATE ON agent_publication_heads
    FOR EACH ROW EXECUTE FUNCTION durable_v3_validate_publication_head();
