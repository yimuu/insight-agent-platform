...
## Re-review fixes: redirect safety

Addressed the redirect bypass in `src/tools/http_get.rs`.

### Changes made

1. Disabled automatic redirects in the `reqwest` client
   - client builder now uses `reqwest::redirect::Policy::none()`
   - this prevents an initially valid HTTPS URL from being transparently redirected to:
     - a non-HTTPS target
     - a host outside the optional allowlist

2. Added explicit redirect-response rejection
   - `http_get` now treats any 3xx response as a blocked redirect
   - returned error is sanitized:
     - `http_get redirect blocked`
   - no response location or URL content is included in the error

3. Preserved phase-scoped default behavior
   - `default_tool_registry()` still registers `HttpGetTool::default()`
   - that keeps the plan/spec-mandated default of “allow all HTTPS hosts unless a deployment constructs the tool with an allowlist”
   - no platform-level default allowlist was added in this phase
...
```bash
cargo test redirect_status_is_rejected_with_sanitized_error
```

Initial failure:

- missing `HttpGetTool::validate_response_status`, confirming the redirect-response guard did not yet exist

Because `reqwest` does not expose client redirect policy for direct inspection, I added a code-level unit around redirect response handling and enforced redirect blocking in client construction via `Policy::none()`.
...
- `cargo test tools::http_get::tests`: 5 passed, 0 failed
- `cargo test --test runner`: 9 passed, 0 failed
- `cargo check`: passed
