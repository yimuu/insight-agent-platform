...
if [[ -z "$candidate" || -z "$output" ]]; then
  echo "PLATFORM_CANDIDATE_MANIFEST and PLATFORM_QUALIFICATION_OUTPUT_DIR are required" >&2
  exit 2
fi
if [[ ! -f "$profile" || ! -f "$candidate" ]]; then
  echo "qualification profile and candidate manifest must be readable files" >&2
  exit 2
fi
if [[ -e "$output" ]]; then
