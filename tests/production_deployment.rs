...
fn register_model_worker(
    ...
    workers
        .register(
            ...
            Arc::new(LlmTaskExecutor::new(models.clone())),
        )
        .unwrap();
}
...
"#,
    ...
    let (admin, schema, repository) =
        postgres_repository(&database_url, "deployment_archive").await;
    let artifact_store =
        shared_artifact_store(temporary.path().join("artifacts"), "deployment-archive").await;
...
"#,
    ...
    let actions = ActionRegistry::default();
    let without_adapters = DeployedAgent::publish(
        Arc::clone(&published),
    ...
    let rejected = DeployedAgent::publish(
        Arc::new(compile_agent_dir(&rejected_root).unwrap()),
        &ProductionLeafDeploymentResolver::new(&models, &actions)
            .with_external_leaf_adapters(&adapters),
        SubflowContractRegistry::new(),
    )
    ...
    let (admin, schema, repository) = postgres_repository(&database_url, "external_leaf").await;
    let artifact_store =
        shared_artifact_store(temporary.path().join("artifacts"), "external-leaf").await;
