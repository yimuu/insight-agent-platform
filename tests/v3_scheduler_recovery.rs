...
const DEPENDENCY_CHAIN_AGENT: &str = r#"api_version: insight.agent/v3
kind: agent
inputs:
  question: string
output: string
...
const DYNAMIC_MAP_AGENT: &str = r#"api_version: insight.agent/v3
kind: agent
inputs:
  items: string[]
output: string[]
...
const DYNAMIC_LOOP_AGENT: &str = r#"api_version: insight.agent/v3
kind: agent
inputs: {seed: string}
output: string
workflow:
...
fn compile_subflow_fixture() -> (
    ...
    let child_source = r#"api_version: insight.agent/v3
kind: agent
...
async fn isolated_postgres() -> Option<(PostgresDurableRepository, PgPool, PgPool, String, String)>
{
    ...
    let schema = format!("scheduler_recovery_v3_{}", Uuid::new_v4().simple());
