...
const LINEAR_AGENT: &str = r#"api_version: insight.agent/v3
kind: agent
inputs:
  question: string
output: string
...
const LARGE_ARTIFACT_AGENT: &str = r#"api_version: insight.agent/v3
kind: agent
inputs:
  question: string
output: any
...
const PARALLEL_FAILURE_AGENT: &str = r#"api_version: insight.agent/v3
kind: agent
inputs:
  question: string
output: any
...
const PARALLEL_ALL_SETTLED_AGENT: &str = r#"api_version: insight.agent/v3
kind: agent
inputs:
  question: string
output: any
...
const NESTED_PARALLEL_AGENT: &str = r#"api_version: insight.agent/v3
kind: agent
inputs:
  question: string
output: any
...
const BRANCH_AGENT: &str = r#"api_version: insight.agent/v3
kind: agent
inputs:
  ready: boolean
output: string
...
async fn isolated_repository() -> Option<(PostgresDurableRepository, PgPool, PgPool, String)> {
    ...
    let schema = format!("scheduler_v3_{}", Uuid::new_v4().simple());
...
fn real_crash_child_process(
    ...
    child
        ...
        .arg("--nocapture")
        .arg("--test-threads=1")
        .env("V3_REAL_CRASH_CHILD", "1")
        .env("V3_REAL_CRASH_SCOPED_URL", scoped_url)
        .env("V3_REAL_CRASH_RUN_ID", run_id.as_str())
        .env("V3_REAL_CRASH_MODE", mode)
        .env("V3_REAL_CRASH_POINT", crash_point_name(crash_point));
    if let Some(artifact_root) = artifact_root {
        child.env("V3_REAL_CRASH_ARTIFACT_ROOT", artifact_root);
    }
...
async fn postgres_real_process_crash_child() {
    if std::env::var("V3_REAL_CRASH_CHILD").ok().as_deref() != Some("1") {
        return;
    }
    let scoped_url = std::env::var("V3_REAL_CRASH_SCOPED_URL").unwrap();
    let run_id = RunId::new(std::env::var("V3_REAL_CRASH_RUN_ID").unwrap()).unwrap();
    let mode = std::env::var("V3_REAL_CRASH_MODE").unwrap();
    let crash_point = parse_crash_point(&std::env::var("V3_REAL_CRASH_POINT").unwrap());
    let repository = PostgresDurableRepository::connect(&scoped_url)
        .await
        .unwrap();
    let control = PgPoolOptions::new()
    ...
    match mode.as_str() {
        ...
        "artifact_result" => {
            ...
            let store = LocalContentAddressedArtifactStore::open(
                PathBuf::from(std::env::var("V3_REAL_CRASH_ARTIFACT_ROOT").unwrap()),
                1,
            )
            .await
            .unwrap();
...
async fn postgres_real_process_crash_matrix_recovers_from_durable_state() {
    ...
    let database_url = std::env::var("V3_TEST_POSTGRES_URL").unwrap();
    let separator = if database_url.contains('?') { '&' } else { '?' };
    let scoped_url = format!("{database_url}{separator}options=-csearch_path%3D{schema}");
    sqlx::query(
