...
#[path = "compiled_agent_fixture.rs"]
mod compiled_agent_fixture;
use compiled_agent_fixture::{compile_agent_apply_fixture, fixture_schema_documents};

#[path = "combined_agent_journey.rs"]
mod combined_agent_journey;
...
fn runtime_binary_path(project: &Path, binary: &str) -> std::path::PathBuf {
    ...
    let directory = if release_identity.starts_with("source:") {
        let output = Command::new("cargo")
            ...
            .output()
            .expect("Cargo target metadata");
        assert!(
            output.status.success() && output.stdout.len() <= 8_388_608,
            "Cargo target metadata must be bounded and successful"
        );
        let metadata: Value = serde_json::from_slice(&output.stdout).expect("Cargo target JSON");
        let target = Path::new(
        ...
        assert!(
            target.is_absolute(),
            "Cargo owns the actual absolute target directory"
        );
        target.join("release")
...
fn public_cli_deterministic_first_run() {
    ...
    native_artifact_cors::verify(insight, project);
    native_cli_authoring::verify(insight, project);
    ...
    let plan = json!({
        "plan_version": 6,
        "schema_documents": fixture_schema_documents(&[&closed_schema]),
    ...
    compile_agent_apply_fixture(insight, project, fixture.path(), &plan, &mut agent_manifest);
    ...
    assert_eq!(
        ...
        agent_manifest["publish"]["plan_content_digest"]
    );
    ...
    let mut replacement = restart_orchestration_worker(insight, project);
    let watched = run_json_lines(
        insight,
        &[
            "run",
    ...
    product_read_probe::run(project, run_id);
