...
pub(super) fn run(
    ...
    let plan = json!({
        "plan_version": 6,
        "schema_documents": fixture_schema_documents(&[&agent_manifest["create"]["document"]["spec"]["input_schema"]]),
            ...
            let mut claimed = transaction
                .claim_orchestration_jobs(ClaimOrchestrationJobs {
                    worker_manifest: insight_platform_contracts::WorkerManifest {
                        manifest_version: insight_platform_contracts::WORKER_MANIFEST_VERSION, worker_role: "orchestration-worker".to_owned(), work_class: insight_platform_contracts::WorkClass::Orchestration,
                        adapter_runtime_digest: format!("sha256:{}", "a".repeat(64)).parse().unwrap(), worker_build_digest: format!("sha256:{}", "b".repeat(64)).parse().unwrap(),
                        execution_capabilities: insight_platform_plan::execution::program_execution_capabilities(), protocol_version: insight_platform_contracts::WORKER_PROTOCOL_VERSION, max_concurrency: 2, critical_control_reserved_slots: 1,
                    },
