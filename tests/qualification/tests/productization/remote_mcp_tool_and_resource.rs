...
fn install_exact_mcp_codec(
    ...
    let mut execution = insight_platform_plan::execution::program_execution_capabilities();
    for (kind, field) in [
        ("http", "installed_http_codecs"),
        ("grpc", "installed_grpc_codecs"),
        ("mcp", "installed_mcp_codecs"),
    ] {
        for codec in config[field]
            .as_array()
            .expect("closed installed codec directory")
        {
            let installed: insight_platform_contracts::InstalledCapabilityCodecRef = serde_json::from_value(json!({
                "schema_version": 1, "backend_kind": kind,
                "codec_id": codec["codec_id"], "codec_version": codec["codec_version"],
                "module_digest": codec["module_digest"], "worker_protocol_version": codec["worker_protocol_version"],
                "descriptor_digest": codec["descriptor_digest"],
            })).expect("fixture codec has an owning installed descriptor");
            let capability =
                insight_platform_contracts::remote_capability_adapter_execution_capability(
                    &installed,
                )
                .expect("fixture codec has the exact installed execution identity");
            if !execution.capabilities.contains(&capability) {
                execution.capabilities.push(capability);
            }
        }
    }
    execution
        .validate()
        .expect("fixture manifest has a closed current capability catalog");
    config["worker_manifest"]["execution_capabilities"] = serde_json::to_value(execution).unwrap();
...
fn publish_agent(
    ...
    let plan = json!({
        "plan_version": 6, "schema_documents": {}, "interface_contract_digest": contract_digest, "entry_node_id": "start",
    ...
    super::compile_agent_apply_fixture(insight, project, fixture, &plan, &mut manifest);
