    ...
    pub(super) fn report(&self, revision: &str) -> Value {
        ...
        json!({
            ...
            "assertions": [
                ...
                check("inline_result", "passed", &self.combined_journey.as_ref().map(|proof| format!("the Model Run returned its typed Inline result; combined public Run {} also completed retrieval, actual model tool exchange, durable approval {} across orchestration process restart, and child {}, preserving the typed result without redispatch; raw proof is in runtime/logs/combined-agent-journey.json", proof["run_id"], proof["task_id"], proof["child_run_id"])).unwrap_or_else(|| "the terminal Run returned the schema-checked deterministic structured Inline result".to_owned())),
...
fn closed_schema(schema: Value) -> Value {
    serde_json::to_value(
        insight_platform_contracts::ClosedJsonSchema::build(schema)
            .expect("fixture uses the owning closed schema profile"),
    )
    .unwrap()
}
...
pub(super) fn run(
    ...
    compile_agent_apply_fixture(insight, project, fixture, &plan, &mut agent_manifest);
    ...
    let combined_journey = (feature_enabled("context") && feature_enabled("remote-capability"))
        .then(|| {
            combined_agent_journey::run(insight, project, fixture, &agent_manifest, replacement)
        });
    drop(remote);
    ExactModelEvidence {
        combined_journey,
