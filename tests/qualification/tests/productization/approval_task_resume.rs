...
pub(super) fn run(
    ...
    compile_agent_apply_fixture(
        insight,
        project,
        fixture,
        &human_task_plan,
        &mut human_task_manifest,
    );
...
fn run_real_gateway_console_journey(
    ...
    let profile: Value = serde_json::from_slice(
        &fs::read(project.join(".insight/runtime/profile.json")).expect("actual runtime profile"),
    )
    .expect("typed runtime profile JSON");
    let management_port = profile["ports"]["gateway_management"]
        .as_u64()
        .and_then(|port| u16::try_from(port).ok())
        .expect("actual Management Gateway port");
    ...
    let output = Command::new(node)
        ...
        .env("INSIGHT_CONSOLE_AUTHORING_JOURNEY", "1")
