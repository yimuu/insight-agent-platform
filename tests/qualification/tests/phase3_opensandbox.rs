#[path = "support/sandbox_scan_isolation.rs"]
mod sandbox_scan_isolation;
mod support;

static SANDBOX_RECOVERY_FIXTURE_LOCK: tokio::sync::Mutex<()> = tokio::sync::Mutex::const_new(());
...
use insight_platform_sandbox::contracts::{
    AuthorizeCandidateCreateV1, AuthorizeSandboxActivationV1, ClaimedSandboxCleanupV1,
    CommitSandboxTerminalV1, OpaqueActivationToken, OpenSandboxId, PhysicalDecision,
    ReconcileSandboxControlsV1, RecordProvisioningIntentV1, RecordSandboxCleanupObservationV1,
    RecordSandboxObservationV1, RunnerBootId, SandboxCandidateMetadataV1,
    SandboxCandidatePurposeV1, SandboxCandidateV1, SandboxClaimV1, SandboxCleanupClaimV1,
    SandboxCleanupObservationV1, SandboxControlKindV1, SandboxDispatcherJobPayloadV1,
    SandboxDurableObservationV1, SandboxExecutionPlanV1, SandboxExecutionRequestV1,
    SandboxFailureClassV1, SandboxFencedIdentityV1, SandboxJobRepository, SandboxNetworkMode,
    SandboxOrphanDispositionV1, SandboxPhysicalPhaseV1, SandboxProvisioningLimitsV1,
    SandboxProvisioningTokenV1, SandboxResourceLimitsV1, SandboxRunnerOutcomeV1,
    SandboxRunnerPhaseV1, SandboxRunnerResultFrameV1, SandboxRunnerStateFrameV1,
    SandboxTerminalOutcomeV1, SelectSandboxCandidateV1,
};
use serde::Serialize;
...
async fn opensandbox_shared_job_is_fenced_atomic_and_recoverable() {
    ...
    let Ok(database_url) = std::env::var("PLATFORM_TEST_DATABASE_URL") else {
        panic!("PLATFORM_TEST_DATABASE_URL is unset; real OpenSandbox PostgreSQL L2 requires its declared fixture environment");
    };
    ...
    for _ in 0..520 {
        let (first, second) = tokio::join!(
            SandboxJobRepository::claim(&fixture.repository, claim_one.clone()),
            SandboxJobRepository::claim(&fixture.repository, claim_two.clone()),
        );
        batches = [first.unwrap(), second.unwrap()];
    ...
    *evidence_digest = digest('c');
    ...
    let cleanup = claim_expected_cleanup(
        &fixture.repository,
        &cleanup_process,
        &fixture.request.tenant_id,
        &fixture.request.job_id,
    ...
    .await;
    let cleanup_replay = claim_expected_cleanup(
        &fixture.repository,
        &cleanup_process,
        &fixture.request.tenant_id,
        &fixture.request.job_id,
...
async fn claim_expected_cleanup(
    repository: &PgRepository,
    process_generation_id: &ResourceId,
    tenant_id: &ResourceId,
    job_id: &ResourceId,
) -> ClaimedSandboxCleanupV1 {
    let mut after = None;
    for _ in 0..512 {
        let page = SandboxJobRepository::claim_cleanup(
            repository,
        ...
        if let Some(claimed) = page
            .records
            .into_iter()
            .find(|claim| (&claim.fence.tenant_id, &claim.fence.job_id) == (tenant_id, job_id))
        {
            return claimed;
        }
        assert!(
            !page.exhausted,
            "expected cleanup was absent from one complete finite sweep"
        );
    }
    panic!("cleanup fixture exceeded its bounded page budget")
}

async fn complete_expected_cleanup(
    repository: &PgRepository,
    tenant_id: &ResourceId,
    job_id: &ResourceId,
    mut claimed: ClaimedSandboxCleanupV1,
) {
    while claimed.payload.cleanup.required {
        assert_eq!(
            (&claimed.fence.tenant_id, &claimed.fence.job_id),
            (tenant_id, job_id),
            "a qualification test must only complete cleanup for the job it created"
        );
        let physical = claimed
            .payload
            .physical
            .as_deref()
            .expect("cleanup claim has physical evidence");
        let sandbox_id = physical
            .candidate_ids
            .iter()
            .find(|candidate| !physical.absent_candidate_ids.contains(candidate))
            .cloned()
            .expect("required cleanup has an unobserved candidate");
        claimed = SandboxJobRepository::record_cleanup_observation(
            repository,
...
async fn opensandbox_boot_rollover_is_durable_unknown_outcome() {
    let _fixture_guard = SANDBOX_RECOVERY_FIXTURE_LOCK.lock().await;
    let Ok(database_url) = std::env::var("PLATFORM_TEST_DATABASE_URL") else {
        panic!("PLATFORM_TEST_DATABASE_URL is unset; OpenSandbox boot rollover L2 requires its declared fixture environment");
    };
    let pool = PgPoolOptions::new()
        .max_connections(8)
        .connect(&database_url)
        ...
        .unwrap();
    verify_schema(&pool).await.unwrap();
    let (fixture, _) = prepare_boot_rollover_unknown_outcome(pool, "sandbox-boot-rollover").await;
    let process_generation_id = id(ResourceKind::WorkerProcessGeneration);
    let cleanup = claim_expected_cleanup(
        &fixture.repository,
    ...
    .await;
    let recovered = SandboxJobRepository::recover(
        &fixture.repository,
        &fixture.request.tenant_id,
        &fixture.request.job_id,
    )
    .await
    .unwrap();
    assert!(!recovered.payload.cleanup.required);
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn opensandbox_cleanup_claim_replays_live_owner_fence_across_pages() {
    let _fixture_guard = SANDBOX_RECOVERY_FIXTURE_LOCK.lock().await;
    let Ok(database_url) = std::env::var("PLATFORM_TEST_DATABASE_URL") else {
        panic!("PLATFORM_TEST_DATABASE_URL is unset; cleanup replay L2 requires its declared fixture environment");
    };
    let pool = PgPoolOptions::new()
        .max_connections(8)
        .connect(&database_url)
    ...
    let (first_fixture, _) =
        prepare_boot_rollover_unknown_outcome(pool.clone(), "cleanup-priority-a").await;
    tokio::time::sleep(std::time::Duration::from_millis(10)).await;
    let (second_fixture, _) =
        prepare_boot_rollover_unknown_outcome(pool, "cleanup-priority-b").await;
    let process_generation_id = id(ResourceKind::WorkerProcessGeneration);
    let first = claim_expected_cleanup(
        &first_fixture.repository,
        &process_generation_id,
        &first_fixture.request.tenant_id,
        &first_fixture.request.job_id,
    )
    .await;

    let first_updated_at: DateTime<Utc> = sqlx::query_scalar(
        "SELECT updated_at FROM insight_platform.jobs WHERE tenant_id = $1 AND job_id = $2",
    )
    .bind(first_fixture.request.tenant_id.to_string())
    .bind(first_fixture.request.job_id.to_string())
    .fetch_one(&first_fixture.pool)
    .await
    .unwrap();
    let second_updated_at: DateTime<Utc> = sqlx::query_scalar(
        "SELECT updated_at FROM insight_platform.jobs WHERE tenant_id = $1 AND job_id = $2",
    )
    .bind(second_fixture.request.tenant_id.to_string())
    .bind(second_fixture.request.job_id.to_string())
    .fetch_one(&second_fixture.pool)
    .await
    .unwrap();
    assert!(
        first_updated_at > second_updated_at,
        "a cleanup claim updates physical state without changing its immutable scan position"
    );
    let unowned_second = SandboxJobRepository::recover(
        &second_fixture.repository,
        &second_fixture.request.tenant_id,
        &second_fixture.request.job_id,
    )
    .await
    .unwrap();
    assert!(unowned_second.payload.cleanup.required);
    assert!(unowned_second
        .payload
        .cleanup
        .owner_process_generation_id
        .is_none());

    let replay = claim_expected_cleanup(
        &first_fixture.repository,
        &process_generation_id,
        &first_fixture.request.tenant_id,
        &first_fixture.request.job_id,
    ...
    .await;
    assert_eq!(
        replay.fence, first.fence,
        "the same process and build recover their exact live fence during a finite sweep"
    );
    ...
    .await;
    let second = claim_expected_cleanup(
        &second_fixture.repository,
        &process_generation_id,
        &second_fixture.request.tenant_id,
        &second_fixture.request.job_id,
...
async fn claim_after_partition_rounds(
    repository: &PgRepository,
    request: SandboxClaimV1,
) -> Result<Vec<insight_platform_sandbox::contracts::LeasedSandboxJobV1>, RepositoryError> {
    for _ in 0..520 {
        let batch = SandboxJobRepository::claim(repository, request.clone()).await?;
        if !batch.is_empty() {
            return Ok(batch);
        }
        tokio::task::yield_now().await;
...
async fn exercise_lease_rollover_and_stale_result(pool: PgPool) {
    ...
    let leased = claim_after_partition_rounds(&fixture.repository, first_claim)
        .await
        .unwrap()
        .pop()
        .unwrap();
    ...
    let continued =
        claim_after_partition_rounds(&fixture.repository, claim(&second_worker, digest('7')))
            .await
            .unwrap()
            .pop()
            .unwrap();
...
async fn exercise_cancel_timeout_and_quota(pool: PgPool) {
    ...
    let leased = claim_after_partition_rounds(
    ...
    let cancelled = reconcile_expected_control(&cancelled_fixture).await;
    ...
    let timed_out = reconcile_expected_control(&timeout_fixture).await;
...
async fn reconcile_expected_control(
    fixture: &Fixture,
) -> insight_platform_sandbox::contracts::SandboxRepositoryDecisionV1 {
    let mut after = None;
    for _ in 0..512 {
        let page = SandboxJobRepository::reconcile_controls(
            &fixture.repository,
            ReconcileSandboxControlsV1 { after, limit: 1 },
        )
        .await
        .unwrap();
        after = page.next_cursor;
        if let Some(decision) = page.records.into_iter().find(|decision| {
        ...
        assert!(
            !page.exhausted,
            "expected control was absent from one complete finite sweep"
        );
    }
    panic!("control fixture exceeded its bounded page budget")
}
...
async fn prepare_boot_rollover_unknown_outcome(
    pool: PgPool,
    sandbox_name: &str,
) -> (Fixture, SandboxCandidateV1) {
    let fixture = seed_fixture(pool).await;
    let leased = claim_after_partition_rounds(
        &fixture.repository,
    ...
    .into_inner()
    .decision;
    let candidate = candidate(&fixture.request, sandbox_name);
    let observed = SandboxJobRepository::record_physical_observation(
        &fixture.repository,
        RecordSandboxObservationV1 {
...
async fn opensandbox_kubernetes_l3_dispatcher_kill_reclaims_same_started_runner() {
    if std::env::var("PLATFORM_OPENSANDBOX_L3_DISPATCHER").as_deref() != Ok("1") {
        panic!(
            "PLATFORM_OPENSANDBOX_L3_DISPATCHER is unset; real Dispatcher kill/reclaim L3 requires its declared fixture environment"
        );
    }
...
async fn opensandbox_kubernetes_l3_running_cancel_intent_survives_dispatcher_exit() {
    if std::env::var("PLATFORM_OPENSANDBOX_L3_CONTROL_PHASE").as_deref() != Ok("cancel-intent") {
        panic!(
            "PLATFORM_OPENSANDBOX_L3_CONTROL_PHASE=cancel-intent is unset; real control L3 requires its declared fixture environment"
        );
    }
    ...
    if std::env::var("PLATFORM_OPENSANDBOX_L3_ABORT_AFTER_INTENT").as_deref() == Ok("1") {
        let marker_path = std::env::var("PLATFORM_OPENSANDBOX_L3_ABORT_MARKER")
            .expect("the abrupt-exit qualification requires a run-scoped marker path");
        let marker = serde_jcs::to_vec(&json!({
            "schema_version": 1,
            "kind": "insight.productization.opensandbox-abort-marker/v1",
            "phase": "cancel-intent",
            "job_id": fixture.request.job_id.to_string(),
            "sandbox_id": sandbox_id.as_str(),
        }))
        .expect("the abrupt-exit marker is canonicalizable");
        let mut marker_file = OpenOptions::new()
            .write(true)
            .create_new(true)
            .open(Path::new(&marker_path))
            .expect("the abrupt-exit marker is created exactly once");
        marker_file
            .write_all(&marker)
            .and_then(|_| marker_file.sync_all())
            .expect("the abrupt-exit marker is durable before abort");
        std::process::abort();
    }
}
...
async fn opensandbox_kubernetes_l3_cancel_terminal_commits_while_server_is_unavailable() {
    if std::env::var("PLATFORM_OPENSANDBOX_L3_CONTROL_PHASE").as_deref() != Ok("cancel-terminal") {
        panic!(
            "PLATFORM_OPENSANDBOX_L3_CONTROL_PHASE=cancel-terminal is unset; real control L3 requires its declared fixture environment"
        );
    }
    let pool = l3_pool().await;
    verify_schema(&pool).await.unwrap();
    ...
    let terminal = 'reconcile: loop {
        ...
        .await
        .unwrap();
        control_after = page.next_cursor;
...
async fn opensandbox_kubernetes_l3_cancel_cleanup_resumes_after_server_recovery() {
    if std::env::var("PLATFORM_OPENSANDBOX_L3_CONTROL_PHASE").as_deref() != Ok("cancel-cleanup") {
        panic!(
            "PLATFORM_OPENSANDBOX_L3_CONTROL_PHASE=cancel-cleanup is unset; real control L3 requires its declared fixture environment"
        );
    }
    let pool = l3_pool().await;
    verify_schema(&pool).await.unwrap();
...
async fn opensandbox_kubernetes_l3_running_deadline_terminal_and_cleanup() {
    if std::env::var("PLATFORM_OPENSANDBOX_L3_CONTROL_PHASE").as_deref() != Ok("timeout") {
        panic!(
            "PLATFORM_OPENSANDBOX_L3_CONTROL_PHASE=timeout is unset; real control L3 requires its declared fixture environment"
        );
    }
    let pool = l3_pool().await;
    verify_schema(&pool).await.unwrap();
    ...
    let terminal = loop {
        ...
        .await
        .unwrap();
        control_after = reconciled.next_cursor;
        if let Some(decision) = reconciled.records.into_iter().find(|decision| {
            decision.job.tenant_id == fixture.request.tenant_id
                && decision.job.job_id == fixture.request.job_id
        }) {
            break decision;
...
async fn cleanup_l3_control_job(
    ...
    loop {
        ...
        .await
        .unwrap();
        cleanup_after = claims.next_cursor;
        for mut claim in claims
            .records
            .into_iter()
            .filter(|claim| (&claim.fence.tenant_id, &claim.fence.job_id) == (tenant_id, job_id))
        {
            loop {
                assert_eq!(
                    (&claim.fence.tenant_id, &claim.fence.job_id),
                    (tenant_id, job_id),
                    "an L3 cleanup fixture must not consume another job's durable cleanup"
                );
...
async fn seed_fixture_with(pool: PgPool, options: FixtureOptions) -> Fixture {
    ...
    let execution_requirement = insight_platform_plan::execution::program_execution_requirement(
        value_digest(&json!({ "fixture": "sandbox-controller-contract", "definition_version": 1 })),
        insight_platform_plan::execution::PROGRAM_IR_ABI_V6,
    )
    .unwrap();
    let execution_requirement_digest = execution_requirement.canonical_digest().unwrap();
    sqlx::query(
        r#"
...
fn deferred_invocation(
    ...
    let mut admission = CapabilityAdmissionSnapshot {
        schema_version: 2,
        adapter_execution_requirement: None,
...
fn identity(
    leased: &insight_platform_sandbox::contracts::LeasedSandboxJobV1,
...
fn decision_identity(
    decision: &insight_platform_sandbox::contracts::SandboxRepositoryDecisionV1,
) -> SandboxFencedIdentityV1 {
    SandboxFencedIdentityV1 {
