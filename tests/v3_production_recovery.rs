...
fn mapped_migration_catalog(root: &std::path::Path) -> DeployedAgentCatalog {
    ...
    let published = compile_enabled_v3_agents(root, &enabled).unwrap();
...
async fn postgres_formal_migrate_derives_cross_revision_mapping_and_reuses_renamed_leaf() {
    ...
    let admin = PgPoolOptions::new()
        ...
        .await
        .unwrap();
    let schema = format!("production_mapped_migrate_v3_{}", Uuid::new_v4().simple());
...
async fn postgres_service_redrive_migrate_and_continue_preserve_durable_lineage() {
    ...
    let admin = PgPoolOptions::new()
        ...
        .await
        .unwrap();
    let schema = format!("production_recovery_v3_{}", Uuid::new_v4().simple());
    sqlx::query(AssertSqlSafe(format!("CREATE SCHEMA {schema}")))
        .execute(&admin)
        .await
        .unwrap();
...
async fn postgres_stale_runtime_migrate_uses_the_durable_target_head() {
    ...
    let admin = PgPoolOptions::new()
        ...
        .unwrap();
    let schema = format!(
        "production_migration_durable_head_v3_{}",
        Uuid::new_v4().simple()
    );
    sqlx::query(AssertSqlSafe(format!("CREATE SCHEMA {schema}")))
        .execute(&admin)
...
async fn postgres_service_migrate_replays_frozen_pending_intent_after_alias_advance() {
    ...
    let admin = PgPoolOptions::new()
        ...
        .unwrap();
    let schema = format!(
        "production_migration_crash_replay_v3_{}",
        Uuid::new_v4().simple()
    );
    sqlx::query(AssertSqlSafe(format!("CREATE SCHEMA {schema}")))
        .execute(&admin)
...
async fn postgres_public_redrive_and_selected_fork_reuse_override_and_cas_are_durable() {
    ...
    let admin = PgPoolOptions::new()
        ...
        .await
        .unwrap();
    let schema = format!("production_reuse_v3_{}", Uuid::new_v4().simple());
    sqlx::query(AssertSqlSafe(format!("CREATE SCHEMA {schema}")))
        .execute(&admin)
        .await
        .unwrap();
