...
use insight_agent_platform::{
    ...
    dsl::{
        v3::{
            CompileOptions, GraphAuthorDocument, GraphDocumentId, GraphSemanticEdit,
...
async fn postgres_graph_author_product_contract_matches_sqlite() {
    ...
    let schema = format!("graph_product_v3_{}", Uuid::new_v4().simple());
...
async fn postgres_live_runtimes_admit_new_runs_from_the_durable_graph_head() {
    ...
    let schema = format!("graph_route_v3_{}", &suffix[..16]);
    let admin = PgPoolOptions::new()
        .max_connections(2)
        .connect(&database_url)
        .await
...
async fn postgres_subflow_publication_rejects_stale_snapshot_and_keeps_old_parent_frozen() {
    ...
    let suffix = Uuid::new_v4().simple().to_string();
    let schema = format!("graph_publication_cas_v3_{}", &suffix[..16]);
    let admin = PgPoolOptions::new()
        .max_connections(2)
        .connect(&database_url)
        .await
...
async fn postgres_stale_runtime_restores_exact_subflow_closure_and_takes_over_run() {
    ...
    let suffix = Uuid::new_v4().simple().to_string();
    let schema = format!("graph_takeover_v3_{}", &suffix[..16]);
    let admin = PgPoolOptions::new()
        .max_connections(2)
        .connect(&database_url)
        .await
