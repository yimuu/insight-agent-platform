...
"#,
        ...
        .unwrap();
        let parallel = Arc::new(compile_agent_dir(&parallel_directory).unwrap());
        deployed.push(Arc::new(
            DeployedAgent::publish(
                parallel,
                &publication_resolver,
...
"#,
        ...
        .unwrap();
        let loop_agent = Arc::new(compile_agent_dir(&loop_directory).unwrap());
        deployed.push(Arc::new(
            DeployedAgent::publish(
                loop_agent,
                &publication_resolver,
...
"#,
        ...
        .unwrap();
        let wait = Arc::new(compile_agent_dir(&wait_directory).unwrap());
        deployed.push(Arc::new(
            DeployedAgent::publish(wait, &publication_resolver, SubflowContractRegistry::new())
                .unwrap(),
        ));
