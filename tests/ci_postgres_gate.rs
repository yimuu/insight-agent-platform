...
async fn ci_requires_the_shared_real_postgres_16_gate() {
    ...
    let database_url = std::env::var("TEST_POSTGRES_URL")
        .expect("CI must set TEST_POSTGRES_URL for durable PostgreSQL gates");
    let artifact_url = std::env::var("TEST_POSTGRES_URL")
        .expect("CI must set TEST_POSTGRES_URL for artifact gates");
    ...
    assert_eq!(
        ...
        "all PostgreSQL gates must use the same authoritative CI service"
    );
    assert_eq!(
    ...
    assert!(
        ...
        "durable CI gate requires PostgreSQL 16, found {version}"
    );
}
