...
pub(crate) const INITIAL_BINARY_NAMES: [&str; 15] = [
...
pub(crate) struct PortBindings {
    ...
    pub(crate) outbox_observability: u16,
    pub(crate) history_observability: u16,
}
...
pub(crate) fn initial_configs(
    ...
    let model_manifest = worker_builds.manifest(
        "platform-model-worker",
        "model-worker",
        insight_platform_contracts::WorkClass::Model,
        digests.model_adapter,
        (4, 1),
        crate::worker_profile::model(digests.anthropic_contract, digests.openai_contract),
    );
...
pub(crate) fn initial_process_launches(
    ...
    .map(|mut launch| {
        if launch.role == "capability-remote" && !selected_profile.has_mcp() {
            launch
                .environment
