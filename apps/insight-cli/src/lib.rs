...
use insight_platform_deployment_tooling::tls::{
    native_identity_specs as expected_local_tls_leaf_identities, LocalTlsIdentitySpec,
    LocalTlsUsage, PUBLIC_GATEWAY_WORKLOAD_IDENTITY, RUNTIME_ARTIFACT_DATA_CERTIFICATE_FILE,
    RUNTIME_ARTIFACT_DATA_PRIVATE_KEY_FILE, RUNTIME_ARTIFACT_GATEWAY_CERTIFICATE_FILE,
    RUNTIME_ARTIFACT_GATEWAY_PRIVATE_KEY_FILE, RUNTIME_CA_CERTIFICATE_FILE,
    RUNTIME_CA_PRIVATE_KEY_FILE, RUNTIME_GATEWAY_CLIENT_CERTIFICATE_FILE,
    RUNTIME_GATEWAY_CLIENT_PRIVATE_KEY_FILE, RUNTIME_NATS_CLIENT_CERTIFICATE_FILE,
    RUNTIME_NATS_CLIENT_PRIVATE_KEY_FILE, RUNTIME_NATS_SERVER_CERTIFICATE_FILE,
    RUNTIME_NATS_SERVER_PRIVATE_KEY_FILE, RUNTIME_ORCHESTRATION_CLIENT_CERTIFICATE_FILE,
    RUNTIME_ORCHESTRATION_CLIENT_PRIVATE_KEY_FILE,
    RUNTIME_REGISTRY_VALIDATION_CLIENT_CERTIFICATE_FILE,
    RUNTIME_REGISTRY_VALIDATION_CLIENT_PRIVATE_KEY_FILE, SCHEDULER_WORKLOAD_IDENTITY,
};
...
mod agent;
#[cfg(test)]
mod agent_entry_tests;
mod apply;
mod apply_journal;
mod artifact;
mod artifact_journal;
mod history_profile;
mod local_artifact_cors;
mod outbox_profile;
use insight_platform_deployment_tooling::private_state;
mod public_client;
mod release;
mod run;
mod run_journal;
mod runtime_config_transition;
mod task;
mod task_journal;
#[cfg(test)]
mod workspace_assets;

use insight_platform_deployment_tooling::base_profile::local_digest;
use insight_platform_deployment_tooling::{
    base_runtime_binary_paths, dev_profile, full_profile, runtime_binary_paths, worker_profile,
    DevProfile,
};
...
use insight_platform_contracts::{
    canonical_digest, parse_strict_json, ArtifactPurpose, DataClassification, JsonLimits,
    PublicJobState, ResourceDocument, ResourceId, ResourceKind, RunState, UtcTimestamp, ValueRef,
};
use rcgen::{
    CertificateParams, ExtendedKeyUsagePurpose, Issuer, KeyPair, PublicKeyData, PKCS_RSA_SHA256,
};
#[cfg(test)]
use rcgen::{KeyUsagePurpose, SanType};
...
#[cfg(test)]
const LOCAL_ACCESS_TOKEN_TTL_SECONDS: i64 =
    insight_platform_deployment_contracts::installation::INSTALLATION_SESSION_SECONDS as i64;
...
pub enum CliCommand {
    ...
    TaskGet {
        root: PathBuf,
        task_id: String,
        purpose: insight_platform_tasks::TaskQueryPurpose,
    },
    TaskList {
        root: PathBuf,
        filters: insight_platform_api::task::TaskInboxFiltersV1,
        page_size: u16,
    ...
    AgentEvaluationReport {
        ...
        file: PathBuf,
        output: PathBuf,
        run_id: insight_platform_contracts::ResourceId,
    },
    AgentEvaluation {
        action: evaluation::EvaluationAction,
        root: PathBuf,
        file: PathBuf,
        output: PathBuf,
    },
    ...
    AgentDependencies {
        root: PathBuf,
        kind: insight_platform_contracts::DependencySlotKind,
...
impl std::fmt::Display for CliError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::ModelConfiguration(detail) => write!(formatter, "model configuration: {detail}"),
            Self::Usage => write!(
                formatter,
...
pub fn parse_command(arguments: &[OsString]) -> Result<CliCommand, CliError> {
    ...
    match command {
        "model" => model::parse(&arguments[1..]).map(CliCommand::Model),
        "connect" => connection::parse(&arguments[1..]).map(CliCommand::Connect),
        "qualification-aws" => parse_aws_qualification(&arguments[1..]),
        "help" | "--help" | "-h" => Ok(CliCommand::Help),
        "doctor" => parse_doctor(&arguments[1..]),
        "version" => parse_version(&arguments[1..]),
        "update" => parse_update(&arguments[1..]),
        "init" | "token" | "dev" | "start" | "status" | "logs" | "stop" | "reset" => {
            Ok(CliCommand::InstallationHelp)
        }
        "apply" => parse_apply(&arguments[1..]),
        "operation" => parse_operation(&arguments[1..]),
        "recovery" => recovery::parse(&arguments[1..]),
...
fn parse_aws_qualification(arguments: &[OsString]) -> Result<CliCommand, CliError> {
    let Some(command) = arguments.first().and_then(|value| value.to_str()) else {
        return Err(CliError::Usage);
    };
    match command {
...
fn parse_task(arguments: &[OsString]) -> Result<CliCommand, CliError> {
    ...
    if action == "list" || action == "get" {
        let start = if action == "get" { 2 } else { 1 };
        let mut values = std::collections::BTreeMap::<String, String>::new();
        let mut root = PathBuf::from(".");
        let mut offset = start;
        while offset < arguments.len() {
            let flag = arguments[offset].to_string_lossy().into_owned();
            if ![
                "--path",
                "--purpose",
                "--state",
            ...
            .contains(&flag.as_str())
                || (action == "get" && flag != "--path" && flag != "--purpose")
            {
                return Err(CliError::UnsupportedOption(flag));
            }
            let value = arguments.get(offset + 1).ok_or(CliError::Usage)?;
            if values
        ...
        let purpose = match values
            .get("--purpose")
            .map(String::as_str)
            .unwrap_or("respondable")
        {
            "respondable" => insight_platform_tasks::TaskQueryPurpose::Respondable,
            "viewable" => insight_platform_tasks::TaskQueryPurpose::Viewable,
        ...
        let filters = insight_platform_api::task::TaskInboxFiltersV1 {
            purpose,
            state: values
                .get("--state")
        ...
        let page_size = values
            ...
            .transpose()
            .map_err(|_| CliError::Usage)?
            .unwrap_or(insight_platform_api::product::PRODUCT_LIST_DEFAULT_PAGE_SIZE);
        ...
        if page_size == 0
            ...
            || filters
                .run_id
                .as_ref()
                .is_some_and(|id| id.kind() != insight_platform_contracts::ResourceKind::Run)
            || cursor.as_ref().is_some_and(|value| {
                value.is_empty()
                    || value.len() > insight_platform_contracts::MAX_OPAQUE_CURSOR_BYTES
                    || value.chars().any(char::is_control)
            })
        {
            return Err(CliError::Usage);
        }
        return Ok(CliCommand::TaskList {
            root,
            filters,
            page_size,
            cursor,
...
fn parse_agent(arguments: &[OsString]) -> Result<CliCommand, CliError> {
    ...
    match action {
        "signal" => agent_signal::parse(&arguments[1..]),
        "restore-source" => {
            let mut root = PathBuf::from(".");
            ...
            while at < arguments.len() {
                let flag = lossy(&arguments[at]);
                let value = arguments
                    .get(at + 1)
                    .and_then(|v| v.to_str())
                    .ok_or(CliError::Usage)?;
                match flag.as_str() {
                    "--path" => root = PathBuf::from(value),
                    "--agent" if agent_id.is_none() => {
                        agent_id = Some(
                            ResourceId::parse_expected(value, ResourceKind::Agent)
                                .map_err(|_| CliError::Usage)?,
                        )
                    }
                    "--version" if version_id.is_none() => {
                        version_id = Some(value.parse::<ResourceId>().map_err(|_| CliError::Usage)?)
                    }
                    "--output" if output.is_none() => output = Some(PathBuf::from(value)),
                    _ => return Err(CliError::UnsupportedOption(flag)),
                }
                at += 2;
            }
            Ok(CliCommand::AgentRestoreSource {
                root,
                agent_id: agent_id.ok_or(CliError::MissingValue("--agent"))?,
                version_id: version_id.ok_or(CliError::MissingValue("--version"))?,
                output: output.ok_or(CliError::MissingValue("--output"))?,
            })
        }
        "evaluation-report" => {
            ...
            while at < arguments.len() {
                ...
                match flag.as_str() {
                    ...
                    "--run" if run_id.is_none() => {
                        run_id = Some(
                            insight_platform_contracts::ResourceId::parse_expected(
                                value,
                                insight_platform_contracts::ResourceKind::Run,
                            )
                            .map_err(|_| CliError::Usage)?,
                        )
                    }
            ...
            Ok(CliCommand::AgentEvaluationReport {
                root,
                file: file.ok_or(CliError::MissingValue("--file"))?,
                output: output.ok_or(CliError::MissingValue("--output"))?,
                run_id: run_id.ok_or(CliError::MissingValue("--run"))?,
            })
        }
        "evaluation-init" | "evaluation-input" => {
            let mut root = PathBuf::from(".");
            let mut file = None;
            let mut output = None;
            let mut at = 1;
            ...
            Ok(CliCommand::AgentEvaluation {
                action: if action == "evaluation-init" {
                    evaluation::EvaluationAction::Init
                } else {
                    evaluation::EvaluationAction::Input
                },
                root,
                file: file.ok_or(CliError::MissingValue("--file"))?,
                output: output.ok_or(CliError::MissingValue("--output"))?,
        ...
        "dependencies" => {
            let kind = arguments
                ...
                .and_then(|v| v.to_str())
                .and_then(|v| {
                    v.parse::<insight_platform_contracts::DependencySlotKind>()
                        .ok()
                })
                .ok_or(CliError::Usage)?;
            let mut root = PathBuf::from(".");
            ...
            while at < arguments.len() {
                ...
                match flag.as_str() {
                    ...
                    "--environment" if environment.is_none() => {
                        environment = Some(value.to_owned())
                    }
                    "--cursor" if cursor_value.is_none() => cursor_value = Some(value.to_owned()),
                    _ => return Err(CliError::UnsupportedOption(flag)),
                };
                at += 2;
            }
            Ok(CliCommand::AgentDependencies {
                root,
                kind,
                environment,
                cursor: cursor_value,
...
struct RuntimeProfileSelection<'a> {
    preserved: Option<&'a runtime_config_transition::PreservedRuntimeInputs<'a>>,
    worker_builds: &'a worker_profile::WorkerBuilds,
...
fn initialize_local_identity(
    ...
    let jwks = insight_platform_deployment_tooling::identity::jwks_for_key_pair(&key_pair, &key_id)
        .map_err(|_| {
            invalid_local_identity(&identity_directory, "cannot derive issuer public key")
        })?;
    ...
    let private_key_der =
        insight_platform_deployment_tooling::identity::pkcs1_private_key_from_pkcs8(
            &private_key_der,
        )
        .ok_or_else(|| {
            invalid_local_identity(
...
fn initialize_local_runtime_identity(state_directory: &Path) -> Result<(), CliError> {
    ...
    write_local_leaf_certificate(
        ...
        full_profile::GATEWAY_EGRESS_CLIENT_PRIVATE_KEY_FILE,
        &[],
        Some(insight_platform_contracts::GATEWAY_WORKLOAD_IDENTITY),
    ...
    for spec in insight_platform_deployment_tooling::tls::outbox_identity_specs() {
        write_local_leaf_certificate(
            &tls_directory,
            spec.certificate,
            spec.private_key,
    ...
    write_local_leaf_certificate(
        &tls_directory,
        RUNTIME_REGISTRY_VALIDATION_CLIENT_CERTIFICATE_FILE,
        RUNTIME_REGISTRY_VALIDATION_CLIENT_PRIVATE_KEY_FILE,
        &[],
        Some("spiffe://insight.platform/workload/registry-validation-worker"),
        ExtendedKeyUsagePurpose::ClientAuth,
        &issuer,
    )?;
    Ok(())
...
fn local_runtime_ca_parameters(tls_directory: &Path) -> Result<CertificateParams, CliError> {
    insight_platform_deployment_tooling::tls::authority_parameters().map_err(|_| {
        invalid_local_identity(
            tls_directory,
...
fn write_local_leaf_certificate(
    ...
    let material = insight_platform_deployment_tooling::tls::create_leaf(
        dns_names,
        workload_identity,
        usage,
        issuer,
    )
    .map_err(|_| invalid_local_identity(tls_directory, "cannot prepare local TLS certificate"))?;
    write_sensitive_new(
...
pub fn prepare_runtime_profile(
    ...
    prepare_runtime_profile_with_ports(
        ...
        RuntimeProfileSource {
            source_fingerprint,
            release_identity: &format!("source:{source_fingerprint}"),
            binary_directory,
        },
    )
}

struct RuntimeProfileSource<'a> {
    source_fingerprint: &'a str,
    release_identity: &'a str,
    binary_directory: &'a Path,
}
...
fn prepare_runtime_profile_with_ports(
    ...
    let worker_builds = worker_profile::WorkerBuilds::read(binary_directory, selected_profile)?;
    ...
    let result = result.and_then(|profile| {
        ...
        write_new(&runtime_directory.join(RUNTIME_PROFILE_STATE_FILE), &bytes)?;
        Ok(profile.config_digests)
    });
    if result.is_err() {
...
fn prepare_runtime_profile_inner(
    ...
    let secret_provider_id = preserved
        .map(|snapshot| snapshot.profile.secret_provider_id.clone())
        .unwrap_or_else(|| fresh_resource_id(ResourceKind::SecretProvider));
    let capability_protocol_profile_revision_id = preserved
        .map(|snapshot| {
            snapshot
                .profile
        ...
        .unwrap_or_else(|| fresh_resource_id(ResourceKind::PolicyRevision));
    ...
    let write_storage_binding_digest = catalog["write_storage_binding_digest"]
        .as_str()
        .ok_or_else(|| CliError::RuntimeState("Artifact write binding missing".into()))?
        .parse()
        .map_err(|_| CliError::RuntimeState("Artifact write binding invalid".into()))?;
    let bootstrap=insight_platform_deployment_tooling::bootstrap::artifact_authority(write_storage_binding_digest,identity.artifact_encryption_domain_id.parse().map_err(|_|CliError::RuntimeState("Artifact encryption domain invalid".into()))?,preserved.map(|snapshot|&snapshot.artifact_bootstrap)).map_err(|_|CliError::RuntimeState("preserved development bootstrap policy differs from current provider and policy semantics".into()))?;
    let runtime = state_directory.join(RUNTIME_DIRECTORY);
    let network = native_runtime_network(ports)?;
    let encryption_domain_id = identity
        .artifact_encryption_domain_id
        .parse()
        .map_err(|_| CliError::RuntimeState("Artifact encryption domain ID invalid".into()))?;
    let validator_principal_id = identity
        .registry_validator_principal_id
        .parse()
        .map_err(|_| CliError::RuntimeState("Registry validator principal ID invalid".into()))?;
    let mut configs = insight_platform_deployment_tooling::base_profile::configurations(
        insight_platform_deployment_tooling::base_profile::BaseConfigInputs {
            network: &network,
            worker_builds,
            identity: insight_platform_deployment_tooling::base_profile::BaseIdentity {
                encryption_domain_id: &encryption_domain_id,
                registry_validator_principal_id: &validator_principal_id,
            },
    ...
    let feature_configs = selected_feature_configs(
        ...
        LocalProviderCatalogs {
            artifact: &catalog,
            secret: &secret_provider_catalog,
        },
        &capability_protocol_profile_revision_id,
        selected_profile,
        worker_builds,
...
struct LocalProviderCatalogs<'a> {
...
fn selected_feature_configs(
    ...
    ports: &RuntimePortBindings,
    identity: &LocalIdentityState,
    providers: LocalProviderCatalogs<'_>,
    capability_protocol_profile_revision_id: &ResourceId,
    selected_profile: DevProfile,
    worker_builds: &worker_profile::WorkerBuilds,
) -> Result<BTreeMap<String, (&'static str, serde_json::Value)>, CliError> {
    let LocalProviderCatalogs {
        artifact: artifact_provider_catalog,
        secret: secret_provider_catalog,
    } = providers;
    ...
    let mut configs = full_profile::initial_configs(
        worker_builds,
        &full_profile::aws_qualification_network(
            &ports.full,
            ports.artifact_data_controller,
            ports.artifact_data_observability,
        ),
        artifact_provider_catalog,
        Some(capability_protocol_profile_revision_id),
    ...
    if !selected_profile.has_mcp() {
        if let Some((_, config)) = configs.get_mut("capability-remote") {
            config["installed_mcp_codecs"] = serde_json::json!([]);
            config["mcp_host"] = serde_json::Value::Null;
            let closure = serde_json::json!({"schema_version":1,"http":config["installed_http_codecs"],"grpc":config["installed_grpc_codecs"],"mcp":config["installed_mcp_codecs"]});
            let digest = canonical_digest(&closure)
                .map_err(|error| CliError::RuntimeState(error.to_string()))?;
            config["worker_manifest"] = worker_builds
                .manifest(
                    "platform-capability-remote-worker",
                    "capability.remote",
                    insight_platform_contracts::WorkClass::CapabilityRemote,
                    &digest,
                    (4, 1),
                    worker_profile::remote_capability(&closure),
                )
                .ok_or_else(|| {
                    CliError::RuntimeState(
                        "selected remote Capability worker binary is unavailable".into(),
                    )
                })?;
        }
    }
...
fn regenerate_fixture_profile(
    ...
    let root = state_directory.parent().expect("fixture project root");
    let _lock = acquire_runtime_lifecycle_lock(root)?;
    *state = runtime_config_transition::regenerate(
        state_directory,
        identity,
        state,
        selected_profile,
...
fn local_artifact_provider_catalog(kms_key_arn: &str) -> Result<serde_json::Value, CliError> {
    let origin = insight_platform_deployment_contracts::installation::ServiceOrigin::parse(
        LOCAL_AWS_ENDPOINT,
    )
    .map_err(|_| CliError::RuntimeState("local provider endpoint invalid".into()))?;
    insight_platform_deployment_tooling::provider_config::artifact_provider_catalog(
        insight_platform_deployment_tooling::provider_config::ProviderEndpoints {
            artifact: &origin,
            kms: &origin,
            secrets: &origin,
        },
        insight_platform_deployment_tooling::provider_config::ARTIFACT_BUCKET,
        kms_key_arn,
    )
    .map_err(|_| CliError::RuntimeState("local Artifact provider configuration invalid".into()))
}
...
fn local_secret_provider_catalog(
    ...
    .map_err(|_| CliError::RuntimeState("local provider endpoint invalid".into()))?;
    insight_platform_deployment_tooling::provider_config::secret_provider_catalog(
        insight_platform_deployment_tooling::provider_config::ProviderEndpoints {
            artifact: &origin,
            kms: &origin,
            secrets: &origin,
    ...
    .map_err(|_| CliError::RuntimeState("local Secret provider configuration invalid".into()))
}
...
fn run_development_profile_locked(
    ...
    ensure_local_artifact_cors(&workspace, &compose_project, &runtime)?;
    ...
    outbox_profile::provision(&binary_directory, &runtime)?;
    history_profile::provision(&binary_directory, &runtime)?;
    let binding = runtime_process_binding(
        &runtime,
        &project.identity.tenant_id,
        &compose_project,
    ...
    if let Err(error) = rotate_local_access_token(root, SystemTime::now())
        .and_then(|_| connection::export_qualification(root))
    {
        return Err(abort_runtime_start(&runtime, &mut state, error));
    }
...
fn compose_command(workspace: &Path, project: &str, runtime: &Path) -> ProcessCommand {
    ...
    command
        .current_dir(runtime)
        .env(
            "INSIGHT_DEV_NATS_CONFIG_PATH",
            runtime.join(outbox_profile::NATS_CONFIG_FILE),
        )
        .env(
...
fn initialize_localstack_artifact_dependency(
    ...
    let cors = local_artifact_cors::configuration_json();
    compose_awslocal(
        workspace,
        compose_project,
        runtime,
...
fn ensure_local_artifact_cors(
    workspace: &Path,
    compose_project: &str,
    runtime: &Path,
) -> Result<(), CliError> {
    let cors = compose_awslocal(
        workspace,
        compose_project,
        runtime,
        &[
    ...
    local_artifact_cors::validate_observed(&cors)?;
    Ok(())
}
...
fn cargo_target_directory(workspace: &Path) -> Result<PathBuf, CliError> {
    #[derive(Deserialize)]
    struct Metadata {
    ...
    let output = ProcessCommand::new("cargo")
        ...
        .output()
        .map_err(|error| {
            CliError::WorkspaceUnavailable(format!("cannot read Cargo target directory: {error}"))
        })?;
    if !output.status.success() || output.stdout.len() > 8_388_608 {
        return Err(CliError::WorkspaceUnavailable(
            "Cargo metadata could not determine the actual target directory".into(),
        ));
    }
    let metadata: Metadata = serde_json::from_slice(&output.stdout).map_err(|_| {
        CliError::WorkspaceUnavailable("Cargo target directory metadata is invalid".into())
    })?;
    if !metadata.target_directory.is_absolute() {
        return Err(CliError::WorkspaceUnavailable(
            "Cargo target directory is not absolute".into(),
        ));
    }
...
fn ensure_runtime_binaries(
    ...
    let binary_directory = cargo_target_directory(workspace)?.join("release");
    ...
    command.current_dir(workspace).args([
        ...
        "--release",
        "-p",
        "insight-platform-storage-tooling",
        "--bin",
        "platform-schema",
        "--bin",
        ...
        "-p",
        "insight-platform-outbox-worker",
        "--bin",
        "platform-outbox-worker",
        "--bin",
        "platform-jetstream-provision",
        "-p",
        "insight-platform-maintenance-worker",
        "--bin",
...
fn native_runtime_network(
    ports: &RuntimePortBindings,
) -> Result<insight_platform_deployment_contracts::installation::NetworkTopologyV1, CliError> {
    let mut network = full_profile::aws_qualification_network(
        &ports.full,
        ports.artifact_data_controller,
        ports.artifact_data_observability,
    );
    use insight_platform_deployment_contracts::installation::{
        InstallationProcess as Process, ProcessNetworkV1, ServiceOrigin,
    };
    let loopback = |port| std::net::SocketAddr::from(([127, 0, 0, 1], port));
    for (process, listener, observability, tls) in [
        (
            Process::GatewayManagement,
            Some(ports.gateway_management),
            ports.gateway_management,
            false,
        ),
        (
            Process::GatewayRuntime,
            Some(ports.gateway_runtime),
            ports.gateway_runtime,
            false,
        ),
        (
            Process::ArtifactGateway,
            Some(ports.artifact_gateway),
            ports.artifact_gateway_observability,
            true,
        ),
        (
            Process::Orchestration,
            None,
            ports.orchestration_observability,
            false,
        ),
        (
            Process::CapabilityNative,
            None,
            ports.capability_native_observability,
            false,
        ),
        (
            Process::RegistryValidation,
            None,
            ports.registry_validation_observability,
            false,
        ),
        (
            Process::Outbox,
            None,
            ports.full.outbox_observability,
            false,
        ),
        (
            Process::HistoryMaintenance,
            None,
            ports.full.history_observability,
            false,
        ),
    ] {
        let origin = listener
        ...
        network.processes.push(ProcessNetworkV1 {
            process,
            listen_address: listener.map(loopback),
            observability_address: loopback(observability),
...
fn start_profile_processes(
    ...
    let tls = runtime.join(RUNTIME_TLS_DIRECTORY);
    let database_url = "postgres://insight:insight@127.0.0.1:5432/insight_platform";
    let outbox_database_url = format!(
        "postgres://insight_outbox_dev:{}@127.0.0.1:5432/insight_platform",
        outbox_profile::read_password(runtime)?
    );
    let history_database_url = format!(
        "postgres://insight_history_dev:{}@127.0.0.1:5432/insight_platform",
        history_profile::password(runtime)?
    );
    let network = native_runtime_network(&profile.ports)?;
    let cursor_path = runtime.join(RUNTIME_CURSOR_KEY_FILE);
    let cursor_digest = cursor_key_digest(&cursor_path)?;
    let mut specs = Vec::new();
    use insight_platform_deployment_contracts::installation::InstallationProcess as Process;
    for process in [
        ...
        let primary = match process {
            Process::Outbox => outbox_database_url.as_str(),
            Process::HistoryMaintenance => history_database_url.as_str(),
            _ => database_url,
        };
        let launch = insight_platform_deployment_tooling::process_environment::process_launch(
            insight_platform_deployment_tooling::process_environment::ProcessEnvironmentInputs {
                process,
                ...
                network: &network,
                configuration_digest: profile.config_digests.get(process.name()).ok_or_else(
                    || CliError::RuntimeState("selected process configuration missing".into()),
                )?,
                database:
                    insight_platform_deployment_tooling::process_environment::ProcessDatabaseUrls {
                        primary,
                        read: Some(database_url),
                        work: Some(database_url),
                    },
                cursor_key_path: Some(&cursor_path),
                cursor_key_digest: Some(&cursor_digest),
                aws_credentials_path: None,
            },
        )
        .map_err(|error| CliError::RuntimeState(error.to_string()))?;
        specs.push(RuntimeLaunchSpec {
            role: launch.role,
...
fn expected_runtime_closure(
    ...
    let mut config_files = BTreeMap::from([
        ("outbox".to_owned(), outbox_profile::CONFIG_FILE),
        (
            "history-maintenance".to_owned(),
            history_profile::CONFIG_FILE,
        ),
        (
    ...
    let mut processes = BTreeMap::from([
        (
            "outbox".to_owned(),
            loopback_address(profile.ports.full.outbox_observability),
        ),
        (
            "history-maintenance".to_owned(),
            loopback_address(profile.ports.full.history_observability),
        ),
        (
            "artifact-data".to_owned(),
...
fn read_runtime_profile_state(
    ...
    runtime_config_transition::reject_pending(runtime)?;
...
fn read_runtime_file_bytes(path: &Path, maximum_bytes: u64) -> Result<Option<Vec<u8>>, CliError> {
...
fn read_runtime_json<T: serde::de::DeserializeOwned>(path: &Path) -> Result<Option<T>, CliError> {
    ...
    let Some(bytes) = read_runtime_file_bytes(path, MAX_RUNTIME_STATE_BYTES)? else {
        return Ok(None);
    };
    let value = parse_strict_json(
        &bytes,
...
fn sign_local_access_token(
    ...
    use insight_platform_deployment_contracts::installation::{
        LocalSessionIdentityV1, LocalSessionKind,
    };
    insight_platform_deployment_tooling::identity::sign_session(
        &LocalSessionIdentityV1 {
            schema_version: 1,
            issuer: identity.issuer.clone(),
            audience: identity.audience.clone(),
            key_id: identity.key_id.clone(),
            tenant_id: identity.tenant_id.parse().map_err(|_| ())?,
            subject: identity.developer_subject.clone(),
            principal_kind: LocalSessionKind::AgentAuthor,
        },
        private_key_der,
        issued_at_unix_seconds,
    )
...
fn rotate_local_access_token(root: &Path, issued_at: SystemTime) -> Result<String, CliError> {
    ...
    let jwks = insight_platform_deployment_tooling::identity::jwks_for_key_pair(
        &key_pair,
...
pub fn execute(
    ...
    match command {
        ...
        CliCommand::Connect(command) => connection::execute(command, current_directory),
        CliCommand::InstallationHelp => Ok(installation_usage().to_owned()),
        CliCommand::Model(command) => model::execute(command, current_directory),
        ...
        CliCommand::Recovery { root, file, action } => recovery::execute(
            &resolve_root(current_directory, root),
            &resolve_root(current_directory, file),
            &action,
        ),
        ...
        CliCommand::TaskGet {
            ...
            let root = resolve_root(current_directory, root);
            read_local_task(&root, &task_id, purpose)
        }
        CliCommand::TaskList {
            root,
            filters,
            ...
            render_json(
                &task::list_tasks(&client, &filters, page_size, cursor.as_deref())
        ...
        CliCommand::AgentValidate {
            ...
            let root = resolve_root(current_directory, root);
            let file = resolve_agent_manifest_path(current_directory, &root, file)?;
        ...
        CliCommand::AgentSignal {
            ...
            let root = resolve_root(current_directory, root);
            let file = file.map(|file| resolve_root(current_directory, file));
            agent_signal::execute(&root, &run_id, &key, file.as_deref())
        }
        CliCommand::AgentRestoreSource {
            root,
            agent_id,
            version_id,
            output,
            ...
            let root = resolve_root(current_directory, root);
            let output = resolve_root(&root, output);
            let (client, _) = local_public_http_client(&root)?;
            render_json(
                &agent_restore::restore_published_agent(&client, &agent_id, &version_id, &output)
                    .map_err(CliError::Agent)?,
            )
        }
        CliCommand::AgentEvaluationReport {
            root,
            file,
            output,
            run_id,
            ...
            let file = resolve_root(current_directory, file);
            let output = resolve_root(&root, output);
            let _upload_lock = connection::acquire_mutation_lock(&root)?;
            let (client, tenant) = local_runtime_http_client(&root)?;
            let mut result =
                evaluation::report(&client, &file, &run_id, &output).map_err(CliError::Agent)?;
            let uploader = artifact::HttpsArtifactObjectUploader::with_additional_roots(
                client.additional_roots(),
            )
            ...
            let uploaded = artifact::upload_artifact(
                ...
                artifact::ArtifactUploadOptions {
                    purpose: insight_platform_contracts::ArtifactPurpose::Export,
                    classification: insight_platform_contracts::DataClassification::Restricted,
                    declared_media_type: Some("application/json".into()),
                    display_name: Some("evaluation-report.json".into()),
                    operation_timeout: std::time::Duration::from_secs(120),
                },
                &root.join(PROJECT_DIRECTORY).join("artifact-upload"),
            )
            .map_err(CliError::Artifact)?;
            result["artifact"] = serde_json::to_value(uploaded).map_err(|_| CliError::Usage)?;
            render_json(&result)
        }
        CliCommand::AgentEvaluation {
            action,
            root,
            file,
            output,
            ...
            let file = resolve_root(current_directory, file);
            let output = resolve_root(&root, output);
            let client = Some(local_public_http_client(&root)?.0);
            render_json(
                &evaluation::execute(action, &root, &file, &output, client.as_ref())
                    .map_err(CliError::Agent)?,
            )
        }
        CliCommand::AgentDependencies {
            root,
            kind,
            environment,
            cursor,
            ...
            let root = resolve_root(current_directory, root);
            let (client, _) = local_public_http_client(&root)?;
            let filters = insight_platform_registry::authoring::AuthoringDependencyFiltersV1 {
                kind,
                environment,
                interface_contract_digest: None,
            };
            filters.validate().map_err(|_| CliError::Usage)?;
            let mut query = vec![
                ("kind".to_owned(), kind.as_str().to_owned()),
                ("page_size".to_owned(), "50".to_owned()),
            ];
            if let Some(environment) = &filters.environment {
                query.push(("environment".into(), environment.clone()));
            }
            if let Some(cursor) = cursor {
                query.push(("cursor".into(), cursor));
            }
            let page: insight_platform_api::product::ListPageV1<
                insight_platform_registry::authoring::AuthoringDependencyV1,
            > = client
                .get_body_json_query(
                    "/v1/agent-authoring-dependencies",
                    &query,
                    ...
                    .any(|item| item.validate_for(&filters).is_err())
            {
                return Err(CliError::PublicClient(
                    public_client::PublicClientError::InvalidResponse(
                        "authoring dependency page does not match query".into(),
                    ),
                ));
            }
            render_json(&page)
...
fn usage() -> &'static str {
    concat!("Insight Platform\n\nInstallation:\n  docker compose -f compose.json up -d\n  python3 tools/install/platform_native.py --help\n  helm upgrade --install installation <chart> -n <namespace> -f values.json --set node=<node> --wait --wait-for-jobs\n\nConnect this workspace to an installation:\n  insight connect --path <directory> --endpoint <Console-or-Management-origin> --tenant <ten_...> --token-file <private-file> [--runtime-endpoint <origin>] [--ca-file <public-PEM>]\n",
        "\nModels (all require --endpoint <origin> --tenant <ten_...> --token-file <private-file>; optional --ca-file <public-PEM>):\n  insight model configure --file <models.json> [--state-dir <private-directory>] [--new-attempt]\n  insight model sources\n  insight model source --source <alias-or-ID>\n  insight model list\n  insight model get --model <alias-or-ID>\n  insight model default [--model <alias-or-ID>|--clear] [--new-attempt]\n  insight model quota --model <alias-or-ID-or-deployment> [--file <limits.json>] [--state-dir <private-directory>] [--new-attempt]\n  insight model probe --model <alias-or-ID>\n  insight model credential --binding <sbd_...>\n  insight model revoke --binding <sbd_...> [--state-dir <private-directory>]\n",
        "\nAgent journey:\n  insight agent validate --file <agent.yaml>\n  insight agent publish --file <agent.yaml> [--output text|json]\n  insight agent list [--output text|json]\n  insight agent get <name-or-agent-id> [--output text|json]\n  insight agent adopt <name> --agent-id <agt_...>\n  insight agent run <name-or-agent-id> (--input <json>|--file <input.json>) [--detach]\n  insight agent logs <name-or-run-id> [--follow]\n  insight agent result <run-id> [--output text|json]\n  insight agent signal <run-id> <signal-key> [--file <typed-signal-request.json>]\n  insight agent dependencies <model|capability|context|child_agent|skill> [--environment <name>] [--cursor <cursor>]\n  insight agent restore-source --agent <agt_...> --version <arev_...> --output <new-directory>\n  insight agent evaluation-init --file <evaluation-request.json> --output <new-directory>\n  insight agent evaluation-input --file <evaluation-request.json> --output <input.json>\n  insight agent evaluation-report --file <evaluation-request.json> --run <run_...> --output <report.json>\n\nInstall and update:\n  insight version [--json]\n  insight update check\n  insight update apply --version <exact-version>\n\nUse `insight advanced` for Platform automation, diagnostics, and lifecycle commands.\n")
}

fn installation_usage() -> &'static str {
    "Prepare installation configuration, then start services with the deployment tool.\n\nCompose: docker compose -f compose.json up -d\nNative foreground services: python3 tools/install/platform_native.py --help\nKubernetes: helm upgrade --install installation <chart> -n <namespace> -f values.json --set node=<node> --wait --wait-for-jobs\n\nUse the explicit installation-session operation to obtain a private token file, then `insight connect` with its endpoint, tenant and private token-file. Native Ctrl-C stops the current foreground services and retains dependency data.\n\nThe explicit `insight qualification-aws` namespace is reserved for the separate AWS physical qualification fixture. It is not the default service installation.\n"
}

fn advanced_usage() -> &'static str {
    "Insight Platform automation and diagnostics\n\nUsage:\n  insight doctor [--json]\n  insight qualification-aws token [--path <directory>]\n  insight qualification-aws start [--path <directory>]\n  insight qualification-aws status [--path <directory>]\n  insight qualification-aws logs [--path <directory>] [--role <role>]\n  insight qualification-aws stop [--path <directory>]\n  insight qualification-aws reset [--path <directory>] [--confirm <project-name>]\n  insight apply --file <manifest.json> [--path <directory>] [--timeout-seconds <1..3600>]\n  insight recovery hold-place|hold-release|pkce-recover --file <request.json> [--path <directory>]\n  insight run create --file <request.json> [--path <directory>]\n  insight run get|result|watch <run_id> [--path <directory>]\n  insight run pause|resume|cancel <run_id> [--path <directory>]\n  insight task list [--purpose respondable|viewable] [--state <state>] [--kind <kind>] [--run-id <run_id>] [--page-size <1..50>] [--cursor <cursor>] [--path <directory>]\n  insight task get <task_id> [--purpose respondable|viewable] [--path <directory>]\n  insight task approve|reject|cancel <task_id> [--path <directory>]\n  insight task submit-input <task_id> --file <input.json> [--path <directory>]\n  insight artifact upload --file <file> --purpose <purpose> --classification <classification> [options]\n  insight artifact get <artifact_id> [--path <directory>]\n  insight artifact read <artifact_id> --output <file> [--path <directory>]\n  insight operation wait <job_id> [--path <directory>] [--timeout-seconds <1..3600>]\n\nThe explicitly selected AWS qualification `reset` first prints its exact destructive scope; deletion requires the displayed project name. All mutations use the public `/v1` authority. Receipt, ETag, Operation, and cursor details remain managed by the client unless an Agent command explicitly enables `--debug-authority`.\n"
}
...
fn resolve_agent_manifest_path(
    ...
    let absolute = resolve_root(current_directory, file);
    // Keep lexical components intact: the source loader must still reject parent traversal
    // and symbolic links instead of losing that evidence through file canonicalization.
    absolute
        .strip_prefix(root)
        .map(Path::to_path_buf)
        .map_err(|_| {
            CliError::Agent(agent::AgentCommandError::InvalidLocalState(
                "manifest lies outside the selected project root".into(),
            ))
        })
}
...
fn validate_local_agent(
    ...
    let captured = agent::capture_project_sources(root, file).map_err(CliError::Agent)?;
    let profile = if online {
        let (client, _) = local_public_http_client(root)?;
        agent::online_compiler_profile(&client, root).map_err(CliError::Agent)?
    } else if let Some(profile) =
        agent::restored_compiler_profile(root, file).map_err(CliError::Agent)?
    {
        profile
    } else {
        agent::offline_compiler_profile(root).map_err(CliError::Agent)?
    };
    let compiled = if online {
        let (client, _) = local_public_http_client(root)?;
        agent::compile_project_online(root, captured, profile, &client)
    } else {
        agent::compile_project(root, captured, profile)
    }
    .map_err(CliError::Agent)?;
...
fn publish_local_agent(
    ...
    let captured = agent::capture_project_sources(root, file).map_err(CliError::Agent)?;
    let _publication_lock = connection::acquire_mutation_lock(root)?;
    ...
    let profile =
        agent::online_compiler_profile(&management_client, root).map_err(CliError::Agent)?;
    let compiled = agent::compile_project_online(root, captured, profile, &management_client)
        .map_err(CliError::Agent)?;
...
fn run_local_agent(
    ...
    let (expected_deployment, spec) =
        agent::published_run_defaults(&management, &agent_id).map_err(CliError::Agent)?;
    ...
    let request = run::CreateRunRequestV1 {
        expected_agent_deployment: Some(expected_deployment),
...
fn local_public_http_client_for(
    ...
    let connection = connection::load(root)?;
    let client = connection.client(matches!(surface, LocalGatewaySurface::Runtime))?;
    Ok((client, connection.tenant_id))
}
...
fn read_local_task(
    ...
    let (client, _) = local_runtime_http_client(root)?;
    let view = task::read_task(&client, &task_id, purpose).map_err(CliError::Task)?;
    render_json(&view)
}
...
fn resolve_local_task(
    ...
    let task_id = parse_task_id(task_id)?;
    let action = match action {
        CliTaskAction::SubmitInput => insight_platform_tasks::TaskAction::SubmitInput,
        CliTaskAction::Approve => insight_platform_tasks::TaskAction::Approve,
        CliTaskAction::Reject => insight_platform_tasks::TaskAction::Reject,
        CliTaskAction::Cancel => insight_platform_tasks::TaskAction::Cancel,
    };
...
impl From<insight_platform_deployment_tooling::DeploymentError> for CliError {
    fn from(error: insight_platform_deployment_tooling::DeploymentError) -> Self {
        Self::RuntimeState(error.to_string())
    }
}
...
mod tests {
    ...
    fn tls_identity_registry_has_the_closed_profile_counts() {
        for (profile, expected) in [
            (DevProfile::starter(), 11),
            (DevProfile::parse(Some("sandbox"), false, true).unwrap(), 11),
            (DevProfile::parse(Some("model"), false, true).unwrap(), 15),
            (
                DevProfile::parse(Some("remote-capability"), false, true).unwrap(),
                15,
            ),
            (DevProfile::parse(Some("context"), false, true).unwrap(), 19),
            (DevProfile::parse(Some("mcp"), false, true).unwrap(), 22),
            (DevProfile::parse(Some("all"), false, true).unwrap(), 27),
        ] {
            assert_eq!(
    ...
    fn runtime_profile_is_closed_digest_bound_and_keeps_private_material_out_of_config() {
        ...
        for (role, file) in [
            ("outbox", outbox_profile::CONFIG_FILE),
            ("history-maintenance", history_profile::CONFIG_FILE),
        ...
        assert_eq!(
            ...
            insight_platform_artifacts::execution::integrity_scanner_contract_digest().to_string()
        );
    ...
    #[test]
    fn feature_configs_regenerate_without_rotating_starter_authority() {
        let directory = TempDir::new().unwrap();
        let project =
            initialize_project(directory.path(), Some("demo"), SystemTime::now()).unwrap();
        let kms = "arn:aws:kms:us-east-1:000000000000:key/12345678-1234-1234-1234-123456789012";
        ...
        .unwrap();
        assert_eq!(state.secret_provider_id, previous.secret_provider_id);
        assert_eq!(
            state.capability_protocol_profile_revision_id,
            previous.capability_protocol_profile_revision_id
        );
        assert_eq!(state.ports, previous.ports);
        assert_eq!(
        ...
        .unwrap();
        assert_eq!(state.config_digests.len(), 25);
        for role in [
            "context-native",
            "model-worker",
            "capability-remote",
    ...
    fn additive_feature_config_recovers_only_an_exact_preexisting_file() {
        ...
        let generated = selected_feature_configs(
            ...
            LocalProviderCatalogs {
                ...
                secret: &secret_catalog,
            },
            &state.capability_protocol_profile_revision_id,
            selected,
            &worker_profile::WorkerBuilds::read(
                &workspace_assets::worker_binary_fixtures(directory.path()),
                DevProfile::parse(Some("all"), false, true).unwrap(),
            )
            .unwrap(),
        ...
        let error = regenerate_fixture_profile(
            &state_directory,
            &project.identity,
            &mut state,
            selected,
        ...
        assert!(matches!(
            error,
            CliError::RuntimeState(detail) if detail.contains("config") && detail.contains("digest")
        ));
    }
    ...
    fn remote_capability_additive_recovery_reuses_persisted_protocol_revision() {
        ...
        assert!(config["mcp_host"].is_null());
        assert_eq!(config["installed_mcp_codecs"], serde_json::json!([]));
        let codec_closure = serde_json::json!({"schema_version":1,"http":config["installed_http_codecs"],"grpc":config["installed_grpc_codecs"],"mcp":[]});
        assert_eq!(
            config["worker_manifest"]["adapter_runtime_digest"],
            canonical_digest(&codec_closure).unwrap()
        );
        assert_eq!(
            config["worker_manifest"]["execution_capabilities"],
            serde_json::to_value(worker_profile::remote_capability(&codec_closure)).unwrap()
        );
        assert!(!first_generation.contains_key("mcp-host"));
        let with_mcp = selected_feature_configs(
            &runtime,
            &state.ports,
            &project.identity,
            LocalProviderCatalogs {
            ...
            &state.capability_protocol_profile_revision_id,
            DevProfile::parse(Some("remote-capability,mcp"), false, true).unwrap(),
            &worker_profile::WorkerBuilds::read(
                &workspace_assets::worker_binary_fixtures(directory.path()),
                DevProfile::parse(Some("all"), false, true).unwrap(),
            )
        ...
        .unwrap();
        assert!(with_mcp.contains_key("mcp-host"));
        let full = &with_mcp["capability-remote"].1;
        assert!(full["mcp_host"].is_object());
        assert!(!full["installed_mcp_codecs"].as_array().unwrap().is_empty());
        assert_ne!(
            full["worker_manifest"]["adapter_runtime_digest"],
            config["worker_manifest"]["adapter_runtime_digest"]
        );
    ...
    fn command_parser_rejects_unclosed_options() {
        let arguments = vec![
            OsString::from("qualification-aws"),
            OsString::from("init"),
            OsString::from("--path"),
        ];
        assert!(matches!(
    ...
    #[test]
    fn ordinary_lifecycle_only_explains_the_shared_installation_entrypoints() {
        for name in [
            "init", "dev", "start", "status", "logs", "stop", "reset", "token",
        ] {
            assert_eq!(
                parse_command(&[OsString::from(name)]).unwrap(),
                CliCommand::InstallationHelp
            );
        }
        let help = installation_usage();
        assert!(help.contains("platform_native.py") && help.contains("insight connect"));
    }
    ...
    fn command_parser_accepts_starter_feature_lifecycle_commands() {
        assert!(matches!(
            parse_command(&[
                OsString::from("qualification-aws"), OsString::from("dev"),
                OsString::from("--path"),
                OsString::from("demo"),
        ...
        assert!(matches!(
            parse_command(&[
                OsString::from("qualification-aws"), OsString::from("start"),
                OsString::from("--path"),
                OsString::from("demo"),
            ]),
        ...
        assert!(matches!(
            parse_command(&[
                OsString::from("qualification-aws"), OsString::from("status"),
                OsString::from("--path"),
                OsString::from("demo"),
            ]),
        ...
        assert!(matches!(
            parse_command(&[
                OsString::from("qualification-aws"), OsString::from("logs"),
                OsString::from("--path"),
                OsString::from("demo"),
                OsString::from("--role"),
        ...
        assert!(matches!(
            parse_command(&[
                OsString::from("qualification-aws"), OsString::from("stop"),
                OsString::from("--path"),
                OsString::from("demo"),
            ]),
        ...
        assert!(matches!(
            parse_command(&[
                OsString::from("qualification-aws"), OsString::from("reset"),
                OsString::from("--path"),
                OsString::from("demo"),
    ...
    #[test]
    fn task_list_and_read_purpose_are_closed_and_bounded() {
        let parse =
            |values: &[&str]| parse_command(&values.iter().map(OsString::from).collect::<Vec<_>>());
        assert!(matches!(
            parse(&["task", "list"]).unwrap(),
            CliCommand::TaskList {
                filters: insight_platform_api::task::TaskInboxFiltersV1 {
                    purpose: insight_platform_tasks::TaskQueryPurpose::Respondable,
                    ..
                },
                page_size: insight_platform_api::product::PRODUCT_LIST_DEFAULT_PAGE_SIZE,
                ..
            }
        ));
        assert!(matches!(
            ...
            CliCommand::TaskList {
                filters: insight_platform_api::task::TaskInboxFiltersV1 {
                    purpose: insight_platform_tasks::TaskQueryPurpose::Viewable,
                    ..
                },
                page_size: 50,
                cursor: Some(_),
        ...
        assert!(matches!(
            parse(&["task", "get", "int_test", "--purpose", "viewable"]).unwrap(),
            CliCommand::TaskGet {
                purpose: insight_platform_tasks::TaskQueryPurpose::Viewable,
                ..
            }
        ...
        for invalid in [
            vec!["task", "list", "--purpose", "all"],
            vec!["task", "list", "--page-size", "0"],
            vec!["task", "list", "--page-size", "51"],
            vec!["task", "get", "int_test", "--state", "pending"],
            vec![
                "task",
                "list",
                "--purpose",
    ...
    #[test]
    fn source_runtime_uses_cargos_actual_target_and_rejects_missing_metadata() {
        let workspace = crate::workspace_assets::workspace_path("");
        let actual = cargo_target_directory(&workspace).unwrap();
        ...
        if let Some(configured) = std::env::var_os("CARGO_TARGET_DIR") {
            ...
            assert_eq!(
                actual, expected,
                "source role paths must follow Cargo's configured target directory"
            );
        }
        let directory = tempfile::TempDir::new().unwrap();
        assert!(
            matches!(
                cargo_target_directory(directory.path()),
                Err(CliError::WorkspaceUnavailable(_))
            ),
            "missing metadata must never fall back to a guessed workspace target"
        );
    }
}
