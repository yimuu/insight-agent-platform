...
pub(crate) struct WorkerBuilds(BTreeMap<&'static str, Sha256Digest>);
impl WorkerBuilds {
    pub(crate) fn read(directory: &Path, selected: DevProfile) -> Result<Self, CliError> {
        ...
        for (name, path) in crate::runtime_binary_paths(directory, selected) {
            ...
            loop {
                let count = file.read(&mut buffer).map_err(|error| {
                    CliError::RuntimeState(format!("cannot hash worker executable: {error}"))
                })?;
                if count == 0 {
                    break;
                }
    ...
    pub(crate) fn executable(&self, binary: &str) -> Option<&Sha256Digest> {
        self.0.get(binary)
    }
...
pub(crate) fn programs() -> WorkerExecutionCapabilities {
    insight_platform_plan::execution::program_execution_capabilities()
}
pub(crate) fn registry() -> WorkerExecutionCapabilities {
...
#[cfg(test)]
pub(crate) fn fixture_binaries(root: &Path) -> std::path::PathBuf {
...
pub(crate) fn subscription() -> WorkerExecutionCapabilities {
    WorkerExecutionCapabilities {
        schema_version: EXECUTION_REQUIREMENT_VERSION,
        capabilities: vec![
...
pub(crate) fn dataset(adapter: &str) -> WorkerExecutionCapabilities {
    WorkerExecutionCapabilities {
        schema_version: EXECUTION_REQUIREMENT_VERSION,
        capabilities: vec![
...
pub(crate) fn model(anthropic: &str, openai: &str) -> WorkerExecutionCapabilities {
    with_adapters(
        [
        ...
        .map(|(name, digest)| {
            insight_platform_contracts::model_adapter_execution_capability_from_contract(
                name,
                &digest.parse().expect("model contract digest"),
            )
            .expect("closed Model adapter")
        }),
    )
}
pub(crate) fn native_capability(module: &str) -> WorkerExecutionCapabilities {
    with_adapters([
...
pub(crate) fn native_context(contract: &str, installed: &str) -> WorkerExecutionCapabilities {
    with_adapters([
...
pub(crate) fn remote_capability(closure: &serde_json::Value) -> WorkerExecutionCapabilities {
...
pub(crate) fn remote_context() -> WorkerExecutionCapabilities {
    with_adapters([
