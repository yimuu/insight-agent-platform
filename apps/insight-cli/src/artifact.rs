...
use crate::{
    artifact_journal::{self, ArtifactJournalError, ArtifactUploadJournalV2},
...
pub fn upload_artifact<U: ArtifactObjectUploader>(
    ...
    let mut journal = match artifact_journal::load(&journal_path)? {
        Some(journal) => {
            validate_upload_journal(&journal, expected_tenant_id, &request_digest, &request)?;
            journal
        }
        None => ArtifactUploadJournalV2::new(
...
fn validate_upload_journal(
    journal: &ArtifactUploadJournalV2,
    expected_tenant_id: &ResourceId,
    ...
    if &journal.request_digest != request_digest
        ...
        || &journal.tenant_id != expected_tenant_id
    {
        return Err(ArtifactClientError::InvalidResponse(
...
fn validate_refreshed_prepare(
    ...
    if refreshed.artifact_id != previous.artifact_id
        ...
        || refreshed.upload_expires_at != previous.upload_expires_at
    {
        return Err(ArtifactClientError::InvalidResponse(
            "prepare replay changed the Artifact, Operation, Grant, generation fence, or upload deadline".to_owned(),
        ));
    }
    Ok(())
}
