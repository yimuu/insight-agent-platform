...
mod tests {
    ...
    #[test]
    fn near_expiry_upload_replays_the_same_deadline_then_waits_for_exact_ready_artifact() {
        let directory = TempDir::new().unwrap();
        let source = directory.path().join("input.txt");
        fs::write(&source, b"upload body").unwrap();
        ...
        let prepared = PrepareArtifactUploadResponseV1 {
            ...
            upload_expires_at: UtcTimestamp::from_datetime(
                Utc::now() + chrono::Duration::seconds(25),
            ),
        };
        let mut drifted = prepared.clone();
        drifted.upload_expires_at =
            UtcTimestamp::from_datetime(Utc::now() + chrono::Duration::hours(1));
        assert!(validate_refreshed_prepare(&prepared, &drifted).is_err());
        assert!(validate_refreshed_prepare(&prepared, &prepared).is_ok());
                ...
                match step {
                    0 | 1 => {
                        ...
                        assert!(header_value(&head, "idempotency-key")
                            .is_some_and(|value| value.ends_with("-prepare")));
                        let actual = (
                            header_value(&head, "idempotency-key").unwrap().to_owned(),
                            body.clone(),
                        );
                        if let Some(first) = &first_prepare {
                            assert_eq!(
                                &actual, first,
                                "near expiry keeps the original preparation intent"
                            );
                        } else {
                            first_prepare = Some(actual);
                        }
    ...
    #[test]
    fn separate_upload_intents_use_distinct_receipts_and_retry_keeps_identity_over_http() {
        let directory = TempDir::new().unwrap();
            ...
            for _ in 0..3 {
                ...
                let (mut stream, _) = loop {
                    match listener.accept() {
                        ...
                        Err(error) => panic!("bounded upload fixture accept: {error}"),
                    }
                };
                let (head, body) = read_request(&mut stream);
                requests.push((
                ...
                let failure = problem(
                    ...
                    true,
                    Some(100),
                    request_trace_id(&head).parse().unwrap(),
                );
                write_problem_response(&mut stream, "503 Service Unavailable", &failure);
            }
            assert_eq!(
                requests[0], requests[1],
                "same unknown outcome must retain original request identity"
            );
            assert_ne!(
                requests[0].0, requests[2].0,
                "new local upload intent is not content identity"
            );
        ...
        for folder in ["a", "a", "b"] {
            assert!(matches!(
                ...
                Err(ArtifactClientError::Public(PublicClientError::Problem(_)))
            ));
        }
        server.join().unwrap();
        assert_eq!(puts.load(Ordering::SeqCst), 0);
        ...
        let path = fs::read_dir(directory.path().join("a"))
            .unwrap()
            .next()
            .unwrap()
            .unwrap()
        ...
        for corrupt in [
            ...
            },
            {
                ...
                format!("{{\"schema_version\":2,{}", &text[1..]).into_bytes()
            },
            vec![b' '; 32_769],
        ] {
            fs::write(&path, corrupt).unwrap();
            assert!(artifact_journal::load(&path).is_err());
        }
        ...
        #[cfg(unix)]
        {
            ...
            assert!(artifact_journal::load(&linked).is_err());
