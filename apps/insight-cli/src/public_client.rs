    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidResponse(detail) => {
                write!(
                    formatter,
                    "public API returned an invalid response: {detail}"
                )
            }
...
impl PublicHttpClient {
    ...
    /// A bounded GET query; the route is validated separately from URL-encoded
    /// pairs. It grants no mutation/Receipt exemption and accepts no raw URL.
    pub fn get_body_json_query<T: DeserializeOwned>(
        &self,
        path: &str,
        query: &[(String, String)],
        expected_status: StatusCode,
    ...
    /// The single registered POST query; no arbitrary path or command exemption is accepted.
    pub fn resolve_agent_bindings(
        &self,
        request: &insight_platform_registry::authoring::ResolveAgentBindingsRequestV1,
    ) -> Result<
    ...
    > {
        ...
        request.validate().map_err(|_| {
            PublicClientError::InvalidConfiguration("invalid authoring selection query")
        })?;
        let body = serde_json::to_vec(request).map_err(|_| {
            PublicClientError::InvalidConfiguration("authoring query is not serializable")
        })?;
        let response = self
            .client
            .post(format!(
            ...
            .send()
            .map_err(|error| PublicClientError::Transport(error.to_string()))?;
        let response: PublicBodyResponse<ResolveAgentBindingsResponseV1> =
            decode_body_response_with_limit(
                response,
                StatusCode::OK,
                MAX_AUTHORING_QUERY_RESPONSE_BYTES,
            )?;
        response.body.validate_for(request).map_err(|_| {
            PublicClientError::InvalidResponse(
                "authoring resolution violates its request binding".to_owned(),
            )
        })?;
        Ok(response.body)
    }
    ...
    /// A command whose owning public contract returns an empty 204 response.
    pub fn post_json_no_content<Request: Serialize>(
        &self,
        path: &str,
        body: &Request,
        idempotency_key: &str,
    ) -> Result<(), PublicClientError> {
        validate_public_path(path)?;
        let body = serde_json::to_vec(body).map_err(|_| {
            PublicClientError::InvalidConfiguration("public request body is not serializable")
        })?;
        ...
        let key = bounded_header(idempotency_key, "idempotency key")?;
        ...
        let response = self
            ...
            .header(TRACEPARENT, format!("00-{trace}-{}-01", SpanId::new()))
            .body(body)
            .send()
            .map_err(|error| PublicClientError::Transport(error.to_string()))?;
        let status = response.status();
        let returned_trace = required_header(&response, "trace-id")?
            .parse::<TraceId>()
            .map_err(|_| PublicClientError::InvalidResponse("trace-id is invalid".into()))?;
        if trace != returned_trace {
            return Err(PublicClientError::InvalidResponse(
                "trace-id differs from command request".into(),
            ));
        }
        if status != StatusCode::NO_CONTENT {
            ...
            require_json_content_type(&response)?;
            return decode_problem(response, status, returned_trace);
        }
        if required_header(&response, CACHE_CONTROL.as_str())? != OPERATION_CACHE_CONTROL
            || response.content_length().is_some_and(|length| length != 0)
            || !read_bounded_body_with_limit(response, 1)?.is_empty()
        {
            return Err(PublicClientError::InvalidResponse(
                "empty command response violates its private 204 contract".into(),
            ));
        }
        Ok(())
    }
...
fn decode_body_response<T: DeserializeOwned>(
    ...
    decode_body_response_with_limit(response, expected_status, MAX_PUBLIC_RESPONSE_BYTES)
}
fn decode_body_response_with_limit<T: DeserializeOwned>(
    response: Response,
    expected_status: StatusCode,
    maximum: usize,
) -> Result<PublicBodyResponse<T>, PublicClientError> {
    ...
    let body = read_bounded_body_with_limit(response, maximum)?;
...
fn validate_public_query(query: &[(String, String)]) -> Result<(), PublicClientError> {
    ...
    {
        return Err(PublicClientError::InvalidConfiguration(
            "public GET query exceeds its closed bounds",
        ));
    }
    Ok(())
}
...
fn read_bounded_body(response: Response) -> Result<Vec<u8>, PublicClientError> {
    read_bounded_body_with_limit(response, MAX_PUBLIC_RESPONSE_BYTES)
}
fn read_bounded_body_with_limit(
    mut response: Response,
    maximum: usize,
) -> Result<Vec<u8>, PublicClientError> {
    if response
        .headers()
        .get(CONTENT_LENGTH)
        .and_then(|value| value.to_str().ok())
