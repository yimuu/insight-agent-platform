...
use insight_platform_api::task::{SubmitTaskInputV1, TaskViewV2};
    ...
    fn validate(&self) -> Result<(), TaskJournalError> {
        if self.schema_version != 1
            ...
            || self
                .result
                .as_ref()
                .is_some_and(|result| result.task_id != self.task_id || result.validate().is_err())
        {
