...
use production_orchestration::{
    start_production_orchestration, ProductionOrchestrationConfig, RunningProductionOrchestration,
};
...
use insight_platform_observability::{
    DurableJobQueueSnapshot, DurableOutboxSnapshot, OrchestrationOperationalMetrics,
    OrchestrationOperationalSnapshot, ProcessHttpMetrics, PROCESS_OBSERVABILITY_OPERATIONS,
};
use insight_platform_plan::{ExpressionLimits, PlanLimits};
...
use insight_platform_runtime::{
    CoordinatorTiming, OrchestrationCoordinatorConfig, OrchestrationExecutorConfig,
    OrchestrationExecutorTiming, OrchestrationSafetyConfig, SafetyDriverTiming,
...
async fn run() -> Result<(), ProcessError> {
    ...
    let supported_catalog = insight_platform_plan::execution::program_execution_capabilities();
            ...
            _ = health.tick() => {
                ...
                match observe_durable_outbox(bulkheads.critical_control_pool()).await {
                    Ok(snapshot) => operational_metrics.observe_durable_outbox(DurableOutboxSnapshot {
                        ...
                        expired_oldest_lag_seconds: snapshot.expired_oldest_lag_seconds,
                        incompatible_events: snapshot.incompatible_events,
                    }),
                    Err(_) => operational_metrics.observe_durable_observation_failure(),
                }
...
fn update_operational_metrics(
    ...
    metrics.update(OrchestrationOperationalSnapshot {
        ...
        recovery_invalid_objects: recovery.invalid_objects,
