...
use insight_platform_artifact_rpc::{
    ...
    ArtifactDataWorkerGrpcService, ArtifactInternalRpcLimits, ArtifactSchedulerGrpcService,
    ArtifactWorkloadStageAuthority, ArtifactWorkloadStageError, LeasedArtifactBytes,
    SchedulerRunValueResponseBroker, SchedulerSkillPackageResponseBroker,
    SchedulerTypedPlanResponseBroker, SchedulerWorkloadIdentity, WorkloadArtifactProducerIdentity,
};
...
use insight_platform_contracts::{
    canonical_digest, parse_strict_json, CommandOutcome, JobKind, JsonLimits, Sha256Digest,
};
use insight_platform_observability::{
    DurableJobQueueMetrics, ProcessHttpMetrics, PROCESS_OBSERVABILITY_OPERATIONS,
};
        ...
        let staged = tokio::time::timeout(
            ...
            self.provider.stage_bytes(
                AwsArtifactUploadRequest {
                    ...
                    expires_at: authorized.deadline.into(),
                },
    ...
    tokio::select! {
        ...
        signal = shutdown_signal() => {
            ...
            tokio::time::timeout(
                ...
                async {
                    let (controller, worker, metrics_server, _, _, _) = tokio::join!(
...
mod tests {
        ...
        serde_json::from_value(serde_json::json!({
            ...
            "scan_worker": {
                "worker_manifest": insight_platform_contracts::WorkerManifest {
                    ...
                    worker_role: "artifact-data-worker".to_owned(), work_class: insight_platform_contracts::WorkClass::Artifact,
                    worker_build_digest: insight_platform_worker::execution::executable_digest(&std::env::current_exe().unwrap()).unwrap(),
                    adapter_runtime_digest: format!("sha256:{}", "a".repeat(64)).parse().unwrap(),
                    execution_capabilities: insight_platform_artifacts::execution::data_worker_execution_capabilities(&insight_platform_artifacts::execution::integrity_scanner_contract_digest()),
                    protocol_version: insight_platform_contracts::WORKER_PROTOCOL_VERSION, max_concurrency: 8, critical_control_reserved_slots: 1,
                },
    ...
    fn process_config_closes_provider_database_and_rpc_bounds() {
        ...
        let mut invalid_scanner = valid.clone();
        invalid_scanner.scan_worker.scanner_contract_digest =
            format!("sha256:{}", "f".repeat(64)).parse().unwrap();
        assert!(invalid_scanner.validate().is_err());
