...
use insight_platform_postgres::{
    ...
    repository::{ArtifactWorkerRole, ClaimArtifactJobs, PgRepository},
};
...
impl ScanWorkerConfig {
    pub(crate) fn validate(&self) -> Result<(), ()> {
        if self.scanner_contract_digest
...
pub(crate) async fn run_scan_worker(
    ...
    insight_platform_worker::execution::validate_installed_worker(
        ...
        &insight_platform_artifacts::execution::data_worker_execution_capabilities(
            &config.scanner_contract_digest,
        ),
        "artifact-data-worker",
        insight_platform_contracts::WorkClass::Artifact,
    )
    .map_err(|error| error.to_string())?;
    let worker_id = new_id(ResourceKind::WorkerProcessGeneration)?;
    let pools = LocalWorkerPools::new(config.worker_manifest.clone(), worker_id.clone())
        .map_err(|error| error.to_string())?;
    let claim_limit = ClaimBatchHardLimit::from_profile(
    ...
    loop {
        ...
        let Some(reservation) = pools
            .reserve_claim_capacity(
                insight_platform_contracts::WorkClass::Artifact,
                usize::from(config.claim_batch),
                claim_limit,
            )
            .map_err(|error| error.to_string())?
        else {
            return Err("Artifact execution capacity unavailable".to_owned());
        };
            ...
            .map(|job| {
                Ok(ClaimedJobIdentity {
                    job_id: job
                        .job_id
                        .parse()
                        .map_err(|_| "Artifact claim Job identity invalid".to_owned())?,
                    lease_generation: u64::try_from(job.lease_epoch)
                        .map_err(|_| "Artifact claim epoch invalid".to_owned())?,
                })
            })
        ...
        for (claimed_job, permit) in claimed.into_iter().zip(permits) {
...
async fn execute_claimed_scan(
    ...
    claimed: insight_platform_jobs::store::JobRecord,
) -> Result<(), String> {
