...
use insight_platform_registry_validation_worker::{
    RegistryArtifactClient, RegistryValidationWorkerConfig, RegistryValidationWorkerDriver,
    REGISTRY_VALIDATION_WORKER_ROLE,
};
...
async fn run() -> Result<(), ProcessError> {
    ...
    let artifacts = Arc::new(
        RegistryArtifactClient::install(
            &required("PLATFORM_REGISTRY_VALIDATION_ARTIFACT_ENDPOINT")?,
            &read_bounded(&required_absolute_path(
