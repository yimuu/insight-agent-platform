...
use insight_platform_jobs::store::{
    SafeScanDiagnostic, SafetyScanCursor, SafetyScanDiagnosticCode, SafetyScanPage,
    SafetyScanPhase, SafetyScanShard,
};
...
use insight_platform_model_adapters::{
    ...
    ModelAdapterFailureClass, ModelAdapterWorker, ModelLiveDeltaSink, ModelOutputMaterializer,
    ...
    async fn heartbeat_model_execution(
        ...
        let record = self
            .heartbeat_job(insight_platform_jobs::store::HeartbeatJob {
                fence: insight_platform_jobs::store::JobCommandFence {
...
pub trait ModelCancellationSource: Send + Sync + 'static {
    async fn scan_model_cancellations(
        ...
        limit: u16,
        limits: ModelTurnLimits,
    ) -> Result<SafetyScanPage<ModelCancellationCandidate>, ModelCancellationSourceFailure>;
}
...
impl ModelCancellationSource for PgRepository {
    async fn scan_model_cancellations(
        ...
        let page = self
            .scan_cancelling_model_executions(worker_process_generation_id, after, limit)
            .await
        ...
        for controlled in page.records {
            if controlled.turn.deadline <= now {
                continue;
            }
            // PG already validated the stored owner. Disagreement with process limits is fatal.
            controlled
                .turn
                .validate(limits)
                .map_err(|_| ModelCancellationSourceFailure::Invariant)?;
            // The process adapter registry is shared configuration, not an isolatable row.
            if !matches!(
                controlled
                    .turn
                    .payload
            ...
            let diagnostic = SafeScanDiagnostic {
                schema_version: 1,
                tenant_id: controlled.turn.tenant_id.clone(),
                item_id: controlled.turn.model_turn_id.clone(),
            ...
            match candidate {
                ...
                Err(ModelCancellationSourceFailure::Invariant) => diagnostics.push(diagnostic),
...
pub struct ModelCancellationDriver<S, A, P, I> {
    ...
    scan_cursor: tokio::sync::Mutex<Option<SafetyScanCursor>>,
}
    ...
    I: ModelWorkerIdentityFactory + 'static,
{
    pub fn new(
        ...
        Ok(Self {
            ...
            pools,
            config,
            scan_cursor: tokio::sync::Mutex::new(None),
        })
    }
    ...
    pub async fn drive_once(
        ...
        // Serialize local cycles so concurrent callers cannot regress the read cursor.
        let mut scan_cursor = self.scan_cursor.lock().await;
        ...
        if (!page.exhausted && page.next_cursor.is_none())
            || page
                .next_cursor
                .as_ref()
                .is_some_and(|cursor| cursor.validate(ResourceKind::Job).is_err())
        {
            return Err(ModelCancellationDriverError::CorruptCandidate);
        }
        // Progress follows scanned rows, including invalid and deferred objects.
        *scan_cursor = if page.exhausted {
            None
        } else {
            page.next_cursor
        };
        for diagnostic in &page.diagnostics {
            tracing::warn!(phase = ?diagnostic.phase, code = ?diagnostic.code,
                "invalid persisted Model cancellation object retained");
        }
        let mut report = ModelCancellationCycleReport {
            discovered: page.records.len() as u64,
            ..Default::default()
        };
    ...
    I: ModelWorkerIdentityFactory + 'static,
{
    ...
    pub async fn drive_once(
        ...
        for ((claim, binding), permit) in claimed.into_iter().zip(bindings).zip(permits) {
            let trace = ExecutionTraceContext::start(binding.trace, TraceFlags::NotSampled)
...
mod tests {
    ...
    impl ModelCancellationSource for CancellationSource {
        ...
        ) -> Result<SafetyScanPage<ModelCancellationCandidate>, ModelCancellationSourceFailure>
        {
            ...
            Ok(SafetyScanPage {
                records: self.candidates.lock().unwrap().take().unwrap_or_default(),
                diagnostics: Vec::new(),
                next_cursor: None,
                exhausted: true,
            })
    ...
    struct PagedCancellationSource {
        expected_generation: ResourceId,
        pages: Mutex<
            std::collections::VecDeque<(
                Option<SafetyScanCursor>,
                SafetyScanPage<ModelCancellationCandidate>,
            )>,
        >,
    }

    #[async_trait]
    impl ModelCancellationSource for PagedCancellationSource {
        async fn scan_model_cancellations(
            &self,
            worker_process_generation_id: &ResourceId,
            after: Option<SafetyScanCursor>,
        ...
        ) -> Result<SafetyScanPage<ModelCancellationCandidate>, ModelCancellationSourceFailure>
        {
            assert_eq!(worker_process_generation_id, &self.expected_generation);
            assert_eq!(limit, 1);
            let (expected_after, page) = self.pages.lock().unwrap().pop_front().unwrap();
    ...
    #[tokio::test]
    async fn cancellation_cursor_crosses_invalid_and_deferred_pages_then_restarts() {
        for deferred in [false, true] {
            let process_generation = resource(ResourceKind::WorkerProcessGeneration);
            let pools =
                LocalWorkerPools::new(model_manifest(1), process_generation.clone()).unwrap();
            let first = cancellation_candidate(&pools);
            let second = cancellation_candidate(&pools);
            let now = Utc::now();
            let head = SafetyScanCursor {
                sort_at: now,
                tenant_id: first.tenant_id.clone(),
                item_id: resource(ResourceKind::Job),
            };
            let cursor_first = SafetyScanCursor {
                sort_at: now + ChronoDuration::milliseconds(1),
                tenant_id: first.tenant_id.clone(),
                item_id: first.job_id.clone(),
            };
            let cursor_second = SafetyScanCursor {
                sort_at: now + ChronoDuration::milliseconds(2),
                tenant_id: second.tenant_id.clone(),
                item_id: second.job_id.clone(),
            };
            let diagnostic = SafeScanDiagnostic {
                schema_version: 1,
                tenant_id: head.tenant_id.clone(),
                item_id: head.item_id.clone(),
                phase: SafetyScanPhase::JobDecode,
                code: SafetyScanDiagnosticCode::InvalidPersistedObject,
            };
            let source = Arc::new(PagedCancellationSource {
                expected_generation: process_generation,
                pages: Mutex::new(std::collections::VecDeque::from([
                    ...
                    (
                        ...
                        SafetyScanPage {
                            ...
                            diagnostics: vec![],
                            next_cursor: Some(cursor_first.clone()),
                            exhausted: false,
                        },
                    ),
                    (
                        ...
                        SafetyScanPage {
                            ...
                            diagnostics: vec![],
                            next_cursor: Some(cursor_second.clone()),
                            exhausted: false,
                        },
                    ),
                    (
            ...
            let port = Arc::new(CancellationPort {
                response: if deferred {
                    CancelResponse::Failure(ModelAdapterFailure {
                        class: ModelAdapterFailureClass::RetryableAfterDispatch,
                        safe_code: "model_cancel_temporarily_unavailable".to_owned(),
                        safe_message: "Model cancellation is temporarily unavailable".to_owned(),
                        evidence_digest: digest('9'),
                        request_sent: true,
                        retry_at: Some(first.deadline - ChronoDuration::seconds(1)),
                    })
                } else {
                    CancelResponse::Outcome(ModelAdapterCancelOutcome::Accepted)
                },
                requests: Mutex::new(Vec::new()),
            });
            let mut config = cancellation_config();
            config.scan_limit = 1;
            let driver = ModelCancellationDriver::new(
                Arc::clone(&source),
                Arc::clone(&authority),
            ...
            .unwrap();
            assert_eq!(
                driver.drive_once().await.unwrap(),
                ModelCancellationCycleReport::default()
            );
            for _ in 0..2 {
                let report = driver.drive_once().await.unwrap();
                assert_eq!(report.discovered, 1);
                assert_eq!(report.deferred, u64::from(deferred));
                assert_eq!(report.cancelled, u64::from(!deferred));
            }
            for _ in 0..2 {
                assert_eq!(
                    driver.drive_once().await.unwrap(),
            ...
            assert_eq!(
                requests
                    .iter()
                    .map(|(_, request)| &request.job_id)
                    .collect::<Vec<_>>(),
                vec![&first.job_id, &second.job_id]
            );
            assert_eq!(
                authority.commands.lock().unwrap().len(),
                if deferred { 0 } else { 2 }
            );
        }
    }
    ...
    #[tokio::test]
    #[ignore = "requires the real mTLS NATS qualification harness"]
    async fn real_tls_nats_publishes_live_delta() {
        let server = std::env::var("PLATFORM_TEST_NATS_URL")
            .expect("PLATFORM_TEST_NATS_URL is required for real mTLS NATS");
        let ca = std::env::var("PLATFORM_TEST_NATS_CA_PATH")
            .expect("PLATFORM_TEST_NATS_CA_PATH is required for real mTLS NATS");
        let certificate = std::env::var("PLATFORM_TEST_NATS_CERT_PATH")
            .expect("PLATFORM_TEST_NATS_CERT_PATH is required for real mTLS NATS");
        let private_key = std::env::var("PLATFORM_TEST_NATS_KEY_PATH")
            .expect("PLATFORM_TEST_NATS_KEY_PATH is required for real mTLS NATS");
        let limits = ModelTurnLimits::from_profile(&checked_in_hard_limit_profile()).unwrap();
