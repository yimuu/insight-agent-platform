...
use insight_platform_postgres::{
    ...
    repository::{ClaimJobs, PgRepository, RepositoryError},
};
...
impl ContextDatasetBuilderDriver {
    pub fn new(
        ...
        let capabilities = insight_platform_contracts::WorkerExecutionCapabilities {
            schema_version: 1,
            capabilities: sources
                .iter()
                .map(|source| source.binding.adapter_contract_digest.clone())
        ...
        let pools = LocalWorkerPools::new(manifest, process_generation_id.clone())
            .map_err(|_| ContextDatasetBuilderError::InvalidConfiguration)?;
        Ok(Self {
            repository,
            stager,
    ...
    pub async fn drive_once(
        ...
        *recovery_cursor = page.next_cursor;
        ...
        for diagnostic in &page.diagnostics {
            eprintln!(
                "Context Dataset retained invalid durable object: phase={:?}, code={:?}",
                diagnostic.phase, diagnostic.code,
            );
        }
        let recovered = page.records.len() as u64;
