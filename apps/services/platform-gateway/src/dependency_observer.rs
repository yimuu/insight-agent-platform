...
struct GatewayPostgresObserver(Arc<DependencyObservationMetrics>);

impl PostgresHealthObserver for GatewayPostgresObserver {
    fn observe(&self, outcome: PostgresHealthOutcome) {
        self.0
            ...
            .expect("Gateway roles install their PostgreSQL dependency metric");
    }
}
...
> {
    ...
    let observer: Arc<dyn PostgresHealthObserver> =
        Arc::new(GatewayPostgresObserver(Arc::clone(&metrics)));
    Ok((metrics, observer))
}
...
mod tests {
    ...
    fn adapter_maps_only_fixed_postgresql_outcomes() {
        ...
        let rendered = ProcessHttpMetrics::install("runtime-api", PROCESS_OBSERVABILITY_OPERATIONS)
            .unwrap()
