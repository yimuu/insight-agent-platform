...
use insight_platform_api::{
    ...
    run::{
        ...
        ListRunsIntent, ReadRunEventsIntent, ReadRunIntent, ReadRunValueIntent, RunApplication,
        RunApplicationError, RunEventCursorCodec, RunEventProjectionV1, RunHttpState,
        RunResultViewV1, RunValueMetadataV1, RunViewV1, SignalRunIntent, SystemRunClock,
    },
    task::{
        build_task_router, task_etag, ListTasksIntent, ReadTaskIntent, ResolveTaskIntent,
        SystemTaskClock, TaskApplication, TaskApplicationError, TaskFormV2, TaskHttpState,
...
use insight_platform_contracts::{
    ...
    JsonLimits, OperationViewV1, PrincipalKind, PrincipalSnapshot, ReadOperation,
    RegistryResourceKind, ResourceDocument, ResourceDraftPayload, ResourceId, ResourceKind,
    RunBindingsSnapshot, Sha256Digest, UtcTimestamp, ValueRef, MAX_ARTIFACT_BYTES,
};
...
use insight_platform_orchestrator::{AdmitRun, RequestRunCancel, RunInputValue, SetRunPause};
...
use insight_platform_tasks::{TaskAction, TaskDefinition, TaskPayload, TaskQueryPurpose};
...
#[derive(Clone)]
struct PgAuthoring(Arc<PgRepository>);
#[async_trait]
impl insight_platform_api::authoring::AuthoringApplication for PgAuthoring {
    async fn discover(
        &self,
        intent: insight_platform_api::authoring::DiscoverAuthoringIntent,
    ) -> Result<
        insight_platform_api::product::AuthorityListPage<
            insight_platform_registry::authoring::AuthoringDependencyV1,
        >,
        insight_platform_registry::authoring::AuthoringQueryError,
    > {
        use insight_platform_api::product::{AuthorityListPage, ListKeysetBoundary};
        use insight_platform_registry::authoring::{
            AuthoringQueryError, DiscoverAuthoringDependencies,
        };
        let boundary = match intent.boundary {
            Some(ListKeysetBoundary::AuthoringDependency {
                created_at,
                deployment_id,
            }) => Some((
                chrono::DateTime::parse_from_rfc3339(created_at.as_str())
        ...
        let page = tokio::time::timeout(
            ...
            self.0
                .discover_agent_authoring_dependencies(DiscoverAuthoringDependencies {
        ...
        Ok(AuthorityListPage {
            ...
            next_boundary: page.next_boundary.map(|(created_at, deployment_id)| {
                ListKeysetBoundary::AuthoringDependency {
                    created_at: UtcTimestamp::from_datetime(created_at),
                    deployment_id,
                }
            }),
        })
    ...
    async fn resolve(
        &self,
        intent: insight_platform_api::authoring::ResolveAuthoringIntent,
    ) -> Result<
        insight_platform_registry::authoring::ResolveAgentBindingsResponseV1,
        insight_platform_registry::authoring::AuthoringQueryError,
    > {
        tokio::time::timeout(
            std::time::Duration::from_secs(5),
            self.0.resolve_agent_authoring_bindings(
...
trait ArtifactMutationForwarder: Send + Sync {
    async fn read_registry_input(
        &self,
        _principal: &insight_platform_api::authentication::AuthenticatedPrincipal,
        _request: &insight_platform_artifacts::RegistryArtifactReadRequestV1,
    ) -> Result<insight_platform_artifacts::RegistryArtifactReadResponseV1, ArtifactApplicationError>
    {
...
impl ArtifactMutationForwarder for MtlsArtifactMutationForwarder {
    async fn read_registry_input(
        &self,
        principal: &insight_platform_api::authentication::AuthenticatedPrincipal,
        request: &insight_platform_artifacts::RegistryArtifactReadRequestV1,
    ) -> Result<insight_platform_artifacts::RegistryArtifactReadResponseV1, ArtifactApplicationError>
    {
        use insight_platform_artifacts::MAX_REGISTRY_ARTIFACT_RESPONSE_BYTES;
        ...
        if response
            .content_length()
            .is_some_and(|length| length > MAX_REGISTRY_ARTIFACT_RESPONSE_BYTES as u64)
        {
            return Err(ArtifactApplicationError::TooLarge);
        }
        ...
        let result: insight_platform_artifacts::RegistryArtifactReadResponseV1 =
            serde_json::from_value(value).map_err(|_| ArtifactApplicationError::Internal)?;
        result
            .decode_verified()
            .map_err(|_| ArtifactApplicationError::Internal)?;
        Ok(result)
...
#[derive(Clone)]
struct PgTasks(Arc<PgRepository>, Arc<dyn ArtifactMutationForwarder>);
...
impl TaskApplication for PgTasks {
    async fn list_tasks(
        ...
        let boundary = match intent.boundary {
            Some(ListKeysetBoundary::Task {
                created_at,
                task_id,
            }) => Some((
                parse_utc_timestamp(&created_at).map_err(|_| TaskApplicationError::Invalid)?,
                task_id,
            )),
            None => None,
            _ => return Err(TaskApplicationError::Invalid),
        };
        let page = self
            .0
            .list_task_inbox(insight_platform_tasks::store::TaskInboxQuery {
                tenant_id: intent.principal.tenant_id,
                principal_id: intent.principal.principal_id,
                principal_kind: intent.principal.principal_kind,
                purpose: intent.filters.purpose,
                state: intent.filters.state,
                kind: intent.filters.kind,
                run_id: intent.filters.run_id,
                snapshot_at: intent.snapshot_at,
                boundary,
                page_size: intent.page_size,
            })
        ...
        Ok(AuthorityListPage {
            ...
            items: page
                .records
                .into_iter()
                .map(task_view_from_record)
                .collect::<Result<Vec<_>, _>>()?,
            next_boundary: page.next_scanned_boundary.map(|(at, task_id)| {
                ListKeysetBoundary::Task {
                    created_at: UtcTimestamp::from_datetime(at),
                    task_id,
                }
            }),
        })
    ...
    async fn read_task_form(
        ...
        if intent.purpose != TaskQueryPurpose::Respondable {
        ...
        let record = self
            ...
            .await
            .map_err(map_task_repository_error)?;
        let view = task_view_from_record(record.clone())?;
        let record = record.task;
        let payload = decode_task_payload(&record)?;
        let response_schema = match (&payload.definition, payload.response_schema) {
            (
                TaskDefinition::Interaction { .. } | TaskDefinition::HumanWork { .. },
                Some(schema),
            ) if record.payload.schema_version == 2 => {
                insight_platform_contracts::InteractionSchemaDocument {
                    schema_version: schema.schema_version,
                    profile: schema.profile,
                    schema: schema.schema,
                    canonical_digest: schema.canonical_digest,
                }
            }
            (
            ...
            _ => return Err(TaskApplicationError::FormUnavailable),
        };
        let form = TaskFormV2 {
            schema_version: 2,
            etag: view.etag,
            ...
            version: u64::try_from(record.version).map_err(|_| TaskApplicationError::Internal)?,
    ...
    async fn resolve_task(
        ...
        if intent.task_id.kind() == ResourceKind::ApprovalTask {
            if current.owner_kind == "artifact" {
                ...
                let decision = match intent.action {
                    TaskAction::Approve => ArtifactDeletionApprovalDecision::Approve,
                    TaskAction::Reject => ArtifactDeletionApprovalDecision::Reject,
                    TaskAction::Cancel => ArtifactDeletionApprovalDecision::Cancel,
                    TaskAction::SubmitInput => return Err(TaskApplicationError::Invalid),
                };
            ...
            let decision = match intent.action {
                TaskAction::Approve => CapabilityApprovalDecision::Approve,
                TaskAction::Reject => CapabilityApprovalDecision::Reject,
                TaskAction::Cancel => CapabilityApprovalDecision::Cancel,
                TaskAction::SubmitInput => return Err(TaskApplicationError::Invalid),
            };
            let payload = decode_task_payload(&current)?;
        ...
        if matches!(
            decode_task_payload(&current)?.definition,
            TaskDefinition::CapabilityInput { .. }
        ) {
            use insight_platform_invocations::{
                CapabilityInputAction, CapabilityInputResponse, ResolveCapabilityInput,
            };
            let control = self
                .0
                .read_capability_task_control(
                    &intent.principal.tenant_id,
                    &intent.principal.principal_id,
                    intent.principal.principal_kind,
                    &intent.task_id,
                ...
                .await
                .map_err(map_task_repository_error)?;
            if control.task.version != current.version
                || control.task.payload.digest != current.payload.digest
            {
                return Err(TaskApplicationError::Conflict);
            }
            let TaskDefinition::CapabilityInput {
                ...
                eligible_principal_rule_digest,
                ..
            } = decode_task_payload(&control.task)?.definition
            else {
                return Err(TaskApplicationError::Internal);
            };
            let action = match intent.action {
                TaskAction::SubmitInput => CapabilityInputAction::Accept,
                TaskAction::Reject => CapabilityInputAction::Decline,
                TaskAction::Cancel => CapabilityInputAction::Cancel,
                TaskAction::Approve => return Err(TaskApplicationError::Invalid),
            };
            let response = intent
                .input
                .as_ref()
                .map(
                    |input| -> Result<CapabilityInputResponse, TaskApplicationError> {
                        Ok(CapabilityInputResponse {
                            value_id: new_task_id(ResourceKind::RunValue)?,
                            classification: input.classification,
                            schema_digest: input.schema_digest.clone(),
                            content_digest: match &input.value {
                                ValueRef::Inline { value } => canonical_digest(value)
                                    .map_err(|_| TaskApplicationError::InvalidTaskInput)?
                                    .parse()
                                    .map_err(|_| TaskApplicationError::Internal)?,
                                ...
                                _ => return Err(TaskApplicationError::InvalidTaskInput),
                            },
                            artifact_link_id: matches!(&input.value, ValueRef::Artifact { .. })
                                .then(|| new_task_id(ResourceKind::ArtifactLink))
                                .transpose()?,
                ...
                .transpose()?;
            let validated_input = match &response {
                Some(response) => Some(
            ...
            let command = ResolveCapabilityInput {
                validated_input,
                audit: task_command_audit(&intent)?,
                invocation_id: current
                    ...
                    .map_err(|_| TaskApplicationError::Internal)?,
                job_id,
                input_task_id: intent.task_id.clone(),
                expected_invocation_version: control.invocation_version,
                expected_job_version: control.job_version,
                expected_wake_generation: wake_generation,
                expected_task_generation: u64::try_from(current.generation)
                    .map_err(|_| TaskApplicationError::Internal)?,
                expected_task_version: intent.expected_task_version,
                eligible_principal_rule_digest,
            ...
            let mut transaction = self
                ...
                .map_err(map_task_repository_error)?;
            transaction
                .resolve_capability_input(command)
                .await
                .map_err(map_task_repository_error)?;
            transaction
                .commit()
        ...
        if intent.task_id.kind() != ResourceKind::Interaction
            || matches!(intent.action, TaskAction::Approve)
        {
            return Err(TaskApplicationError::Invalid);
        }
        let target = intent
            .action
            .target(current.task_kind)
        ...
        let validated_input = match &response {
            Some(response) => Some(
                self.validate_response(&intent, &current, &response.schema_digest, &response.value)
                    .await?,
            ),
            None => None,
        };
...
impl PgTasks {
    async fn validate_response(
        &self,
        intent: &ResolveTaskIntent,
        current: &insight_platform_tasks::store::TaskRecord,
        schema_digest: &Sha256Digest,
        value: &ValueRef,
    ) -> Result<insight_platform_tasks::ValidatedTaskInput, TaskApplicationError> {
        let authority = self
            .0
            .read_task_form_for_principal(
                &intent.principal.tenant_id,
            ...
            .await
            .map_err(map_task_repository_error)?;
        let authority = authority.task;
        if authority.version != current.version
            || authority.payload.digest != current.payload.digest
        {
            return Err(TaskApplicationError::Conflict);
        }
        let payload = decode_task_payload(&authority)?;
        let schema = match (&payload.definition, payload.response_schema) {
            (
                TaskDefinition::CapabilityInput {
                    response_schema, ..
                },
            ...
            (
                TaskDefinition::HumanWork { .. } | TaskDefinition::Interaction { .. },
                Some(schema),
            ) => insight_platform_contracts::InteractionSchemaDocument {
                schema_version: schema.schema_version,
                profile: schema.profile,
        ...
        if schema_digest != &schema.canonical_digest {
            return Err(TaskApplicationError::InvalidTaskInput);
        }
        match value {
            ...
            ValueRef::Artifact { artifact } => {
                if schema.profile == insight_platform_contracts::MCP_FORM_SCHEMA_PROFILE_ID
                    || artifact.byte_length() > 65_536
                {
                    return Err(TaskApplicationError::InvalidTaskInput);
                }
                let frozen = insight_platform_contracts::ClosedJsonSchema {
                    schema_version: schema.schema_version,
                    profile: schema.profile,
                    schema: schema.schema,
                    canonical_digest: schema.canonical_digest,
                ...
                let mut stream = self
                    .1
                    .read_content(ReadArtifactIntent {
                        principal: intent.principal.clone(),
                        artifact_id: artifact.artifact_id().clone(),
                        deadline: intent.deadline,
                    })
                    .await
                    .map_err(map_task_artifact_error)?;
                if stream.content_length != artifact.byte_length() {
                    return Err(TaskApplicationError::InvalidTaskInput);
                }
                let mut bytes = Vec::with_capacity(stream.content_length as usize);
                while let Some(chunk) = futures::StreamExt::next(&mut stream.stream).await {
                    let chunk = chunk.map_err(|_| TaskApplicationError::Unavailable)?;
                    if bytes.len().saturating_add(chunk.len()) > 65_536 {
                        return Err(TaskApplicationError::InvalidTaskInput);
                    }
                    bytes.extend_from_slice(&chunk);
                }
                insight_platform_tasks::ValidatedTaskInput::validate_artifact(
...
fn task_view_from_record(
    access: insight_platform_tasks::store::TaskAccessRecord,
) -> Result<TaskViewV2, TaskApplicationError> {
    let record = access.task;
    ...
    let owner = if let Some(invocation_id) = record.invocation_id {
        ...
        TaskOwnerLinkV2::Artifact {
            artifact_id: record
                .owner_id
                .parse()
                .map_err(|_| TaskApplicationError::Internal)?,
        }
    } else if record.owner_kind == "mcp_authorization_binding" {
        TaskOwnerLinkV2::McpAuthorization {
            authorization_binding_id: record
                .owner_id
                .parse()
                .map_err(|_| TaskApplicationError::Internal)?,
        }
...
fn decode_task_payload(
    record: &insight_platform_tasks::store::TaskRecord,
...
impl RunApplication for PgRuns {
    async fn list_child_runs(
        &self,
        intent: insight_platform_api::run::ListChildRunsIntent,
    ) -> Result<AuthorityListPage<insight_platform_api::run::ChildRunViewV1>, RunApplicationError>
    {
        let remaining = (intent.deadline - chrono::Utc::now())
            .to_std()
            .map_err(|_| RunApplicationError::Unavailable)?;
        let boundary = match intent.boundary {
            Some(ListKeysetBoundary::ChildRun { created_at, run_id }) => Some((
                chrono::DateTime::parse_from_rfc3339(created_at.as_str())
        ...
        let page = tokio::time::timeout(
            remaining,
            self.0.list_child_runs_for_principal(
                insight_platform_orchestrator::store::ChildRunLinksQuery {
                    tenant_id: intent.principal.tenant_id,
                    principal_id: intent.principal.principal_id,
                    principal_kind: intent.principal.principal_kind,
                    parent_run_id: intent.run_id,
                    parent_node_id: intent.node_id,
                    page_size: intent.page_size,
                    snapshot_at: intent.snapshot_at,
                    boundary,
                },
        ...
        Ok(AuthorityListPage {
            items: page
                .items
                .into_iter()
                .map(insight_platform_api::run::ChildRunViewV1::from_record)
                .collect(),
            snapshot_at: page.snapshot_at,
            next_boundary: page.next_boundary.map(|(created_at, run_id)| {
                ListKeysetBoundary::ChildRun {
                    created_at: UtcTimestamp::from_datetime(created_at),
                    run_id,
                }
            }),
    ...
    async fn list_run_values(
        ...
        let page = self
            .0
            .list_run_values_for_principal(insight_platform_orchestrator::store::RunValuesQuery {
                tenant_id: intent.principal.tenant_id,
                principal_id: intent.principal.principal_id,
                principal_kind: intent.principal.principal_kind,
        ...
        Ok(AuthorityListPage {
            items: page
                ...
                .collect(),
            snapshot_at: page.snapshot_at,
            next_boundary: page.next_boundary.map(|(created_at, value_id)| {
                ListKeysetBoundary::RunValue {
                    created_at: UtcTimestamp::from_datetime(created_at),
                    value_id,
                }
            }),
    ...
    async fn list_runs(
        ...
        let boundary = match intent.boundary {
            ...
            Some(_) => return Err(RunApplicationError::Invalid),
            None => None,
        };
        let page = self
            .0
    ...
    ) -> Result<insight_platform_api::run::RunDefinitionV1, RunApplicationError> {
        if intent.deadline <= chrono::Utc::now() {
            return Err(RunApplicationError::Unavailable);
        }
        let value = self
            ...
            .await
            .map_err(map_run_repository_error)?;
        Ok(insight_platform_api::run::RunDefinitionV1 {
            schema_version: 1,
            run_id: value.run_id,
            agent_id: value.agent_id,
            agent_deployment: value.agent_deployment,
            agent_interface: value.agent_interface,
            plan: value.plan,
        })
    }
    ...
    async fn read_run_events(
    ...
    ) -> Result<insight_platform_api::run::RunEventPageV1, RunApplicationError> {
        if intent.deadline <= chrono::Utc::now() {
            return Err(RunApplicationError::Unavailable);
        }
        let page = self.0
            .read_public_run_events_for_principal(
                ...
                intent.principal.principal_kind,
                &intent.run_id,
                match intent.after_sequence {
                    Some(sequence) => insight_platform_orchestrator::history::PublicRunReadPosition::AfterSequence { sequence },
                    None => insight_platform_orchestrator::history::PublicRunReadPosition::Initial,
                },
                intent.limit,
            )
            .await
        ...
        let events = page
            ...
            .collect::<Result<Vec<_>, RunApplicationError>>()?;
        Ok(insight_platform_api::run::RunEventPageV1 {
            events,
            replay_floor: page.replay_floor,
            high_water_sequence: page.high_water_sequence,
            started_after_sequence: page.started_after_sequence,
...
impl PgRuns {
    ...
    ) -> Result<insight_platform_orchestrator::store::RunRecord, RunApplicationError> {
        self.0
            .read_run_for_principal(
                &intent.principal.tenant_id,
                &intent.principal.principal_id,
...
fn run_view_from_record(
    record: insight_platform_orchestrator::store::RunRecord,
) -> Result<RunViewV1, RunApplicationError> {
...
impl ResourceApplication for PgResources {
    ...
    async fn list_agents(
        ...
        let boundary = match intent.boundary {
            ...
            Some(_) => return Err(ResourceApplicationError::Invalid),
            None => None,
        };
        let page = self
            .repository
    ...
    async fn validate_resource_draft(
        ...
        if let Some(job_id) = self
            .repository
            .lookup_registry_validation_receipt(
                &insight_platform_registry::LookupRegistryValidationReceipt {
                    tenant_id: intent.principal.tenant_id.clone(),
                    principal_id: intent.principal.principal_id.clone(),
                    principal_kind: intent.principal.principal_kind,
                    resource_id: intent.resource_id.clone(),
        ...
        let expected_resource_version = i64::try_from(intent.expected_resource_version)
            .map_err(|_| ResourceApplicationError::Invalid)?;
        let compilation_input = if intent.resource_kind == RegistryResourceKind::Agent {
            let request = insight_platform_artifacts::RegistryArtifactReadRequestV1 {
                schema_version: 1,
                authority:
                    insight_platform_artifacts::RegistryArtifactReadAuthorityV1::RegistryDraft {
                        resource_id: intent.resource_id.clone(),
                        expected_resource_version: intent.expected_resource_version,
                    },
                selector: insight_platform_artifacts::RegistryArtifactSelectorV1::Authoring,
                deadline: intent.deadline.min(now + chrono::Duration::seconds(30)),
            };
            let source = self
                .artifacts
                .read_registry_input(&intent.principal, &request)
                .await
                .map_err(|error| match error {
                    ArtifactApplicationError::Denied => ResourceApplicationError::Denied,
                    ArtifactApplicationError::NotFound => ResourceApplicationError::NotFound,
                    ArtifactApplicationError::Unauthenticated => ResourceApplicationError::Denied,
                    ArtifactApplicationError::Unavailable => ResourceApplicationError::Unavailable,
                    _ => ResourceApplicationError::Invalid,
                })?;
            let bytes = source
                .decode_verified()
                .map_err(|_| ResourceApplicationError::Invalid)?;
            Some(
                insight_platform_agent_compiler::inspect_frozen_source_bundle(&bytes)
                    .map_err(|_| ResourceApplicationError::RecompileRequired)?,
            )
        } else {
            None
        };
...
fn decode_agent_resource_draft(
    mut payload: insight_platform_contracts::TypedPayload,
...
fn map_resource_repository_error(error: RepositoryError) -> ResourceApplicationError {
    match error {
        ...
        | RepositoryError::LeaseExpired => ResourceApplicationError::Unavailable,
        RepositoryError::QuotaExceeded
        | RepositoryError::PublicHistoryGap { .. }
        | RepositoryError::CorruptRow(_)
        | RepositoryError::InvalidPersistedObject(_) => ResourceApplicationError::Internal,
    }
}
...
fn map_run_repository_error(error: RepositoryError) -> RunApplicationError {
    match error {
        ...
        RepositoryError::QuotaExceeded
        | RepositoryError::CorruptRow(_)
        | RepositoryError::InvalidPersistedObject(_) => RunApplicationError::Internal,
    }
}

fn map_task_artifact_error(error: ArtifactApplicationError) -> TaskApplicationError {
    match error {
        ArtifactApplicationError::Unauthenticated => TaskApplicationError::Unauthenticated,
        ArtifactApplicationError::Denied => TaskApplicationError::Denied,
        ArtifactApplicationError::NotFound => TaskApplicationError::NotFound,
        ArtifactApplicationError::Unavailable => TaskApplicationError::Unavailable,
        _ => TaskApplicationError::InvalidTaskInput,
    }
}

fn map_task_repository_error(error: RepositoryError) -> TaskApplicationError {
    match error {
        RepositoryError::InvalidInput(message) if message == "invalid_task_input" => {
        ...
        RepositoryError::Database(_)
        | RepositoryError::CapacityUnavailable
        | RepositoryError::LeaseExpired => TaskApplicationError::Unavailable,
        RepositoryError::QuotaExceeded
        | RepositoryError::PublicHistoryGap { .. }
        | RepositoryError::CorruptRow(_)
        | RepositoryError::InvalidPersistedObject(_) => TaskApplicationError::Internal,
    }
}
...
fn build_router(
    ...
    let protected = match role {
        ProcessRole::ManagementApi => operation
            ...
            .merge(insight_platform_api::authoring::build_authoring_router(
                insight_platform_api::authoring::AuthoringHttpState::new(
                    Arc::new(PgAuthoring(repository.clone())),
                    list_cursor_codec.clone(),
                ),
            ))
        ...
        ProcessRole::RuntimeApi => {
            ...
            operation
                .merge(build_run_router(
                    ...
                    .with_list_cursor_codec(list_cursor_codec.clone()),
                ))
                .merge(build_task_router(
                    TaskHttpState::new(
                        Arc::new(PgTasks(repository, artifact_mutation_forwarder.clone())),
        ...
        ProcessRole::ManagementApi => (
            ...
            Some(Arc::new(MtlsArtifactMutationForwarder::install(
                config
                    .artifact_gateway
                    .as_ref()
                    .expect("validated Artifact Gateway target"),
...
mod tests {
    ...
    async fn gateway_postgresql_capacity_tracks_a_real_checked_out_connection() {
        let database_url = std::env::var("PLATFORM_TEST_DATABASE_URL").expect(
            "PLATFORM_TEST_DATABASE_URL is required for the real PostgreSQL capacity fixture",
        );
        let pool = PgPoolOptions::new()
            .max_connections(2)
            .connect(&database_url)
