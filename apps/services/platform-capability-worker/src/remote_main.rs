...
use insight_platform_worker::telemetry::{run_worker_permit_sampler, update_worker_permits};
...
use insight_platform_capability_worker::{
    ...
    UuidCapabilityWorkerIdentityFactory, CAPABILITY_REMOTE_WORKER_ROLE,
};
...
impl McpHostConfig {
    fn validate(&self) -> Result<(), ProcessError> {
        let endpoint =
            Url::parse(&self.endpoint).map_err(|_| ProcessError::InvalidConfiguration)?;
    ...
    fn rpc_limits(&self) -> Result<McpHostInternalRpcLimits, ProcessError> {
...
impl ProcessConfig {
    ...
    fn validate_mcp_host(&self) -> Result<(), ProcessError> {
        if let Some(config) = &self.mcp_host {
            config.validate()?;
        }
        Ok(())
    }
...
async fn run() -> Result<(), ProcessError> {
    ...
    let mut supported_catalog = insight_platform_plan::execution::program_execution_capabilities();

    for codec in &config.installed_http_codecs {
        let descriptor = insight_platform_contracts::InstalledCapabilityCodecRef {
            schema_version: insight_platform_contracts::INSTALLED_CAPABILITY_CODEC_MANIFEST_VERSION,
            backend_kind: insight_platform_contracts::CapabilityBackendKind::Http,
        ...
        supported_catalog.capabilities.push(
            insight_platform_contracts::remote_capability_adapter_execution_capability(&descriptor)
                .map_err(|_| ProcessError::InvalidConfiguration)?,
        );
    }

    for codec in &config.installed_grpc_codecs {
        let descriptor = insight_platform_contracts::InstalledCapabilityCodecRef {
            schema_version: insight_platform_contracts::INSTALLED_CAPABILITY_CODEC_MANIFEST_VERSION,
            backend_kind: insight_platform_contracts::CapabilityBackendKind::Grpc,
            codec_id: codec.codec_id.clone(),
            codec_version: codec.codec_version.clone(),
            module_digest: codec.module_digest.clone(),
            worker_protocol_version: codec.worker_protocol_version,
    ...
    for codec in &config.installed_mcp_codecs {
        let descriptor = insight_platform_contracts::InstalledCapabilityCodecRef {
            schema_version: insight_platform_contracts::INSTALLED_CAPABILITY_CODEC_MANIFEST_VERSION,
            backend_kind: insight_platform_contracts::CapabilityBackendKind::Mcp,
            codec_id: codec.codec_id.clone(),
            codec_version: codec.codec_version.clone(),
            module_digest: codec.module_digest.clone(),
            worker_protocol_version: codec.worker_protocol_version,
    ...
    insight_platform_worker::execution::validate_installed_worker(
        &config.worker_manifest,
        &supported_catalog,
...
mod tests {
    ...
    fn config() -> ProcessConfig {
        ...
        .unwrap();
        let adapter_runtime_digest: insight_platform_contracts::Sha256Digest =
            canonical_digest(&installed_value).unwrap().parse().unwrap();
        ProcessConfig {
            schema_version: 1,
            ...
            worker_manifest: WorkerManifest {
                ...
                worker_build_digest: adapter_runtime_digest.clone(),
    ...
    fn refresh_adapter_digest(config: &mut ProcessConfig) {
        let closure = serde_json::to_value(InstalledCodecClosure {
            schema_version: 1,
        ...
        config.worker_manifest.adapter_runtime_digest =
            canonical_digest(&closure).unwrap().parse().unwrap();
    }

    #[test]
    fn mcp_host_is_present_exactly_when_mcp_codecs_are_installed() {
        let mut http_grpc = config();
        http_grpc.installed_mcp_codecs.clear();
        http_grpc.mcp_host = None;
        refresh_adapter_digest(&mut http_grpc);
        http_grpc.validate().unwrap();
        let mut orphan_host = http_grpc.clone();
        orphan_host.mcp_host = config().mcp_host;
        assert!(orphan_host.validate().is_err());
        let mut missing_host = config();
        missing_host.mcp_host = None;
        assert!(missing_host.validate().is_err());
        assert!(config().validate().is_ok());
        let mut old_digest = config();
        old_digest.installed_mcp_codecs.clear();
        old_digest.mcp_host = None;
        assert!(
            old_digest.validate().is_err(),
            "actual closure identity must change"
        );
    }
    ...
    fn config_binds_role_codecs_egress_and_database_budget() {
        ...
        wrong_mcp_host.mcp_host.as_mut().unwrap().endpoint =
            "http://platform-mcp-host:9443/".to_owned();
