...
//!
//! The process drives bounded OAuth Task expiry and shared Job cleanup claim/settlement through
//! their owning PostgreSQL ports, including exact MCP cleanup authorization. Secret deletion crosses the MCP Host mTLS Egress RPC; this process has
//! no Secret Manager credential or unrestricted external egress.

use insight_platform_mcp_runtime::{McpOAuthPkceCleanupConsumer, McpOAuthPkceCleanupWorker};
...
use insight_platform_mcp_host::McpOAuthPkceCleanupWorkerConfig;
...
async fn run() -> Result<(), ProcessError> {
    ...
    let catalog = insight_platform_contracts::WorkerExecutionCapabilities {
        schema_version: 1,
    ...
    fn process_config_rejects_unbounded_cleanup_and_non_https_egress() {
        let mut config = ProcessConfig {
            worker_manifest: insight_platform_contracts::WorkerManifest {
                ...
                worker_role: "mcp-cleanup-worker".to_owned(),
                ...
                execution_capabilities: insight_platform_contracts::WorkerExecutionCapabilities {
                    schema_version: 1,
                    capabilities: vec![
                        insight_platform_mcp_host::mcp_oauth_cleanup_execution_capability(),
                    ],
                },
