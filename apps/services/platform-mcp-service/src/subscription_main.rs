...
use insight_platform_mcp_runtime::{
    ...
    McpNotificationIngressService, McpStreamableHttpSubscriptionIngress, McpSubscriptionWorker,
    UuidMcpSubscriptionIngressIdentityFactory,
};
use insight_platform_mcp_transport::StreamableHttpMcpSubscriptionTransport;
...
use insight_platform_mcp_host::{
    McpNotificationIngressLimits, McpNotificationRateLimits,
    McpStreamableHttpSubscriptionConnector, McpStreamableHttpSubscriptionSink,
};
...
async fn run() -> Result<(), ProcessError> {
    ...
    insight_platform_worker::execution::validate_installed_worker(
        ...
        insight_platform_contracts::WorkClass::Mcp,
    )
    .map_err(|_| ProcessError::InvalidConfiguration)?;
...
mod tests {
    use super::*;
    fn fixture_manifest() -> insight_platform_contracts::WorkerManifest {
        insight_platform_contracts::WorkerManifest {
            manifest_version: insight_platform_contracts::WORKER_MANIFEST_VERSION,
            worker_role: "mcp-subscription-worker".to_owned(),
            work_class: insight_platform_contracts::WorkClass::Mcp,
            adapter_runtime_digest: format!("sha256:{}", "1".repeat(64)).parse().unwrap(),
    ...
    fn config() -> ProcessConfig {
        ProcessConfig {
            worker_manifest: fixture_manifest(),
            schema_version: 1,
