...
use insight_platform_mcp_host::{
    ...
    McpSubscriptionRemoteIoLease, McpSubscriptionWorkerAudit, McpSubscriptionWorkerAudits,
    McpSubscriptionWorkerError, RecoverDueMcpSubscription, WakeMcpSubscriptionReconcile,
    MAX_MCP_SUBSCRIPTION_RECONCILE_SCAN, MIN_MCP_SUBSCRIPTION_RECONCILE_IDLE_MILLISECONDS,
};
use insight_platform_mcp_runtime::McpSubscriptionWorker;
use insight_platform_postgres::repository::{ClaimJobs, PgRepository, RepositoryError};
...
pub struct McpSubscriptionDriver {
    ...
    reconcile_cursor: tokio::sync::Mutex<Option<insight_platform_jobs::store::SafetyScanCursor>>,
}
...
impl McpSubscriptionCapacity {
    ...
    pub fn available(&self) -> usize {
        self.pools.snapshot().business_available
    }
}
...
impl McpSubscriptionDriver {
    pub fn new(
        ...
        worker_process_generation_id: ResourceId,
        config: McpSubscriptionDriverConfig,
        manifest: insight_platform_contracts::WorkerManifest,
    ) -> Result<Self, McpSubscriptionDriverError> {
        config.validate()?;
        insight_platform_worker::execution::validate_worker_catalog(
            &manifest,
        ...
        if usize::from(manifest.max_concurrency) != config.maximum_concurrency {
            return Err(McpSubscriptionDriverError::InvalidConfiguration);
        }
        let pools = LocalWorkerPools::new(manifest, worker_process_generation_id.clone())
            .map_err(|_| McpSubscriptionDriverError::InvalidConfiguration)?;
        ...
        Ok(Self {
            ...
            reconcile_cursor: tokio::sync::Mutex::new(None),
        })
    }
    ...
    pub async fn drive_once(
        ...
        let Some(reservation) = self
            ...
            .reserve_claim_capacity(
                WorkClass::Mcp,
                usize::from(self.config.claim_batch_size),
                ClaimBatchHardLimit::from_profile(
        ...
        let identities = claims
            ...
            .map(|claim| {
                Ok(ClaimedJobIdentity {
                    ...
                    lease_generation: u64::try_from(claim.lease_epoch)
                        .map_err(|_| McpSubscriptionDriverError::CorruptClaim)?,
                })
            })
            .collect::<Result<Vec<_>, McpSubscriptionDriverError>>()?;
    ...
    pub async fn recover_once(&self) -> Result<usize, McpSubscriptionDriverError> {
        ...
        let page = self
            .repository
            .list_due_mcp_subscription_recoveries_global(
        ...
        *cursor = if page.exhausted {
        ...
        for diagnostic in &page.diagnostics {
            tracing::warn!(phase=?diagnostic.phase,code=?diagnostic.code,"invalid persisted subscription retained");
        }
    ...
    pub async fn reconcile_once(&self) -> Result<usize, McpSubscriptionDriverError> {
        let mut cursor = self.reconcile_cursor.lock().await;
        let page = self
            .repository
