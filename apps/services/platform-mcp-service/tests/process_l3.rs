...
async fn production_host_kill_and_restart_preserves_safe_replay_boundary() {
    let Ok(binary) = std::env::var("PLATFORM_MCP_HOST_BIN") else {
        panic!("PLATFORM_MCP_HOST_BIN is required for the physical MCP Host fixture");
    };
    let tls = tls_fixture();
    ...
    let trace =
        ExecutionTraceContext::start(TraceIdentityV1::generate(), TraceFlags::NotSampled).unwrap();
