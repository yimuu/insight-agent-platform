import type { ConsoleServer } from './gateway-server.ts'

export function runUntilSignal(start: () => Promise<ConsoleServer>) {
  start()
    .then((server) => {
      process.stdout.write(
        `${JSON.stringify({ kind: 'insight.console.transport/v1', origin: server.origin })}\n`,
      )
