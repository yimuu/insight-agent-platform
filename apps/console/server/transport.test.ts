...
async function fixture(t, handler, overrides = {}) {
  ...
  const origin = `http://127.0.0.1:${tcpPort(gateway)}`
...
test('whole-process buffering rejects concurrent body admission and releases capacity after cancellation', async (t) => {
  ...
  const proxy = await fixture(
    ...
    (input, response) => {
      calls++
      input.resume()
      input.once('end', () => response.end('accepted'))
    },
    { max_request_bytes: 16384, max_buffered_request_bytes: 16384 },
  )
  const unfinished = request(`${proxy.origin}/v1/agents`, {
    method: 'POST',
...
test('lost mutation response is not retried and upstream failures disclose no raw details', async (t) => {
  ...
  const reply = await send(proxy.origin, {
    headers: { authorization: 'Bearer private', 'idempotency-key': 'same-request' },
    chunks: ['complete mutation'],
  })
...
test('a flowing SSE survives the request deadline and idle cancellation releases its upstream', async (t) => {
  ...
  assert.equal(
    chunks.join(''),
    Array.from({ length: 6 }, (_, index) => `data: ${index + 1}\n\n`).join(''),
  )
})
...
test('absolute request targets and CONNECT cannot choose a third upstream', async (t) => {
  ...
  for (const target of [
    'GET http://example.invalid/v1/agents HTTP/1.1',
    'GET //example.invalid/v1/agents HTTP/1.1',
...
test('a paused SSE reader backpressures the real producer and cancellation releases the connection slot', async (t) => {
  ...
  let client
  const reader = await new Promise<import('node:http').IncomingMessage>((resolve, reject) => {
    client = request(`${proxy.origin}/v1/runs/run/events`, { agent: false }, (response) => {
      response.pause()
      resolve(response)
    })
    client.once('error', reject)
  ...
  assert.ok(
    written > 0 && written < maximumProduced,
    'a paused reader must stop the actual Gateway producer before its finite cap',
  )
...
test('request and upstream response header bytes obey the configured parser bound', async (t) => {
  ...
  const oversizedRequest = await send(proxy.origin, {
    headers: { 'x-oversized-header': 'x'.repeat(2048) },
  })
...
test('admitted request storage is zeroed after transmission, cancellation and overflow', async (t) => {
  ...
  for (const outcome of ['cancel', 'overflow']) {
    ...
    await wait(30)
    assert.ok(
      allocations.slice(firstAllocation).some((bytes) => bytes.includes(secret)),
      'the test must observe the actual admitted private body',
    )
    if (outcome === 'cancel') client.destroy()
    else {
      client.end('x'.repeat(138))
      assert.equal(await completed, 413)
    }
    await wait(30)
    assert.ok(
      allocations.slice(firstAllocation).every((bytes) => bytes.every((byte) => byte === 0)),
    )
  }
  assert.deepEqual(delivered, [secret])
})
