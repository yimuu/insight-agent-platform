...
export interface ResolvedAgentBindings {
  deployment_features?: import('../api/authoring-query.ts').DeploymentFeatures[]
  slots?: Json[]
  model: null | {
    manifest_ref: string
...
function executionKind(value: unknown): AgentExecutionKind {
  if (
    ...
    value === 'model_chat' ||
    value === 'full_plan' ||
    value === 'framework_graph'
  )
    return value
  throw new AgentCompilerError(
    'compiler_internal',
    'The shared compiler returned an unsupported execution kind.',
  )
}

type Operation =
  'inspect_sources' | 'inspect' | 'compile' | 'compile_frozen' | 'digest' | 'build_expression'
...
async function wasm(operation: Operation, input: unknown, signal?: AbortSignal): Promise<unknown> {
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      reject(
        new AgentCompilerError(
          'compiler_unavailable',
          'The shared compiler could not be initialized.',
        ),
      )
      return
    }
    const timer = setTimeout(() => {
      ...
      reject(
        new AgentCompilerError('compiler_limit_exceeded', 'Compilation exceeded its time budget.'),
      )
    }, 30_000)
    const onAbort = () => {
      done()
    ...
    worker.addEventListener(
      'message',
      (event: MessageEvent<{ bytes?: Uint8Array; failed?: boolean }>) => {
        done()
        if (event.data.failed || !event.data.bytes) {
          reject(
            new AgentCompilerError(
              'compiler_unavailable',
              'The shared compiler could not complete the request.',
            ),
          )
          return
        }
        try {
          resolve(JSON.parse(decoder.decode(event.data.bytes)))
        } catch {
          reject(
            new AgentCompilerError(
              'compiler_internal',
              'The shared compiler returned an invalid response.',
            ),
          )
        }
      },
    ...
    worker.addEventListener(
      ...
      () => {
        ...
        reject(new AgentCompilerError('compiler_unavailable', 'The shared compiler failed.'))
      },
      { once: true },
    )
...
function accepted(value: unknown): Record<string, unknown> {
  const response = value as {
    outcome?: string
    diagnostics?: { code: string; safe_detail: string }[]
  }
  if (response.outcome === 'rejected') {
    const diagnostic = response.diagnostics?.[0]
    throw new AgentCompilerError(
      diagnostic?.code ?? 'compiler_internal',
      diagnostic?.safe_detail ?? 'Compilation failed.',
    )
  }
  if (response.outcome !== 'compiled' && response.outcome !== 'inspected') {
    throw new AgentCompilerError(
      'compiler_internal',
      'The shared compiler returned an unsupported response.',
    )
  }
...
export async function inspectAgentManifest(source: string): Promise<{
  ...
  const resolution = response.resolution as {
  ...
  return {
    executionKind: executionKind(resolution.execution_kind),
    modelRef: resolution.model_ref,
    inputSchemaPath: resolution.input_schema_path,
    outputSchemaPath: resolution.output_schema_path,
    planPath: resolution.plan_path,
  }
}
...
export async function digestJson(value: Json): Promise<string> {
  ...
  if (!result.Ok)
    throw new AgentCompilerError(
      result.Err ?? 'compiler_internal',
      'Canonical digest could not be computed.',
    )
  return result.Ok
}

/** The owning Rust builder computes stack depth and semantic identity. */
export async function rebuildExpression(
  expression: { [key: string]: Json },
  signal?: AbortSignal,
  ...
  const response = (await wasm(
    ...
    {
      schema_version: 1,
      input_ports: expression.input_ports,
      instructions: expression.instructions,
      output_schema_digest: expression.output_schema_digest,
    },
    signal,
  )) as {
    outcome?: string
  ...
  if (response.outcome === 'rejected') {
    const diagnostic = response.diagnostics?.[0]
    throw new AgentCompilerError(
      diagnostic?.code ?? 'agent_compile_failed',
      diagnostic?.safe_detail ?? 'Rust could not build this expression.',
    )
  }
  if (response.outcome !== 'built' || response.schema_version !== 1 || !response.expression)
    throw new AgentCompilerError(
      'compiler_internal',
      'The shared expression builder returned an invalid response.',
    )
...
export async function verifyAgentAuthoringProfile(
  ...
  if ((await digestJson(payload as unknown as Json)) !== profile_digest) {
    throw new AgentCompilerError(
      ...
      'Authoring profile digest does not match its exact bindings.',
    )
  }
}

export type AgentSourceInput = Pick<
  AgentCompilerInput,
  'manifest' | 'manifestPath' | 'inputSchema' | 'outputSchema' | 'plan'
>
...
export async function inspectAgentSources(input: AgentSourceInput, signal?: AbortSignal) {
  ...
  const file = (path: string, text: string) => {
    if (Object.hasOwn(files, path) && files[path] !== text)
      throw new AgentCompilerError(
        'source_bundle_invalid',
        'One source path was provided with conflicting file contents.',
      )
    files[path] = text
  }
  ...
  if (inspected.planPath) {
    if (input.plan === undefined)
      throw new AgentCompilerError(
        ...
        'The referenced Plan source has not been supplied.',
      )
    file(inspected.planPath, input.plan)
  } else if (input.plan !== undefined)
    throw new AgentCompilerError(
      'source_bundle_invalid',
      'The manifest does not reference the supplied Plan source.',
    )
...
export async function compileCapturedAgentSources(
...
export async function compileFrozenSourceBundle(
  source: string,
  signal?: AbortSignal,
): Promise<CompiledAgent> {
  if (encoder.encode(source).length > 8_388_608)
    throw new AgentCompilerError('compiler_limit_exceeded', 'Source bundle exceeds its byte limit.')
  const result = compilationResult(
    accepted(await wasm('compile_frozen', JSON.parse(source), signal)),
  )
  if (result.sourceBundle !== source)
    throw new AgentCompilerError(
      'source_bundle_invalid',
      'The persisted package is not the exact canonical source bundle.',
    )
  return result
}
...
function compilationResult(response: Record<string, unknown>): CompiledAgent {
  const result = response.compilation as {
    ...
    compiled: {
      name: string
      execution_kind: AgentExecutionKind
      canonical_manifest_bytes: number[]
      manifest_digest: string
      resource_intent: Json & { contract_digest: string }
      typed_plan_bytes: number[]
      typed_plan_digest: string
      deployment_intent: Json
      required_features: RequiredAgentFeature[]
      lifecycle_plan: Json
    }
  }
  const compiled = result.compiled
  if (
    !Array.isArray(result.source_map_bytes) ||
    !result.source_map_bytes.every((byte) => Number.isInteger(byte) && byte >= 0 && byte <= 255) ||
    !/^sha256:[0-9a-f]{64}$/.test(result.source_map_digest)
  )
    throw new AgentCompilerError(
      'compiler_internal',
      'The shared compiler returned no valid source map.',
    )
  return {
    name: compiled.name,
    executionKind: executionKind(compiled.execution_kind),
    sourceBundle: decoder.decode(new Uint8Array(result.source_bundle_bytes)),
    sourceBundleDigest: result.source_bundle_digest,
    sourceMap: decoder.decode(new Uint8Array(result.source_map_bytes)),
    sourceMapDigest: result.source_map_digest,
    canonicalManifest: decoder.decode(new Uint8Array(compiled.canonical_manifest_bytes)),
    manifestDigest: compiled.manifest_digest,
    contractDigest: compiled.resource_intent.contract_digest,
    resourceIntent: compiled.resource_intent,
    typedPlan: decoder.decode(new Uint8Array(compiled.typed_plan_bytes)),
    typedPlanDigest: compiled.typed_plan_digest,
    deploymentIntent: compiled.deployment_intent,
    requiredFeatures: compiled.required_features,
    lifecyclePlan: compiled.lifecycle_plan,
  }
}
