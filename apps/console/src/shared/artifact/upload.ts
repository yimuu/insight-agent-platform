...
function conflict(): never {
  throw new Error(
    'artifact_upload_conflict: Retain the original upload attempt and reconcile current Artifact authority.',
  )
}
...
function text(value: unknown, limit: number): value is string {
  return typeof value === 'string' && value.length > 0 && encoder.encode(value).length <= limit
}
...
export function validateArtifactUploadRecovery(value: ArtifactUploadRecovery): void {
  ...
  if (
    ...
    !id(value.artifact_id, 'art') ||
    !id(value.operation_id, 'job') ||
    !id(value.upload_grant_id, 'grt') ||
    typeof value.artifact_etag !== 'string' ||
    !new RegExp(`^"${value.artifact_id}-[1-9][0-9]*"$`).test(value.artifact_etag) ||
    !expiry(value.upload_expires_at) ||
    !/^sha256:[0-9a-f]{64}$/.test(value.request_digest) ||
    !text(value.prepare_receipt, 255) ||
    !text(value.complete_receipt, 255) ||
...
function validatePrepared(value: PrepareArtifactUploadResponse): void {
  if (
    ...
    !id(value.operation_id, 'job') ||
    !id(value.upload_grant_id, 'grt') ||
    !text(value.artifact_etag, 255) ||
    !expiry(value.upload_expires_at) ||
    !value.upload_target ||
    !text(value.upload_target.url, 8192) ||
    !text(value.upload_target.completion_proof, 4096) ||
    !/^[\x21-\x7e]+$/.test(value.upload_target.completion_proof)
  )
    conflict()
  let url: URL
  try {
    url = new URL(value.upload_target.url)
  } catch {
    conflict()
  }
...
function samePrepared(old: ArtifactUploadRecovery, value: PrepareArtifactUploadResponse): void {
  ...
  if (
    old.artifact_id !== value.artifact_id ||
    old.operation_id !== value.operation_id ||
    old.upload_grant_id !== value.upload_grant_id ||
    old.artifact_etag !== value.artifact_etag ||
    old.upload_expires_at !== value.upload_expires_at
  )
    conflict()
}
function currentArtifact(
  response: AuthorityResponse<ArtifactView>,
  state: ArtifactUploadRecovery,
  request: JsonObject,
): ArtifactView {
  const value = response.data
  if (
    value.schema_version !== 1 ||
    value.artifact_id !== state.artifact_id ||
    !positive(value.version) ||
    value.etag !== `"${value.artifact_id}-${value.version}"` ||
    response.etag !== value.etag ||
    value.purpose !== request.purpose ||
    value.classification !== request.classification ||
    value.expected_size_bytes !== request.expected_size_bytes ||
    value.declared_media_type !== request.declared_media_type
  )
    conflict()
  return value
}
function ready(
  ...
  if (
    value.state !== 'ready' ||
    !content ||
    content.artifact_id !== state.artifact_id ||
    content.content_digest !== request.expected_digest ||
    content.byte_length !== request.expected_size_bytes ||
    content.media_type !== request.declared_media_type ||
    content.classification !== request.classification ||
...
export async function uploadArtifact(
  ...
  saved: ArtifactUploadRecovery | null,
  ...
  if (state) {
    ...
    if (
      ...
      state.prepare_receipt !== prepareReceipt ||
      state.complete_receipt !== completeReceipt
    )
      conflict()
  } else {
    ...
    state = {
      ...
      artifact_etag: prepared.artifact_etag,
  ...
  if (current.state === 'staging') {
    if (current.etag !== state.artifact_etag || Date.parse(state.upload_expires_at) <= Date.now())
      conflict()
    ...
    if (!state.uploaded) {
      await client.putArtifactObject(
        prepared.upload_target.url,
        bytes,
        String(request.declared_media_type),
      )
      state = { ...state, uploaded: true }
      persist(state)
    }
    if (Date.parse(state.upload_expires_at) <= Date.now()) conflict()
    await client.completeArtifactUpload(
      state.artifact_id,
      { schema_version: 1, completion_proof: prepared.upload_target.completion_proof },
      state.artifact_etag,
      state.complete_receipt,
    )
  ...
  if (operation.data.operation_id !== state.operation_id || operation.data.state !== 'succeeded')
    throw new Error(
      'artifact_verification_pending: Inspect the original Artifact operation before resuming.',
    )
