...
export const equal = (a: Json, b: Json): boolean =>
...
export function treeSchema(input: JsonObject, nominals: NominalSchemas = new Map()): TreeSchema {
  if (bytes(JSON.stringify(input)) > TREE_LIMITS.schemaBytes)
    throw new Error('Schema exceeds the form byte limit.')
  let nodes = 0
  const rootIds = new WeakMap<object, number>()
  const rootId = (root: object) => {
  ...
  const visit = (
    ...
    if (++nodes > TREE_LIMITS.schemaNodes || depth > TREE_LIMITS.schemaDepth)
      throw new Error('Schema exceeds the bounded tree capacity.')
    ...
    if ('$ref' in schema) {
      ...
      if (ref.startsWith('#/$defs/')) {
        ...
        if (ref.slice(8).includes('/') || /~(?![01])/.test(ref.slice(8)))
          throw new Error('Unsupported local schema pointer.')
        target = object(root.$defs) ? root.$defs[key] : undefined
      } else {
        target = nominals.get(ref)
        if (object(target)) targetRoot = target
      }
      if (!object(target))
        throw new Error(`Unknown local or exact pinned schema reference: ${ref.slice(0, 200)}`)
      const resolved = visit(target, targetRoot, depth + 1, new Set([...references, identity]))
      const { $ref: ignored, ...siblings } = schema
      void ignored
      if (
        Object.keys(siblings).every((k) =>
          ['$schema', '$id', '$defs', 'title', 'description', 'x-platform-classification'].includes(
            k,
          ),
        )
      )
        return { ...resolved, schema: { ...resolved.schema, ...siblings } }
      ...
      return {
        schema,
        properties: { ...resolved.properties, ...overlay.properties },
        items: overlay.items ?? resolved.items,
        additional: overlay.additional ?? resolved.additional,
        alternatives: overlay.alternatives.length ? overlay.alternatives : resolved.alternatives,
      }
    }
    ...
    if (Array.isArray(schema.allOf)) {
      ...
      const { allOf: ignored, ...base } = schema
      void ignored
      ...
      return branches.reduce(
        (a, b) => ({
          ...
          schema: {
            ...
            required: [
              ...new Set([
                ...(Array.isArray(a.schema.required) ? a.schema.required : []),
                ...(Array.isArray(b.schema.required) ? b.schema.required : []),
              ]),
            ],
          },
          properties: { ...b.properties, ...a.properties },
          items: a.items ?? b.items,
          alternatives: a.alternatives.length ? a.alternatives : b.alternatives,
        }),
        main,
      )
    }
      ...
      properties: object(schema.properties)
        ? Object.fromEntries(
            Object.entries(schema.properties).map(([k, v]) => [
              k,
              visit(v, root, depth + 1, references),
            ]),
          )
        : {},
      items: object(schema.items) ? visit(schema.items, root, depth + 1, references) : undefined,
      additional: object(schema.additionalProperties)
        ? visit(schema.additionalProperties, root, depth + 1, references)
        : undefined,
      alternatives: Array.isArray(alternatives)
        ? alternatives.map((s) => visit(s, root, depth + 1, references))
        : [],
    }
  }
  ...
  if (s.type === 'object')
    return Object.fromEntries(
      Object.entries(node.properties)
        .filter(([k]) => Array.isArray(s.required) && s.required.includes(k))
        .map(([k, n]) => [k, initialValue(n, depth + 1)]),
    )
...
export function branchIndex(node: TreeSchema, value: Json): number {
  return node.alternatives.findIndex((branch) => {
    ...
    if (object(value) && s.type === 'object') {
      const tags = Object.entries(branch.properties).filter(([, child]) =>
        Object.hasOwn(child.schema, 'const'),
      )
      return tags.every(
        ([k, child]) => Object.hasOwn(value, k) && equal(child.schema.const, value[k]),
      )
    }
    return typeMatches(s.type, value)
  })
}
const typeMatches = (type: Json | undefined, value: Json): boolean =>
  type === undefined ||
  (type === 'null'
    ? value === null
    : type === 'object'
...
export function valueBounds(value: Json, limit = TREE_LIMITS.bytes): Record<string, string> {
  ...
  const visit = (v: Json, path: string, depth: number) => {
    if (depth > TREE_LIMITS.depth) {
      errors[path] = 'Response exceeds depth 32.'
      return
    }
    if (Array.isArray(v)) {
      items += v.length
      if (items > TREE_LIMITS.items) {
        errors[path] = 'Response exceeds 4096 array items.'
        return
      }
      v.forEach((child, i) => visit(child, pointer(path, i), depth + 1))
    } else if (object(v)) {
      properties += Object.keys(v).length
      if (properties > TREE_LIMITS.properties) {
        errors[path] = 'Response exceeds 1024 properties.'
        return
      }
      Object.entries(v).forEach(([k, child]) => visit(child, pointer(path, k), depth + 1))
    } else if (
      typeof v === 'string' &&
      (bytes(v) > TREE_LIMITS.stringBytes ||
        [...v].some((c) => {
          const n = c.codePointAt(0)!
          return n >= 0xd800 && n <= 0xdfff
        }))
    )
      errors[path] = 'Text exceeds its UTF-8 limit or contains invalid Unicode.'
    else if (
      typeof v === 'number' &&
      (!Number.isFinite(v) || Math.abs(v) > Number.MAX_SAFE_INTEGER)
    )
      errors[path] = 'Use a finite interoperable JSON number.'
  }
  visit(value, '', 0)
...
export function validateTree(node: TreeSchema, value: Json): Record<string, string> {
  ...
  const visit = (n: TreeSchema, v: Json, path: string, depth: number) => {
    ...
    if (!typeMatches(s.type, v)) {
      fail(`Expected ${String(s.type)}.`)
      return
    }
    if (Object.hasOwn(s, 'const') && !equal(s.const, v))
      fail('Value must equal the schema constant.')
    if (Array.isArray(s.enum) && !s.enum.some((c) => equal(c, v)))
      fail('Select a declared enum value.')
    if (n.alternatives.length) {
      const index = branchIndex(n, v)
      if (index < 0) fail('Select a schema variant.')
      else visit(n.alternatives[index], v, path, depth + 1)
    }
    if (object(v)) {
      if (Array.isArray(s.required))
        for (const key of s.required)
          if (typeof key === 'string' && !Object.hasOwn(v, key))
            errors[pointer(path, key)] = 'This field is required.'
      for (const [k, child] of Object.entries(v)) {
        const spec = n.properties[k] ?? n.additional
        if (spec) visit(spec, child, pointer(path, k), depth + 1)
        else if (s.additionalProperties === false)
          errors[pointer(path, k)] = 'This property is not declared by the schema.'
      }
    }
    if (Array.isArray(v)) {
      if (typeof s.minItems === 'number' && v.length < s.minItems)
        fail(`Add at least ${s.minItems} items.`)
      if (typeof s.maxItems === 'number' && v.length > s.maxItems)
        fail(`Use at most ${s.maxItems} items.`)
      if (
        s.uniqueItems === true &&
        new Set(v.map((item) => JSON.stringify(sorted(item)))).size !== v.length
      )
    ...
    if (typeof v === 'string') {
      if (typeof s.minLength === 'number' && [...v].length < s.minLength)
        fail(`Use at least ${s.minLength} characters.`)
      if (typeof s.maxLength === 'number' && [...v].length > s.maxLength)
        fail(`Use at most ${s.maxLength} characters.`)
      if (typeof s['x-platform-max-bytes'] === 'number' && bytes(v) > s['x-platform-max-bytes'])
        fail(`Text exceeds ${s['x-platform-max-bytes']} UTF-8 bytes.`)
    ...
    if (typeof v === 'number') {
      if (typeof s.minimum === 'number' && v < s.minimum)
        fail(`Use a value of at least ${s.minimum}.`)
      if (typeof s.maximum === 'number' && v > s.maximum)
        fail(`Use a value of at most ${s.maximum}.`)
      if (typeof s.exclusiveMinimum === 'number' && v <= s.exclusiveMinimum)
        fail(`Use a value greater than ${s.exclusiveMinimum}.`)
      if (typeof s.exclusiveMaximum === 'number' && v >= s.exclusiveMaximum)
        fail(`Use a value less than ${s.exclusiveMaximum}.`)
