...
const event = (eventId, cursor = `opaque-${eventId}`) => ({
  id: cursor,
  event: 'run.updated',
...
const historyHeaders = {
  ...
  'x-insight-run-high-water': '100',
  'x-insight-history-truncated': 'false',
}
const response = (body) =>
  new Response(body, { headers: { 'content-type': 'text/event-stream', ...historyHeaders } })
...
test('invalid or oversized public event data and unbounded framing fail closed', () => {
  ...
  assert.throws(
    () =>
      parseEventStream(
        Array.from({ length: 129 }, (_, index) => frame(event(String(index)))).join(''),
      ),
    /128 events/,
  )
  const decoder = new RunEventStreamDecoder()
  ...
  assert.throws(
    () => new RunEventStreamDecoder().push(new Uint8Array(34 * 1024 * 1024 + 1), () => {}),
    /34 MiB/,
  )
})
  ...
  for (const tail of [
    ...
    await assert.rejects(
      readRunEventStream(
        ...
        { onEvent: (item) => accepted.push(item) },
      ),
      RunEventTransportError,
    )
...
test('a failed response body keeps earlier complete events and cancels its reader', async () => {
  ...
  await assert.rejects(
    readRunEventStream(response(body), { onEvent: (item) => accepted.push(item) }),
    RunEventTransportError,
  )
  assert.deepEqual(accepted, [event('first')])
...
test('abort cancels a blocked body and cannot publish late events', async () => {
  ...
  let cancelled = false
  const accepted = []
  const pending = readRunEventStream(
    response(
      new ReadableStream({
        cancel() {
          cancelled = true
...
test('reissued cursors deduplicate by durable event_id and the projection remains bounded', async () => {
  ...
  const cursors = []
  let page = 0
  await followRunEventPages(
    async (cursor, { onEvent }) => {
      cursors.push(cursor)
      if (page++ === 0) {
  ...
  assert.deepEqual(
    updates.at(-1).events.map((item) => item.data.event_id),
    Array.from({ length: 128 }, (_, i) => String(i + 1)),
  )
})
...
test('interruption retries with the last complete opaque cursor, never a partial frame', async (context) => {
  ...
  const updates = []
  const cursors = []
  const pending = followRunEventPages(
    async (cursor, pageOptions) => {
      cursors.push(cursor)
      if (cursors.length === 1) {
        return readRunEventStream(
          response(frame(event('first', 'opaque+/cursor==')) + 'id: partial\ndata: {'),
          pageOptions,
        )
      }
...
test('cursor rejection, history unavailability, and malformed events stop without resetting history', async () => {
  ...
  for (const code of ['cursor_expired', 'cursor_invalid', 'history_unavailable']) {
    ...
    const updates = []
    const clears = []
    const problem = Object.assign(new Error(`Public history cannot continue: ${code}`), {
      code,
      retryable: false,
      status: 400,
    })
    ...
    await assert.rejects(
      followRunEventPages(
        ...
        options(controller, updates, clears),
      ),
      (error) => error === problem,
    )
    assert.equal(calls, 2)
...
test('authorization loss clears protected content and stops even if marked retryable', async () => {
  for (const status of [401, 403]) {
    ...
    const clears = []
    let calls = 0
    const problem = Object.assign(new Error('Authorization was revoked'), {
      status,
      retryable: true,
    })
    await assert.rejects(
      followRunEventPages(
        ...
        options(new AbortController(), updates, clears),
      ),
      (error) => error === problem,
    )
    assert.equal(calls, 2)
...
test('PlatformClient streams finite pages with scoped auth, opaque resume header, and AbortSignal', async (context) => {
  ...
  Object.assign(expected.data, { message: '中文🦀' })
  let request
  ...
  assert.equal(request.init.signal.aborted, controller.signal.aborted)
...
test('PlatformClient follows a transport failure but preserves a closed cursor error for the UI', async (context) => {
  ...
  context.mock.method(globalThis, 'fetch', async () => {
    ...
    return new Response(
      ...
      {
        status: 400,
        headers: { 'content-type': 'application/problem+json', 'trace-id': 'safe-trace-id' },
      },
    )
  })
...
test('current SSE headers are required, bounded u64 values and retained history is reported before an empty page', async () => {
  ...
  const headers = {
    'content-type': 'text/event-stream',
    'x-insight-run-replay-floor': '9007199254740993',
    'x-insight-run-high-water': '18446744073709551615',
    'x-insight-history-truncated': 'true',
  }
  assert.deepEqual(
    await readRunEventStream(new Response('', { headers }), {
      onHistory: (history) => seen.push(history),
    }),
    [],
  )
  assert.deepEqual(seen, [
    { replayFloor: '9007199254740993', highWaterSequence: '18446744073709551615', truncated: true },
  ])
  for (const changed of [
    { 'x-insight-run-replay-floor': undefined },
    { 'x-insight-run-high-water': '18446744073709551616' },
    { 'x-insight-run-replay-floor': '01' },
    { 'x-insight-run-high-water': '8' },
    { 'x-insight-history-truncated': 'unknown' },
  ]) {
    const current = { ...headers, ...changed }
    for (const key of Object.keys(current)) if (current[key] === undefined) delete current[key]
    await assert.rejects(
      readRunEventStream(new Response('', { headers: current })),
      /Required Run history boundaries/,
    )
  }
})
