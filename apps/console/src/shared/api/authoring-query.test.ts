...
import {
  parseBindingSelections,
  parseBindingResolution,
  parseDependencyPage,
  exactFeatureSelections,
  exactResolvedFeatures,
} from './authoring-query.ts'
const digest = (character) => `sha256:${character.repeat(64)}`
const deployment = {
  deployment_id: 'mdep_0198f1c3-8f49-7c3e-b1f3-773c28367b90',
  resource_kind: 'model_deployment',
  deployment_digest: digest('a'),
}
const policy = {
  deployment: { ...deployment, deployment_id: 'pdep_policy', resource_kind: 'policy_deployment' },
  revision: {
    revision_id: 'prev_policy',
  ...
  slots: [
    {
      ...
      target: {
        kind: 'model',
        candidates: [{ kind: 'active', resource_id: 'mpr_model', environment: 'dev' }],
  ...
  slots: [
    {
      ...
      resolution: {
        ...
        deployment_features: [],
        binding,
        observed_contract_digests: [digest('d')],
        contract_match: null,
...
test('authoring query POST is a cancellable read with no receipt or mutation preconditions', async (context) => {
  ...
  const response = await client.resolveAgentBindings(
    parseBindingSelections(JSON.stringify(selections)),
  ...
  assert.equal(response.data.slots[0].resolution.kind, 'resolved')
  if (response.data.slots[0].resolution.kind !== 'resolved')
...
test('discovery preserves empty continuations and keeps compatibility independent from call permission', async (context) => {
  const item = {
    schema_version: 1,
    kind: 'model',
    resource_id: 'mpr_model',
    environment: 'dev',
    deployment,
    interface_contract_digest: digest('d'),
  ...
  const filters: DependencyFilters = {
    kind: 'model',
    environment: 'dev',
    interfaceContractDigest: digest('e'),
    cursor: 'opaque+/old==',
  }
  let url
  context.mock.method(globalThis, 'fetch', async (input) => {
    url = new URL(input)
    return new Response(JSON.stringify({ ...page, items: [] }))
  })
  const client = new PlatformClient('https://gateway.example', 'token')
  ...
  assert.throws(
    () =>
      parseDependencyPage(JSON.stringify(page), {
        ...filters,
        interfaceContractDigest: digest('d'),
      }),
    /mismatched response/,
  )
})
...
test('strict dynamic authoring rejects duplicates, unknown fields, reordering and foreign binding identities', () => {
  assert.throws(
    () =>
      parseBindingSelections(
        JSON.stringify(selections).replace(
          '"slot_id":"model"',
          '"slot_id":"bad","slot_id":"model"',
        ),
      ),
    /duplicate keys/,
  )
  assert.throws(
    () => parseBindingSelections(JSON.stringify({ ...selections, mutation: false })),
    /unsupported shape/,
  )
  assert.throws(
    () =>
  ...
  const wrong = structuredClone(resolved)
  wrong.slots[0].resolution.binding.requirement_digest = digest('e')
  assert.throws(
    () => parseBindingResolution(JSON.stringify(wrong), selections),
    /mismatched response/,
  )
  const swapped = structuredClone(resolved)
  swapped.slots[0].slot_id = 'other'
  assert.throws(
    () => parseBindingResolution(JSON.stringify(swapped), selections),
    /mismatched response/,
  )
  const forged = structuredClone(resolved)
  Object.assign(forged.slots[0].resolution.binding, { binding_digest: digest('a') })
  assert.throws(
    () => parseBindingResolution(JSON.stringify(forged), selections),
    /unsupported shape/,
  )
  const wrongCount = structuredClone(resolved)
  wrongCount.slots[0].resolution.binding.target.candidates.push(deployment)
  assert.throws(
    () => parseBindingResolution(JSON.stringify(wrongCount), selections),
    /mismatched response/,
  )
  const oversized = {
    ...selections,
    slots: Array.from({ length: 65 }, (_, n) => ({ ...selections.slots[0], slot_id: `s${n}` })),
  }
...
test('exact feature queries carry server projections without inferring a Capability backend', () => {
  const cap = {
    ...deployment,
    deployment_id: 'cdep_0198f1c3-8f49-7c3e-b1f3-773c28367b90',
    resource_kind: 'capability_deployment',
  }
  const input = {
    slot_id: 'tool',
    requirement_digest: digest('e'),
    target: { kind: 'capability', candidates: [cap], selection_policy: policy, tool_alias: null },
  }
  ...
  const evidence = {
    ...
    deployment: cap,
    interface_contract_digest: digest('d'),
    required_features: ['remote-capability', 'mcp'],
  }
  const result = {
    schema_version: 1,
    slots: [
  ...
  assert.deepEqual(exactResolvedFeatures(parseBindingResolution(JSON.stringify(result), request)), [
    evidence,
  ])
  for (const alter of [
    (v) => (v.slots[0].resolution.deployment_features = []),
    (v) => (v.slots[0].resolution.deployment_features[0].interface_contract_digest = digest('f')),
    (v) =>
      (v.slots[0].resolution.deployment_features[0].required_features = [
        'mcp',
        'remote-capability',
      ]),
  ]) {
    ...
    alter(invalid)
    assert.throws(() => parseBindingResolution(JSON.stringify(invalid), request))
  }
})
