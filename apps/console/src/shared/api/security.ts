const SENSITIVE_KEY =
  /(?:authorization|cookie|credential|password|secret|token|completion_proof|upload_target|raw_(?:prompt|body)|prompt|tool_(?:input|output))/i
...
export function redact(value: unknown, depth = 0): unknown {
  ...
  return Object.fromEntries(
    Object.entries(value as Record<string, unknown>)
      ...
      .map(([key, item]) => [
        key,
        key !== 'safe_prompt_key' && SENSITIVE_KEY.test(key) ? REDACTED : redact(item, depth + 1),
      ]),
  )
}
