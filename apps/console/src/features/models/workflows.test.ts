import type { ModelCredentialMetadata, ModelDefault } from '../../shared/api/model-types.ts'
import type { ExactDeploymentRef } from '../../shared/api/types.ts'
import type { ModelPublicationRequest } from '../../shared/api/model-types.ts'
...
const tenant = id('ten', 1),
  provider = id('spr', 2)
const exactModel = (n: number): ExactDeploymentRef => ({
  resource_kind: 'model_deployment',
  deployment_id: id('mdep', n),
  deployment_digest: sha('c'),
})
function memory(t) {
...
test('credential revoke resumes the original generation, version and Receipt after a lost response', async (t) => {
  const storage = memory(t),
    client = new PlatformClient('https://platform.example', 'session'),
    calls = []
  const original: ModelCredentialMetadata = {
    schema_version: 1,
    tenant_id: tenant,
    secret_binding_id: id('sbd', 3),
  ...
  const revoked: ModelCredentialMetadata = {
    ...original,
    state: 'revoked',
    generation: 8,
    version: 10,
  ...
  t.mock.method(client, 'revokeModelCredential', async (...args) => {
    calls.push(args)
    current = revoked
    if (calls.length === 1) throw new Error('response lost')
    return { data: revoked, etag: revoked.etag }
  })
  ...
  await assert.rejects(
    revokeCredential(client, tenant, {
      ...original,
      secret_binding_id: id('sbd', 4),
      etag: `"${id('sbd', 4)}-9"`,
    }),
    /pending/,
  )
...
test('probe is a single call with the exact deployment and rejects a changed target or invented outcome', async (t) => {
  const client = new PlatformClient('https://platform.example', 'session'),
    target = exactModel(20),
    calls = []
  let value = {
    schema_version: 1,
    model_deployment: target,
    provider_deployment: {
      resource_kind: 'model_provider_deployment',
      deployment_id: id('mpdep', 21),
      deployment_digest: sha('d'),
    },
    model_identity: { value: 'example-model', stability: 'externally_mutable' },
    protocol: 'open_ai_responses',
    observed_at: '2026-09-09T10:00:00Z',
    outcome: 'timed_out',
  }
  t.mock.method(client, 'probeModel', async (...args) => {
    calls.push(args)
    return { data: value }
  })
...
test('credential response loss reuses exact operation, never persists the key, and refuses another tenant or source', async (t) => {
  ...
  await assert.rejects(
    importSourceCredential(client, intent, secret),
    (error) =>
      error instanceof Error && !error.message.includes(secret) && /pending/.test(error.message),
  )
  ...
  await assert.rejects(
    importSourceCredential(client, { ...intent, alias: 'different' }, secret),
    /pending/,
  )
...
test('default recovery retains original CAS and refuses concurrent drift after replay', async (t) => {
  ...
  const client = new PlatformClient('https://platform.example', 'session'),
    calls = []
  const current = (version: number): ModelDefault => ({
    schema_version: 1,
    tenant_id: tenant,
    version,
    etag: `"${tenant}-${version}"`,
  ...
  t.mock.method(client, 'setModelDefault', async (body, etag, receipt) => {
    ...
    if (lost) {
      lost = false
      value = { ...current(5), default_model: selected }
  ...
  assert.deepEqual(
    await resumeModelDefault(client, { ...current(5), default_model: selected }),
    value,
  )
  assert.equal(hasPendingModelDefault(), false)
  assert.deepEqual(calls[0], calls[1])
  assert.equal(calls[1].etag, `"${tenant}-4"`)
  t.mock.method(client, 'getModelDefault', async () => ({
    data: { ...current(6), default_model: other },
    etag: current(6).etag,
  }))
...
async function fixture(
  t,
  { drop = null, update = false, kind = 'source', deferUpload = false, prepareDrift = null } = {},
) {
  const storage = memory(t),
    receipts = new Map(),
    calls = [],
  ...
  const source = kind === 'source',
    noun = source ? 'model-providers' : 'models',
    resourceId = id(source ? 'mpr' : 'mdl', 10)
  let resource = update
    ? {
        schema_version: 1,
        resource_id: resourceId,
        resource_kind: source ? 'model_provider' : 'model_profile',
        ...
        draft_generation: 2,
        etag: `"${resourceId}-7"`,
        draft: { alias: 'primary', display_name: 'Old', document: {}, validation: null },
      }
    : null
  ...
  const quota = () => ({
    ...
    tenant_concurrency: { limit: 8, reserved: 0, used: 0 },
    etag: `"model-quota-${(quotaAllocation ? 'b' : 'a').repeat(64)}"`,
  })
  const advance = () => {
    resource.version++
    resource.etag = `"${resourceId}-${resource.version}"`
  }
  const server = createServer(async (request, response) => {
    try {
      const bytes = []
      for await (const chunk of request) bytes.push(chunk)
      const raw = Buffer.concat(bytes).toString(),
      ...
      if (path.startsWith('/v1/model-quotas/')) {
        ...
        assert.deepEqual(body, {
          schema_version: 1,
          model_deployment: quota().model_deployment,
          limits: INITIAL_QUOTA,
        })
        ...
        if (receipts.has(receipt)) {
          assert.deepEqual(receipts.get(receipt), body)
          return send(200, quota())
        }
        receipts.set(receipt, body)
        quotaAllocation = {
          limits: body.limits,
          reserved: { requests: 0, tokens: 0, cost_microunits: 0 },
          used: { requests: 0, tokens: 0, cost_microunits: 0 },
        }
        if (drop === 'quota') {
          drop = null
          response.destroy()
      ...
      if (request.method === 'GET') {
        if (path.startsWith('/v1/artifacts/')) {
          ...
          if (drop === 'ready-get' && artifact.state === 'ready') {
            drop = null
            response.destroy()
            return
          }
        ...
        if (path.startsWith('/v1/operations/'))
          return send(200, {
            ...
            state: deferUpload ? 'running' : 'succeeded',
          })
      ...
      if (path === '/v1/model-configuration:declare') {
        ...
        declaration = {
          schema_version: 1,
          content,
          content_digest: await digestJson(content),
          size_bytes: 0,
        }
      ...
      if (path === '/v1/model-configuration:compile') {
        ...
        compiled = {
          ...
          draft: {
            ...
            document: {
              kind: source ? 'model_provider' : 'model_profile',
              declaration_digest: declaration.content_digest,
            },
          },
          deployment: {
            resource_kind: source ? 'model_provider' : 'model_profile',
            bindings: { fixed_policy_digest: sha('d') },
          },
        }
      ...
      const phase =
        path === '/v1/artifacts:prepare-upload'
          ? 'prepare'
          : path.includes(':complete-upload')
            ? 'complete'
      ...
      if (receipts.has(key)) {
        ...
        if (phase === 'prepare' && artifacts.get(saved.value.artifact_id)?.state !== 'staging') {
          return send(409, {
            code: 'conflict',
            title: 'Terminal upload preparation cannot be replayed.',
          })
        }
        assert.equal(saved.fingerprint, fingerprint)
        if (phase === 'prepare' && prepareDrift) {
          const changed = structuredClone(saved.value)
          if (prepareDrift === 'expiry')
            changed.upload_expires_at = new Date(Date.parse(changed.upload_expires_at) + 1000)
              .toISOString()
              .replace(/(\.\d{3})Z$/, '$1000Z')
          else
            changed[prepareDrift] =
              prepareDrift === 'artifact_etag'
                ? `"${changed.artifact_id}-2"`
                : id(
                    { artifact_id: 'art', operation_id: 'job', upload_grant_id: 'grt' }[
                      prepareDrift
                    ],
                    999,
                  )
      ...
      if (!['prepare', 'complete', 'create'].includes(phase))
        assert.equal(request.headers['if-match'], resource.etag)
      let value,
        status = 200
      if (phase === 'prepare') {
        ...
        value = {
          ...
          operation_id: id('job', counter++),
          upload_grant_id: id('grt', counter++),
          upload_expires_at: new Date(Date.now() + 120_000)
            .toISOString()
            .replace(/(\.\d{3})Z$/, '$1000Z'),
          upload_target: {
            url: 'https://objects.example/exact',
            completion_proof: 'private-upload-proof',
          },
        }
        ...
        const content = {
          ...
          byte_length: declaration.size_bytes,
        ...
        artifacts.set(artifactId, {
          ...
          version: 4,
          etag: `"${artifactId}-4"`,
          state: deferUpload ? 'uploaded' : 'ready',
          content,
        })
        value = {}
        ...
        if (!resource)
          resource = {
            ...
            draft_generation: 0,
            etag: '',
            draft: { alias: 'primary', display_name: 'Primary', document: {}, validation: null },
          }
        ...
        status = 202
        resource.draft.validation = { valid: true }
        advance()
        ...
        value = {
          ...
          published_versions: [
            {
              resource_version_id: id(source ? 'mprev' : 'mdrev', counter++),
              revision_no: resource.draft_generation,
              content_digest: declaration.content_digest,
              artifact_id: artifactId,
              etag: '"revision"',
            },
        ...
        const deploymentId = id(source ? 'mpdep' : 'mdep', counter++),
          digest = await digestJson({ schema_version: 1, ...body.closure })
        status = 201
        advance()
        deployment = {
          schema_version: 1,
          ...
          etag: `"${deploymentId}-${digest.slice(7)}"`,
        }
        value = deployment
      } else {
        resource.gate_state = 'enabled'
        resource.active_deployment_id = deployment.deployment_id
        advance()
        value = resource
      }
      ...
      response.end(JSON.stringify({ code: 'fixture_failed', detail: error.message }))
    }
  })
  ...
  const client = new PlatformClient(`http://127.0.0.1:${tcpPort(server)}`, 'session')
  const uploadMock = t.mock.method(client, 'putArtifactObject', async (_target, bytes) => {
    assert.deepEqual(bytes, await declarationBytes(declaration))
    if (drop === 'object-put') {
      drop = null
      throw new Error('object PUT outcome unknown')
    }
  ...
  const configuration = source
    ...
    : {
        ...
        display_name: 'Primary',
        source: {
          deployment_id: id('mpdep', 5),
          deployment_digest: sha('e'),
          resource_kind: 'model_provider_deployment',
        },
        model: 'fixture-model',
        maximum_input_tokens: 8192,
        maximum_output_tokens: 1024,
        declared_at: '2026-09-09T00:00:00.000000Z',
      }
  return {
    client,
    uploadMock,
    ...
    request: {
      tenant_id: tenant,
      installation_digest: sha('b'),
      input: { kind, configuration },
      quota: source ? null : { ...INITIAL_QUOTA },
      existing,
    } as ModelPublicationRequest,
    change: () => advance(),
    current: () => structuredClone(resource),
    releaseUpload: () => {
      deferUpload = false
      if (artifactId) artifacts.get(artifactId).state = 'ready'
    },
  }
}
...
test('model source and profile publish actual declaration bytes through all public lifecycle stages', async (t) => {
  for (const kind of ['source', 'model'])
    await t.test(kind, async (t) => {
      ...
      assert.deepEqual(
        f.calls.map((call) => call.phase),
        ['prepare', 'complete', 'create', 'validate', 'publish', 'deploy', 'activate'],
      )
    })
})
...
test('lost publication responses recover through exact readback or the original Receipt and CAS', async (t) => {
  for (const phase of [
    ...
    await t.test(phase, async (t) => {
      ...
      if (phase === 'complete')
        assert.equal(repeated.length, 1) // Ready Artifact readback resolves the uncertainty.
      else {
...
test('pending publication cannot accept a changed tenant, original CAS, declaration or post-activation drift', async (t) => {
  ...
  await assert.rejects(publishModelConfiguration(f.client, f.request, () => {}))
  await assert.rejects(
    resumeModelPublication(f.client, id('ten', 99), sha('b'), () => {}),
    /conflict/,
  )
  const saved = f.storage.get(publicationKey),
    handle = JSON.parse(saved)
  handle.existing.etag = `"${handle.existing.id}-9"`
  handle.existing.version = 9
  f.storage.set(publicationKey, JSON.stringify(handle))
  await assert.rejects(
    resumeModelPublication(f.client, tenant, sha('b'), () => {}),
    /conflict/,
  )
...
test('uploaded and Ready artifacts recover after wait timeout or final read loss without terminal prepare replay', async (t) => {
  for (const failure of ['wait-timeout', 'ready-get'])
    await t.test(failure, async (t) => {
      const f = await fixture(t, {
        drop: failure === 'ready-get' ? 'ready-get' : null,
        deferUpload: failure === 'wait-timeout',
      })
...
test('staging recovery refuses changed Artifact, Operation, Grant, CAS and original deadline before another PUT', async (t) => {
  for (const prepareDrift of [
    ...
    await t.test(prepareDrift, async (t) => {
      const f = await fixture(t, { drop: 'object-put', prepareDrift })
      await assert.rejects(
        publishModelConfiguration(f.client, f.request, () => {}),
        /PUT outcome unknown/,
      )
      ...
      assert.equal(f.uploadMock.mock.callCount(), 1)
...
test('a model handle without the required upload metadata is retained and rejected before HTTP', async (t) => {
  ...
  f.storage.set(publicationKey, original)
...
test('publication retains frozen quota and resumes its original allocation after activation', async (t) => {
  ...
  await assert.rejects(publishModelConfiguration(f.client, f.request, () => {}))
  const raw = f.storage.get(publicationKey),
    pending = JSON.parse(raw)
  assert.equal(
    pending.deployment.deployment_id,
    pending.quota_intent.model_deployment.deployment_id,
  )
  assert.deepEqual(pending.quota, INITIAL_QUOTA)
  await assert.rejects(
    publishModelConfiguration(
      f.client,
      { ...f.request, quota: { ...INITIAL_QUOTA, requests: 30 } },
      () => {},
    ),
    /conflict/,
  )
