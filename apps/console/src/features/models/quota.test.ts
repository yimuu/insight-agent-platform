import type { ModelQuotaView } from '../../shared/api/model-types.ts'
...
const tenant = id('ten', 1),
  target = {
    ...
    deployment_id: id('mdep', 2),
    deployment_digest: `sha256:${'a'.repeat(64)}`,
  }
const zero = { requests: 0, tokens: 0, cost_microunits: 0 }
const view = (letter: string, allocation: ModelQuotaView['allocation'] = null): ModelQuotaView => ({
  schema_version: 1,
  tenant_id: tenant,
  model_deployment: target,
  allocation,
  tenant_concurrency: { limit: 8, reserved: 0, used: 0 },
  etag: `"model-quota-${letter.repeat(64)}"`,
})
function memory(t) {
  const values = new Map(),
    previous = globalThis.sessionStorage
...
test('quota loss replays exact original deployment, limits, CAS and Receipt while observing new usage', async (t) => {
  const storage = memory(t),
    client = new PlatformClient('https://platform.example', 'private-session'),
    calls = []
  const original = view('a'),
    allocated = view('b', { limits: INITIAL_QUOTA, reserved: zero, used: zero })
  let current = allocated
  t.mock.method(client, 'setModelQuota', async (...args) => {
    calls.push(args)
    if (calls.length === 1) throw new Error('response lost')
    return { data: allocated, etag: allocated.etag }
  })
  t.mock.method(client, 'getModelQuota', async (id) => {
    assert.equal(id, target.deployment_id)
    return { data: current, etag: current.etag }
  })
  ...
  await assert.rejects(
    saveModelQuota(client, allocated, { ...INITIAL_QUOTA, requests: 30 }),
    /conflict/,
  )
  await assert.rejects(resumeModelQuota(client, id('ten', 4)), /conflict/)
  await assert.rejects(
    resumeModelQuota(new PlatformClient('https://different.example', 'session'), tenant),
    /conflict/,
  )
  ...
  assert.equal(calls[1][2], original.etag)
...
test('quota projections are closed, finite and internally consistent', () => {
  for (const invalid of [
    null,
    { ...INITIAL_QUOTA, requests: -1 },
    { ...INITIAL_QUOTA, requests: 1.5 },
    { ...INITIAL_QUOTA, requests: Number.MAX_SAFE_INTEGER + 1 },
    { ...INITIAL_QUOTA, extra: 0 },
    { requests: 1, tokens: 1 },
  ])
    assert.equal(validQuotaLimits(invalid), false)
  ...
  assert.equal(
    validQuotaView(view('b', { limits: zero, reserved: zero, used: { ...zero, requests: 1 } })),
    false,
  )
  assert.equal(
    validQuotaView({ ...view('a'), tenant_concurrency: { limit: 8, reserved: 8, used: 1 } }),
    false,
  )
  assert.equal(
    // @ts-expect-error Deliberately omit reserved to verify runtime rejection.
    validQuotaView({ ...view('a'), allocation: { limits: INITIAL_QUOTA, used: zero } }),
    false,
  )
})
