...
const write = (draft, field, raw) => {
  draft.children[field] = changeRaw(draft.children[field], raw)
}

test('typed required fields preserve false and enum types while optional fields remain absent', () => {
  const { node, draft } = shape(
    {
      name: text,
  ...
  write(draft, 'name', 'answer')
  write(draft, 'count', '2')
  write(draft, 'consent', 'false')
  write(draft, 'choice', '1')
  assert.deepEqual(validateDraft(node, draft).value, {
    name: 'answer',
    count: 2,
    consent: false,
    choice: 1.5,
  ...
  assert.equal(objectValue(validateDraft(node, draft).value).optional, 'hello')
})
...
test('text limits count Unicode characters and UTF-8 bytes, with overflow never silently submitted', () => {
  ...
  write(draft, 'text', '😀😀')
  assert.equal(objectValue(validateDraft(node, draft).value).text, '😀😀')
  write(draft, 'text', '😀😀😀')
  ...
  write(draft, 'text', 'ok')
  assert.equal(objectValue(validateDraft(node, draft).value).text, 'ok')
})

test('numbers do not coerce blank/hex/NaN/unsafe integers and respect open and closed bounds', () => {
  const { node, draft } = shape({
    count: { type: 'integer', exclusiveMinimum: 1, maximum: 3 },
    number: { type: 'number', minimum: -2, exclusiveMaximum: 1.5 },
  })
...
test('arrays validate minimum, maximum, duplicate object values, nested fields and escaped paths', () => {
  ...
  const { node, draft } = shape({
    entries: { type: 'array', minItems: 1, maxItems: 2, uniqueItems: true, items: nested },
  })
...
test('unsupported features and resource bounds disable the whole form explicitly', () => {
  for (const keyword of [
    ...
    const analysis = analyzeTaskSchema(
      object({ value: { ...text, [keyword]: 'unsupported' } }),
      digest,
    )
  ...
  for (const schema of [
    ...
    object({ value: { type: 'array', minItems: 0, maxItems: 65, items: text } }),
    ...
    object({
      value: {
        ...
        minItems: 0,
        maxItems: 64,
        items: { type: 'array', minItems: 0, maxItems: 64, items: text },
      },
    }),
  ]) {
  ...
  assert.equal(
    analyzeTaskSchema(
      object({ text: { ...text, description: 'x'.repeat(FORM_LIMITS.schemaBytes) } }),
      digest,
    ).node,
    undefined,
  )
...
test('envelope scope mismatch cannot submit; MCP root is implicitly closed and multi-select unique', () => {
  const schema = {
    schema_version: 1,
    profile: 'mcp.form-json-schema/2025-11-25',
    ...
    schema: {
      ...
      properties: {
        choices: {
          type: 'array',
          items: { type: 'string', enum: ['one', 'two'], enumNames: ['First', 'Second'] },
        },
      },
      required: ['choices'],
    },
  ...
  const itemNode = node.properties[0].node.items
  draft.children.choices.items = [
    changeRaw(createDraft(itemNode), '0'),
    changeRaw(createDraft(itemNode), '0'),
  ]
...
test('property names are data, and total response size is bounded', () => {
  ...
  const value = objectValue(validateDraft(node, draft).value)
  ...
  const large = shape(
    Object.fromEntries(
      Array.from({ length: 10 }, (_, index) => [
        String(index),
        { type: 'string', minLength: 0, maxLength: 8192, 'x-platform-max-bytes': 8192 },
      ]),
    ),
  )
  Object.values(large.draft.children).forEach((item) => {
    item.raw = 'x'.repeat(8192)
  })
