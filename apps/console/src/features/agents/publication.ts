...
import type { CompiledAgent } from '../../shared/compiler/compiler.ts'
...
function invalidHandle(): never {
  throw new Error(
    'publication_conflict: Publication recovery state is invalid; reconcile its existing server operation before another publication',
  )
}
...
function bounded(value: unknown, maximum = 512): value is string {
  return (
    typeof value === 'string' && value.length > 0 && encoder.encode(value).byteLength <= maximum
  )
}
...
function validateArtifact(value: unknown): void {
  ...
  if (
    !identity(item.artifact_id, 'art') ||
    !digest(item.content_digest) ||
    !positive(item.byte_length) ||
    !bounded(item.media_type) ||
    !['public', 'internal', 'confidential', 'restricted'].includes(String(item.classification)) ||
    (item.display_name !== null && !bounded(item.display_name))
  )
    invalidHandle()
}
...
function validateHandle(value: unknown): asserts value is PublicationHandle {
  ...
  if (
    ...
    !bounded(item.gateway_origin, 2048) ||
    !digest(item.source_bundle_digest) ||
    !digest(item.manifest_digest) ||
    !bounded(item.agent_name)
  )
    invalidHandle()
  for (const [key, prefix] of [
    ['existing_agent_id', 'agt'],
    ['resource_id', 'agt'],
    ['validation_operation_id', 'job'],
    ['deployment_id', 'adep'],
  ]) {
  ...
  for (const key of ['authoring_upload', 'plan_upload'])
    if (item[key] !== null) validateArtifactUploadRecovery(item[key] as ArtifactUploadRecovery)
  if (item.published_versions !== null) {
    if (!Array.isArray(item.published_versions) || item.published_versions.length !== 2)
      invalidHandle()
    for (const entry of item.published_versions) {
      const version = closed(entry, [
        'resource_version_id',
        'revision_no',
        'content_digest',
        'artifact_id',
      ...
      if (
        (!identity(version.resource_version_id, 'aif') &&
          !identity(version.resource_version_id, 'arev')) ||
        !positive(version.revision_no) ||
        !digest(version.content_digest) ||
        !identity(version.artifact_id, 'art') ||
        !bounded(version.etag, 256)
      )
        invalidHandle()
    }
  }
  ...
  if (
    present('existing_agent_id') !== present('existing_agent_etag') ||
    ...
    (present('existing_agent_id') &&
      present('resource_id') &&
      item.existing_agent_id !== item.resource_id)
  )
    invalidHandle()
}
...
function loadHandle(
  ...
  if (raw) {
    ...
    if (
      (value.authoring_artifact &&
        value.authoring_artifact.content_digest !== compiled.sourceBundleDigest) ||
      (value.plan_artifact && value.plan_artifact.content_digest !== compiled.typedPlanDigest)
    )
      invalidHandle()
    return value
  }
...
function requireResource(resource: ResourceView, expectedId: string | null): void {
  if (
    ...
    resource.resource_kind !== 'agent' ||
    !identity(resource.resource_id, 'agt') ||
    (expectedId !== null && resource.resource_id !== expectedId) ||
    !positive(resource.version) ||
    !bounded(resource.etag, 256) ||
...
function requireActive(resource: ResourceView, handle: PublicationHandle): void {
  ...
  if (resource.gate_state !== 'enabled' || resource.active_deployment_id !== handle.deployment_id) {
    throw new Error(
      'publication_conflict: The current active deployment differs from this publication',
    )
  }
}
...
export async function publishCompiledAgent(
  ...
  handle.authoring_artifact = await uploadArtifact(
    client,
    encoder.encode(compiled.sourceBundle),
    object(intent.authoring_artifact, 'authoring artifact intent'),
    receipt(handle.attempt_id, 'authoring-prepare'),
    receipt(handle.attempt_id, 'authoring-complete'),
    handle.authoring_upload,
    (state) => {
      handle.authoring_upload = state
      saveHandle(handle)
    },
  )
  saveHandle(handle)
  handle.plan_artifact = await uploadArtifact(
    client,
    encoder.encode(compiled.typedPlan),
    object(intent.typed_plan_artifact, 'Typed Plan artifact intent'),
    receipt(handle.attempt_id, 'plan-prepare'),
    receipt(handle.attempt_id, 'plan-complete'),
    handle.plan_upload,
    (state) => {
      handle.plan_upload = state
      saveHandle(handle)
    },
  )
  saveHandle(handle)
  ...
  if (!handle.resource_id || !handle.resource_etag) {
    const resource =
      handle.existing_agent_id && handle.existing_agent_etag
        ? await client.updateAgent(
            handle.existing_agent_id,
            draft,
            handle.existing_agent_etag,
            receipt(handle.attempt_id, 'update'),
          )
  ...
  if (validation.data.state !== 'succeeded') {
    throw new Error(
      `agent_validation_failed: ${validation.data.error?.code ?? validation.data.state}`,
    )
  }
  if (!handle.validated_resource_etag || !handle.draft_generation) {
    ...
    if (
      handle.resource_version === null ||
      validated.data.version !== handle.resource_version + 1 ||
      validated.data.draft_generation !== handle.resource_draft_generation ||
      validated.data.draft.display_name !== draft.display_name ||
      (await digestJson(validated.data.draft.document as Json)) !==
  ...
  if (!handle.published_versions || !handle.published_resource_etag) {
    const published = await client.publishAgent(
      handle.resource_id,
      {
        kind: 'agent',
      ...
      handle.validated_resource_etag,
      receipt(handle.attempt_id, 'publish'),
    )
    requireResponseEtag(published)
    if (
      published.data.schema_version !== 1 ||
      published.data.resource_id !== handle.resource_id ||
      published.data.resource_kind !== 'agent' ||
      published.data.draft_generation !== handle.draft_generation ||
  ...
  if (
    handle.published_versions.some(
      (version) => version.artifact_id !== handle.plan_artifact?.artifact_id,
    )
  ) {
    throw new Error('publish_invalid: Published versions must bind this Typed Plan Artifact')
  }
  const interfaceRevision = exactVersion(
    handle.published_versions,
    'aif_',
    String(intent.contract_digest),
  )
  ...
  if (!handle.deployment_id) {
    const deployment = await client.createAgentDeployment(
      handle.resource_id,
      {
      ...
      handle.published_resource_etag,
      receipt(handle.attempt_id, 'deploy'),
    )
    requireResponseEtag(deployment)
    if (
      deployment.data.schema_version !== 1 ||
      deployment.data.resource_id !== handle.resource_id ||
      deployment.data.resource_kind !== 'agent' ||
      deployment.data.resource_version_id !== planRevision.revision_id ||
  ...
  if (!handle.activation_etag) {
    ...
    if (
      handle.published_resource_version === null ||
      beforeActivation.data.version !== handle.published_resource_version + 1
    ) {
