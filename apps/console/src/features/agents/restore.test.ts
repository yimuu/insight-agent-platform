...
async function fixture() {
  ...
  const compiled = await compileAgentManifest({
    manifest: await readFile(new URL(c.manifest, base), 'utf8'),
    manifestPath: 'agents/restored/agent.yaml',
    inputSchema: await readFile(new URL(c.input_schema, base), 'utf8'),
    outputSchema: await readFile(new URL(c.output_schema, base), 'utf8'),
    profile: corpus.profile,
    bindings: c.bindings,
  })
  const ref = (id, digest, content) => ({
    artifact_id: id,
  ...
  const version = {
    ...
    revision_no: 1,
    content_digest: compiled.typedPlanDigest,
    artifact_id: planId,
    payload: { document: materializeDocument(compiled, authoring, plan), validation: {} },
  ...
  const current = {
    resource_id: agent,
    resource_kind: 'agent',
    etag: '"current-draft-CAS"',
    draft: {
      document: { resource_kind: 'agent', spec: { authoring_name: 'current-draft-different' } },
    },
  }
...
function transport(
  ...
  alter: (path: string, sample: Awaited<ReturnType<typeof fixture>>) => Response | undefined = () =>
    undefined,
) {
  const calls = []
  context.mock.method(globalThis, 'fetch', async (url, init) => {
    ...
    const altered = alter(path, sample)
    if (altered) return altered
    const json = (body, headers = {}) =>
      new Response(JSON.stringify(body), {
        headers: { 'content-type': 'application/json', ...headers },
      })
    if (path === `/v1/agents/${agent}/versions/${versionId}`)
      return json(sample.version, { etag: sample.version.etag })
    if (path === `/v1/agents/${agent}`) return json(sample.current, { etag: sample.current.etag })
    if (path === `/v1/artifacts/${authoringId}`)
      return json({
        artifact_id: authoringId,
        state: 'ready',
        purpose: 'authoring_document',
        content: sample.authoring,
      })
    if (path === `/v1/artifacts/${planId}`)
      return json({
        artifact_id: planId,
        state: 'ready',
        purpose: 'typed_plan',
    ...
    if (path === `/v1/artifacts/${authoringId}/content`)
      return new Response(sample.compiled.sourceBundle, {
        headers: {
          'content-length': String(sample.authoring.byte_length),
          'content-type': 'application/json',
          etag: `"${sample.authoring.content_digest}"`,
        },
      })
...
test('published restore uses exact authorized sources, original manifest path and frozen model; current draft only supplies edit CAS', async (context) => {
  ...
  const result = await restorePublishedSources(
    new PlatformClient('https://platform.example', 'token'),
    agent,
    versionId,
    controller.signal,
  )
  ...
  assert.equal(
    ...
    JSON.parse(f.compiled.sourceBundle).sources.files['agents/restored/agent.yaml'],
  )
...
test('published restore stops on current content 403 before editor authority; no partial result', async (context) => {
  const f = await fixture()
  const calls = transport(context, f, (path) =>
    path.endsWith('/content')
      ? new Response(
          JSON.stringify({
            code: 'permission_denied',
            detail: 'Content read denied.',
            retryable: false,
          }),
          { status: 403 },
        )
...
test('published restore rejects altered bytes, exact version mismatch and complete document drift', async (context) => {
  ...
  for (const variant of ['bytes', 'identity', 'document', 'etag', 'plan']) {
    const f = await fixture()
    if (variant === 'identity')
      f.version.resource_version_id = 'arev_01950000-0000-7000-8000-000000000099'
    if (variant === 'document')
      (f.version.payload.document.spec as Record<string, unknown>).author_instructions =
        'silently changed'
    if (variant === 'etag') f.version.etag = '"different"'
    if (variant === 'plan') f.plan.content_digest = `sha256:${'f'.repeat(64)}`
    const calls = transport(context, f, (path) =>
      variant === 'bytes' && path.endsWith('/content')
        ? new Response(f.compiled.sourceBundle.replace('model', 'mOdel'), {
            headers: {
              'content-length': String(f.authoring.byte_length),
              'content-type': 'application/json',
              etag: `"${f.authoring.content_digest}"`,
            },
          })
        : undefined,
    )
...
function frozenDefinition(f) {
  ...
  const exact = {
    ...
    resource_kind: 'agent_deployment' as const,
    deployment_digest: `sha256:${'a'.repeat(64)}`,
  }
  const plan = {
    revision_id: versionId,
    resource_kind: 'agent_plan_revision',
    semantic_digest: f.compiled.typedPlanDigest,
  }
  const contract = {
    revision_id: 'aif_01950000-0000-7000-8000-000000000006',
    resource_kind: 'agent_interface_revision',
    semantic_digest: f.compiled.contractDigest,
  }
  const definition = {
    schema_version: 1,
    run_id: 'run_01950000-0000-7000-8000-000000000007',
    agent_id: agent,
    agent_deployment: exact,
    agent_interface: contract,
    plan,
  }
  const deployment = {
    schema_version: 1,
    ...
    resource_kind: 'agent',
    deployment_id: deploymentId,
    closure_digest: exact.deployment_digest,
    closure: { resource_kind: 'agent', bindings: { plan, interface: contract } },
    etag: `"${deploymentId}-${exact.deployment_digest.slice(7)}"`,
  }
  return {
    definition,
    deployment,
    summary: { agent_id: agent, active_deployment: exact },
    run: { run_id: definition.run_id, agent_deployment_id: deploymentId },
  }
}
test('Run defaults use the exact active published Plan while the editable draft differs', async (context) => {
  const f = await fixture(),
    exact = frozenDefinition(f)
  const calls = transport(context, f, (path) =>
    path.endsWith(`/deployments/${exact.deployment.deployment_id}`)
      ? new Response(JSON.stringify(exact.deployment), { headers: { etag: exact.deployment.etag } })
      : undefined,
  )
  const defaults = await publishedRunDefaults(
    new PlatformClient('https://platform.example', 'token'),
    exact.summary,
  )
  assert.equal(
    defaults.schemaDigest,
    (
      (f.version.payload.document.spec as Record<string, unknown>).input_schema as Record<
        string,
        unknown
      >
    ).canonical_digest,
...
test('Run source map is rebuilt from its exact frozen definition and current authorized Artifact', async (context) => {
  ...
  const calls = transport(context, f, (path) =>
    path.endsWith('/definition')
      ? new Response(JSON.stringify(exact.definition))
      : path.endsWith(`/deployments/${exact.deployment.deployment_id}`)
        ? new Response(JSON.stringify(exact.deployment), {
            headers: { etag: exact.deployment.etag },
          })
        : undefined,
  )
  const source = await frozenRunSources(
    new PlatformClient('https://platform.example', 'token'),
    exact.run,
  )
  ...
  assert.ok(
    calls.every(({ init }) => init.cache === 'no-store' && !init.headers.has('idempotency-key')),
  )
})
test('Run source and defaults reject altered exact references and content revocation without alternate lookups', async (context) => {
  for (const mode of ['head', 'plan', 'content', 'deployment_identity', 'etag']) {
    ...
    transport(context, f, (path) =>
      path.endsWith('/definition')
        ...
        : path.endsWith(`/deployments/${exact.deployment.deployment_id}`)
          ...
          : mode === 'content' && path.endsWith('/content')
    ...
    await assert.rejects(
      mode === 'head'
        ? publishedRunDefaults(client, exact.summary)
        : frozenRunSources(client, exact.run),
    )
    context.mock.restoreAll()
  }
})
