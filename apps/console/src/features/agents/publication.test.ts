...
const id = (prefix, counter) =>
  prefix + '_0198f1cc-32e4-75e1-a9e8-' + counter.toString(16).padStart(12, '0')

async function compilation(displayName?: string) {
  ...
  const manifest = displayName
    ? original.replace('  name: echo-agent', '  name: echo-agent\n  displayName: ' + displayName)
    : original
  return compileAgentManifest({
    manifest,
    inputSchema: await text(item.input_schema),
    outputSchema: await text(item.output_schema),
...
async function fixture(
  ...
  options: {
    drop?: string
    failPublish?: boolean
    invalidCurrentEtag?: boolean
    driftDuringValidation?: string
    driftBeforeActivation?: boolean
    dropUpload?: string
  } = {},
) {
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      const send = (status, value) => {
        response.writeHead(status, {
          ...
          ...(value?.etag
            ? {
                etag:
                  options.invalidCurrentEtag &&
                  request.method === 'GET' &&
                  value.gate_state === 'enabled'
                    ? '"different"'
                    : value.etag,
              }
            : {}),
      ...
      const phase = path.endsWith(':activate')
        ? 'activate'
        : path.endsWith('/deployments')
          ? 'deploy'
          : path.endsWith('draft:publish')
            ? 'publish'
            : path.endsWith('draft:validate')
              ? 'validate'
              : request.method === 'PUT'
                ? 'update'
                : 'create'
      ...
      if (replay) {
        if (replay.fingerprint !== fingerprint)
          return send(409, { code: 'receipt_conflict', detail: 'Receipt request changed' })
        return send(200, replay.value)
      }
      if (phase !== 'create' && request.headers['if-match'] !== resource.etag)
        return send(409, { code: 'version_conflict', detail: 'CAS changed' })
      ...
      if (phase === 'create') {
        ...
        if (options.driftDuringValidation === 'document')
          resource.draft.document.spec.default_deadline_seconds++
        ...
        published = [
          {
            resource_version_id: id('aif', counter++),
          ...
          {
            resource_version_id: id('arev', counter++),
            revision_no: resource.draft_generation,
        ...
        value = {
          ...
          resource_id: agentId,
          resource_kind: 'agent',
          draft_generation: resource.draft_generation,
  ...
  const client = Object.assign(new PlatformClient('http://127.0.0.1:' + tcpPort(server), ''), {
    prepareArtifactUpload: async (body, key) => {
      if (uploads.has(key)) {
        const previous = uploads.get(key)
        if (artifactStates.get(previous.data.artifact_id) !== 'staging')
          throw new Error('terminal prepare replay rejected')
        return previous
      }
      ...
      artifacts.set(artifactId, {
        ...
        content_digest: body.expected_digest,
        byte_length: body.expected_size_bytes,
        media_type: body.declared_media_type,
        classification: body.classification,
        display_name: body.display_name,
      })
      const result: { purpose?: string; data: Record<string, unknown> } = {
        data: {
          schema_version: 1,
          artifact_id: artifactId,
    ...
    completeArtifactUpload: async (artifactId) => {
      ...
      if (options.dropUpload === 'complete') {
        options.dropUpload = null
        throw new Error('completion response lost')
      }
      return { data: {} }
    },
    waitOperation: async (operation_id) => ({
      data: { operation_id, state: 'succeeded', error: null },
    }),
    getArtifact: async (artifactId) => {
      const content = artifacts.get(artifactId)
      const state = artifactStates.get(artifactId)
      const version = state === 'staging' ? 1 : 4
      const intent = [...uploads.values()].find((value) => value.data.artifact_id === artifactId)
      return {
        ...
        data: {
          ...
          expected_size_bytes: content.byte_length,
          declared_media_type: content.media_type,
          state,
          content: state === 'ready' ? content : null,
        },
      }
    },
  })
  return {
    ...
    observe,
    drift: () => {
      resource.active_deployment_id = id('adep', 999)
      advance()
    },
  }
}
...
test('actual HTTP publish binds Plan Artifact and retries the same receipt without storing source', async (t) => {
  const compiled = await compilation(),
    f = await fixture(t, { failPublish: true })
  await assert.rejects(
    publishCompiledAgent(f.client, compiled, null, () => {}),
    /unavailable/,
  )
...
test('A to B to C to B creates separate publication commands for repeated source content', async (t) => {
  ...
  const variants = await Promise.all(
    ['A', 'B', 'C', 'B'].map((name) => compilation('Agent ' + name)),
  )
  ...
  for (const variant of variants) {
    ...
    assert.equal(
      current.draft.document.spec.authoring_package.artifact.content_digest,
      variant.sourceBundleDigest,
    )
  }
...
test('a historical activation receipt cannot claim a different current head is ready', async (t) => {
  const compiled = await compilation(),
    f = await fixture(t, { drop: 'activate' })
  await assert.rejects(publishCompiledAgent(f.client, compiled, null, () => {}))
  f.drift()
  const stages = []
  await assert.rejects(
    publishCompiledAgent(f.client, compiled, null, (stage) => stages.push(stage)),
    /current active deployment differs/,
  )
...
test('a concurrent Resource mutation after deployment cannot be swallowed by activation', async (t) => {
  const compiled = await compilation(),
    f = await fixture(t, { driftBeforeActivation: true })
  await assert.rejects(
    publishCompiledAgent(f.client, compiled, null, () => {}),
    /changed after this deployment/,
  )
  assert.equal(
    f.calls.some((call) => call.phase === 'activate'),
    false,
  )
})
...
test('invalid, oversized or out of order handles fail before HTTP and preserve the pending intent', async (t) => {
  const compiled = await compilation(),
    f = await fixture(t, { failPublish: true })
  await assert.rejects(publishCompiledAgent(f.client, compiled, null, () => {}))
  const valid = f.memory.get(handleKey),
...
for (const drift of ['version', 'generation', 'document']) {
  test('validation cannot absorb a concurrent ' + drift + ' change', async (t) => {
    const compiled = await compilation(),
      f = await fixture(t, { driftDuringValidation: drift })
    await assert.rejects(
      publishCompiledAgent(f.client, compiled, null, () => {}),
      /draft changed during this validation/,
    )
    assert.equal(
      f.calls.some((call) => call.phase === 'publish'),
      false,
    )
    assert.ok(f.memory.has(handleKey))
  })
...
test('mismatched current HTTP and body ETags cannot clear recovery or report ready', async (t) => {
  const compiled = await compilation(),
    f = await fixture(t, { invalidCurrentEtag: true })
  const stages = []
  await assert.rejects(
    publishCompiledAgent(f.client, compiled, null, (stage) => stages.push(stage)),
    /ETag differs/,
...
test('Agent upload completion loss retains prepared metadata and never retries terminal prepare', async (t) => {
  const compiled = await compilation(),
    f = await fixture(t, { dropUpload: 'complete' })
  await assert.rejects(
    publishCompiledAgent(f.client, compiled, null, () => {}),
...
test('previous Agent handle remains visible and is rejected without any HTTP', async (t) => {
  ...
  delete saved.authoring_upload
  delete saved.plan_upload
