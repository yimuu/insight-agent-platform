...
} from '../../shared/compiler/compiler.ts'
import {
  buildFormManifest,
  exactSlotBindings,
  manifestFormFields,
...
async function seed() {
  ...
  const input = {
    manifest: await source('deterministic.yaml'),
    inputSchema: await source('schema-message.json'),
    outputSchema: await source('schema-message.json'),
...
test('a Full Plan compiles unchanged through actual WASM and its complete source bundle imports without loss', async () => {
  ...
  const recompiled = await compileAgentManifest({
    ...input,
    manifest: buildFormManifest(imported.fields),
    plan: imported.plan,
    bindings: { model: null, slots: exactSlotBindings(imported.slotBindings) },
  })
...
test('unsupported nodes and exact slot structures reach Rust errors without a template fallback', async () => {
  ...
  assert.equal(
    planNodeOutline(JSON.stringify(plan)).nodes.find((node) => node.entry).kind,
    'not_a_supported_node',
  )
  await assert.rejects(
    compileAgentManifest({ ...input, plan: JSON.stringify(plan) }),
  ...
  await assert.rejects(
    compileAgentManifest({ ...input, bindings: { model: null, slots } }),
    (error) => error instanceof AgentCompilerError,
  )
  await assert.rejects(
    manifestFormFields(input.manifest.replace('full_plan', 'future_template')),
    (error) => error instanceof AgentCompilerError,
  )
})
...
test('editor JSON rejects duplicate slot keys, unbounded input, and malformed containers', () => {
  assert.throws(
    () => exactSlotBindings('[{"slot_id":"first","slot_id":"second"}]'),
    /duplicate keys/,
  )
  ...
  const nodes = Object.fromEntries(
    Array.from({ length: 129 }, (_, index) => [String(index), { kind: 'return' }]),
  )
...
test('bundle import does not discard missing, unreferenced, or unsupported source data', async () => {
  ...
  await assert.rejects(
    readEditableSourceBundle(JSON.stringify(bundle)),
    /supported complete Agent source bundle/,
  )
})
...
test('framework graph keeps the static export as source and compiles at Platform node granularity through WASM', async () => {
  ...
  const adapterDigest = `sha256:${createHash('sha256')
    .update(JSON.stringify(descriptor, Object.keys(descriptor).sort()))
    .digest('hex')}`
  const graph = {
    schema_version: 1,
  ...
  const manifest = buildFormManifest({
    ...(await manifestFormFields(input.manifest)),
    executionKind: 'framework_graph',
    planPath: 'framework.json',
  })
  ...
  await assert.rejects(
    compileAgentManifest({ ...input, manifest, plan: 'def graph(): pass' }),
    AgentCompilerError,
  )
  await assert.rejects(
    compileAgentManifest({ ...input, manifest, plan: JSON.stringify({ ...graph, reducers: {} }) }),
    AgentCompilerError,
  )
})
