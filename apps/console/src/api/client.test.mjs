...
test('Run signal uses its exact 204 public port with stable Receipt and AbortSignal', async (context) => {
  const requests = []
  let status = 204
  context.mock.method(globalThis, 'fetch', async (url, init) => { requests.push({ url, init }); return new Response(status === 204 ? null : '{}', { status }) })
  const client = new PlatformClient('https://platform.example', 'token')
  const controller = new AbortController()
  const body = { payload: { classification: 'internal', schema_digest: `sha256:${'a'.repeat(64)}`, value: { kind: 'inline', value: { approved: false } } } }
  for (let i = 0; i < 2; i++) assert.equal((await client.signalRun('run_/exact', 'approval', body, 'same-receipt', { signal: controller.signal })).data, null)
  assert.ok(requests.every(({ url, init }) => url.endsWith('/runs/run_%2Fexact/signals/approval') && init.method === 'POST' && init.headers.get('idempotency-key') === 'same-receipt' && !init.headers.has('if-match') && init.signal === controller.signal && init.cache === 'no-store'))
  assert.deepEqual(JSON.parse(requests[0].init.body), body)
  status = 200
  await assert.rejects(client.signalRun('run_exact', 'approval', { payload: null }, 'same-receipt'), /success status/)
})
...
test('Task inbox/form use exact filters, preserve empty-page continuation and pass cancellation', async (context) => {
  ...
  context.mock.method(globalThis, 'fetch', async (url, init) => {
    calls.push({ url, init })
    return new Response(JSON.stringify({ schema_version: 1, items: [], next_cursor: 'opaque+/next==' }), { status: 200 })
  })
  const client = new PlatformClient('https://platform.example', 'token')
  const abort = new AbortController()
  const page = await client.listTasks({ state: 'pending', kind: 'human_work', runId: 'run_id', cursor: 'opaque+/cursor==' }, { signal: abort.signal })
  assert.equal(calls[0].url, 'https://platform.example/v1/tasks?page_size=25&state=pending&kind=human_work&run_id=run_id&cursor=opaque%2B%2Fcursor%3D%3D')
  ...
  await client.taskAction('int_/id', 'submit-input', '"task-v2"', 'stable-receipt', { classification: 'internal', schema_digest: 'digest', value: { kind: 'inline', value: { accepted: false } } }, { signal: abort.signal })
  ...
  assert.ok(calls.every(({ init }) => init.signal === abort.signal && init.cache === 'no-store'))
