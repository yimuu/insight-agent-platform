...
function parseBlock(lines: string[]): RunEvent | null {
  ...
  if (id.includes('\0')) throw new RunEventProtocolError('invalid_event: SSE cursor contains a null byte')
  const raw = data.join('\n')
...
export class RunEventStreamDecoder {
  ...
  push(bytes: Uint8Array, accept: (event: RunEvent) => void): void {
    ...
    let text: string
    try { text = this.decoder.decode(bytes, { stream: true }) } catch {
  ...
  private append(fragment: string): void {
    ...
    if (this.frameBytes > MAX_FRAME_BYTES) {
      throw new RunEventProtocolError('event_too_large: SSE frame exceeded the 256 KiB data and bounded framing budget')
    }
    this.line += fragment
  }

  finish(): void {
    try { this.decoder.decode() } catch { throw new RunEventTransportError() }
...
export interface RunEventHistory { readonly replayFloor: string; readonly highWaterSequence: string; readonly truncated: boolean }
...
export async function readRunEventStream(response: Response, options: RunEventPageOptions = {}): Promise<RunEvent[]> {
  ...
  const validU64 = (value: string | null): value is string => value !== null && /^(0|[1-9][0-9]{0,19})$/.test(value) && BigInt(value) <= 18_446_744_073_709_551_615n
  if (!validU64(replayFloor) || !validU64(highWaterSequence) || BigInt(replayFloor) > BigInt(highWaterSequence) || (truncated !== 'true' && truncated !== 'false')) {
    await response.body?.cancel()
    throw new RunEventProtocolError('invalid_event_history: Required Run history boundaries are missing or invalid.')
  }
...
export async function followRunEventPages(readPage: RunEventPageReader, options: RunEventFollowOptions): Promise<void> {
    ...
    while (!signal.aborted) {
      ...
      try {
        await readPage(cursor, {
          signal,
          onHistory(history) { if (!signal.aborted) options.onHistory?.(history) },
          onEvent(event) {
            if (signal.aborted) return
            const eventId = event.data.event_id
            if (typeof eventId !== 'string' || !eventId) {
              throw new RunEventProtocolError('invalid_event: Durable public event is missing event_id')
            }
            if (!seen.has(eventId)) {
              seen.add(eventId)
        ...
        if (accepted > 0) { retryMilliseconds = INITIAL_RETRY_MILLISECONDS; continue }
      } catch (error) {
        if (signal.aborted) break
        if (!retryable(error)) {
          if (error instanceof Error && 'status' in error && (error.status === 401 || error.status === 403)) clear()
          throw error
        }
      }
