...
export class PlatformClient {
  ...
  private async request<T>(path: string, init: RequestInit = {}, decode?: (text: string) => T, expectedStatus?: number): Promise<AuthorityResponse<T>> {
    ...
    if (expectedStatus !== undefined && response.status !== expectedStatus) {
      ...
      throw new Error('unexpected_response_status: Public operation returned an unsupported success status')
    }
  ...
  getAgentAuthoringProfile(options: { signal?: AbortSignal } = {}) { return this.request<AgentAuthoringProfile>('/agent-authoring-profile', { signal: options.signal }) }
  listAuthoringDependencies(filters: DependencyFilters, options: { signal?: AbortSignal } = {}) {
    ...
    if (filters.interfaceContractDigest) query.set('interface_contract_digest', filters.interfaceContractDigest)
    if (filters.cursor) query.set('cursor', filters.cursor)
    return this.request(`/agent-authoring-dependencies?${query}`, options, (text) => parseDependencyPage(text, filters))
  }
  resolveAgentBindings(request: BindingSelections, options: { signal?: AbortSignal } = {}) {
    return this.request('/agent-authoring-bindings:resolve', { ...options, method: 'POST', body: JSON.stringify(request) }, (text) => parseBindingResolution(text, request))
  }
  ...
  getRun(id: string, options: { signal?: AbortSignal } = {}) { return this.request<RunView>(`/runs/${encodeURIComponent(id)}`, { signal: options.signal }) }
  getRunDefinition(id: string, options: { signal?: AbortSignal } = {}) { return this.request<RunDefinition>(`/runs/${encodeURIComponent(id)}/definition`, options) }
  getRunResult(id: string, options: { signal?: AbortSignal } = {}) { return this.request<JsonObject>(`/runs/${encodeURIComponent(id)}/result`, { signal: options.signal }) }
  listRunValues(id: string, filters: { nodeId?: string; cursor?: string } = {}, options: { signal?: AbortSignal } = {}) {
    const query = new URLSearchParams({ page_size: '25' })
    if (filters.nodeId) query.set('node_id', filters.nodeId)
    if (filters.cursor) query.set('cursor', filters.cursor)
    return this.request<ListPage<RunValueMetadata>>(`/runs/${encodeURIComponent(id)}/values?${query}`, options)
  }
  getRunValueContent(runId: string, valueId: string, options: { signal?: AbortSignal } = {}) {
    return this.request<JsonObject>(`/runs/${encodeURIComponent(runId)}/values/${encodeURIComponent(valueId)}/content`, options)
  }
  listTasks(filters: { purpose?: 'respondable' | 'viewable'; state?: string; kind?: string; runId?: string; cursor?: string } = {}, options: { signal?: AbortSignal } = {}) {
    const query = new URLSearchParams({ page_size: '25' })
  ...
  getTask(id: string, options: { signal?: AbortSignal; purpose?: 'respondable' | 'viewable' } = {}) { const query = new URLSearchParams({ purpose: options.purpose ?? 'respondable' }); return this.request<TaskView>(`/tasks/${encodeURIComponent(id)}?${query}`, { signal: options.signal }) }
  getTaskForm(id: string, options: { signal?: AbortSignal } = {}) { return this.request<TaskForm>(`/tasks/${encodeURIComponent(id)}/form`, options) }
  getArtifact(id: string, options: { signal?: AbortSignal } = {}) { return this.request<ArtifactView>(`/artifacts/${encodeURIComponent(id)}`, options) }
  getResourceVersion(noun: string, id: string, versionId: string, options: { signal?: AbortSignal } = {}) {
    return this.request<ResourceVersionView>(`/${encodeURIComponent(noun)}/${encodeURIComponent(id)}/versions/${encodeURIComponent(versionId)}`, options)
  }
  getOperation(id: string) { return this.request<OperationView>(`/operations/${encodeURIComponent(id)}`) }
  getResource(noun: string, id: string, options: { signal?: AbortSignal } = {}) { return this.request<ResourceView>(`/${encodeURIComponent(noun)}/${encodeURIComponent(id)}`, options) }
  getDeployment(noun: string, resourceId: string, deploymentId: string, options: { signal?: AbortSignal } = {}) {
    return this.request<DeploymentView>(`/${encodeURIComponent(noun)}/${encodeURIComponent(resourceId)}/deployments/${encodeURIComponent(deploymentId)}`, options)
  }
  ...
  async getRunEvents(id: string, cursor?: string, options: RunEventPageOptions = {}): Promise<RunEvent[]> {
  ...
  followRunEvents(id: string, options: RunEventFollowOptions): Promise<void> {
    return followRunEventPages((cursor, pageOptions) => this.getRunEvents(id, cursor, pageOptions), options)
  }

  taskAction(id: string, action: 'submit-input' | 'approve' | 'reject' | 'cancel', etag: string, receipt: string, body?: JsonObject, options: { signal?: AbortSignal } = {}) {
  ...
  signalRun(id: string, key: string, body: JsonObject, receipt: string, options: { signal?: AbortSignal } = {}) {
    return this.request<null>(`/runs/${encodeURIComponent(id)}/signals/${encodeURIComponent(key)}`, {
      ...options, method: 'POST', headers: { 'Idempotency-Key': receipt }, body: JSON.stringify(body),
    }, undefined, 204)
  }

  async downloadArtifact(id: string, options: { signal?: AbortSignal; maximumBytes?: number } = {}): Promise<{ blob: Blob; mediaType: string; etag: string | null }> {
    ...
    if (lengthHeader === null || !/^\d+$/.test(lengthHeader) || !Number.isSafeInteger(declared) || declared < 0 || declared > maximum) {
      await response.body?.cancel()
    ...
    try {
      while (true) {
        ...
        if (received > declared || received > maximum) throw new Error('content_length_mismatch: Artifact download exceeded its exact bound')
