...
test('WASM adapter preserves the Rust v2 corpus and binds the new complete source Artifact', async () => {
  ...
  for (const fixture of corpus.cases) {
    ...
    assert.equal(compiled.resourceIntent.authoring_artifact.content_digest, compiled.sourceBundleDigest)
    assert.equal(compiled.resourceIntent.authoring_artifact.byte_length, new TextEncoder().encode(compiled.sourceBundle).length)
    assert.equal(compiled.sourceBundleDigest, `sha256:${createHash('sha256').update(compiled.sourceBundle).digest('hex')}`)
  }
})
...
test('TypeScript adapter rejects unsafe YAML and impossible deterministic schemas', async () => {
  ...
  await assert.rejects(
    compileAgentManifest({
      manifest: manifest.replace('output:\n    schema: schema-message.json', 'output:\n    schema: schema-answer.json'),
      inputSchema: await text('schema-message.json'),
...
test('actual WASM preserves local schema definitions, embedded IDs and literal ref data in echo and full Plan compilation', async () => {
  ...
  const schema = {
    ...
    $defs: {
      Message: {
        ...
        type: 'string', minLength: 1, maxLength: 128, 'x-platform-max-bytes': 512,
      },
      Literal: {
  ...
  const input = {
    manifest, inputSchema: schemaSource, outputSchema: schemaSource,
    profile: corpus.profile, bindings: { model: null, slots: [] },
  }
  ...
  const response = JSON.parse(new TextDecoder().decode(compile_agent(new TextEncoder().encode(compiled.sourceBundle))))
  ...
  assert.equal(decoded(response.compilation.compiled.canonical_manifest_bytes), compiled.canonicalManifest)
  ...
  assert.equal(JSON.parse(fullPlan.sourceBundle).sources.files['echo-plan.json'], compiled.typedPlan)
...
test('source-only WASM preflight rejects local faults without profile or bindings and compiles the same captured snapshot', async () => {
  const corpus = JSON.parse(await text('corpus.json'))
  const input = { manifest: await text('deterministic.yaml'), inputSchema: await text('schema-message.json'), outputSchema: await text('schema-message.json') }
  ...
  const compiled = await compileCapturedAgentSources(captured.sources, corpus.profile, { model: null, slots: [] })
  assert.equal(JSON.parse(compiled.sourceBundle).sources.files['agent.yaml'], captured.sources.files['agent.yaml'])
  for (const invalid of [
    { ...input },
    { manifest: await text('model-chat.yaml'), inputSchema: '{ invalid schema', outputSchema: await text('schema-answer.json') },
    { manifest: (await text('deterministic.yaml')).replace('kind: deterministic', 'kind: full_plan\n    plan: graph.json'), inputSchema: await text('schema-message.json'), outputSchema: await text('schema-message.json'), plan: '{ invalid Plan' },
