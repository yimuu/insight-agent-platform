...
import { branchIndex } from '../schema/tree.ts'
import { compileAgentManifest, rebuildExpression } from './compiler.ts'

const descriptor = readNodeEditorDescriptor(await readFile(new URL('../../../../contracts/platform-v1/agent-node-editor.v1.json', import.meta.url), 'utf8'))
...
test('actual Rust source map bytes are digest exact and map selected Plan nodes to real source positions', async () => {
  ...
  const input = { manifest: await read('deterministic.yaml'), inputSchema: await read('schema-message.json'), outputSchema: await read('schema-message.json'), profile: corpus.profile, bindings: { model: null, slots: [] } }
  const echo = await compileAgentManifest(input)
  assert.equal(echo.sourceMapDigest, `sha256:${createHash('sha256').update(echo.sourceMap).digest('hex')}`)
  ...
  for (const id of Object.keys(JSON.parse(echo.typedPlan).nodes)) {
    ...
    assert.ok(locations.every((location) => location.file === 'agent.yaml' && location.line > 0 && location.column > 0))
  }
})
...
test('actual WASM expression rebuild computes exact stack/digest and rejects invalid instruction programs', async () => {
  ...
  const rebuilt = await rebuildExpression({ ...draft, maximum_stack_depth: 65535, semantic_digest: `sha256:${'f'.repeat(64)}`, unknown_authored_field: 'preserved for complete compile diagnostics' })
  ...
  await assert.rejects(rebuildExpression({ ...draft, instructions: [{ op: 'boolean_and' }] }), /expression|instruction|stack/i)
})
