import type { ArtifactRef, ExactDeploymentRef, Json, JsonObject, ResourceView } from '../api/types.ts'
...
export type ModelConnectionOutcome = 'response_received' | 'credentials_rejected' | 'model_unavailable' | 'rate_limited' | 'provider_unavailable' | 'invalid_response' | 'timed_out' | 'transport_unavailable'
...
export interface ModelQuotaLimits { requests: number; tokens: number; cost_microunits: number }
export interface ModelQuotaView {
  schema_version: 1
  tenant_id: string
  model_deployment: ExactDeploymentRef
  allocation: { limits: ModelQuotaLimits; reserved: ModelQuotaLimits; used: ModelQuotaLimits } | null
