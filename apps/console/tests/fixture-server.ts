...
const root = resolve(
  process.env.INSIGHT_CONSOLE_BUNDLE_ROOT ?? fileURLToPath(new URL('../dist', import.meta.url)),
)
initSync({
  module: readFileSync(
    new URL(
...
const taskResponseSchema = {
  ...
  schema: taskSchema,
  canonical_digest: digest(taskSchema),
}
const policyBinding = {
  deployment: {
    deployment_id: policyDeploymentId,
    resource_kind: 'policy_deployment',
    deployment_digest: `sha256:${'b'.repeat(64)}`,
  },
  revision: {
...
Object.assign(authoringProfile, { profile_digest: digest(authoringProfile) })
...
const server = createServer(async (request, response) => {
  ...
  if (request.method === 'PUT' && objectUpload) {
    try {
      ...
      if (
        !prepared ||
        Buffer.byteLength(text) !== prepared.expected_size_bytes ||
        digest(text) !== prepared.expected_digest
      )
        throw new Error('actual upload differs from declared Artifact')
      if (prepared.purpose === 'authoring_document') {
        const result = JSON.parse(
          new TextDecoder().decode(compile_agent(new TextEncoder().encode(text))),
        )
        if (
          result.outcome !== 'compiled' ||
          result.compilation.compiled.execution_kind !== 'full_plan'
        )
          throw new Error('browser did not publish a shared compiler Full Plan')
        uploadedCompilation = result.compilation
        if (
          uploadedCompilation.source_bundle_digest !== prepared.expected_digest ||
          new TextDecoder().decode(new Uint8Array(uploadedCompilation.source_bundle_bytes)) !== text
        )
          throw new Error('uploaded source is not exact canonical compiler input')
        process.stdout.write(
          `${JSON.stringify({ fixture_observation: 'full_plan_source_uploaded', execution_kind: 'full_plan', digest: prepared.expected_digest })}\n`,
        )
      } else if (prepared.purpose === 'typed_plan') {
        if (
          !uploadedCompilation ||
          JSON.parse(text).plan_version !== 6 ||
          text !==
            new TextDecoder().decode(new Uint8Array(uploadedCompilation.compiled.typed_plan_bytes))
        )
          throw new Error('Plan upload is not the exact shared compiler output')
        process.stdout.write(
          `${JSON.stringify({ fixture_observation: 'exact_plan_uploaded', plan_version: 6, digest: prepared.expected_digest })}\n`,
        )
      ...
      items: [
        {
          ...
          display_name: 'Preexisting Agent',
          agent_id: 'agt_0198f1c3-8f49-7c3e-b1f3-773c28367bc0',
          state: 'ready',
          ...
          ? [
              {
                ...
                active_deployment: {
                  ...
                  deployment_digest: authoredDeployment.closure_digest,
                },
              },
            ]
          : []),
  ...
  if (request.method === 'POST' && url.pathname === '/v1/artifacts:prepare-upload') {
    ...
    sendJson(
      ...
      201,
      {
        ...
        upload_target: {
          url: `https://objects.example/${artifactId}`,
          completion_proof: `proof-${index}`,
        },
  ...
  if (request.method === 'POST' && completeArtifact) {
    ...
    if (!prepared?.uploaded)
      return problem(
        response,
        409,
        'fixture_upload_missing',
        'Actual object bytes must arrive before completion.',
      )
    prepared.completed = true
    sendJson(response, 202, {
      schema_version: 1,
      artifact_id: completeArtifact[1],
      artifact_etag: `"${completeArtifact[1]}-2"`,
  ...
  if (request.method === 'GET' && artifactContent) {
    ...
    if (!prepared?.uploaded || typeof prepared.bytes !== 'string')
      return problem(response, 404, 'not_found', 'Uploaded Artifact content is unavailable.')
    response.writeHead(
      200,
      headers({
        'content-type': prepared.declared_media_type,
        'content-length': Buffer.byteLength(prepared.bytes),
        etag: `"${prepared.expected_digest}"`,
      }),
    )
  ...
  if (request.method === 'POST' && url.pathname === '/v1/agents') {
    ...
    if (
      !/^browser-full-plan-[a-f0-9]{16}$/.test(authoredName ?? '') ||
      !uploadedCompilation ||
      authoredName !== uploadedCompilation.compiled.name ||
      authoredDocument.spec.typed_plan_digest !== uploadedCompilation.compiled.typed_plan_digest
    )
      return problem(
        response,
        400,
        'fixture_agent_invalid',
        'Expected the unique uploaded Full Plan Agent.',
      )
    process.stdout.write(
      `${JSON.stringify({ fixture_observation: 'unique_full_plan_agent_created', name: authoredName, preexisting_agents: 1 })}\n`,
    )
  ...
  if (request.method === 'POST' && url.pathname === `/v1/agents/${agentId}/draft:validate`) {
    ...
    sendJson(response, 202, {
      operation_id: 'job_0198f1c3-8f49-7c3e-b1f3-773c28367bb3',
      state: 'ready',
    })
    return
  }
  ...
  if (request.method === 'POST' && url.pathname === `/v1/agents/${agentId}/draft:publish`) {
    ...
    authoredVersion = {
      ...
      revision_no: 1,
      content_digest: body.plan_content_digest,
      artifact_id: authoredDocument.spec.typed_plan_artifact_id,
      payload: { document: structuredClone(authoredDocument), validation: authoredValidation },
      created_at: '2026-09-01T00:00:02.000000Z',
      etag: `"${versionId}-${body.plan_content_digest.slice(7)}"`,
    }
    sendJson(
      response,
      200,
      {
        ...
        published_versions: [
          {
            resource_version_id: 'aif_0198f1c3-8f49-7c3e-b1f3-773c28367bb4',
            revision_no: 1,
            content_digest: body.interface_content_digest,
            artifact_id: body.artifact_id,
            etag: '"interface"',
          },
          {
            resource_version_id: 'arev_0198f1c3-8f49-7c3e-b1f3-773c28367bb5',
            revision_no: 1,
            content_digest: body.plan_content_digest,
            artifact_id: body.artifact_id,
            etag: '"plan"',
  ...
  if (request.method === 'POST' && url.pathname === `/v1/agents/${agentId}/deployments`) {
    ...
    authoredDeployment = {
      ...
      created_at: '2026-09-01T00:00:02.000000Z',
      etag: `"${agentDeploymentId}-${closureDigest.slice(7)}"`,
    }
  ...
  if (
    request.method === 'GET' &&
    url.pathname === `/v1/agents/${agentId}/deployments/${agentDeploymentId}`
  ) {
    sendJson(response, 200, authoredDeployment, { etag: authoredDeployment.etag })
    return
  }
  if (
    request.method === 'GET' &&
    url.pathname === `/v1/agents/${agentId}/versions/${authoredVersion?.resource_version_id}`
  ) {
    sendJson(response, 200, authoredVersion, { etag: authoredVersion.etag })
    return
  }
  if (
    request.method === 'POST' &&
    url.pathname === `/v1/agents/${agentId}/deployments/${agentDeploymentId}:activate`
  ) {
  ...
  if (request.method === 'POST' && url.pathname === '/v1/runs') {
    ...
    if (
      ...
      body.input?.schema_digest !== authoredDocument.spec.input_schema.canonical_digest ||
      canonical(body.expected_agent_deployment) !==
        canonical({
          deployment_id: agentDeploymentId,
          resource_kind: 'agent_deployment',
          deployment_digest: authoredDeployment.closure_digest,
        }) ||
      body.input?.classification !== authoredDocument.spec.input_classification ||
      body.input?.value?.kind !== 'inline' ||
      canonical(body.input.value.value) !==
        canonical({ message: `full-plan-echo-${authoredName.slice('browser-full-plan-'.length)}` })
    )
      return problem(
        response,
        400,
        'fixture_run_invalid',
        'Expected explicit input for the exact newly published Agent.',
      )
    authoredInput = body.input.value.value
    process.stdout.write(
      `${JSON.stringify({ fixture_observation: 'exact_full_plan_run_input', agent_id: body.agent_id, run_id: authoredRunId, explicit_inline_echo: true })}\n`,
    )
  ...
  if (request.method === 'GET' && url.pathname === `/v1/runs/${authoredRunId}/definition`) {
    sendJson(response, 200, {
      ...
      agent_interface: authoredDeployment.closure.bindings.interface,
      plan: authoredDeployment.closure.bindings.plan,
    })
    return
  }
  ...
  if (request.method === 'GET' && url.pathname === `/v1/runs/${authoredRunId}/result`) {
    sendJson(response, 200, {
      schema_version: 1,
      run_id: authoredRunId,
      value: { kind: 'inline', value: authoredInput },
    })
    return
  }
  ...
  if (request.method === 'GET' && operationRead && operationRead[1] !== 'job_capacity') {
    sendJson(response, 200, {
      operation_id: operationRead[1],
      tenant_id: 'ten_0198f1c3-8f49-7c3e-b1f3-773c28367bbc',
      kind: 'resource_validation',
      target: { kind: 'agent', agent_id: agentId },
      state: 'succeeded',
      progress: null,
      result: { result_digest: `sha256:${'f'.repeat(64)}` },
      error: null,
      created_at: '2026-09-01T00:00:00Z',
      updated_at: '2026-09-01T00:00:01Z',
      etag: '"operation-v2"',
  ...
  if (
    request.method === 'GET' &&
    [runId, emptyRunId].some((id) => url.pathname === `/v1/runs/${id}`)
  ) {
    ...
    if (postTaskRunRead && postTaskRunReadInFlight) {
      process.stdout.write(
        `${JSON.stringify({ fixture_observation: 'overlapping_post_task_run_read' })}\n`,
      )
      problem(
        response,
        409,
        'overlapping_run_read',
        'Run authority reads must complete before the next refresh.',
      )
      return
    }
  ...
  if (request.method === 'GET' && url.pathname === `/v1/runs/${runId}/events`) {
    ...
    const encodeEvent = (sequence, eventType, sourceKind, sourceId, sourceVersion) => {
      ...
      const envelope = {
        ...
        data: {
          ...
          source_id: sourceId,
          source_projection_version: sourceVersion,
          safe_summary: null,
        },
      }
    ...
    const events =
      cursor === 'cur-browser-2'
        ? ''
        : cursor === 'cur-browser-1'
          ? runState === 'succeeded'
            ? encodeEvent(2, 'run.completed', 'run', runId, 2)
            : ''
    ...
    response.writeHead(
      ...
      headers({
        'content-type': 'text/event-stream',
        'x-insight-run-replay-floor': '0',
        'x-insight-run-high-water': runState === 'succeeded' ? '2' : '1',
        'x-insight-history-truncated': 'false',
  ...
  if (request.method === 'GET' && url.pathname === `/v1/runs/${runId}/result`) {
    ...
    sendJson(response, 200, {
      ...
      value: {
        kind: 'inline',
        value: { message: 'completed', tool_output: 'authorized-output-canary' },
      },
    })
    return
  }
  if (request.method === 'POST' && url.pathname.startsWith(`/v1/runs/${runId}:`)) {
    ...
    if (action === 'pause')
      return problem(response, 409, 'invalid_state_transition', 'Run cannot pause while waiting.')
    if (action === 'cancel')
      return problem(response, 412, 'precondition_failed', 'Run ETag changed before cancellation.')
    return problem(
      response,
      429,
      'capacity_exhausted',
      'Control capacity is temporarily exhausted.',
      true,
    )
  }
  if (request.method === 'GET' && url.pathname === '/v1/tasks') {
    const matches =
      (!url.searchParams.get('state') || url.searchParams.get('state') === taskState) &&
  ...
  if (request.method === 'GET' && url.pathname === `/v1/tasks/${taskId}/form`) {
    sendJson(
      ...
      200,
      {
        ...
        response_schema_digest: taskResponseSchema.canonical_digest,
        allowed_actions: taskView().allowed_actions,
        etag: taskView().etag,
        safe_prompt_key: taskView().safe_prompt_key,
      },
      { etag: taskView().etag },
    )
    return
  ...
  if (request.method === 'POST' && url.pathname === `/v1/tasks/${taskId}:submit-input`) {
    ...
    if (
      !String(request.headers['idempotency-key'] ?? '').startsWith('console-task-submit-input-')
    ) {
    ...
    if (
      input.schema_digest !== taskResponseSchema.canonical_digest ||
      input.value?.kind !== 'inline' ||
      typeof input.value.value?.message !== 'string'
    ) {
      problem(
        response,
        400,
        'invalid_task_input',
        'Task response does not match the frozen fixture schema.',
      )
      return
    }
    taskState = 'responded'
  ...
  if (request.method === 'GET' && url.pathname === '/v1/operations/job_capacity') {
    problem(
      ...
      429,
      'capacity_exhausted',
      'Operation read capacity is temporarily exhausted.',
      true,
    )
    return
  }
...
server.listen(port, host, () => {
  process.stdout.write(
    `console fixture ready http://${host}:${tcpPort(server)} run=${runId} task=${taskId}\n`,
  )
})
