...
function checkedUpstream(value) {
  ...
  if (upstream.protocol !== 'http:' || !loopbackHosts.has(upstream.hostname) || upstream.username || upstream.password || upstream.pathname !== '/' || upstream.search || upstream.hash) {
    throw new Error('Console Gateway origins must be origin-only loopback HTTP URLs')
  }
  return upstream
}
...
function proxy(request, response, upstream, requestUrl) {
  ...
  // A browser can cancel an event read after sending its complete GET request. Its response
  // closing, rather than the request body ending, releases the corresponding upstream stream.
  response.once('close', () => {
...
export async function startGatewayConsoleServer({ gatewayOrigin, managementGatewayOrigin, port = 0, bundleRoot: requestedBundleRoot } = {}) {
  const bundleRoot = checkedBundleRoot(requestedBundleRoot ?? process.env.INSIGHT_CONSOLE_BUNDLE_ROOT ?? defaultBundleRoot)
  const runtime = checkedUpstream(gatewayOrigin ?? process.env.INSIGHT_CONSOLE_GATEWAY_ORIGIN ?? '')
  const management = checkedUpstream(managementGatewayOrigin ?? process.env.INSIGHT_CONSOLE_MANAGEMENT_GATEWAY_ORIGIN ?? '')
  const server = createServer((request, response) => {
    const url = new URL(request.url ?? '/', 'http://127.0.0.1')
    if (url.pathname === '/readyz' || url.pathname === '/v1' || url.pathname.startsWith('/v1/')) {
      // Mirror the physical Gateway routing boundary; each actual server still validates its
      // closed route set, permissions and command contracts. No local business routing authority.
      const noun = url.pathname.split('/')[2]?.split(':')[0]
      const upstream = url.pathname === '/readyz' || ['runs', 'tasks', 'artifacts'].includes(noun) ? runtime : management
