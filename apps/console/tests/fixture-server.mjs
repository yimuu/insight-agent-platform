...
import { initSync, compile_agent } from '../src/agent/generated/insight_platform_agent_compiler_wasm.js'
import { createHash } from 'node:crypto'
import { createServer } from 'node:http'
import { extname, join, normalize, resolve } from 'node:path'
import { fileURLToPath } from 'node:url'

const root = resolve(process.env.INSIGHT_CONSOLE_BUNDLE_ROOT ?? fileURLToPath(new URL('../dist', import.meta.url)))
initSync({ module: readFileSync(new URL('../src/agent/generated/insight_platform_agent_compiler_wasm_bg.wasm', import.meta.url)) })
...
let authoredName = null
let authoredInput = null
let uploadedCompilation = null
let authoredValidation = null
let authoredDeployment = null
let authoredVersion = null
...
function taskView() {
  return {
    schema_version: 2,
    allowed_actions: taskState === 'pending' ? ['submit_input','cancel'] : [],
    task_id: taskId,
    task_kind: 'interaction_form',
    state: taskState,
    generation: 1,
    version: taskVersion,
    safe_prompt_key: 'interaction.confirm_release',
    response_schema_digest: taskResponseSchema.canonical_digest,
...
const taskResponseSchema = { schema_version: 1, profile: 'insight.closed-json-schema/1', schema: taskSchema, canonical_digest: digest(taskSchema) }
const policyBinding = {
...
function authoredResource() {
  return {
    schema_version: 1,
    resource_id: agentId,
    active_deployment_id: authoredDeployment ? agentDeploymentId : null,
    resource_kind: 'agent',
...
const server = createServer(async (request, response) => {
  ...
  const objectUpload = url.pathname.match(/^\/__fixture\/objects\/(art_[^/]+)$/)
  if (request.method === 'PUT' && objectUpload) {
    try {
      if (authorizationPresent) throw new Error('object upload must not receive bearer')
      const prepared = preparedArtifacts.get(objectUpload[1])
      const text = await readBody(request, 1024 * 1024)
      if (!prepared || Buffer.byteLength(text) !== prepared.expected_size_bytes || digest(text) !== prepared.expected_digest) throw new Error('actual upload differs from declared Artifact')
      if (prepared.purpose === 'authoring_document') {
        const result = JSON.parse(new TextDecoder().decode(compile_agent(new TextEncoder().encode(text))))
        if (result.outcome !== 'compiled' || result.compilation.compiled.execution_kind !== 'full_plan') throw new Error('browser did not publish a shared compiler Full Plan')
        uploadedCompilation = result.compilation
        if (uploadedCompilation.source_bundle_digest !== prepared.expected_digest || new TextDecoder().decode(new Uint8Array(uploadedCompilation.source_bundle_bytes)) !== text) throw new Error('uploaded source is not exact canonical compiler input')
        process.stdout.write(`${JSON.stringify({ fixture_observation: 'full_plan_source_uploaded', execution_kind: 'full_plan', digest: prepared.expected_digest })}\n`)
      } else if (prepared.purpose === 'typed_plan') {
        if (!uploadedCompilation || JSON.parse(text).plan_version !== 6 || text !== new TextDecoder().decode(new Uint8Array(uploadedCompilation.compiled.typed_plan_bytes))) throw new Error('Plan upload is not the exact shared compiler output')
        process.stdout.write(`${JSON.stringify({ fixture_observation: 'exact_plan_uploaded', plan_version: 6, digest: prepared.expected_digest })}\n`)
      } else throw new Error('unexpected authoring upload purpose')
      prepared.uploaded = true
      prepared.bytes = text
      response.writeHead(200, { 'content-length': '0' }); response.end()
    } catch (error) {
      problem(response, 400, 'fixture_upload_invalid', error.message)
    }
    return
  }
  ...
  if (request.method === 'GET' && url.pathname === '/v1/agents') {
    sendJson(response, 200, {
      schema_version: 1,
      items: [{
        schema_version: 1, name: 'preexisting-agent', display_name: 'Preexisting Agent',
        agent_id: 'agt_0198f1c3-8f49-7c3e-b1f3-773c28367bc0', state: 'ready', environment: 'development',
        updated_at: '2026-09-01T00:00:02Z', published_at: '2026-09-01T00:00:02Z', required_features: [], latest_run_state: null, active_deployment: null,
      }, ...(authoredAgentReady ? [{
        schema_version: 1,
        name: authoredName,
        ...
        active_deployment: {deployment_id:agentDeploymentId,resource_kind:'agent_deployment',deployment_digest:authoredDeployment.closure_digest},
      }] : [])],
      next_cursor: null,
    })
    return
  ...
  if (request.method === 'GET' && artifactContent) {
    const prepared = preparedArtifacts.get(artifactContent[1])
    if (!prepared?.uploaded || typeof prepared.bytes !== 'string') return problem(response, 404, 'not_found', 'Uploaded Artifact content is unavailable.')
    response.writeHead(200, headers({ 'content-type':prepared.declared_media_type, 'content-length':Buffer.byteLength(prepared.bytes), etag:`"${prepared.expected_digest}"` }))
    response.end(prepared.bytes)
    return
  }
  ...
  if (request.method === 'POST' && url.pathname === '/v1/agents') {
    ...
    authoredName = body.document?.spec?.authoring_name
    if (!/^browser-full-plan-[a-f0-9]{16}$/.test(authoredName ?? '') || !uploadedCompilation || authoredName !== uploadedCompilation.compiled.name
      || authoredDocument.spec.typed_plan_digest !== uploadedCompilation.compiled.typed_plan_digest) return problem(response, 400, 'fixture_agent_invalid', 'Expected the unique uploaded Full Plan Agent.')
    process.stdout.write(`${JSON.stringify({ fixture_observation: 'unique_full_plan_agent_created', name: authoredName, preexisting_agents: 1 })}\n`)
  ...
  if (request.method === 'POST' && url.pathname === `/v1/agents/${agentId}/draft:publish`) {
    ...
    const versionId = 'arev_0198f1c3-8f49-7c3e-b1f3-773c28367bb5'
    authoredVersion = {schema_version:1,resource_id:agentId,resource_kind:'agent',resource_version_id:versionId,revision_no:1,content_digest:body.plan_content_digest,artifact_id:authoredDocument.spec.typed_plan_artifact_id,payload:{document:structuredClone(authoredDocument),validation:authoredValidation},created_at:'2026-09-01T00:00:02.000000Z',etag:`"${versionId}-${body.plan_content_digest.slice(7)}"`}
    sendJson(response, 200, {
      schema_version: 1,
      resource_id: agentId,
      resource_kind: 'agent',
  ...
  if (request.method === 'POST' && url.pathname === `/v1/agents/${agentId}/deployments`) {
    ...
    const closureDigest = digest({schema_version:1,...body.closure})
    authoredDeployment = { schema_version: 1, deployment_id: agentDeploymentId, resource_id: agentId, resource_kind: 'agent', resource_version_id: body.resource_version_id, environment: body.environment, closure_digest: closureDigest, closure: body.closure, created_at: '2026-09-01T00:00:02.000000Z', etag: `"${agentDeploymentId}-${closureDigest.slice(7)}"` }
    sendJson(response, 201, authoredDeployment, {etag:authoredDeployment.etag})
    return
  }
  if (request.method === 'GET' && url.pathname === `/v1/agents/${agentId}/deployments/${agentDeploymentId}`) {
    sendJson(response,200,authoredDeployment,{etag:authoredDeployment.etag}); return
  }
  if (request.method === 'GET' && url.pathname === `/v1/agents/${agentId}/versions/${authoredVersion?.resource_version_id}`) {
    sendJson(response,200,authoredVersion,{etag:authoredVersion.etag}); return
  }
  ...
  if (request.method === 'POST' && url.pathname === '/v1/runs') {
    const body = JSON.parse(await readBody(request))
    if (!authoredAgentReady || body.agent_id !== agentId || body.input?.schema_digest !== authoredDocument.spec.input_schema.canonical_digest
      || canonical(body.expected_agent_deployment) !== canonical({deployment_id:agentDeploymentId,resource_kind:'agent_deployment',deployment_digest:authoredDeployment.closure_digest})
      || body.input?.classification !== authoredDocument.spec.input_classification || body.input?.value?.kind !== 'inline'
      || canonical(body.input.value.value) !== canonical({ message: `full-plan-echo-${authoredName.slice('browser-full-plan-'.length)}` })) return problem(response, 400, 'fixture_run_invalid', 'Expected explicit input for the exact newly published Agent.')
    authoredInput = body.input.value.value
    process.stdout.write(`${JSON.stringify({ fixture_observation: 'exact_full_plan_run_input', agent_id: body.agent_id, run_id: authoredRunId, explicit_inline_echo: true })}\n`)
  ...
  if (request.method === 'GET' && url.pathname === `/v1/runs/${authoredRunId}/definition`) {
    sendJson(response, 200, { schema_version:1, run_id:authoredRunId, agent_id:agentId,
      agent_deployment:{deployment_id:agentDeploymentId,resource_kind:'agent_deployment',deployment_digest:authoredDeployment.closure_digest},
      agent_interface:authoredDeployment.closure.bindings.interface, plan:authoredDeployment.closure.bindings.plan })
    return
  }
  if (request.method === 'GET' && url.pathname === `/v1/runs/${authoredRunId}/events`) {
    response.writeHead(200, headers({ 'content-type': 'text/event-stream', 'x-insight-run-replay-floor': '0', 'x-insight-run-high-water': '0', 'x-insight-history-truncated': 'false', 'content-length': 0 }))
    response.end()
    return
  }
  if (request.method === 'GET' && url.pathname === `/v1/runs/${authoredRunId}/result`) {
    sendJson(response, 200, { schema_version: 1, run_id: authoredRunId, value: { kind: 'inline', value: authoredInput } })
    return
  }
  ...
  if (request.method === 'GET' && url.pathname === `/v1/runs/${emptyRunId}/events`) {
    response.writeHead(200, headers({
      'content-type': 'text/event-stream', 'x-insight-run-replay-floor': '0', 'x-insight-run-high-water': '0', 'x-insight-history-truncated': 'false',
      'content-length': 0,
    }))
    response.end()
    return
  ...
  if (request.method === 'GET' && url.pathname === `/v1/runs/${runId}/events`) {
    ...
    const encodeEvent = (sequence, eventType, sourceKind, sourceId, sourceVersion) => {
      const eventCursor = `cur-browser-${sequence}`
      const envelope = {
        event_id: `evt_0198f1c3-8f49-7c3e-b1f3-773c28367b9${sequence + 5}`,
        run_id: runId,
        cursor: eventCursor,
        sequence,
        schema_version: 1,
        ...
        data: { source_kind: sourceKind, source_id: sourceId, source_projection_version: sourceVersion, safe_summary: null },
      }
      return `id: ${eventCursor}\nevent: ${eventType}\ndata: ${JSON.stringify(envelope)}\n\n`
    }
    const events = cursor === 'cur-browser-2' ? '' : cursor === 'cur-browser-1'
      ? runState === 'succeeded' ? encodeEvent(2, 'run.completed', 'run', runId, 2) : ''
      : encodeEvent(1, 'interaction.required', 'interaction', taskId, 1)
    response.writeHead(200, headers({
      'content-type': 'text/event-stream', 'x-insight-run-replay-floor': '0', 'x-insight-run-high-water': runState === 'succeeded' ? '2' : '1', 'x-insight-history-truncated': 'false',
  ...
  if (request.method === 'GET' && url.pathname === `/v1/runs/${runId}/result`) {
    ...
    sendJson(response, 200, {
      ...
      value: { kind: 'inline', value: { message: 'completed', tool_output: 'authorized-output-canary' } },
    })
    return
  }
  ...
  if (request.method === 'GET' && url.pathname === '/v1/tasks') {
    const matches = (!url.searchParams.get('state') || url.searchParams.get('state') === taskState)
      && (!url.searchParams.get('kind') || url.searchParams.get('kind') === 'interaction_form')
      && (!url.searchParams.get('run_id') || url.searchParams.get('run_id') === runId)
    sendJson(response, 200, { schema_version: 1, items: matches ? [taskView()] : [], next_cursor: null })
    return
  }
  if (request.method === 'GET' && url.pathname === `/v1/tasks/${taskId}/form`) {
    sendJson(response, 200, { schema_version: 2, task_id: taskId, generation: 1, version: taskVersion, response_schema: taskResponseSchema, response_schema_digest: taskResponseSchema.canonical_digest, allowed_actions: taskView().allowed_actions, etag:taskView().etag, safe_prompt_key:taskView().safe_prompt_key }, {etag:taskView().etag})
    return
  }
  ...
  if (request.method === 'POST' && url.pathname === `/v1/tasks/${taskId}:submit-input`) {
    if (request.headers['if-match'] !== `"${taskId}-1"`) {
    ...
    if (input.schema_digest !== taskResponseSchema.canonical_digest || input.value?.kind !== 'inline' || typeof input.value.value?.message !== 'string') {
      problem(response, 400, 'invalid_task_input', 'Task response does not match the frozen fixture schema.')
      return
    }
    taskState = 'responded'
    taskVersion = 2
...
if (!existsSync(join(root, 'index.html'))) {
  throw new Error('Build apps/console before starting the browser fixture')
}
