...
import { parseArguments, readPrivateSession, runInstalledModelChat } from './model-chat-installation-journey.mjs'

test('installed Model chat requires an explicit fresh intent and safe Gateway origin', () => {
  const args = ['--endpoint', 'http://127.0.0.1:8080', '--session-file', '/private/tmp/session', '--agent-name', 'qwen-check', '--evidence-directory', '/private/tmp/new-observation']
  ...
  for (const endpoint of ['http://external.example', 'https://user:secret@model.example', 'https://model.example/path', 'https://model.example/?secret=x', 'https://model.example/#x', 'file:///tmp/a']) {
    assert.throws(() => parseArguments([...args.slice(0, 1), endpoint, ...args.slice(2)]))
  }
  for (const invalid of [args.slice(0, -1), [...args, '--endpoint', 'https://model.example'], [...args, '--approve', 'true'], [...args.slice(0, 5), '../bad', ...args.slice(6)]]) {
...
test('an existing evidence directory fails before browser startup and preserves the previous intent', async () => {
  ...
  try {
    ...
    const report = await runInstalledModelChat({ endpoint: 'http://127.0.0.1:1', sessionFile: session, agentName: 'existing', evidenceDirectory: directory })
...
test('session read uses a bounded private regular inode and rejects ambiguous or unsafe bytes', async () => {
  ...
  try {
    ...
    await assert.rejects(readPrivateSession(path))
    await rm(alias)
    for (const bytes of ['', 'two\nlines', 'trailing\n\n', ' leading', Buffer.from([0xff]), 'x'.repeat(65538)]) {
