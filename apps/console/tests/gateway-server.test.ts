...
import { checkedBundleRoot } from '../server/gateway-server.ts'
...
test('rejects symbolic links anywhere in a candidate bundle', () => {
  ...
  try {
    ...
    assert.throws(() => checkedBundleRoot(root), /must not contain symbolic links/)
  } finally {
    rmSync(root, { recursive: true })
...
test('transparent local proxy preserves both physical Gateway surfaces and opaque command/SSE headers', async () => {
  ...
  const { startGatewayConsoleServer } = await import('../server/native.ts')
  const root = mkdtempSync(join(tmpdir(), 'insight-console-dual-gateway-'))
  writeFileSync(join(root, 'index.html'), '<!doctype html>')
  const calls = []
  const upstream = (surface) =>
    createServer(async (request, response) => {
      ...
      if (request.url.startsWith('/v1/unknown')) {
        response.writeHead(404, { 'cache-control': 'no-store', etag: '"actual-404"' })
        response.end('actual upstream missing')
        return
      }
  ...
  try {
    await Promise.all(
      [runtime, management].map(
        (server) => new Promise<void>((resolve) => server.listen(0, '127.0.0.1', resolve)),
      ),
    )
    proxy = await startGatewayConsoleServer({
      bundleRoot: root,
      gatewayOrigin: `http://127.0.0.1:${tcpPort(runtime)}`,
      managementGatewayOrigin: `http://127.0.0.1:${tcpPort(management)}`,
    })
    for (const [path, method, surface] of [
      ['/v1/agent-authoring-profile', 'GET', 'management'],
      ['/v1/agents?cursor=opaque%2B%2F', 'GET', 'management'],
      ['/v1/agent-authoring-bindings:resolve', 'POST', 'management'],
      ['/v1/agents/agent:publish', 'POST', 'management'],
      ['/v1/operations/job', 'GET', 'management'],
      ['/v1/tasks/task:submit-input', 'POST', 'runtime'],
      ['/v1/artifacts/art:upload', 'POST', 'runtime'],
    ]) {
      const headers = {
        authorization: 'Bearer actual-test-token',
      ...
      for (const name of ['authorization', 'idempotency-key', 'if-match'])
        assert.equal(forwarded.headers[name], headers[name])
      if (method === 'POST') assert.equal(forwarded.body, '{"exact":true}')
    }
    const response = await fetch(`${proxy.origin}/v1/runs/run/events`, {
    ...
    for (const unsafe of [
      ...
      await assert.rejects(
        startGatewayConsoleServer({
          bundleRoot: root,
          gatewayOrigin: proxy.origin,
          managementGatewayOrigin: unsafe,
        }),
        /origin-only loopback|Invalid Console transport configuration/,
      )
    }
  } finally {
    if (proxy) await proxy.close()
    await Promise.all(
      [runtime, management].map((server) => new Promise((resolve) => server.close(resolve))),
    )
    rmSync(root, { recursive: true })
  }
})
...
test('disconnecting a Console event reader closes the actual upstream stream', async () => {
  ...
  try {
    ...
    const origin = `http://127.0.0.1:${tcpPort(upstream)}`
    proxy = await startGatewayConsoleServer({
      bundleRoot: root,
      gatewayOrigin: origin,
      managementGatewayOrigin: origin,
    })
    ...
    await Promise.race([
      ...
      new Promise((_, reject) => {
        timer = setTimeout(
          () => reject(new Error('disconnected reader retained its upstream stream')),
          2_000,
        )
      }),
    ])
