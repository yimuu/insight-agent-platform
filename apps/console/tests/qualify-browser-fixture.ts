...
export function fixtureReadyOrigin(output) {
  const lines = output
    .split('\n')
    .slice(0, -1)
    .filter((line) => line.startsWith('console fixture ready '))
  if (lines.length === 0) return null
  if (lines.length !== 1) throw new BrowserProcessError('invalid_fixture_endpoint')
  const match =
    /^console fixture ready http:\/\/127\.0\.0\.1:([1-9][0-9]{0,4}) run=([^ ]+) task=([^ ]+)$/.exec(
      lines[0],
    )
  if (!match || Number(match[1]) > 65535 || match[2] !== runId || match[3] !== taskId)
    throw new BrowserProcessError('invalid_fixture_endpoint')
...
export async function waitForFixture(output, owner, timeoutMs, signal) {
  ...
  while (performance.now() < deadline) {
    ...
    await withinSignal(
      delay(Math.min(50, deadline - performance.now())),
      AbortSignal.any([signal, owner.stopped]),
    )
  }
...
async function runJourney(origin, signal) {
  const owner = spawnFixtureChild(process.execPath, [`${directory}fixture-browser-journey.ts`], {
    env: {
      ...process.env,
  ...
  try {
    ...
    if (status.code !== 0)
      throw new Error(`browser journey failed (${JSON.stringify(status)})\n${stderr}\n${stdout}`)
    evidence = JSON.parse(stdout)
    if (
      evidence.kind !== 'insight.console.synthetic-gateway-journey/v1' ||
...
function verifyRequestLog(output) {
  ...
  const publicRequests = records.filter(
    (record) => typeof record.path === 'string' && record.path.startsWith('/v1/'),
  )
  const mutationIndex = publicRequests.findIndex(
    (record) => record.method === 'POST' && record.path === `/v1/tasks/${taskId}:submit-input`,
  )
  const mutation = publicRequests[mutationIndex]
  const runReadsAfterMutation = publicRequests
    .slice(mutationIndex + 1)
    .filter((record) => record.method === 'GET' && record.path === `/v1/runs/${runId}`)
  const postTaskRunResponses = records.filter(
    (record) => record.fixture_observation === 'post_task_run_response',
  )
  const overlappingRunReads = records.filter(
    (record) => record.fixture_observation === 'overlapping_post_task_run_read',
  )
  ...
  if (runReadsAfterMutation.length < 3) {
    throw new Error(
      'browser journey did not refresh the Run authority through running, succeeded, and reload re-read',
    )
  }
  if (
    JSON.stringify(postTaskRunResponses.slice(0, 3).map((record) => record.state)) !==
    JSON.stringify(['running', 'succeeded', 'succeeded'])
  ) {
    throw new Error(
      'browser journey did not observe ordered running, succeeded, then reload succeeded Run responses',
    )
  }
  ...
  for (const observation of [
    'full_plan_source_uploaded',
    'exact_plan_uploaded',
    'unique_full_plan_agent_created',
    'exact_full_plan_run_input',
  ]) {
    if (records.filter((record) => record.fixture_observation === observation).length !== 1)
      throw new Error(`Missing unique authoring evidence: ${observation}`)
  }
...
async function main() {
  ...
  const fixtureOwner = spawnFixtureChild(process.execPath, [`${directory}fixture-server.ts`], {
    env: {
      ...process.env,
  ...
  fixture.stdout.on('data', (chunk) => {
    fixtureOutput = `${fixtureOutput}${chunk}`.slice(-256 * 1024)
  })
  fixture.stderr.on('data', (chunk) => {
    fixtureError = `${fixtureError}${chunk}`.slice(-64 * 1024)
  })
  let evidence
  let failure
  try {
  ...
  if (failure) throw failure
  if (fixtureError.includes(token))
    throw new BrowserProcessError('fixture_diagnostic_contains_token')
