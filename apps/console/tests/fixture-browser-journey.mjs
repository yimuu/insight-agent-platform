...
import { runGatewayJourney } from './real-gateway-journey.mjs'
...
async function configureSyntheticBrowser({ client, consoleOrigin, gatewayOrigin }) {
  ...
  client.on('Fetch.requestPaused', async ({ requestId, request, networkId }) => {
    ...
    try {
      ...
      if (url.origin !== 'https://objects.example') throw new Error('unexpected synthetic object origin')
      if (request.method === 'OPTIONS') status = 204
      else if (request.method === 'PUT') {
        const body = request.postData ?? (await client.call('Network.getRequestPostData', { requestId: networkId })).postData
