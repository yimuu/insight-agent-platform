...
RUN cargo chef cook --locked --release --workspace --recipe-path recipe.json \
    ...
    --bin platform-history-maintenance \
    --bin platform-outbox-worker \
    --bin platform-jetstream-provision \
    --bin platform-database-role \
...
RUN musl-gcc -std=c11 -O2 -Wall -Wextra -Werror -fPIE -static -pie \
      ...
      apps/sandbox/platform-sandbox-runner/native/launcher.c
...
COPY --from=builder /workspace/target/release/platform-history-maintenance /usr/local/bin/platform-history-maintenance
COPY --from=builder /workspace/target/release/platform-outbox-worker /usr/local/bin/platform-outbox-worker
COPY --from=builder /workspace/target/release/platform-jetstream-provision /usr/local/bin/platform-jetstream-provision
COPY --from=builder /workspace/target/release/platform-database-role /usr/local/bin/platform-database-role
