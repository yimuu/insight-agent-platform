...
当前仓库有模块化Rust workspace、shared Job/Run/Artifact/MCP/Sandbox等候选实现、一个23表baseline候选与大量
开发fixture；现有schema contract为v6。但这些代码是CR-166之前的candidate，包含已推迟的microVM、
Managed stdio session、Model Artifact或过度Artifact role拆分。

因此：

- 可复用的领域与仓储代码必须通过CR-166对照审计后才计入phase evidence；
...
1. **Resource lifecycle**
   ...
   - Draft -> immutable ResourceVersion -> Deployment -> tenant active binding；
...
Platform v2只在Phase 1～4全部exit gate通过、CR-166影响规范推进为Accepted、schema v7与migration实际发布、
