...
状态：**Passed（composition + eight-scenario closure）**。这不是十条黄金场景的完成证据。

## 1. 权威运行

最新 exact-revision 运行：

- Git revision：`b0d8a3247a0ce09f3946312359f0a8cb078f937e`；
- 命令：`scripts/run-productization-base-journey.sh --profile full --console-browser
  --report-directory target/productization-reports-b0d8a324`；
- 环境：macOS Darwin 25.6.0 arm64、Rust 1.94.1、Node 24.14.1、pnpm 11.19.0、Chrome 151、
  Docker Engine 29.4.0、Compose v5.1.2；
- 场景报告时间：`2026-08-30T12:54:43Z` 至 `12:55:51Z`；测试主体 67.67 秒；
- report checker：`validated 8 productization scenario report(s); complete_gate=false`。
...
bearer credential 写入日志。执行前 working tree 为空，八份 report 的 `source_revision` 均精确等于上述 40 字节 revision。
...
Artifact lifecycle/rejection、Context retrieval/citation、exact Model streaming 与 Native/Remote Capability journey。Model 路径通过 public
...
保留为 reconciliation，deny-all catalog 在 dispatch 前拒绝。结束时 runner停止 exact Platform进程并移除 fresh Compose closure。

## 3. Machine-readable reports

目录 `target/productization-reports-b0d8a324` 由 report checker按 manifest、closed schema、exact source revision、
entrypoint、assertion与 failure probe重验通过：

| 场景 | 状态 | SHA-256 |
|---|---|---|
| `approval-task-resume` | Passed | `5e759799c20dbc3af198c9f99521cd772902b87bd884fe70102cde4235db09a1` |
| `artifact-lifecycle-and-rejection` | Passed | `283b84482af43c9eebe0d0ffe22c2fe0527d78849ed51fe8199ec6703fd4f6f2` |
| `context-retrieval-and-citation` | Passed | `db445ca063ba2478c6b0a4e7cb9430e25a972a1c83d67bd93a5b0b21cd06ba50` |
| `deterministic-first-run` | Passed | `42ece607d88db9ea4171a2223c1aeb09af24e98e5e233ab06b5fb1155728a96e` |
| `exact-model-streaming-chat` | Passed | `5cad24fbb0391837de52511b7c8c51cc5d6beb94a3792cca04bd021f315841ce` |
| `native-and-remote-capability` | Passed | `59741f0dfcbe9306d4703b3073de4015f310e85f757f15414f7027354b50031b` |
| `subagent-quota-and-cancel` | Passed | `52d1e1c6655824834c73936e073d5d70faa9a4fece7175f2a8ca427aada9c76e` |
| `timer-signal-restart-recovery` | Passed | `08bd67919c248289491fb4ea3f73cbc5b9501439835b3e4e12c03f62bba379d6` |

## 4. 明确边界

本次运行证明 full profile 可以在 fresh authority 上完成整体构建、25 角色监督启动、public `/v1`、CLI、真实
Console、Artifact、Context、Model 与 Capability 用户旅程及受控清理。严格 M4 仍缺
`remote-mcp-tool-and-resource` 与 `wasi-and-remote-framework-capability` 两份 Passed report；
