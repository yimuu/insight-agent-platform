...
| `full` | Model、remote Capability、MCP、Context、Artifact maintenance、WASI | `base` 加 Egress/Security、对应 worker、Artifact Maintenance 与其所需 lifecycle configuration | **实现中**。Context Native/Remote/Subscription、Model Worker、Remote Capability Worker、MCP Host/Resource Host/Discovery/Subscription/Cleanup、Callback API、Artifact Maintenance、Security Authority、Egress Broker 与本地 Attestor/Controller/restricted-WASI Executor 已有 closed/digest-bound 配置、持久化动态端口，并进入只对 full 生效的单次 release build/受监督 launch；base 不额外构建这些二进制。2026-08-30 fresh GitHub Linux run `33283147235` 已从空 project path 完成 24 角色全部 ready、真实 Runtime Gateway、headless Chrome 和三条 base journey 的整体 `full` composition，详见 [`full-journey-evidence.md`](full-journey-evidence.md)。真实 local attestation registry 只记录 UID/GID/PID、boot/start identity 与 project-local instance UID，不伪造 Pod/cgroup；生产 Helm 仍固定 Linux procfs/private-node 语义并拒绝 loopback。Egress 的独立 service principal、双向 TLS、分用途 exact state keys、pinned LocalStack dependencies 与每个调用角色的 exact SPIFFE ClientAuth 也已通过增量探针；Remote Capability 在 MCP Host 缺席时仍会 readiness fail closed，Callback authorization-code exchange 只授权专用 workload identity。尚未被 exact Deployment 安装的 remote lane 使用显式空 catalog 作为 deny-all closure。Remote HTTP Capability 已用 public authoring、exact Deployment、Egress TLS fixture、reconciliation 与 catalog rejection 形成 Passed report；仍缺 MCP remote/OAuth 完整旅程、真实 WASI Job 与 WASI limit 负向场景，故 full composition ready 不等于 M1/M4 完成。每个增量依赖只能由所需场景启用。 |
