...
| `full` | Model、remote Capability、MCP、Context、Artifact maintenance、WASI | `base` 加 Egress/Security、对应 worker、Artifact Maintenance 与其所需 lifecycle configuration | **实现中**。Context Native/Remote/Subscription、Model Worker、Remote Capability Worker、MCP Host/Resource Host/Discovery/Subscription/Cleanup、Callback API、Artifact Maintenance、Security Authority 与 Egress Broker 已有 closed/digest-bound 配置、持久化动态端口，并已进入只对 full 生效的 release build/受监督 launch 规格；base 不额外构建这些二进制。fresh 探针已证明 Egress 的独立 service principal、双向 TLS、分用途的 exact 32-byte MCP/OAuth state keys、pinned LocalStack Secrets Manager/KMS readiness，以及每个调用角色的 exact SPIFFE ClientAuth；Model 另通过同一 project CA 完成 NATS mTLS。Remote Capability 在 MCP Host 缺席时会在 readiness 前失败关闭，恢复 exact mTLS Host 后才 ready；四个 MCP/Context 后台 worker与 Callback API 也已在 fresh authority 上 ready，Callback authorization-code exchange 只授权专用 workload identity。尚未被 exact Deployment 安装的 Model、Capability、MCP、OAuth 与 remote Context lane 使用显式空 catalog 作为 deny-all closure，而不是伪造可调用 endpoint。上述角色尚未在一次 `insight dev --profile full` journey 中共同启动，实际 remote fixture endpoint、OAuth lifecycle 与 Sandbox/WASI 闭包仍缺失。每个增量依赖只能由所需场景启用。 |
