...
| `full` | Model、remote Capability、MCP、Context、Artifact maintenance、WASI | `base` 加 Egress/Security、对应 worker、Artifact Maintenance 与其所需 lifecycle configuration | **实现中**。Context Native 与 Artifact Maintenance 已有 closed/digest-bound 配置和持久化动态端口；Security Authority 已有独立 Egress service principal、closed config、ServerAuth/ClientAuth mTLS 资料且 fresh readiness 探针通过。上述 roles 尚未接入 full 进程监督，Egress Broker、MCP、Model、remote worker 与 WASI 闭包仍缺失。每个增量依赖只能由所需场景启用。 |
