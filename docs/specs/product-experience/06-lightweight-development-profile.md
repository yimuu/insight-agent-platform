...
| 属性 | 值 |
|---|---|
| 状态 | Accepted / CR-216 revision 1；OpenSandbox feature implementation/L1～L3 pending |
| 日期 | 2026-09-01 |
| 默认入口 | `insight dev` |
...
- local Kubernetes/OpenSandbox 不可用：`sandbox` feature 保持 not ready 并由 `doctor` 报告 exact dependency；starter 不冒充 Sandbox capability；
- `sandbox` 使用 OpenSandbox Kubernetes/BatchSandbox/containerd-runc，且是 single-node developer preview，不冒充强多租户或 production 隔离；
...
- `sandbox` feature 固定 Sandbox Dispatcher、internal OpenSandbox Server、BatchSandbox Controller、Kubernetes API physical store、
  Armed runner、containerd/runc 与 `Direct | Disabled` CNI policy digest；CLI/Console 不获得 OpenSandbox endpoint/API key；
- concurrent/response-loss create 只产生 bounded inert candidates，PostgreSQL 只选择一个，Package 最多 activation 一次；
  stop/start、boot rollover 与 orphan cleanup 不创建 replacement workload；
