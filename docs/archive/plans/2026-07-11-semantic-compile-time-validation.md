...
```markdown
A0 Action 校验错误安全修复不兼容既有 Run 历史；部署前按[正式 V1 破坏性变更说明](../migrations/formal-v1-breaking-changes.md)停止服务并显式清空历史。应用不会自动删除数据。
```

with:

```markdown
A0 Action 校验错误安全修复不兼容既有 Run 历史；部署前按[正式 V1 破坏性变更说明](../migrations/formal-v1-breaking-changes.md)停止服务并显式清空历史。A5 会让静态非法 Action input、hyphenated node/branch ID、indexed/computed `nodes` access 在启动编译期失败；迁移理由见同一说明。应用不会自动删除数据。
```
