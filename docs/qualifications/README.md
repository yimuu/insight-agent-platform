...
2026-09-12 的[启动与 Console 接口验证](console-startup-2026-09-12.json)覆盖根目录一条 Compose 命令、
管理员初始注册状态、页面所用客户端的实际凭据导入/同源上传/模型发布，以及完整 Plan 发布、运行成功和精确输出。
测试使用合成密钥，没有调用外部厂商；测试数据卷已删除，最后保留干净的 `my-platform` 安装。
工作空间全目标编译、相关 Rust/前端/安装工具测试通过。最终 Console 比 API 验证时的镜像多了
与 owning contract 一致的地址长度限制；该最终镜像通过了构建、就绪和初始注册状态检查。
Mac 锁屏使最后的页面逐项交互验收尚未完成；此次没有重跑 Kubernetes 集群或外部模型资格。

2026-09-11，本工作树的声明式安装通过了本机 Linux ARM64 容器验收：
[Compose 报告](declarative-compose-2026-09-11.json)覆盖原生 `up -d`、只读验证、配置漂移拒绝、
保留数据的容器重建和独立会话交付；[Kind 报告](declarative-kind-2026-09-11.json)覆盖一次 Helm 安装、
重复安装、Pod 恢复、TLS 正反检查及会话/公共 CA 交付。两份报告绑定实际使用的镜像，成功测试资源已清理。
故障排查中的首次 Compose 写入时序失败及首次 Kind 配置文件挂载失败均保留了安全诊断，修正后使用新的
隔离安装重跑通过；不将失败后重建的身份视为原安装恢复。相关三个 Rust crate 的 115 项测试及 77 项
部署/CI Python 回归通过，格式、依赖边界、公共合同与当前数据库 schema 检查通过。
这些是本机安装机制证据，不代表提交后的 CI、其他镜像、外部模型、浏览器业务旅程或生产资格。
...
2026-09-12 的[持久对话开发验证](conversation-live-2026-09-12.json)覆盖实际 Compose 升级、两轮外部模型对话、
终态前实时文字与 Gateway/Console 重启后读取。另有隔离 PostgreSQL 授权、并发、恢复和旧模型部署搭配新 Worker 的测试。
浏览器自动化未完成，不能将该记录当作截图验收、签名发行或生产容量资格。
