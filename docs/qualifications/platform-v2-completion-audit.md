...
1. MCP Host production binary/Helm与ToolsCall process L3已闭合；OAuth Callback/Cleanup/Egress和subscription的真实endpoint多进程
   ...
   owner transaction、Receipt replay与唯一Job L2；r271补齐typed production target及full reconcile acceptance L2，但该target尚未组合进binary，
   Context Job handler/claim loop及对应L3尚未实现。
