...
| Worker/queue telemetry | Orchestration、Model、Capability Native/Remote、Context Native/Remote/Subscription、Sandbox WASI/gVisor共9个pool从各自LocalWorkerPools导出business/critical-control available/used permit；Sandbox Controller另从实际Artifact-response semaphore导出capacity，Orchestration还有active jobs、claim/recovery outcome及PostgreSQL authority的Job/Outbox backlog与lag | Orchestration Job/shared Outbox及10/19 pool动态capacity L1 telemetry闭合；其余role authority及saturation仍待接线 |
...
2. 全部19个ComponentRole workload pool及Sandbox process attestor已有shared process metrics；Orchestration、Model、Capability Native/Remote、
   ...
   backlog/dependency health、其余9个pool的role saturation和production Prometheus scrape证据。
