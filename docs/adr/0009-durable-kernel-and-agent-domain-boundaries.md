...
公开开发发行在写入 OCI 发行 tag 和 GitHub Release 前，使用已验证候选的 exact 镜像闭包，单独核验 GHCR 索引的匿名可读性。
登录后的运行资格不能证明匿名访问。验证只向固定 registry 申请对应 repository 的匿名 pull token，再读取确切 digest 的原始 index 字节并核对 SHA256；
不读取账户凭证、Docker 配置或代理认证，不跟随重定向，不在失败后切换到登录访问，也不修改包的可见性。
候选签名、索引/平台子项闭包、性能、完整产品旅程和最终重新签名仍由既有交付流程裁决。

联合评审要求实际总墙钟期限覆盖 DNS、TLS、响应头和正文；响应与诊断有界，token、正文和原始网络异常不得进入报告。
证据只表示确切 index 匿名可读，不表示完整两架构 layer 下载已通过；失败按可观察类别报告，不能把所有网络失败解释为私有包。
公开包访问设置属于 GitHub 管理配置，验证工具不授予权限或生成持久凭证。

## 开发环境的空闲扫描周期

联合评审接受：CLI 开发 profile 使用秒级 Native Capability、Registry 与 Artifact 扫描周期，并降低 Orchestration 后台推进频率。
Native 和 Registry 各自共用领取与恢复周期，因此降低频率也会延后正常 Job 的下一次领取或验证。
Artifact 扫描在本批完成后等待，后置 finalizer 另行轮询；上传完成的可见延迟会累加两个阶段的等待。
Orchestration 后台扫描还推进到期 Timer、重试、收敛、Task 过期与子任务取消，其业务 Job 领取周期保持独立。
这是开发资源与响应时间的明确取舍，不改变恢复页容量、持久 cursor、租约、心跳、事务或 Helm 的部署输入。
默认值由 CLI 配置生成器拥有，配置和 profile 摘要按正常路径重建；不能改写已签名候选来继承资格。
验收须包含真实空闲 CPU、Native 任务进展、恢复与完整候选旅程，微基准不能替代这些证据。

## 建库、发布与证据

全局领取的开发集成测试必须使用独立数据库隔离其 authority，不能仅靠函数排序或租约时长避开另一夹具的 Job。
Coordinator 恢复与 Q1 领取分别在完整 schema 验证后、任何 admission 前确认 Tenant 为空；发现已用库即拒绝，不删除事实。
子进程传递对应的 exact 数据库配置；PostgreSQL 集群级锁视图的 barrier 观察也限定当前数据库。
隔离修复不改变生产 scheduler、公平性、并发、配额或恢复期限；这些仍由原实际多进程断言验证。
...
联合评审补充：Kind mechanics 使用独立、闭合的开发 CPU 预留输入，并把它纳入 deployment_config_digest。
生成器只覆盖 Rust 服务的 CPU requests，保留生产 chart 的内存预留、全部 limits、双副本、拓扑、安全边界及执行并发。
...
联合评审接受：runtime 与 Sandbox runner 由各自平台的原生构建宿主产出，使用相同的 owning Dockerfile、
固定工具链、基础镜像与 locked 依赖。汇总消费当前源码与工作流批次的有界、版本化记录，只通过精确摘要合并
OCI index，并验证两平台运行镜像及其 attestation 描述符完整保留。缓存不构成构建身份凭据。
Console 只编译一次资产，汇总验证该 archive 后生成两平台 scratch 镜像；此阶段不执行目标架构代码。

构建与推送的计时从最早原生分支开始，到对应最终 index 已推送并验证完成，包含分支、工件传递和调度等待。
原性能预算、签名、完整候选旅程及退出清理、匿名读取与不可变发布顺序保持不变。新的构建拓扑须由自己的
精确候选证据证明通过，不能继承历史源码旅程或其他版本的发行资格。
