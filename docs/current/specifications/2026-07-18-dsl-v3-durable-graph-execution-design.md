...
本文定义当前 DSL v3 与持久化执行内核的架构和可验证合同，覆盖：
...
- 从 Region/SSA 内存运行时到 v3 的九阶段 clean-break 切换结果。

本文是当前 v3 parser、compiler、Plan、durable runtime 与 Graph API 的规范性合同。
...
| [Terminal CAS](../../archive/specs/2026-07-15-independent-connection-terminal-cas-race-design.md) 与 [Public Agent Contract](../../archive/specs/2026-07-15-public-agent-contract-design.md) | 保留唯一终态 CAS 和公共输入 Schema；v3 内部状态通过明确映射投影到现有公共状态/事件 |
| 历史 `core.*` 控制流规范 | 继续为历史文档；v3 的 Fork/Join 是私有 Plan 原语，不重新开放 public `core.*` 节点 |

v3 当前实现仅保留 clean-break 路径；发布级完成以本文状态与证据节所列门禁通过为准。
冲突规范按历史记录保留，当前权威关系由
...
8. v3 中产生值的 `if` 必须有 `else`；不依赖 Compiler 对任意 CEL 条件做穷尽证明。
...
`match` 不得包含步骤、创建 Activation 或改变控制流。v3 删除 `choose` 和 authored
...
GraphAuthorDocument；字段级 Schema 由 v3 GraphAuthorDocument wire contract 定义：
...
5. v3 首批只支持 `all_success` 与 `all_settled`。
...
6. 当前 singleton store advisory lock 在 v3 cutover 时删除，不作为分布式调度基础。
...
| v3 internal terminal | public Run status | public event |
|---|---|---|
...
5. 合并 v3 前必须清除 checked-in Agent、positive fixture 和 active docs 中的旧控制流。
...
| 阶段 | 实施状态 | 主要验证锚点 |
|---|---|---|
| 1. 架构和状态机 | 已实现 / 已验证 | `v3_engine_state_model`、`v3_engine_control_model`：非法转移、终态不可变、stale fence、first-winner、跨 Run 隔离 |
| 2. Canonical Typed Plan | 已实现 / 已验证 | `v3_plan_model`、`v3_plan_schema`；不可执行 Policy、不可达节点、端口/终止合同在发布前拒绝 |
| 3. PostgreSQL durable kernel | 已实现 / 已验证 | `v3_repository_postgres*`、`v3_postgres_migrations`、`v3_projection_rebuild`、非 skip 的 `v3_ci_postgres_gate` |
| 4. 最小纵向切片 | 已实现 / 已验证 | `v3_scheduler_core`、`v3_scheduler_postgres`、真实进程 crash matrix、Artifact 提交/恢复、生产二进制重启恢复 |
| 5. 执行安全合同 | 已实现 / 已验证 | `v3_scheduler_advanced`、SQLite/PostgreSQL boundary suites；retry/timeout 固化、cancel/drain、signal/timeout first-winner |
| 6. Structured v3 Core | 已实现 / 已验证 | `v3_dsl_core`；checked-in Agent/Markdown 编译、旧控制语法负向测试、等价 Plan normalized hash |
| 7. 删除旧内核 | 已实现 / 已验证 | v3 residual scan 为零；format、Clippy、全量测试、真实 PostgreSQL 与 stock production binary gate 通过 |
| 8. 高级控制、恢复与保留 | 已实现 / 已验证 | `v3_dsl_advanced`、AgentLoop、Recovery、Production Recovery/Subflow、Artifact/GC、Human Task first-winner |
| 9. Graph 作者合同与 API | 已实现 / 已验证 | `v3_graph_author`、`v3_graph_product`、`v3_graph_repository`；Graph/View/trace authority 与 Policy edit fail-closed |
...
### 阶段 6：实现 StructuredAuthorDocument v3 Core
...
**退出标准**：所有 Agent 经 v3 compiler 执行；旧控制语法在 positive surface 不可执行；文本 DSL 与手工等价 Plan 的 normalized hash 一致。

本阶段只发布 v3 Core surface。Map、Loop、Subflow、Try/Catch、Human Task 等高级作者语法在
...
| `bash scripts/check-v3-cutover-residuals.sh` | 通过；production source-symbol residual 为零 |
