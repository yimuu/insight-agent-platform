...
database_role_bin=${INSIGHT_KIND_DATABASE_ROLE_BIN:-$(dirname "$schema_bin")/platform-database-role}
jetstream_provision_bin=${INSIGHT_KIND_JETSTREAM_PROVISION_BIN:-$(dirname "$schema_bin")/platform-jetstream-provision}
qualification_bin=${INSIGHT_KIND_QUALIFICATION_BIN:-$root/target/debug/platform-qualification}
postgres_forward_port=${INSIGHT_KIND_POSTGRES_FORWARD_PORT:-15432}
nats_forward_port=${INSIGHT_KIND_NATS_FORWARD_PORT:-14222}
...
for path in "$insight_bin" "$schema_bin" "$database_role_bin" "$jetstream_provision_bin" "$qualification_bin"; do
  if [[ ! -x "$path" ]]; then
...
python3 "$root/tools/release/inspect-platform-oci-image.py" \
...
python3 "$root/tools/checks/verify-platform-sandbox-package-image.py" \
...
docker cp "$image_container:/usr/local/bin/." "$output/image-binaries/"
...
# image. Kind imports of product images bypass Docker's image store: their checked, registry-preserving OCI
...
ensure_namespace platform-outbox outbox-worker restricted l4-outbox platform-outbox
ensure_namespace platform-history-maintenance history-maintenance restricted l4-history platform-history-maintenance
...
issue_server_certificate() {
  ...
  openssl req -new -key "$tls_output/$name-key.pem" -subj "/CN=$name" -out "$tls_output/$name.csr"
...
# Local CLI certificates name localhost only. Each service keeps the same CA and gains
# its exact Kubernetes DNS names; client certificate URI identities remain unchanged.
issue_server_certificate artifact-data-kind-server \
  DNS:localhost,DNS:insight-platform-artifact-data-worker.platform-artifacts.svc,DNS:insight-platform-artifact-data-worker.platform-artifacts.svc.cluster.local
issue_server_certificate egress-kind-server \
  DNS:localhost,DNS:l4-security-insight-platform-security-egress-egress.platform-egress.svc,DNS:l4-security-insight-platform-security-egress-egress.platform-egress.svc.cluster.local
issue_server_certificate authority-kind-server \
  DNS:localhost,DNS:l4-security-insight-platform-security-egress-security-authority.platform-security-authority.svc,DNS:l4-security-insight-platform-security-egress-security-authority.platform-security-authority.svc.cluster.local
issue_server_certificate mcp-host-kind-server \
  DNS:localhost,DNS:insight-platform-mcp-host.platform-mcp-host.svc,DNS:insight-platform-mcp-host.platform-mcp-host.svc.cluster.local
issue_server_certificate mcp-resource-kind-server \
  DNS:localhost,DNS:insight-platform-mcp-resource-host.platform-mcp-host.svc,DNS:insight-platform-mcp-resource-host.platform-mcp-host.svc.cluster.local
...
"$kubectl_bin" -n platform-deps create configmap nats-config \
  --from-file=nats.conf="$root/deploy/dev/nats.conf" \
  --dry-run=client -o yaml | "$kubectl_bin" apply -f - >/dev/null
...
mkdir -m 700 "$output/credentials"
for purpose in outbox history security-authority; do
  (umask 077; openssl rand -hex 16 | tr -d '\n' >"$output/credentials/$purpose-password")
  PLATFORM_DATABASE_ROLE_ADMIN_URL="postgresql://insight:insight-local-only@127.0.0.1:$postgres_forward_port/insight" \
    "$database_role_bin" --profile kind-local "$postgres_forward_port" --purpose "$purpose" "$output/credentials/$purpose-password"
done
mkdir -m 700 "$output/credentials/artifact"
for role in artifact-gateway artifact-data-reader artifact-data-worker artifact-maintenance; do
  (umask 077; openssl rand -hex 16 | tr -d '\n' >"$output/credentials/artifact/$role-password")
done
PLATFORM_DATABASE_ROLE_ADMIN_URL="postgresql://insight:insight-local-only@127.0.0.1:$postgres_forward_port/insight" \
  "$database_role_bin" --profile kind-local "$postgres_forward_port" --purpose artifact "$output/credentials/artifact"
cleanup_port_forward
trap - EXIT INT TERM

# The publisher cannot create or reconfigure streams. Provision once with the separate identity.
port_forward_log="$output/nats-port-forward.log"
"$kubectl_bin" -n platform-deps port-forward service/nats "$nats_forward_port:4222" >"$port_forward_log" 2>&1 &
port_forward_pid=$!
trap cleanup_port_forward EXIT INT TERM
for _ in $(seq 1 60); do
  if grep -q 'Forwarding from' "$port_forward_log"; then break; fi
  if ! kill -0 "$port_forward_pid" >/dev/null 2>&1; then
    sed -n '1,120p' "$port_forward_log" >&2; exit 1
  fi
  sleep 0.25
done
PLATFORM_OUTBOX_PROVISION_NATS_URL="tls://localhost:$nats_forward_port" \
PLATFORM_OUTBOX_PROVISION_CA_PATH="$seed_runtime/tls/ca.pem" \
PLATFORM_OUTBOX_PROVISION_CERT_PATH="$seed_runtime/tls/outbox-provision-client.pem" \
PLATFORM_OUTBOX_PROVISION_KEY_PATH="$seed_runtime/tls/outbox-provision-client-key.pem" \
  "$jetstream_provision_bin" create "$root/deploy/jetstream/committed-events-v1.json"
cleanup_port_forward
trap - EXIT INT TERM
...
deployment_digest=$(
  ruby "$root/tools/qualification/prepare-platform-kind-local.rb" \
    --seed-runtime "$seed_runtime" \
    --worker-binaries "$output/image-binaries" \
    --qualification-tool "$qualification_bin" \
...
config_map platform-outbox insight-platform-outbox-worker-candidate outbox-worker.json "$config_dir/outbox-worker.json"
config_map platform-history-maintenance insight-platform-history-maintenance-candidate history-maintenance.json "$config_dir/history-maintenance.json"
...
# Only these narrowly provisioned credentials enter the Outbox and History workloads.
for purpose in outbox history; do
  role="insight_${purpose}_dev"
  credential=$(cat "$output/credentials/$purpose-password")
  (umask 077; printf 'postgresql://%s:%s@postgres.platform-deps.svc.cluster.local:5432/insight' "$role" "$credential" >"$output/credentials/$purpose-url")
  unset credential
done
for role in security-authority artifact-gateway artifact-data-reader artifact-data-worker artifact-maintenance; do
  case "$role" in
    security-authority) password_file="$output/credentials/security-authority-password" ;;
    *) password_file="$output/credentials/artifact/$role-password" ;;
  esac
  database_role="insight_${role//-/_}_dev"
  credential=$(cat "$password_file")
  (umask 077; printf 'postgresql://%s:%s@postgres.platform-deps.svc.cluster.local:5432/insight' "$database_role" "$credential" >"$output/credentials/$role-url")
  unset credential
done
generic_secret platform-security-authority insight-platform-security-authority-database --from-file=database-url="$output/credentials/security-authority-url"
for role in artifact-gateway artifact-data-reader artifact-data-worker artifact-maintenance; do
  generic_secret platform-artifacts "insight-platform-$role-database" --from-file=database-url="$output/credentials/$role-url"
done
generic_secret platform-outbox insight-platform-outbox-worker-database --from-file=database-url="$output/credentials/outbox-url"
generic_secret platform-history-maintenance insight-platform-history-maintenance-database --from-file=database-url="$output/credentials/history-url"
generic_secret platform-outbox insight-platform-outbox-worker-nats-client-tls \
  --from-file=ca.pem="$seed_runtime/tls/ca.pem" --from-file=client.pem="$seed_runtime/tls/outbox-client.pem" --from-file=client-key.pem="$seed_runtime/tls/outbox-client-key.pem"
generic_secret platform-registry-validation insight-platform-registry-validation-artifact-client-tls \
  --from-file=ca.pem="$seed_runtime/tls/ca.pem" --from-file=client.pem="$seed_runtime/tls/registry-validation-client.pem" --from-file=client-key.pem="$seed_runtime/tls/registry-validation-client-key.pem"
...
remote_mcp_enabled=$(ruby -rjson -e 'config=JSON.parse(File.read(ARGV.fetch(0))); enabled=!config.fetch("installed_mcp_codecs").empty?; abort "MCP codec/Host configuration mismatch" unless enabled == !config["mcp_host"].nil?; puts enabled' "$config_dir/capability-remote-worker.json")
if [[ "$remote_mcp_enabled" == true ]]; then
...
generic_secret platform-egress insight-platform-egress-server-tls \
  --from-file=client-ca.pem="$seed_runtime/tls/ca.pem" --from-file=server.pem="$tls_output/egress-kind-server.pem" --from-file=server-key.pem="$tls_output/egress-kind-server-key.pem"
...
generic_secret platform-security-authority insight-platform-security-authority-server-tls \
  --from-file=client-ca.pem="$seed_runtime/tls/ca.pem" --from-file=server.pem="$tls_output/authority-kind-server.pem" --from-file=server-key.pem="$tls_output/authority-kind-server-key.pem"
...
generic_secret platform-artifacts insight-platform-artifact-data-worker-server-tls \
  --from-file=client-ca.pem="$seed_runtime/tls/ca.pem" --from-file=server.pem="$tls_output/artifact-data-kind-server.pem" --from-file=server-key.pem="$tls_output/artifact-data-kind-server-key.pem"
generic_secret platform-mcp-host insight-platform-mcp-host-server-tls \
  --from-file=client-ca.pem="$seed_runtime/tls/ca.pem" --from-file=server.pem="$tls_output/mcp-host-kind-server.pem" --from-file=server-key.pem="$tls_output/mcp-host-kind-server-key.pem"
...
generic_secret platform-mcp-host insight-platform-mcp-resource-host-server-tls \
  --from-file=client-ca.pem="$seed_runtime/tls/ca.pem" --from-file=server.pem="$tls_output/mcp-resource-kind-server.pem" --from-file=server-key.pem="$tls_output/mcp-resource-kind-server-key.pem"
...
install_chart l4-outbox platform-outbox insight-platform-outbox-worker outbox
install_chart l4-history platform-history-maintenance insight-platform-history-maintenance history
