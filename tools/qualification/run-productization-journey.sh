...
usage() {
  cat <<'EOF'
Usage: tools/qualification/run-productization-journey.sh [options]
...
if [[ -z "$release_candidate" ]]; then
  runtime_binary_root="$(cd "$workspace" && cargo metadata --locked --no-deps --format-version 1 | python3 -c 'import json,pathlib,sys; p=pathlib.Path(json.load(sys.stdin)["target_directory"]); assert p.is_absolute(); print(p/"release")')"
  if [[ "$insight_bin_explicit" == false && -z "${PLATFORM_INSIGHT_BIN:-}" ]]; then insight_bin="$runtime_binary_root/insight"; fi
else
  runtime_binary_root="$(dirname "$insight_bin")"
fi
orphaned_processes="$(python3 - "$runtime_binary_root" <<'PY_TARGET'
import subprocess, sys
prefix = sys.argv[1].rstrip('/') + '/platform-'
for line in subprocess.check_output(['ps', '-axo', 'pid=,command='], text=True).splitlines():
    fields = line.strip().split(None, 1)
    if len(fields) == 2 and fields[1].startswith(prefix): print(fields[0])
PY_TARGET
)"
...
if [[ "$console_browser" == true ]]; then
  if [[ -z "$release_candidate" ]]; then
    PATH="$(dirname "$node_bin"):$PATH" "$corepack_bin" pnpm --dir "$workspace/apps/console" install --frozen-lockfile
    PATH="$(dirname "$node_bin"):$PATH" "$corepack_bin" pnpm --dir "$workspace/apps/console" run build
  fi
fi
...
if [[ -n "$first_run_marker" ]]; then
  python3 tools/qualification/qualify-productization-first-run.py \
    --insight-bin "$insight_bin" \
    --project "$project" \
    --marker "$first_run_marker"

  source_revision="$(git rev-parse HEAD)"
  python3 tools/qualification/write-productization-north-star-report.py \
    --marker "$first_run_marker" \
    --output "$north_star_report" \
    --source-revision "$source_revision" \
    --started-epoch "$journey_started_epoch" \
    --fresh-checkout
  python3 tools/checks/check-productization-north-star-report.py \
    "$north_star_report" \
    --source-revision "$source_revision"
  rm -f "$first_run_marker"
fi
...
env "${test_environment[@]}" \
  cargo test --locked -p insight-platform-qualification-tests --test productization deterministic_first_run::public_cli_deterministic_first_run -- --ignored --exact --nocapture

if [[ -n "$report_directory" ]]; then
  if [[ "$features" == "all" ]]; then
    source_revision="$(git rev-parse HEAD)"
    python3 tools/checks/check-productization-scenario-reports.py \
      "$report_directory" \
      --source-revision "$source_revision" \
