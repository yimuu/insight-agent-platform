...
if [[ -n "$qualification_bin" ]]; then
  ...
  cargo run --locked --manifest-path "$root/Cargo.toml" \
    -p insight-platform-contract-tooling --bin platform-qualification -- \
...
python3 "$root/tools/checks/check-platform-production-topology.py" \
...
python3 "$root/tools/checks/check-platform-production-workloads.py" \
  --candidate "$candidate" \
  --capacity "$capacity" \
