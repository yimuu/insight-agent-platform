...
manifest = (root / "apps/services/platform-callback-api/Cargo.toml").read_text(encoding="utf-8")
source = (root / "apps/services/platform-callback-api/src/main.rs").read_text(encoding="utf-8")
...
if "MCP_OAUTH_CALLBACK_PATH" not in (root / "crates/protocols/platform-api/src/lib.rs").read_text(encoding="utf-8"):
