...
def check_limits(errors):
    ...
    if profile.get("profile_id") != "q1-50" or profile.get("profile_version") != 6:
    ...
    if runtime_bundle != {
        ...
        errors.append("Q1 Sandbox runtime bundle limit differs from the closed v6 contract")
    schema = load(CONTRACT_ROOT / "limits" / "hard-limit-profile.schema.json")
    if schema.get("properties", {}).get("profile_version") != {"const": 6}:
        errors.append("HardLimitProfile schema must accept only profile version 6")
    if profile.get("control_data", {}).get("outbox_admission_backlog") != {
        "unit": "count", "hard_max": 1000000, "q1_default": 100000,
        "overflow_outcome": "temporarily_unavailable",
    }:
        errors.append("Q1 Outbox pressure admission tuple differs from its reviewed contract")
    pressure = schema.get("$defs", {}).get("control_data", {}).get("properties", {}).get("outbox_admission_backlog", {})
    if pressure.get("additionalProperties") is not False or pressure.get("properties") != {
        "unit": {"const": "count"}, "hard_max": {"type": "integer", "minimum": 1, "maximum": 1000000},
        "q1_default": {"type": "integer", "minimum": 1, "maximum": 1000000},
        "overflow_outcome": {"const": "temporarily_unavailable"},
    }:
        errors.append("Outbox pressure admission schema lost its bounded closed semantics")
    expression_limits = {
    ...
    for name, expected in expression_limits.items():
        if profile.get("registry_plan", {}).get(name) != expected:
            errors.append(f"Q1 {name} differs from the closed v6 contract")
        expected_schema = {
            "type": "object",
...
def check_foundation_surfaces(errors):
    ...
    management_contract = [
        ...
        '      pattern: "^(agt|skl|cap|cim|ctx|xim|mpr|mdl|mcp|pol|srt|spk|sxp)_',
        '      pattern: "^(aif|arev|srev|cirev|cimp|xirev|ximp|mprev|mdrev|mrev|prev|srrev|sprev|sxrev)_',
        '      pattern: "^(adep|skdep|cdep|xdep|mpdep|mdep|mcdep|pdep|sxdep)_',
    ...
    for component, required in {
        "TaskViewV2": {"schema_version", "task_id", "task_kind", "state", "generation", "version", "safe_prompt_key", "response_schema_digest", "owner", "deadline", "responded_at", "created_at", "updated_at", "etag", "allowed_actions"},
        "TaskFormV2": {"schema_version", "task_id", "generation", "version", "response_schema", "response_schema_digest", "etag", "safe_prompt_key", "allowed_actions"},
    }.items():
        block = re.search(rf"^    {component}:\n(.*?)(?=^    [A-Za-z]|\Z)", openapi, re.M | re.S)
        body = block.group(1) if block else ""
        declared = re.search(r"^      required: \[(.*?)\]$", body, re.M)
        actual = {field.strip() for field in declared.group(1).split(",")} if declared else set()
        if actual != required or not re.search(r"^        schema_version: \{(?:type: integer, )?const: 2\}$", body, re.M):
            errors.append(f"{component} must require the closed current action and identity fields")
        if 'allowed_actions: {type: array, maxItems: 4, uniqueItems: true, items: {$ref: "#/components/schemas/TaskAction"}}' not in body:
            errors.append(f"{component} must bound and deduplicate owning Task actions")
    if "    TaskAction:\n      type: string\n      enum: [submit_input, approve, reject, cancel]" not in openapi:
        errors.append("Task actions differ from the current owning closed catalog")
    if openapi.count("name: purpose, in: query, schema: {type: string, enum: [respondable, viewable], default: respondable}") != 2:
        errors.append("Task metadata list/detail must bind the closed query purpose with responder default")
    proto = (ROOT / "contracts/proto" / "insight" / "platform" / "v1" / "foundation.proto").read_text(
        encoding="utf-8"
    )
...
def check_machine_registry_contracts(errors):
    ...
    expected_job_kinds = [
        ...
        "capability_invocation", "mcp_discovery", "mcp_subscription", "mcp_oauth_pkce_cleanup",
    ...
    expected_work_owner_pairs = [
        ...
        {"work_class": "recovery", "owner_kind": "interaction"},
    ]
    ...
    expected_job_triples = [
        ...
        {"job_kind": "mcp_oauth_pkce_cleanup", "work_class": "recovery", "owner_kind": "interaction"},
    ]
    ...
    expected_api_problem_codes = [
        "invalid_request", "invalid_task_input", "task_form_unavailable", "schema_validation_failed", "unauthenticated",
...
def check_history_deployment_contracts(errors):
    config = load(CONTRACT_ROOT / "schemas/history-maintenance-config-v1.schema.json")
    properties = config.get("properties", {})
    expected = {"schema_version", "component_role", "executable_digest", "observability_listen_address", "database_max_connections", "database_acquire_timeout_milliseconds", "poll_interval_milliseconds", "maximum_runs", "maximum_events_per_run", "retention_policy"}
    if config.get("additionalProperties") is not False or set(config.get("required", [])) != expected or set(properties) != expected:
        errors.append("History maintenance process config must be closed")
    if properties.get("component_role", {}).get("const") != "history_maintenance":
        errors.append("History maintenance must use its independent closed role")
    for name, minimum, maximum in [("database_max_connections", 2, 8), ("database_acquire_timeout_milliseconds", 1, 5000), ("poll_interval_milliseconds", 1000, 60000), ("maximum_runs", 1, 128), ("maximum_events_per_run", 1, 1000)]:
        if properties.get(name) != {"type": "integer", "minimum": minimum, "maximum": maximum}:
            errors.append(f"History maintenance bound drifted: {name}")
    evidence = load(CONTRACT_ROOT / "schemas/history-maintenance-executable-evidence-v1.schema.json")
    expected = {"schema_version", "component_role", "runtime_image_digest", "executable_digest", "process_config_digest"}
    if evidence.get("additionalProperties") is not False or set(evidence.get("required", [])) != expected or set(evidence.get("properties", {})) != expected:
        errors.append("History maintenance executable evidence must be closed and independent of Job WorkerManifest")
...
def main():
    ...
    check_history_deployment_contracts(errors)
