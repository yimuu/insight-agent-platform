...
manifest = (root / "apps/services/platform-gateway/Cargo.toml").read_text(encoding="utf-8")
source = (root / "apps/services/platform-gateway/src/main.rs").read_text(encoding="utf-8")
...
# Both roles use the public-gateway mTLS identity. Management uses only the
# internal exact Draft authoring read; public Artifact routes remain Runtime-owned.
for role_name, deployment in (("Management", management), ("Runtime", runtime)):
    for authority in ("artifact-tls", "PLATFORM_GATEWAY_ARTIFACT_CA_PATH", "PLATFORM_GATEWAY_ARTIFACT_CERT_PATH", "PLATFORM_GATEWAY_ARTIFACT_KEY_PATH"):
        if authority not in deployment:
            failures.append(f"{role_name} API is missing exact Artifact read authority {authority}")
for forbidden in (
