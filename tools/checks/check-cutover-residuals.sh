...
done < <(find crates apps/services apps/sandbox apps/insight-cli tools/rust tests/qualification -type d -name src -print0)
while IFS= read -r -d '' manifest; do
  member_manifests+=("$manifest")
done < <(find crates apps/services apps/sandbox apps/insight-cli tools/rust tests/qualification -type f -name Cargo.toml -print0)
...
report_matches \
  ...
  apps/insight-cli/src/lib.rs apps/insight-cli/src/full_profile.rs
...
report_matches \
  ...
  tools/rust/platform-storage-tooling/src/bin/platform_dev_bootstrap.rs
report_matches \
  "durable PostgreSQL payload retains an implicit required-field compatibility default" \
  -nE '#\[serde\(default[[:space:]]*\)\]' \
  crates/adapters/platform-postgres/src/repository.rs
if ! grep -Fq 'if identity.schema_version != 3' apps/insight-cli/src/lib.rs; then
  fail "local identity does not require the current schema_version 3"
fi
if ! grep -Fq 'if self.schema_version != 2' \
  tools/rust/platform-storage-tooling/src/bin/platform_dev_bootstrap.rs; then
...
if grep -Fq \
  ...
  deploy/images/platform.Dockerfile; then
...
report_matches \
  ...
  crates contracts/platform-v1 deploy/release examples/productization tests/qualification/tests/productization apps/console
...
if ! grep -Fq 'default-members = ["apps/insight-cli"]' Cargo.toml; then
...
report_matches \
  ...
  deploy/images/platform.Dockerfile
if ! grep -Fq 'ENTRYPOINT ["/usr/local/bin/platform-gateway"]' deploy/images/platform.Dockerfile; then
