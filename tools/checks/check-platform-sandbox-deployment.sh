...
rendered=$(mktemp)
fixture=$(mktemp)
trap 'rm -f "$rendered" "$fixture"' EXIT
...
if grep -R -n -i -E \
  ...
  "$chart" "$root/deploy/images/platform.Dockerfile"; then
...
for binary in platform-sandbox-dispatcher platform-sandbox-runner; do
  if ! grep -q -- "--bin $binary" "$root/deploy/images/platform.Dockerfile"; then
...
if grep -Fq \
  ...
  "$root/deploy/images/platform.Dockerfile"; then
  echo "sandbox deployment: generic runtime still contains the non-launcher runner binary" >&2
  exit 1
fi

python3 "$root/tools/tests/sandbox_helm_fixture.py" --output "$fixture"
if helm template sandbox "$chart" >/dev/null 2>&1; then
  echo "Sandbox must reject absent executable manifest" >&2; exit 1
fi
helm lint "$chart" --values "$fixture" >/dev/null
helm template sandbox "$chart" --values "$fixture" >"$rendered"

reject_values() {
  if helm template sandbox "$chart" --values "$fixture" "$@" >/dev/null 2>&1; then
