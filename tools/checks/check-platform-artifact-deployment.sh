...
manifest = (root / "apps/services/platform-artifact-service/Cargo.toml").read_text()
dockerfile = (root / "deploy/images/platform.Dockerfile").read_text()
postgres = (root / "crates/adapters/platform-postgres/src/repository.rs").read_text()
postgres += (root / "crates/adapters/platform-postgres/src/repository/job_commands.rs").read_text()
artifact_repository = (root / "crates/adapters/platform-postgres/src/artifact_repository.rs").read_text()
data_worker = (root / "apps/services/platform-artifact-service/src/main.rs").read_text()
maintenance = (root / "apps/services/platform-artifact-service/src/bin/maintenance.rs").read_text()
gateway = (root / "apps/services/platform-artifact-service/src/bin/gateway.rs").read_text()
grants = (root / "crates/adapters/platform-postgres/artifact-role-grants.sql").read_text()
failures = []
