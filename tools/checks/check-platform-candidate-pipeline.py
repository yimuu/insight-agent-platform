...
generator = (ROOT / "tools/release/build-platform-production-candidate.py").read_text()
...
required_workflow = (
    ...
    "Extract executable bytes from the verified runtime image", "docker create", "docker cp",
    "--worker-manifests", "--worker-configs", "--worker-binaries", "--history-maintenance-config",
    "verify-history-maintenance-deployment", "history-maintenance-executable-evidence.json",
    "validate-production-candidate", "candidate-manifest.json", "schema-executable-evidence.json", "verify-schema-executable-evidence",
...
if workflow.count("file: deploy/images/platform.Dockerfile") != 2:
    failures.append("both candidate targets must use the current owning Dockerfile path")
...
production_bins = (
    ...
    "platform-subscription-context-worker", "platform-orchestration-worker", "platform-outbox-worker", "platform-jetstream-provision", "platform-database-role", "platform-history-maintenance",
...
if len(production_builds) != 1:
    failures.append("deploy/images/platform.Dockerfile must compile the production closure in one Cargo invocation")
if not production_builds:
    failures.append("deploy/images/platform.Dockerfile production build must select binaries across the workspace")
for build_cache_marker in (
    ...
    if build_cache_marker not in dockerfile:
        failures.append(f"deploy/images/platform.Dockerfile production build misses dependency cache stage {build_cache_marker}")
for binary in production_bins:
    if (
        ...
        failures.append(f"deploy/images/platform.Dockerfile production build misses binary {binary}")
for runner_boundary in ("FROM runtime-base AS sandbox-runner", "ENTRYPOINT [\"/usr/local/bin/platform-sandbox-runner\"]"):
    if runner_boundary not in dockerfile:
        failures.append(f"deploy/images/platform.Dockerfile sandbox runner misses {runner_boundary}")
if "-p insight-platform-sandbox-runner --bin platform-sandbox-runner" not in dockerfile:
    failures.append("deploy/images/platform.Dockerfile sandbox runner misses the isolated static Rust core build")
