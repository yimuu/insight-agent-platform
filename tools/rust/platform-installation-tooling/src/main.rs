...
async fn run() -> Result<(), InstallationError> {
    ...
    if args.as_slice() == ["compose-release-digest"] {
        return upgrade::release_digest();
    }
    if let [command, stopped, expected, digest] = args.as_slice() {
        if command == "compose-rollout"
            && stopped == "--serving-stopped"
            && expected == "--expected-release"
        {
            return upgrade::rollout(
                digest
                    .parse()
    ...
    if args.as_slice() == ["compose-upgrade", "--serving-stopped"] {
        return upgrade::run().await;
    }
    if args.as_slice() == ["compose-prepare"] {
        return compose_startup::prepare();
    }
    if args.as_slice() == ["compose-template"] {
        println!("# Generated by platform-installation compose-template. Configure public options in .env.\n{}", yaml_serde::to_string(
            &insight_platform_deployment_tooling::compose::repository_document()?
        ).map_err(|_| InstallationError::InvalidInput)?);
        return Ok(());
    }
    if let [command, input_flag, file, directory_flag, directory, phase_flag, phase] =
        args.as_slice()
    {
        if command == "wait-gate"
            && input_flag == "--input"
            && directory_flag == "--directory"
            && phase_flag == "--phase"
        {
            use insight_platform_deployment_contracts::installation_provider::InstallationGatePhaseV1 as P;
            ...
            gates::wait(
                &read_input(std::path::Path::new(file))?,
    ...
    if let [command, input_flag, input_file, runtime_flag, runtime, console_flag, console] =
        args.as_slice()
    {
        if matches!(command.as_str(), "compose" | "helm-values")
            && input_flag == "--input"
            ...
            && console_flag == "--console-image"
        {
            ...
            let document = if command == "compose" {
                ...
                serde_json::json!({"plan": insight_platform_deployment_tooling::kubernetes::helm_plan(
    ...
    match command.as_str() {
        "bootstrap" => {
            let identity = provider_bootstrap::bootstrap(&input, &state).await?;
            println!(
                "{}",
                serde_json::json!({"schema_version":1,"input_digest":input.digest()?,"identity_digest":identity,"phase":"provider_ready"})
            );
        }
        "install" => {
            use insight_platform_deployment_contracts::installation_provider::InstallationGatePhaseV1 as P;
            let output = output.as_deref().ok_or(InstallationError::InvalidInput)?;
            let binaries = binaries.as_deref().ok_or(InstallationError::InvalidInput)?;
            let prepared = workflow::prepare(&input, &state, output)?;
            gates::publish(&prepared, &output.join("gates"), P::Prepared)?;
            drop(prepared);
            provider_bootstrap::bootstrap(&input, &state).await?;
            let prepared = workflow::configure(&input, &state, output, binaries, false).await?;
            gates::publish(&prepared, &output.join("gates"), P::Ready)?;
            println!(
                "{}",
                serde_json::json!({"schema_version":1,"input_digest":input.digest()?,"identity_digest":prepared.identity().digest()?,"phase":"ready"})
            );
        }
        "provider-observe" => {
            let prepared = PreparedInstallation::open(&input, &state)?;
            let evidence = openbao_setup::observe(&prepared, &state).await?;
            // Observation cannot commit bootstrap completion or bypass root-token revocation.
            let _ = evidence;
            println!(
                "{}",
                serde_json::json!({
...
fn valid_command_options(command: &str, output: bool, binaries: bool) -> bool {
    matches!(
        ...
        ("prepare", true, false)
            | ("provision" | "verify" | "install", true, true)
            | (
                "session" | "public-trust" | "bootstrap" | "provider-observe",
                false,
                false
            )
    )
            ...
            for output in [false, true] {
                for binaries in [false, true] {
                    let expected = match command {
                        ...
                        "provision" | "verify" | "install" => output && binaries,
                        "session" | "public-trust" | "bootstrap" | "provider-observe" => {
                            !output && !binaries
                        }
                        _ => false,
