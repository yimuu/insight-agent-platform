...
fn load_artifact_authority() -> Result<DevelopmentArtifactAuthoritySeed, ProcessError> {
    let path = required_absolute_path(ARTIFACT_CONFIG_PATH_ENV)?;
    let bytes = read_bounded_file(&path, MAX_DEVELOPMENT_ARTIFACT_BOOTSTRAP_BYTES)?;
...
async fn run() -> Result<(), ProcessError> {
    ...
    let artifact_authority = load_artifact_authority()?;
