...
async fn main() {
    ...
    if !matches!(arguments.as_slice(),[command] if matches!(command.as_str(),"provision"|"verify"|"inventory"))
    {
    ...
    if command == Some("provision") {
        match provision_schema(&pool).await {
            Ok(verification) => println!(
                "insight.platform/v1 schema contract {} provisioned (snapshot {}, inventory {}, tables {})",
                verification.contract_version,
                verification.schema_snapshot_digest,
    ...
    match result {
        Ok(verification) => println!(
            "insight.platform/v1 schema contract {} verified (snapshot {}, inventory {}, tables {})",
            verification.contract_version,
            verification.schema_snapshot_digest,
            verification.schema_inventory_digest,
            verification.table_count
