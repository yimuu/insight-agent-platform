...
ROOT = next(parent for parent in pathlib.Path(__file__).resolve().parents if (parent / "Cargo.toml").is_file())
RUNNER = ROOT / "tools/qualification/run-productization-journey.sh"
WORKFLOW = ROOT / ".github" / "workflows" / "productization-journey.yml"
BOOTSTRAP = ROOT / "tools/qualification/bootstrap-platform-kind-local.sh"
...
class ProductizationJourneyRunnerTests(unittest.TestCase):
    ...
    def test_console_path_installs_exact_dependencies_before_build(self) -> None:
        ...
        install = 'pnpm --dir "$workspace/apps/console" install --frozen-lockfile'
        build = 'pnpm --dir "$workspace/apps/console" run build'
    ...
    def test_north_star_precedes_the_heavy_scenario_test(self) -> None:
        source = RUNNER.read_text(encoding="utf-8")
        qualifier = "python3 tools/qualification/qualify-productization-first-run.py"
        scenario = (
    ...
    def test_all_workflow_bootstraps_and_always_cleans_real_opensandbox(self) -> None:
        ...
        self.assertIn("tools/qualification/bootstrap-platform-kind-local.sh", workflow)
        self.assertIn("tools/qualification/run-productization-sandbox-qualification.sh", workflow)
    ...
    def test_product_images_use_verified_registry_preserving_oci_import(self) -> None:
        ...
        self.assertIn("tools/release/inspect-platform-oci-image.py", source)
        self.assertIn("tools/checks/verify-platform-sandbox-package-image.py", source)
        ...
        self.assertIn("tools/release/inspect-platform-oci-image.py", candidate)
        self.assertIn("tools/checks/verify-platform-sandbox-package-image.py", candidate)
        ...
        self.assertIn("tools/checks/verify-platform-sandbox-package-image.py", bootstrap)
...
"""
    ...
    def test_quick_ci_runs_candidate_and_sandbox_boundary_tests(self) -> None:
        ...
        for path in (
            ...
            "tools/tests/test_inspect_platform_oci_image.py",
    ...
    def test_ci_checker_allows_only_local_unpinned_uses(self) -> None:
        checker = (ROOT / "tools/checks/check-productization-ci.py").read_text(encoding="utf-8")
        self.assertIn('if action.startswith("./"):', checker)
        result = subprocess.run(
            ["python3", str(ROOT / "tools/checks/check-productization-ci.py")],
            cwd=ROOT,
            check=False,
            capture_output=True,
            text=True,
