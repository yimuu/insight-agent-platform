...
SCRIPT = ROOT / "tools/release/build-product-release.py"
SIGNER = ROOT / "tools/release/sign-product-release.py"
IMAGE_METADATA = ROOT / "tools/release/build-release-image-metadata.py"
PERFORMANCE = ROOT / "tools/release/build-release-performance.py"
PRODUCTIZATION_CHECKER = ROOT / "tools/checks/check-productization-scenario-reports.py"
...
class ProductReleaseTests(unittest.TestCase):
    ...
    def fixture(self, root: pathlib.Path) -> pathlib.Path:
        version = "1.2.3"
        profile = json.loads((ROOT / "deploy/release/development-profile-v1.json").read_bytes())
    ...
    def productization_fixture(
        ...
        *,
        ...
        evidence = {
            ...
            "qualifier": "tools/qualification/qualify-platform-sandbox-l3.sh",
            "release_candidate": (
                {
    ...
    def test_signer_binds_the_configured_public_trust_root(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(generated.returncode, 0, "release signature qualification requires OpenSSL with Ed25519 support")
            subprocess.run([
