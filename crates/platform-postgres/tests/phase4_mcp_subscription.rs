...
use insight_platform_contracts::{
    ...
    ArtifactReferenceKind, AuthoringPackage, CapabilityEndpointScheme, CommandAudit,
    CommandOutcome, ContextBackendBinding, ContextBackendContract, ContextBackendKind,
    ContextBackendLimits, ContextCitationContract, ContextCitationStrength, ContextConsistencyMode,
    ...
    ContextLocatorKind, ContextPaginationContract, ContextRankingContract, DataClassification,
    DataRegion, DeploymentClosure, ExactDeploymentRef, ExactSecretBindingRef, ExactVersionRef,
...
async fn insert_ready_artifact(
    ...
    let blob_id = id(ResourceKind::InternalBlob, 0x170);
    let now = Utc::now();
    sqlx::query(
        r#"
...
async fn seed(pool: &PgPool, repository: &PgRepository, now: DateTime<Utc>) -> Fixture {
    ...
    let token_binding = ExactSecretBindingRef::build(
        ...
        1,
        id(ResourceKind::SecretProvider, 0x42),
        token_purpose.clone(),
