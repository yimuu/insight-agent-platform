...
use insight_platform_agent_compiler::{
    AgentCompilationEvidenceV1, AgentCompilationInputIdentityV1,
};
...
use insight_platform_contracts::{DomainOperationRequirements, ExecutionRequirement};
...
pub struct RequestResourceValidation {
    pub compilation_input: Option<AgentCompilationInputIdentityV1>,
    pub audit: CommandAudit,
    pub resource_id: ResourceId,
    pub expected_resource_version: i64,
...
impl RegistryValidationJobPayload {
    pub fn from_request(
        ...
        let execution_requirement = match (&command.compilation_input, resource_kind) {
            (Some(input), RegistryResourceKind::Agent) => {
                input
                    .validate()
                    .map_err(|_| RegistryCommandError::InvalidValidationJob)?;
                ExecutionRequirement::AgentCompilation {
                    validation_input_digest: draft_digest.clone(),
                    compiler_semantic_identity: input.compiler_semantic_identity.clone(),
                    compile_policy_inputs_digest: input.compile_policy_inputs_digest.clone(),
                }
            }
            (None, kind) if kind != RegistryResourceKind::Agent => {
                ExecutionRequirement::DomainOperation {
                    operation_abi_identity: registry_resource_validation_operation_abi_identity(),
                    requirements: DomainOperationRequirements::Validator {
                        validation_policy_digest: command.validation_profile_digest.clone(),
                    },
                }
            }
            _ => return Err(RegistryCommandError::InvalidValidationJob),
        };
        let payload = Self {
    ...
    pub fn validate_for_owner(&self, owner_id: &ResourceId) -> Result<(), RegistryCommandError> {
        ...
        match (&self.execution_requirement, self.resource_kind) {
            (
                ExecutionRequirement::AgentCompilation {
            ...
            ) if validation_input_digest == &self.draft_digest => {}
            (
                ExecutionRequirement::DomainOperation {
                ...
                && operation_abi_identity
                    == &registry_resource_validation_operation_abi_identity()
                && validation_policy_digest == &self.validation_profile_digest => {}
            _ => return Err(RegistryCommandError::InvalidValidationJob),
        }
        Ok(())
    }
...
pub fn build_registry_validation_summary(
    ...
    compilation: Option<&AgentCompilationEvidenceV1>,
) -> Result<ValidationSummary, RegistryCommandError> {
    payload.validate_for_owner(&payload.job_id)?;
    draft
        .validate()
        .map_err(|failure| RegistryCommandError::Contract(failure.to_string()))?;
    if payload.validation_profile_digest != *installed_validation_profile_digest
    ...
    let program_requirement = match (compilation, &draft.document, &payload.execution_requirement) {
        (
            Some(evidence),
            ResourceDocument::Agent(agent),
            ExecutionRequirement::AgentCompilation {
                compiler_semantic_identity,
                compile_policy_inputs_digest,
                ..
            },
        ) => {
            evidence
            ...
            if &evidence.input.compiler_semantic_identity != compiler_semantic_identity
                || &evidence.input.compile_policy_inputs_digest != compile_policy_inputs_digest
                || &evidence.input.source_bundle_digest
                    != agent.authoring_package.artifact.content_digest()
                || evidence.manifest_digest != agent.authoring_package.manifest_digest
                || evidence.typed_plan_digest != agent.typed_plan_digest
                || evidence.contract_digest != agent.contract_digest
            {
                return Err(RegistryCommandError::InvalidValidationResult);
            }
            Some(evidence.program_requirement.clone())
        }
        (None, document, ExecutionRequirement::DomainOperation { .. })
            if document.kind() != RegistryResourceKind::Agent =>
        {
            None
        }
        _ => return Err(RegistryCommandError::InvalidValidationResult),
    };
...
impl RecordResourceValidation {
    pub fn validate_at(&self, now: DateTime<Utc>) -> Result<(), RegistryCommandError> {
        ...
        self.validation
            .validate()
            .map_err(|failure| RegistryCommandError::Contract(failure.to_string()))?;
        if self.resource_id.kind() == ResourceKind::Agent
            || self.expected_resource_version <= 0
...
pub struct NewPublishedVersion {
    /// Immutable publication identity. Equal content does not reuse a previous source/validation.
    pub resource_version_id: ResourceId,
    /// Unique within one Resource and revision kind; an Agent publishes both kinds together.
    pub revision_no: i64,
    /// The owning executable/interface content digest, not the complete publication payload.
...
mod tests {
    ...
    fn registry_validation_summary_is_bound_to_the_exact_job_and_installed_closure() {
        let payload = RegistryValidationJobPayload {
            schema_version: 2,
            execution_requirement: ExecutionRequirement::AgentCompilation {
                validation_input_digest: digest('a'),
                compiler_semantic_identity: digest('2'),
        ...
        payload.execution_requirement = ExecutionRequirement::AgentCompilation {
            validation_input_digest: payload.draft_digest.clone(),
            compiler_semantic_identity: digest('2'),
            compile_policy_inputs_digest: digest('3'),
        };
        // Synthetic typed evidence exercises the summary binder; byte recompilation is
        // independently covered by the compiler's exact Artifact validation tests.
        let evidence =
            AgentCompilationEvidenceV1 {
                schema_version: 1,
                input: AgentCompilationInputIdentityV1 {
                    schema_version: 1,
                    source_bundle_digest: digest('d'),
                    compiler_semantic_identity: digest('2'),
                    compile_policy_inputs_digest: digest('3'),
                },
                ...
                contract_digest: digest('f'),
                program_requirement:
                    insight_platform_plan::execution::program_execution_requirement(digest('1'), 6)
                        .unwrap(),
            };
        ...
        let replacement_build = build_registry_validation_summary(
            &payload,
            &draft,
            &digest('0'),
            &digest('c'),
        ...
        .unwrap();
        assert_eq!(replacement_build.validator_digest, digest('0'));
        assert!(build_registry_validation_summary(
            &payload,
            &draft,
            &digest('0'),
            &digest('9'),
...
/// Read-only lookup of a previously committed validation command. This never
/// creates a Receipt and does not require reading or recompiling source bytes.
#[derive(Debug, Clone)]
pub struct LookupRegistryValidationReceipt {
...
impl LookupRegistryValidationReceipt {
    pub fn validate_at(&self, now: DateTime<Utc>) -> Result<(), RegistryCommandError> {
        if self.tenant_id.kind() != ResourceKind::Tenant
            || self.principal_id.kind() != ResourceKind::Principal
            || !RegistryResourceKind::ALL
                .iter()
                .any(|kind| kind.id_kind() == self.resource_id.kind())
            || self.deadline <= now
...
/// Resource validation semantics, independent of validator process/build identity.
pub fn registry_resource_validation_operation_abi_identity() -> Sha256Digest {
    canonical_digest(&serde_json::json!({"contract":"insight.registry.resource-validation","semantic_version":1,"operation":"resource.validate","resource_document_abi":1}))
        .expect("static validator semantic descriptor is canonical").parse().expect("canonical SHA-256")
}
pub fn registry_resource_validation_execution_capability(
) -> insight_platform_contracts::WorkerExecutionCapability {
