...
pub use convergence::*;
mod child_budget;
pub use child_budget::{derive_child_budget, validate_child_budget_limit};
mod external_leaf_completion;
mod scope_environment;
pub use external_leaf_completion::{ExternalLeafCompletion, ExternalLeafCompletionOwner};
...
use insight_platform_contracts::{
    ...
    Failure, JobState, JsonLimits, NodeExecutionState, PlanNodeKind, PrincipalSnapshot, ResourceId,
    ResourceKind, RunBindingsSnapshot, RunState, Sha256Digest, ValueRef,
};
...
pub enum ControllerObservation {
    None,
    /// A completed external owner and its exact output were independently checked.
    ExternalLeafCompleted,
    Branch {
...
pub fn decide_controller(
    ...
    match (node, observation) {
        (
            RuntimeNode::ChildAgentCall { resume, .. }
            | RuntimeNode::ContextQuery { resume, .. }
            | RuntimeNode::ModelLoop { resume, .. }
            | RuntimeNode::CapabilityCall { resume, .. },
...
pub struct OrchestrationJobPayload {
    ...
    /// A committed result awaiting ordinary structural controller settlement.
    pub external_leaf_completion: Option<ExternalLeafCompletion>,
}

pub const ORCHESTRATION_JOB_PAYLOAD_VERSION: i32 = 2;
pub const ORCHESTRATION_JOB_PAYLOAD_MAX_BYTES: usize = 262_144;
...
impl OrchestrationJobPayload {
    /// Only settlement of a previously committed external outcome uses the reserved lane.
    pub fn scheduling_lane(&self) -> insight_platform_contracts::SchedulingLane {
        if self.external_leaf_completion.is_some() || self.convergence_failure.is_some() {
    ...
    pub fn to_payload(
        &self,
    ) -> Result<insight_platform_contracts::TypedPayload, OrchestratorError> {
        self.validate()?;
        insight_platform_contracts::TypedPayload::with_limit(
            ORCHESTRATION_JOB_PAYLOAD_VERSION,
            self,
            ORCHESTRATION_JOB_PAYLOAD_MAX_BYTES,
        )
        .map_err(|_| OrchestratorError::InvalidRunAdmission)
    }

    pub fn from_payload(
        payload: &insight_platform_contracts::TypedPayload,
    ) -> Result<Self, OrchestratorError> {
        if payload.schema_version != ORCHESTRATION_JOB_PAYLOAD_VERSION {
            return Err(OrchestratorError::InvalidRunAdmission);
        }
        let mut value = payload.value.clone();
        let version = value
            .as_object_mut()
            .and_then(|value| value.remove("schema_version"));
        if version.as_ref().and_then(serde_json::Value::as_i64)
            != Some(i64::from(ORCHESTRATION_JOB_PAYLOAD_VERSION))
        {
            return Err(OrchestratorError::InvalidRunAdmission);
        }
        let result: Self =
            serde_json::from_value(value).map_err(|_| OrchestratorError::InvalidRunAdmission)?;
...
pub struct AdmitRun {
    ...
    pub expected_agent_deployment: Option<ExactDeploymentRef>,
...
impl AdmitRun {
    pub fn validate_shape(&self) -> Result<(), OrchestratorError> {
        self.audit
            .validate_shape()
        ...
        if self
            ...
            .is_some_and(|exact| {
                exact.validate().is_err()
                    || exact.resource_kind != ResourceKind::AgentDeployment
                    || self.admission_scope_id.kind() != ResourceKind::Agent
            })
...
impl From<PlanError> for OrchestratorError {
    fn from(error: PlanError) -> Self {
        match error {
            PlanError::InvalidPlanNodeKey => Self::InvalidPlanNodeKey,
            PlanError::InvalidPlan => Self::InvalidPlan,
            PlanError::UnknownPlanNodeReference => Self::UnknownPlanNodeReference,
            PlanError::Canonicalization => Self::Canonicalization,
        }
    }
}
...
mod tests {
    ...
    fn value_schema(kind: &str) -> insight_platform_contracts::ClosedValueSchema {
        let integer = serde_json::json!({"type":"integer","minimum":-1000,"maximum":1000});
        let mut schema = match kind {
            "object" => {
                serde_json::json!({"type":"object","properties":{"answer":integer,"value":integer},"required":[],"additionalProperties":false})
            }
            "integer" => integer,
            "array" => {
                serde_json::json!({"type":"array","items":integer,"minItems":0,"maxItems":16})
            }
            "boolean" => serde_json::json!({"type":"boolean"}),
            _ => panic!("unknown fixture value kind"),
        };
    ...
    fn fixture_documents() -> BTreeMap<Sha256Digest, insight_platform_contracts::ClosedValueSchema>
    {
        ["object", "boolean", "integer", "array"]
            .into_iter()
            .map(value_schema)
    ...
    fn literal_program(value: serde_json::Value) -> TypedExpressionProgram {
        ...
        let schema_digest = value_schema(kind).canonical_digest;
    ...
    fn exact_port(node: &str, port: &str) -> ExactDataPortRef {
        let kind = match port {
            "predicate" | "unexpected" => "boolean",
            "first" | "second" | "item" => "integer",
            _ => "object",
        };
        ExactDataPortRef::NodeOutput {
            producer_node_id: key(node),
            port_id: DataPortKey::new(port.to_owned()).unwrap(),
            schema_digest: value_schema(kind).canonical_digest,
        }
    }
    ...
    fn runtime_plan_is_closed_bounded_and_digestible() {
        let plan = RuntimePlan {
            schema_documents: fixture_documents(),
            plan_version: 6,
        ...
        assert_eq!(obsolete.validate(limits()), Err(PlanError::InvalidPlan));
    ...
    fn external_leaf_contracts_are_exact_closed_and_bounded() {
        ...
        *capability_slot_id = "missing".to_owned();
        assert_eq!(plan.validate(limits()), Err(PlanError::InvalidPlan));
    }
    ...
    fn terminal_ports_are_exact_declared_and_reachable() {
        ...
        plan.validate(limits()).unwrap();
        plan.validate_terminal_schema_digests(&fixture_schema().canonical_digest, &digest('2'))
            .unwrap();
        assert_eq!(
        ...
        raised.nodes.insert(
            ...
            RuntimeNode::Raise {
                failure: ExactDataPortRef::RunInput {
                    schema_digest: value_schema("boolean").canonical_digest,
                },
            },
        );
        ...
        raised
            .validate_terminal_schema_digests(
                ...
                &value_schema("boolean").canonical_digest,
            )
            .unwrap();
        ...
        *value = exact_port("compute", "forged");
        assert_eq!(undeclared.validate(limits()), Err(PlanError::InvalidPlan));
        ...
        *next = key("other");
        assert_eq!(unreachable.validate(limits()), Err(PlanError::InvalidPlan));
    }
    ...
    fn loop_carried_ports_are_exact_schema_matched_and_body_owned() {
        ...
        assert_eq!(wrong_schema.validate(limits()), Err(PlanError::InvalidPlan));
        ...
        assert_eq!(outside_body.validate(limits()), Err(PlanError::InvalidPlan));
    }
    ...
    fn compute_assignments_are_exact_topological_and_digest_bound() {
        ...
        assert_eq!(branch.validate(limits()), Err(PlanError::InvalidPlan));
    }
        ...
        let increment = TypedExpressionProgram::build(
            ...
            vec![
                ...
                TypedInstruction::Literal {
                    value: ClosedJsonValue::build(
                        value_schema("integer").canonical_digest,
                        serde_json::json!(1),
                    )
                    .unwrap(),
                },
    ...
    fn child_ancestry_rejects_cycles_depth_and_unbounded_delegation() {
        ...
        let child_deployment = ExactDeploymentRef::new(
            id("adep_0198f1c5-0787-75e1-a9e8-d95ca0f37103"),
            fixture_schema().canonical_digest,
        )
        .unwrap();
