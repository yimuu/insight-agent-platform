...
use insight_engine::{
    ...
    PublicEventId, PublicEventKind, PublicEventPayload, RunId, RunLifecycle, TransitionKey,
    EXECUTION_EVENT_SCHEMA_VERSION, PUBLIC_EVENT_SCHEMA_VERSION,
};
...
pub(crate) fn project_terminal_tool_results(
    ...
    for call in &calls {
        let current = adapter::stored_succeeded_model_tool_call_order_key(call);
...
pub(crate) fn response_item_id(
    ...
    attempt_no: insight_engine::AttemptNo,
    model_call_no: u32,
) -> String {
    let identity = format!(
...
/// Workspace-internal surface for concrete storage adapters.
///
/// The DTOs below are defined exactly once in this module. Their crate-private
/// aliases above preserve the Phase 0 paths used by the durable authority,
/// while free functions keep the implementation helpers crate-private.
#[doc(hidden)]
pub mod adapter {
    use chrono::{DateTime, Utc};
    use serde::Serialize;
    ...
    pub struct StoredModelCallUsage {
    ...
    ///
    /// This type intentionally has no `Debug` implementation: both JSON
    /// values may contain executor-private material and must only cross the
    /// closed public projection performed by the durable authority.
    pub struct StoredSucceededModelToolCall {
        pub activation_id: String,
        pub attempt_no: u32,
        pub model_call_no: u32,
        pub call_index: u32,
    ...
    pub(super) fn stored_succeeded_model_tool_call_order_key(
        call: &StoredSucceededModelToolCall,
    ) -> (&str, u32, u32, u32) {
        call.order_key()
    }

    pub struct PreparedModelCallCompletion {
    ...
    pub struct PreparedResponseFunctionPublication {
        pub call_index: u32,
    ...
    pub struct StoredExecutionEventRow {
    ...
    /// witnesses observed at a repository boundary.
    pub struct StoredPublicProjectionDecision {
        pub run_id: String,
    ...
    /// payload authority.
    pub struct ValidatedInlinePayload {
    ...
    pub fn response_item_id(
        run_id: &RunId,
        activation_id: &ActivationId,
        attempt_no: AttemptNo,
        model_call_no: u32,
    ) -> String {
        super::response_item_id(run_id, activation_id, attempt_no, model_call_no)
    }

    pub fn function_call_response_item_id(
        run_id: &RunId,
        activation_id: &ActivationId,
        attempt_no: AttemptNo,
        model_call_no: u32,
        ...
        super::function_call_response_item_id(
            run_id,
            activation_id,
            attempt_no,
            model_call_no,
    ...
    pub fn prepare_model_call_completion(
        completion: &ModelCallCompletion,
        expected_item_id: Option<&str>,
        run_id: &RunId,
        activation_id: &ActivationId,
        ...
        super::prepare_model_call_completion(
            completion,
            expected_item_id,
            run_id,
            activation_id,
    ...
    pub fn validate_incomplete_function_call_item(
        ...
        super::validate_incomplete_function_call_item(value, expected_item_id)
    }

    pub fn canonical_intent_hash<T>(intent: &T) -> Result<IntentHash, RepositoryError>
    where
        T: Serialize + ?Sized,
    {
        super::canonical_intent_hash(intent)
    }

    pub fn canonical_json(value: &Value) -> Result<String, RepositoryError> {
        super::canonical_json(value)
    }

    pub fn decode_execution_event_schema_version(stored: i64) -> Result<(), RepositoryError> {
        super::decode_execution_event_schema_version(stored)
    }

    pub fn decode_stored_execution_event(
        stored: StoredExecutionEventRow,
    ) -> Result<ExecutionEventEnvelope, RepositoryError> {
        super::decode_stored_execution_event(stored)
    }

    pub fn decode_public_projection_decision(
        stored: StoredPublicProjectionDecision,
        execution: &ExecutionEventEnvelope,
    ) -> Result<Option<String>, RepositoryError> {
        super::decode_public_projection_decision(stored, execution)
    }

    pub fn durable_public_event_envelope(
        run_id: &RunId,
        public_event_id: &str,
        causation_event_id: &str,
        seq: u64,
        ...
        super::durable_public_event_envelope(
            run_id,
            public_event_id,
            causation_event_id,
            seq,
    ...
    pub fn wait_late_audit_identity(
        loser_kind: &'static str,
        run_id: &RunId,
        loser_id: &str,
    ) -> Result<(TransitionKey, IntentHash), RepositoryError> {
        super::wait_late_audit_identity(loser_kind, run_id, loser_id)
    }

    pub fn canonical_value(value: &Value) -> Result<(String, ContentHash), RepositoryError> {
        super::canonical_value(value)
    }

    pub fn validate_inline_payload(
    ...
    pub fn scheduler_checkpoint_content_hash(
        run_id: &str,
        checkpoint_id: &str,
        checkpoint_kind: &str,
        transition_key: &str,
        ...
        super::scheduler_checkpoint_content_hash(
            run_id,
            checkpoint_id,
            checkpoint_kind,
            transition_key,
    ...
    pub fn event_id(key: &TransitionKey) -> String {
        super::event_id(key)
    }

    pub const fn public_event_ordinal(kind: PublicEventKind) -> u16 {
        super::public_event_ordinal(kind)
    }

    pub fn public_event_id(run_id: &RunId, key: &TransitionKey, kind: PublicEventKind) -> String {
        super::public_event_id(run_id, key, kind)
    }

    pub fn task_id(key: &TransitionKey) -> String {
        super::task_id(key)
    }

    pub fn timer_id(key: &TransitionKey, purpose: &str) -> String {
        super::timer_id(key, purpose)
    }

    pub fn companion_transition_key(
        primary: &TransitionKey,
        role: &str,
    ) -> Result<TransitionKey, RepositoryError> {
        super::companion_transition_key(primary, role)
    }

    pub fn companion_event_intent_hash(
        primary_transition_key: &TransitionKey,
        primary_event_id: &str,
        role: &str,
        event: &PendingExecutionEvent,
    ) -> Result<IntentHash, RepositoryError> {
        super::companion_event_intent_hash(primary_transition_key, primary_event_id, role, event)
    }

    pub fn fencing_token(key: &TransitionKey) -> String {
        super::fencing_token(key)
    }

    pub fn payload_id(hash: &ContentHash) -> String {
        super::payload_id(hash)
    }

    pub fn i64_from_u64(value: u64) -> Result<i64, RepositoryError> {
        super::i64_from_u64(value)
    }

    pub fn u64_from_i64(value: i64) -> Result<u64, RepositoryError> {
        super::u64_from_i64(value)
    }

    pub fn parse_run_lifecycle(value: &str) -> Result<RunLifecycle, RepositoryError> {
        super::parse_run_lifecycle(value)
    }

    pub fn parse_admission_state(value: &str) -> Result<AdmissionState, RepositoryError> {
        super::parse_admission_state(value)
    }

    pub fn validated_inline_payload_value(payload: &ValidatedInlinePayload) -> &Value {
        payload.value()
    }

    pub fn validated_inline_payload_canonical(payload: &ValidatedInlinePayload) -> &str {
        payload.canonical()
    }

    pub fn validated_inline_payload_content_hash(payload: &ValidatedInlinePayload) -> &ContentHash {
        payload.content_hash()
    }

    pub const fn validated_inline_payload_canonical_bytes(payload: &ValidatedInlinePayload) -> u64 {
...
pub(crate) const fn public_event_ordinal(kind: insight_engine::PublicEventKind) -> u16 {
    match kind {
        insight_engine::PublicEventKind::RunCreated => 10,
        insight_engine::PublicEventKind::RunStarted => 20,
        insight_engine::PublicEventKind::OperationStarted => 30,
        insight_engine::PublicEventKind::OperationCompleted
        | insight_engine::PublicEventKind::OperationFailed => 40,
        insight_engine::PublicEventKind::RunCompleted
        | insight_engine::PublicEventKind::RunFailed
        | insight_engine::PublicEventKind::RunCancelled
        | insight_engine::PublicEventKind::RunInterrupted => 50,
    }
}
...
pub(crate) fn public_event_id(
    run_id: &RunId,
    key: &TransitionKey,
    kind: insight_engine::PublicEventKind,
) -> String {
