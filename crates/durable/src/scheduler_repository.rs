...
use insight_engine::EffectIdempotency;
...
pub(crate) fn scheduler_success_validation_fixture(
    ...
    scheduler_validation_fixture(EffectIdempotency::Idempotent, 1, claim_expires_at)
}

/// Workspace-internal construction surface for cross-crate conformance tests.
#[doc(hidden)]
pub mod adapter {
    ...
    use insight_engine::{
        plan::DataPortId,
        worker::{TaskExecutionResult, WorkerFailureClass},
        EffectEvidence, EffectIdempotency, ValueRef,
    };

    use super::{
        ActivationId, AttemptNo, DateTime, DurableTaskExecutionRequest, FencedSchedulerRunCommand,
        LeaseEpoch, PlanType, PlannedSchedulerAction, RepositoryError, RunId, RuntimeValue,
        SchedulerCheckpointId, SchedulerCommitReceipt, SchedulerDurableRepository,
        SchedulerFailureDisposition, SchedulerStoredValue, SchedulerTaskClaim,
        SchedulerTaskClaimMode, SchedulerTaskCompletionReceipt, SchedulerTaskFailure,
        SchedulerTaskSuccess, TaskExecutionRequest, Utc, WorkerFailure,
    };

    pub const WORKER_DEADLINE_EXCEEDED: &str = super::SCHEDULER_WORKER_DEADLINE_EXCEEDED;

    pub trait SchedulerCommitReceiptAdapter: Sized {
        fn new(
            event_seq: u64,
            event_id: String,
            checkpoint_id: SchedulerCheckpointId,
    ...
    impl SchedulerCommitReceiptAdapter for SchedulerCommitReceipt {
        fn new(
            event_seq: u64,
            event_id: String,
            checkpoint_id: SchedulerCheckpointId,
    ...
    pub trait DurableTaskExecutionRequestAdapter: Sized {
        fn new(
            request: TaskExecutionRequest,
            attempt_no: AttemptNo,
            lease_epoch: LeaseEpoch,
    ...
    impl DurableTaskExecutionRequestAdapter for DurableTaskExecutionRequest {
        fn new(
            request: TaskExecutionRequest,
            attempt_no: AttemptNo,
            lease_epoch: LeaseEpoch,
    ...
    pub trait SchedulerTaskClaimModeAdapter: Sized {
        fn as_str(&self) -> &'static str;
        fn parse(value: &str) -> Result<Self, RepositoryError>;
    }

    impl SchedulerTaskClaimModeAdapter for SchedulerTaskClaimMode {
        fn as_str(&self) -> &'static str {
            SchedulerTaskClaimMode::as_str(*self)
        }
        fn parse(value: &str) -> Result<Self, RepositoryError> {
            SchedulerTaskClaimMode::parse(value)
        }
    }

    pub trait SchedulerTaskClaimAdapter: Sized {
        fn new(
            envelope: DurableTaskExecutionRequest,
            claimed_by: String,
            claim_token: String,
        ...
        fn with_lease_loss_evidence(self, evidence: EffectEvidence) -> Self;
    }

    impl SchedulerTaskClaimAdapter for SchedulerTaskClaim {
        fn new(
            envelope: DurableTaskExecutionRequest,
            claimed_by: String,
            claim_token: String,
        ...
        fn with_lease_loss_evidence(self, evidence: EffectEvidence) -> Self {
            SchedulerTaskClaim::with_lease_loss_evidence(self, evidence)
        }
    }

    pub fn validate_runtime_value_ref(
        runtime_value: &RuntimeValue,
        value_ref: &ValueRef,
    ) -> Result<(), RepositoryError> {
        super::validate_runtime_value_ref(runtime_value, value_ref)
    }

    pub trait SchedulerStoredValueAdapter: Sized {
        #[allow(clippy::too_many_arguments)]
        fn new(
            run_id: RunId,
            port_id: DataPortId,
    ...
    impl SchedulerStoredValueAdapter for SchedulerStoredValue {
        fn new(
            run_id: RunId,
            port_id: DataPortId,
            owner_activation_id: ActivationId,
    ...
    pub trait SchedulerTaskSuccessAdapter {
        fn validate_value_ref_integrity(&self) -> Result<(), RepositoryError>;
    }

    impl SchedulerTaskSuccessAdapter for SchedulerTaskSuccess {
        fn validate_value_ref_integrity(&self) -> Result<(), RepositoryError> {
            SchedulerTaskSuccess::validate_value_ref_integrity(self)
        }
    }

    pub trait SchedulerTaskFailureAdapter: Sized {
        fn from_worker_failure(
            claim: &SchedulerTaskClaim,
            failure: &WorkerFailure,
            effect_evidence: EffectEvidence,
            disposition: SchedulerFailureDisposition,
        ) -> Result<Self, RepositoryError>;
        fn from_runtime_deadline(claim: &SchedulerTaskClaim) -> Result<Self, RepositoryError>;
        fn validate_for_authority(
            &self,
            claim: &SchedulerTaskClaim,
            current_effect_evidence: EffectEvidence,
        ) -> Result<(), RepositoryError>;
        fn is_runtime_deadline(&self) -> bool;
    }

    impl SchedulerTaskFailureAdapter for SchedulerTaskFailure {
        fn from_worker_failure(
            claim: &SchedulerTaskClaim,
            failure: &WorkerFailure,
            effect_evidence: EffectEvidence,
            ...
            SchedulerTaskFailure::from_worker_failure(claim, failure, effect_evidence, disposition)
        }
        ...
        fn validate_for_authority(
            ...
            SchedulerTaskFailure::validate_for_authority(self, claim, current_effect_evidence)
        }

        fn is_runtime_deadline(&self) -> bool {
            SchedulerTaskFailure::is_runtime_deadline(self)
        }
    }

    pub trait SchedulerTaskCompletionReceiptAdapter: Sized {
        fn new(
            commit: SchedulerCommitReceipt,
            task_id: super::SchedulerTaskId,
            attempt_no: AttemptNo,
            lease_epoch: LeaseEpoch,
    ...
    impl SchedulerTaskCompletionReceiptAdapter for SchedulerTaskCompletionReceipt {
        fn new(
            commit: SchedulerCommitReceipt,
            task_id: super::SchedulerTaskId,
            attempt_no: AttemptNo,
    ...
    pub async fn validate_subflow_admission<R: SchedulerDurableRepository + ?Sized>(
        repository: &R,
        fence: &FencedSchedulerRunCommand,
        action: &PlannedSchedulerAction,
    ) -> Result<(), RepositoryError> {
        super::validate_subflow_admission(repository, fence, action).await
    }

    pub fn scheduler_validation_fixture(
        effect_idempotency: EffectIdempotency,
        max_attempts: u32,
        claim_expires_at: DateTime<Utc>,
    ) -> (SchedulerTaskClaim, DataPortId, TaskExecutionResult) {
        super::scheduler_validation_fixture(effect_idempotency, max_attempts, claim_expires_at)
    }

    pub fn scheduler_success_validation_fixture(
        claim_expires_at: DateTime<Utc>,
    ) -> (SchedulerTaskClaim, DataPortId, TaskExecutionResult) {
        super::scheduler_success_validation_fixture(claim_expires_at)
    }

    pub fn scheduler_task_success_unchecked(
        result: TaskExecutionResult,
        value_refs: BTreeMap<DataPortId, ValueRef>,
    ) -> SchedulerTaskSuccess {
        SchedulerTaskSuccess::unchecked_for_test(result, value_refs)
    }

    #[allow(clippy::too_many_arguments)]
    pub fn scheduler_task_failure_unchecked(
        claim: &SchedulerTaskClaim,
...
mod tests {
    ...
    use insight_engine::{
        ...
        worker::{TaskExecutionResult, WorkerFailure, WorkerFailureClass},
        ActivationId, ArtifactId, ArtifactRef, ContentHash, EffectEvidence, EffectIdempotency,
        RunId, ValueRef,
    };

    fn fixture_claim_expires_at() -> chrono::DateTime<chrono::Utc> {
    ...
    fn durable_task_failure_revalidates_safe_error_class_and_code_authorities() {
        let (claim, _, _) = super::scheduler_validation_fixture(
        ...
        .is_err());
        let (idempotent_claim, _, _) = super::scheduler_validation_fixture(
    ...
    fn durable_task_envelope_deserialize_validates_schema_and_lease_fence() {
        let (claim, _, _) = super::scheduler_success_validation_fixture(fixture_claim_expires_at());
