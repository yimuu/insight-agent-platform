...
use insight_engine::{worker::ModelContinuationTurn, EffectEvidence};
...
pub mod adapter {
    ...
    pub type LatestParentModelCallStatuses<'a> =
        (&'a str, Option<&'a str>, Option<&'a str>, Option<&'a str>);

    fn latest_from_statuses(
        call_status: &str,
        finish_reason: Option<&str>,
        execution_status: Option<&str>,
        continuation_status: Option<&str>,
    ) -> LatestParentModelCall {
        LatestParentModelCall {
            model_call_no: 0,
            task_id: String::new(),
            lease_epoch: 0,
            fencing_token: String::new(),
            call_status: call_status.to_owned(),
            finish_reason: finish_reason.map(str::to_owned),
            execution_status: execution_status.map(str::to_owned),
            continuation_status: continuation_status.map(str::to_owned),
        }
    }

    pub fn classify_parent_task_claim_statuses(
        task_state: &str,
        attempt_lifecycle: &str,
        effect_evidence: EffectEvidence,
        latest: Option<LatestParentModelCallStatuses<'_>>,
    ) -> &'static str {
        let latest = latest.map(
            |(call_status, finish_reason, execution_status, continuation_status)| {
                latest_from_statuses(
                    call_status,
                    finish_reason,
                    execution_status,
                    continuation_status,
        ...
        super::classify_parent_task_claim(
            task_state,
            attempt_lifecycle,
            effect_evidence,
            latest.as_ref(),
    ...
    pub fn latest_parent_model_call_is_waiting_tools(
        call_status: &str,
        finish_reason: Option<&str>,
        execution_status: Option<&str>,
        continuation_status: Option<&str>,
    ...
    pub fn latest_parent_model_call_is_checkpointed(
        call_status: &str,
        finish_reason: Option<&str>,
        execution_status: Option<&str>,
        continuation_status: Option<&str>,
    ...
    pub fn latest_parent_model_call_is_ready(
        call_status: &str,
        finish_reason: Option<&str>,
        execution_status: Option<&str>,
        continuation_status: Option<&str>,
    ...
    pub fn model_tool_parent_resume_activate_checkpointed(
        model_call_no: u32,
        operation_deadline: DateTime<Utc>,
    ) -> Result<ModelToolParentResume, RepositoryError> {
        ModelToolParentResume::activate_checkpointed(model_call_no, operation_deadline)
    }

    pub fn model_tool_parent_resume_ready_continue(
        turns: Vec<ModelContinuationTurn>,
        operation_deadline: DateTime<Utc>,
    ) -> Result<ModelToolParentResume, RepositoryError> {
    ...
    pub fn model_tool_parent_resume_ready_failed(
        completed_model_call_no: u32,
        operation_deadline: DateTime<Utc>,
        effect_outcome_unknown: bool,
    ) -> Result<ModelToolParentResume, RepositoryError> {
        ModelToolParentResume::ready_failed(
            completed_model_call_no,
            operation_deadline,
            effect_outcome_unknown,
        )
    ...
    pub fn model_tool_parent_resume_ready_cancelled(
        completed_model_call_no: u32,
        operation_deadline: DateTime<Utc>,
    ) -> Result<ModelToolParentResume, RepositoryError> {
        ModelToolParentResume::ready_cancelled(completed_model_call_no, operation_deadline)
    }
}
...
mod tests {
    ...
    use insight_engine::worker::{ModelContinuationTurn, ModelToolCall, ModelToolResult};
