...
impl ClosedJsonSchema {
    ...
    #[cfg(feature = "runtime-schema-validation")]
    pub fn validate_instance(&self, value: &Value) -> Result<(), SchemaProfileError> {
        self.validate()?;
        validate_exact_schema_instance(&self.schema, value)
    }
}

/// Internal value ports may contain scalars or arrays. Public object inputs keep
/// their separate closed-object profile and are never widened by this type.
pub const CLOSED_VALUE_SCHEMA_PROFILE_ID: &str = "insight.platform/closed-value-schema/v1";
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ClosedValueSchema {
    pub schema_version: u32,
    pub profile: String,
    pub schema: Value,
    pub canonical_digest: Sha256Digest,
}
impl ClosedValueSchema {
    /// Portable authoring preflight. This checks the declared value shape; execution still
    /// validates all JSON Schema assertions against the exact frozen document.
    pub fn validate_literal_shape(&self, value: &Value) -> Result<(), SchemaProfileError> {
        self.validate()?;
        if literal_matches_shape(&self.schema, &self.schema, value, 0) {
            Ok(())
        } else {
            Err(SchemaProfileError::new(
                "$",
                "closed_value_literal_shape",
                "literal does not match the declared value shape",
            ))
        }
    }
    pub fn build(schema: Value) -> Result<Self, SchemaProfileError> {
        validate_closed_schema_document_size(&schema)?;
        validate_schema_document(&schema, false)?;
        Ok(Self {
            schema_version: CLOSED_SCHEMA_DOCUMENT_VERSION,
            profile: CLOSED_VALUE_SCHEMA_PROFILE_ID.to_owned(),
            canonical_digest: schema_digest(&schema)?,
            schema,
        })
    }
    pub fn validate(&self) -> Result<(), SchemaProfileError> {
        if self.schema_version != CLOSED_SCHEMA_DOCUMENT_VERSION
            || self.profile != CLOSED_VALUE_SCHEMA_PROFILE_ID
            || schema_digest(&self.schema)? != self.canonical_digest
        {
            return Err(SchemaProfileError::new(
                "$",
                "closed_value_schema_binding",
                "value schema version, profile or digest is invalid",
            ));
        }
        validate_closed_schema_document_size(&self.schema)?;
        validate_schema_document(&self.schema, false)
    }
    #[cfg(feature = "runtime-schema-validation")]
    pub fn validate_instance(&self, value: &Value) -> Result<(), SchemaProfileError> {
        self.validate()?;
...
fn literal_matches_shape(root: &Value, schema: &Value, value: &Value, depth: usize) -> bool {
    // Local reference cycles are rejected by schema validation. The bound also prevents a
    // valid but deeply nested document from exhausting a compiler's native or WASM stack.
    if depth > 128 {
        return false;
    }
    if let Some(reference) = schema.get("$ref").and_then(Value::as_str) {
        let referenced = if let Some(name) = reference.strip_prefix("#/$defs/") {
            root.get("$defs").and_then(|defs| defs.get(name)).cloned()
        } else {
            crate::nominal::schema_for_pinned_nominal_reference(reference)
        };
        let Some(referenced) = referenced else {
            return false;
        };
        if !literal_matches_shape(root, &referenced, value, depth + 1) {
            return false;
        }
    }
    if let Some(branches) = schema.get("oneOf").and_then(Value::as_array) {
        // Branches can have the same object shape and differ only in a discriminator.
        // Exact oneOf assertion validation remains an execution-boundary responsibility.
        if !branches
            .iter()
            .any(|branch| literal_matches_shape(root, branch, value, depth + 1))
        {
            return false;
        }
    }
    match schema.get("type").and_then(Value::as_str) {
        Some("object") => {
            ...
            object.iter().all(|(name, value)| {
                properties.get(name).map_or(
                    schema.get("additionalProperties") != Some(&Value::Bool(false)),
                    |property| literal_matches_shape(root, property, value, depth + 1),
                )
            })
        }
        Some("array") => value.as_array().is_some_and(|items| {
            schema.get("items").is_some_and(|item_schema| {
                items
                    .iter()
                    .all(|item| literal_matches_shape(root, item_schema, item, depth + 1))
            })
        }),
...
impl TryFrom<ClosedJsonSchema> for ClosedValueSchema {
    type Error = SchemaProfileError;
    fn try_from(value: ClosedJsonSchema) -> Result<Self, Self::Error> {
        value.validate()?;
        Self::build(value.schema)
    }
}
impl TryFrom<ClosedValueSchema> for ClosedJsonSchema {
    type Error = SchemaProfileError;
    fn try_from(value: ClosedValueSchema) -> Result<Self, Self::Error> {
        value.validate()?;
        Self::build(value.schema)
    }
}
#[cfg(feature = "runtime-schema-validation")]
fn validate_exact_schema_instance(schema: &Value, value: &Value) -> Result<(), SchemaProfileError> {
    let validation_schema = inline_pinned_nominals(schema, 0)?;
...
#[cfg(feature = "runtime-schema-validation")]
fn inline_pinned_nominals(schema: &Value, depth: usize) -> Result<Value, SchemaProfileError> {
    if depth > 128 {
        return Err(SchemaProfileError::new(
            "$",
            "closed_schema_depth",
            "schema expansion exceeds its bound",
        ));
    }
    let mut expanded = schema.clone();
    if let Some(reference) = schema.get("$ref").and_then(Value::as_str) {
        if !reference.starts_with("#/$defs/") {
            // The owning nominal registry is the sole resolver. No network or filesystem
            // retrieval is needed, including for references nested in properties or arrays.
            let nominal = crate::nominal::schema_for_pinned_nominal_reference(reference)
                .ok_or_else(|| {
                    SchemaProfileError::new(
                        "$",
                        "schema_ref_forbidden",
                        "unknown nominal reference",
                    )
                })?;
            let siblings = expanded.as_object_mut().ok_or_else(|| {
                SchemaProfileError::new("$", "schema_node", "invalid schema node")
            })?;
            siblings.remove("$ref");
            let mut document = nominal.as_object().cloned().ok_or_else(|| {
                SchemaProfileError::new("$", "schema_node", "invalid nominal node")
            })?;
            // Siblings of $ref are conjunctive in Draft 2020-12; keep both constraints.
            if siblings.is_empty() {
                expanded = Value::Object(document);
            } else {
                document.remove("$id");
                expanded = serde_json::json!({"allOf":[Value::Object(document),Value::Object(siblings.clone())]});
            }
        }
    }
        ...
        for keyword in ["properties", "$defs"] {
            if let Some(children) = object.get_mut(keyword).and_then(Value::as_object_mut) {
                for child in children.values_mut() {
                    *child = inline_pinned_nominals(child, depth + 1)?;
                }
            }
        }
        if let Some(items) = object.get_mut("items") {
            *items = inline_pinned_nominals(items, depth + 1)?;
        }
        for keyword in ["oneOf", "allOf"] {
            if let Some(branches) = object.get_mut(keyword).and_then(Value::as_array_mut) {
                for branch in branches {
                    *branch = inline_pinned_nominals(branch, depth + 1)?;
                }
            }
        }
    }
...
pub fn validate_closed_schema(schema: &Value) -> Result<(), SchemaProfileError> {
    validate_schema_document(schema, true)
}
fn validate_schema_document(schema: &Value, object_input: bool) -> Result<(), SchemaProfileError> {
...
fn validate_local_references(root: &Map<String, Value>) -> Result<(), SchemaProfileError> {
    ...
    let mut document_references = BTreeSet::new();
    collect_local_references(&Value::Object(root.clone()), &mut document_references);
    for reference in &document_references {
        if !definitions.is_some_and(|definitions| definitions.contains_key(reference)) {
            return Err(SchemaProfileError::new(
                "$",
                "schema_ref_missing",
...
fn collect_local_references(value: &Value, references: &mut BTreeSet<String>) {
    ...
    // Only schema-valued positions participate in the reference graph. Objects
    // inside const/enum are instance data, even when a field is named `$ref`.
    for keyword in ["properties", "$defs"] {
    ...
    if let Some(items) = object.get("items") {
        collect_local_references(items, references);
    }
    if let Some(branches) = object.get("oneOf").and_then(Value::as_array) {
        for branch in branches {
            collect_local_references(branch, references);
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn internal_value_schema_preserves_object_boundaries_and_exact_digest() {
        let object = ClosedJsonSchema::build(valid_schema()).unwrap();
        let value = ClosedValueSchema::try_from(object.clone()).unwrap();
        assert_eq!(value.canonical_digest, object.canonical_digest);
        assert_ne!(value.profile, object.profile);
        assert_eq!(ClosedJsonSchema::try_from(value).unwrap(), object);
        let scalar =
            json!({"$schema":"https://json-schema.org/draft/2020-12/schema","type":"boolean"});
        assert!(ClosedJsonSchema::build(scalar.clone()).is_err());
        let value = ClosedValueSchema::build(scalar).unwrap();
        assert!(ClosedJsonSchema::try_from(value.clone()).is_err());
        let mut altered = value;
        altered.schema["type"] = json!("integer");
        assert!(altered.validate().is_err());
        assert!(ClosedValueSchema::build(
    ...
    #[test]
    fn internal_value_literal_shape_is_portable_and_closed() {
        let object = ClosedValueSchema::build(valid_schema()).unwrap();
        assert!(object.validate_literal_shape(&json!({"count":1})).is_ok());
        assert!(object
            .validate_literal_shape(&json!({"count":true}))
            .is_err());
        assert!(object
            .validate_literal_shape(&json!({"count":1,"unexpected":false}))
            .is_err());
        assert!(object.validate_literal_shape(&json!({})).is_err());
        let scalar = ClosedValueSchema::build(
            json!({"$schema":"https://json-schema.org/draft/2020-12/schema","type":"boolean"}),
        )
        .unwrap();
        assert!(scalar.validate_literal_shape(&json!(true)).is_ok());
        assert!(scalar.validate_literal_shape(&json!(1)).is_err());
    }
    #[cfg(feature = "runtime-schema-validation")]
    #[test]
    fn nested_nominal_references_are_exact_and_resolved_locally() {
        let schema=ClosedJsonSchema::build(json!({"$schema":"https://json-schema.org/draft/2020-12/schema","type":"object","properties":{"digest":{"$ref":crate::nominal::pinned_nominal_reference("Digest").unwrap()}},"required":["digest"],"additionalProperties":false})).unwrap();
        assert!(schema
            .validate_instance(&json!({"digest":format!("sha256:{}","a".repeat(64))}))
            .is_ok());
        assert!(schema
            .validate_instance(&json!({"digest":"untyped"}))
            .is_err());
        assert!(schema.validate_instance(&json!({"digest":false})).is_err());
    }
    #[cfg(feature = "runtime-schema-validation")]
    #[test]
    fn internal_value_schema_checks_scalar_and_collection_instances() {
        let scalar = ClosedValueSchema::build(
            json!({"$schema":"https://json-schema.org/draft/2020-12/schema","type":"boolean"}),
        )
        .unwrap();
        assert!(scalar.validate_instance(&json!(true)).is_ok());
        assert!(scalar.validate_instance(&json!({})).is_err());
        let array=ClosedValueSchema::build(json!({"$schema":"https://json-schema.org/draft/2020-12/schema","type":"array","minItems":0,"maxItems":2,"items":{"type":"integer","minimum":0,"maximum":10}})).unwrap();
        assert!(array.validate_instance(&json!([1, 2])).is_ok());
        assert!(array.validate_instance(&json!([1, 2, 3])).is_err());
        assert!(array.validate_instance(&json!([true])).is_err());
    }
    ...
    #[test]
    fn literal_ref_fields_do_not_create_schema_dependencies() {
        let schema = json!({
            "$schema": "https://json-schema.org/draft/2020-12/schema",
            "type": "object", "properties": {"value": {"$ref": "#/$defs/value"}},
            "required": ["value"], "additionalProperties": false,
            "$defs": {"value": {
                "type": "object", "properties": {"$ref": {"type": "string", "minLength": 0, "maxLength": 64, "x-platform-max-bytes": 64}},
                "required": ["$ref"], "additionalProperties": false,
                "const": {"$ref": "#/$defs/nonexistent"},
                "enum": [{"$ref": "#/$defs/nonexistent"}]
            }}
        });
        validate_closed_schema(&schema).unwrap();
        let mut references = BTreeSet::new();
        collect_local_references(&schema, &mut references);
        assert_eq!(references, BTreeSet::from(["value".to_owned()]));
        let mut cyclic = schema;
        cyclic["$defs"]["value"]["properties"]["next"] = json!({"$ref":"#/$defs/value"});
        assert_eq!(
            validate_closed_schema(&cyclic).unwrap_err().code,
            "schema_ref_cycle"
        );
    }

    #[test]
    fn missing_local_references_are_rejected_in_all_schema_positions() {
        for property in [
            json!({"$ref":"#/$defs/missing"}),
            json!({"type":"array","minItems":0,"maxItems":2,"items":{"$ref":"#/$defs/missing"}}),
            json!({"type":"object","additionalProperties":false,"properties":{"nested":{"$ref":"#/$defs/missing"}},"required":[]}),
        ] {
            let schema = json!({"$schema":"https://json-schema.org/draft/2020-12/schema","type":"object","properties":{"value":property},"required":[],"additionalProperties":false});
            assert_eq!(
                validate_closed_schema(&schema).unwrap_err().code,
                "schema_ref_missing"
            );
