...
/// Exact logical authoring input; publication supplies the new owner identities.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
...
impl ContextBindingSnapshot {
    ...
    pub fn validate(&self) -> Result<(), ContextContractError> {
        ContextBindingInputV1 {
            context_deployment: self.context_deployment.clone(),
            consistency: self.consistency.clone(),
            allowed_projection: self.allowed_projection.clone(),
            authorization_policy: self.authorization_policy.clone(),
            ranking_policy: self.ranking_policy.clone(),
        }
        .validate()?;
