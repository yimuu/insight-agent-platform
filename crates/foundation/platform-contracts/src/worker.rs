...
pub struct WorkerManifest {
    ...
    /// SHA-256 of the exact native worker executable bytes in both containers and
    /// local development. Deployment attestation binds that binary to its image;
    /// an OCI manifest, source revision, version string or this manifest's digest
    /// is not a worker binary digest.
    pub worker_build_digest: Sha256Digest,
    pub execution_capabilities: crate::WorkerExecutionCapabilities,
...
impl WorkerManifest {
    pub fn validate(&self) -> Result<(), WorkerManifestError> {
        ...
        self.execution_capabilities.validate().map_err(|_| {
            invalid(
                "execution_capabilities",
                "must declare a bounded supported execution catalog",
            )
        })?;
...
mod tests {
    ...
    fn manifest() -> WorkerManifest {
        WorkerManifest {
            ...
            execution_capabilities: crate::WorkerExecutionCapabilities {
                schema_version: crate::EXECUTION_REQUIREMENT_VERSION,
                capabilities: vec![crate::WorkerExecutionCapability::Program {
                    program_semantic_identity: digest('b'),
                    ir_abi_version: 5,
                }],
            },
