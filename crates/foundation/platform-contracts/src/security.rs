...
/// Narrows existing tenant/Task permissions; it never grants authorization.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(tag = "kind", rename_all = "snake_case", deny_unknown_fields)]
pub enum TaskEligibilityRule {
    AnyAuthorized,
    Creator,
    ExactPrincipal { principal_id: ResourceId },
}

impl TaskEligibilityRule {
    pub fn validate(&self) -> Result<(), SecurityContractError> {
        if matches!(self, Self::ExactPrincipal { principal_id } if principal_id.kind() != ResourceKind::Principal)
        {
            return Err(SecurityContractError::InvalidPrincipalScope);
        }
        Ok(())
    }
    pub fn canonical_digest(&self) -> Result<Sha256Digest, SecurityContractError> {
        self.validate()?;
        canonical_digest(&serde_json::json!({"schema_version":1,"rule":self}))
            .map_err(|_| SecurityContractError::InvalidPrincipalScope)?
            .parse()
            .map_err(|_| SecurityContractError::InvalidPrincipalScope)
    }
    pub fn permits(&self, admitted_creator: &ResourceId, current_principal: &ResourceId) -> bool {
        self.validate().is_ok()
            && match self {
                Self::AnyAuthorized => true,
                Self::Creator => admitted_creator == current_principal,
                Self::ExactPrincipal { principal_id } => principal_id == current_principal,
            }
    }
}
...
/// Current restricted-content permission gate shared by Inline and Artifact exits.
/// Callers resolve the current principal and exact owner; this pure gate cannot
/// be used to grant business dispatch or completion authority.
pub fn permits_content_disclosure(
    principal: &PrincipalSnapshot,
    purpose: crate::ExecutionAuthorizationPurpose,
) -> bool {
    purpose == crate::ExecutionAuthorizationPurpose::ContentDisclosure
        && principal.validate().is_ok()
        && principal
            .permissions
            .contains(crate::Permission::ArtifactRead)
}
