...
pub const MAX_AGENT_DEPLOYMENT_FEATURE_EVIDENCE: usize = 1024;

/// A safe projection of an exact published deployment, independently reproduced at validation.
/// The input is not a client authorization or proof of currently installed worker availability.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct AgentDeploymentFeaturesV1 {
...
pub struct ValidationSummary {
    ...
    pub program_requirement: Option<crate::ExecutionRequirement>,
    ...
    pub fn validate(&self) -> Result<(), ResourceContractError> {
        if self
            ...
            .is_some_and(|requirement| {
                !matches!(requirement, crate::ExecutionRequirement::Program { .. })
...
impl PublishedVersionPayload {
    pub fn validate_for(
        ...
        match (&self.document, &self.validation.program_requirement) {
            (
                ResourceDocument::Agent(agent),
                Some(crate::ExecutionRequirement::Program {
                    definition_digest, ..
                }),
            ) if &agent.typed_plan_digest == definition_digest => {}
            (ResourceDocument::Agent(_), _) | (_, Some(_)) => {
...
#[serde(deny_unknown_fields)]
pub struct AgentSlotBindingInputV1 {
    pub slot_id: String,
    pub requirement_digest: Sha256Digest,
    pub target: AgentSlotTargetInputV1,
}

#[derive(Debug, Clone, PartialEq, Eq, Deserialize, Serialize)]
#[serde(tag = "kind", rename_all = "snake_case", deny_unknown_fields)]
pub enum AgentSlotTargetInputV1 {
    Model {
    ...
    Context {
        binding: Box<crate::ContextBindingInputV1>,
    },
    ChildAgent {
        candidates: Vec<ExactDeploymentRef>,
        selection_policy: ExactPolicyBinding,
...
impl AgentSlotBindingInputV1 {
    pub fn materialize(
        self,
        deployment_id: &ResourceId,
        context_binding_ids: &mut impl Iterator<Item = ResourceId>,
    ) -> Result<FrozenSlotBinding, ResourceContractError> {
        let target = match self.target {
            ...
            AgentSlotTargetInputV1::Context { binding } => {
                let context_binding_id = context_binding_ids
                    .next()
                    .ok_or(ResourceContractError::InvalidAgentContract)?;
            ...
            AgentSlotTargetInputV1::ChildAgent {
                candidates,
                selection_policy,
        ...
        let binding_digest =
            FrozenSlotBinding::binding_digest_for(&self.slot_id, &self.requirement_digest, &target)
                .map_err(|_| ResourceContractError::InvalidAgentContract)?;
    ...
    pub fn validate(&self) -> Result<(), ResourceContractError> {
        ...
        match &self.target {
            ...
            AgentSlotTargetInputV1::Capability {
                ...
                selection_policy.validate()?;
                if tool_alias.as_ref().is_some_and(|value| !is_code(value)) {
                    return Err(ResourceContractError::InvalidCode);
                }
                Ok(())
            }
            AgentSlotTargetInputV1::Context { binding } => binding
                .validate()
                .map_err(|_| ResourceContractError::InvalidContextContract),
            AgentSlotTargetInputV1::ChildAgent {
                candidates,
...
impl FrozenSlotBinding {
    pub fn expected_binding_digest(&self) -> Result<Sha256Digest, ResourceContractError> {
        Self::binding_digest_for(&self.slot_id, &self.requirement_digest, &self.target)
    }
    pub fn binding_digest_for(
        slot_id: &str,
        requirement_digest: &Sha256Digest,
        target: &FrozenSlotTarget,
    ) -> Result<Sha256Digest, ResourceContractError> {
        canonical_digest(&serde_json::json!({"slot_id":slot_id,"requirement_digest":requirement_digest,"target":target}))
            .map_err(|_| ResourceContractError::Canonicalization)?.parse().map_err(|_| ResourceContractError::Canonicalization)
    }
    pub fn validate(&self) -> Result<(), ResourceContractError> {
        if !is_code(&self.slot_id) {
            return Err(ResourceContractError::InvalidCode);
