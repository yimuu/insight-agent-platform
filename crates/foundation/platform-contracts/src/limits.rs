...
impl HardLimitProfile {
    pub fn validate(&self) -> Result<(), LimitProfileError> {
        ...
        if failure.is_none()
            && (self.control_data.outbox_admission_backlog.unit != LimitUnit::Count
                || self.control_data.outbox_admission_backlog.hard_max
                    > u64::from(crate::MAX_OUTBOX_BACKLOG_SCAN)
                || self.control_data.outbox_admission_backlog.overflow_outcome
                    != OverflowOutcome::TemporarilyUnavailable)
        {
            failure = Some(LimitProfileError { field: "control_data.outbox_admission_backlog".to_owned(), message: "Outbox admission must use bounded obligation counts and temporary unavailability" });
        }
