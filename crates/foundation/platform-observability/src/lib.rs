...
pub struct OrchestrationOperationalSnapshot {
    ...
    pub recovery_invalid_objects: u64,
...
pub struct OrchestrationOperationalMetrics {
    ...
    recovery_invalid_objects: AtomicU64,
    ...
    durable_outbox_incompatible_events: AtomicU64,
}
...
impl OrchestrationOperationalMetrics {
    pub fn update(&self, snapshot: OrchestrationOperationalSnapshot) {
        ...
        self.recovery_invalid_objects
            .store(snapshot.recovery_invalid_objects, Ordering::Release);
    ...
    pub fn observe_durable_outbox(&self, snapshot: DurableOutboxSnapshot) {
        ...
        self.durable_outbox_incompatible_events
            .store(snapshot.incompatible_events, Ordering::Release);
    ...
    fn render_prometheus(&self, role: &str, output: &mut String) {
        ...
        for (outcome, value) in [
            ...
            (
                "invalid_object",
                self.recovery_invalid_objects.load(Ordering::Acquire),
            ),
            (
                "capacity_skipped",
...
mod tests {
    ...
    fn orchestration_metrics_export_only_fixed_operational_dimensions() {
        ...
        assert!(rendered.contains("outcome=\"invalid_object\"} 3"));
        ...
        assert!(rendered.contains("queue=\"incompatible\"} 1"));
