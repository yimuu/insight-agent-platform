...
impl McpAuthorizationContext {
    pub fn build(
        now: DateTime<Utc>,
        mut input: NewMcpAuthorizationContext,
...
impl McpHostExecutionContract {
    pub fn build(
        now: DateTime<Utc>,
        input: NewMcpHostExecutionContract,
    ) -> Result<Self, McpHostError> {
...
impl McpOperationRequest {
    ...
    pub fn safe_to_retry_after_unknown(&self) -> bool {
        match self.effect {
...
pub fn digest<T: Serialize>(value: &T) -> Result<Sha256Digest, McpHostError> {
...
pub fn placeholder_digest() -> Result<Sha256Digest, McpHostError> {
    digest(&serde_json::json!({"empty": true}))
}

pub fn static_digest(domain: &str) -> Sha256Digest {
