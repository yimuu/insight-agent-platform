...
impl McpSubscriptionReconcileScan {
    pub fn validate(&self) -> Result<(), McpHostError> {
        if let Some(cursor) = &self.after {
            cursor
                .validate(ResourceKind::McpOperation)
                .map_err(|_| McpHostError::InvalidSubscription)?;
            if cursor.tenant_id != self.tenant_id {
                return Err(McpHostError::InvalidSubscription);
            }
        }
        if self.tenant_id.kind() != ResourceKind::Tenant
...
pub fn canonical_mcp_resource_uri_digest(value: &str) -> Result<Sha256Digest, McpHostError> {
...
mod tests {
    ...
    fn subscription_binding_cannot_drift_from_transport_session_identity() {
        ...
        assert!(
            McpSubscriptionPayload::pending(chrono::Utc::now(), binding, payload.session).is_err()
        );
    }
    ...
    fn payload_fixture(now: DateTime<Utc>) -> (McpSubscriptionPayload, Sha256Digest) {
        ...
        let session = McpSessionRecord::disconnected(chrono::Utc::now(), session_key).unwrap();
        ...
        binding.canonical_digest = digest_without_field(&binding, "canonical_digest").unwrap();
        let payload =
            McpSubscriptionPayload::pending(chrono::Utc::now(), binding, session).unwrap();
