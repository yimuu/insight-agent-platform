...
pub fn decide_schedule_initial_scan(
    ...
    if artifact.version != command.expected_artifact_version
        ...
        || operation.snapshot.scan_policy_revision != command.scan_policy_revision
        || operation.snapshot.scanner_contract_digest != command.scanner_contract_digest
        || operation.snapshot.ruleset_digest != command.ruleset_digest
        || operation.snapshot.evidence_ttl_milliseconds != command.evidence_ttl_milliseconds
        || operation.snapshot.retry_backoff_milliseconds != command.retry_backoff_milliseconds
    {
