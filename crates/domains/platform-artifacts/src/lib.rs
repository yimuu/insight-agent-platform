...
impl PrepareArtifact {
    ...
    pub fn validate_at(
        ...
        self.audit
            .validate_at(now)
            .map_err(|_| ArtifactCommandError::InvalidAudit)?;
        if self.operation_deadline <= now
            || self.grant_expires_at <= now
            || self.grant_expires_at - now > ChronoDuration::seconds(limits.maximum_staging_seconds)
        {
            return Err(ArtifactCommandError::InvalidTimeOrSize);
        }
        Ok(())
...
mod tests {
    ...
    #[test]
    fn prepare_replay_requires_original_live_staging_authority() {
        let now = Utc::now();
        let identity = ArtifactUploadReplayIdentity::from_audit(&command(now).audit);
        let prepared = prepared_bundle(now);
        validate_artifact_prepare_replay(&prepared, &identity, now).unwrap();
        assert_eq!(
            ...
            Err(ArtifactCommandError::InvalidTransition)
        );
        let mut completed = prepared.clone();
        completed.grant.state = ArtifactLinkState::Consumed;
        assert_eq!(
            validate_artifact_prepare_replay(&completed, &identity, now),
            Err(ArtifactCommandError::InvalidTransition)
        );
        completed = prepared.clone();
        completed.artifact.state = ArtifactState::Ready;
        assert_eq!(
            validate_artifact_prepare_replay(&completed, &identity, now),
            Err(ArtifactCommandError::InvalidTransition)
        );
