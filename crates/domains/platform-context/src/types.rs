...
pub(crate) fn context_adapter_requirement(
    binding: &ContextBindingSnapshot,
    closure: &ContextDeploymentClosure,
    interface: &ContextInterfaceResourceSpec,
    implementation: &ContextImplementationResourceSpec,
    policies: &[ExactVersionRef],
) -> Result<insight_platform_contracts::ExecutionRequirement, ContextQueryError> {
    let capability = insight_platform_contracts::context_adapter_execution_capability(
        &closure.backend,
        &implementation.contract.backend,
    )
    .map_err(|_| ContextQueryError::InvalidBinding)?;
    let policy=canonical_digest(&serde_json::json!({"binding":binding,"closure":closure,"interface":interface,"implementation":implementation,"policies":policies})).map_err(|_|ContextQueryError::Canonicalization)?.parse().map_err(|_|ContextQueryError::Canonicalization)?;
