...
use insight_platform_jobs::{
    decide_claim, decide_claim_continuation, decide_expired_lease, decide_owner_terminal,
...
pub fn decide_context_query_admission(
    ...
    let adapter_execution_requirement = crate::types::context_adapter_requirement(
        &facts.binding,
        &facts.context_closure,
        &facts.interface,
...
impl ClaimContextJobs {
    pub fn validate(&self) -> Result<(), ContextQueryError> {
        ...
        for slot in &self.slots {
            slot.validate()?;
            if !std::iter::once(&slot.quota_reservation_id)
...
pub struct ContextControlDecision {
    ...
    pub job_payload: Option<ContextJobPayload>,
    /// Conservative reservation accounting, never an assertion of measured provider usage.
    pub estimated_dispatched: bool,
    pub consumed_query: bool,
    pub result_bytes: u64,
}

/// Context backends are bounded read protocols. Closing the local fenced attempt discards a
/// late read response; it neither proves that remote I/O stopped nor reports actual usage.
pub fn decide_context_control(
    current_query: &ContextQueryRecord,
    current_job: Option<&JobProjection>,
    current_payload: Option<&ContextJobPayload>,
    kind: ContextControlKind,
    database_now: DateTime<Utc>,
    limits: ContextQueryLimits,
) -> Result<ContextControlDecision, ContextQueryError> {
    current_query.validate(limits)?;
    ...
    match (current_job, current_payload) {
        (Some(job), Some(payload)) => {
            payload.validate_for(current_query, job, limits)?;
            if current_query.payload.current_job_id.as_ref() != Some(&job.job_id) {
                return Err(ContextQueryError::InvalidJob);
            }
        }
        (None, None) if current_query.payload.current_job_id.is_none() => {}
        _ => return Err(ContextQueryError::InvalidJob),
    }
    let mut query = current_query.clone();
    ...
    if !current_query.state.can_transition_to(query.state) {
        return Err(ContextQueryError::FirstWinnerLost);
    }
    query.version = increment(query.version)?;
    query.terminal_at = Some(database_now);
    ...
    let job = current_job
        .map(|job| decide_owner_terminal(job, target))
        .transpose()
        .map_err(|_| ContextQueryError::FirstWinnerLost)?;
    let estimated_dispatched = current_job.is_some_and(|job| job.state == JobState::Running);
    let consumed_query =
        estimated_dispatched && current_payload.is_some_and(|payload| !payload.query_consumed);
    let result_bytes = if estimated_dispatched {
        query.payload.admission.quota_ceiling.result_bytes
    } else {
        0
    };
    let mut payload = current_payload.cloned();
    if let Some(payload) = payload.as_mut() {
        payload.wake_contract = None;
        payload.remote_state_digest = None;
        payload.resume_same_attempt = false;
        payload.query_consumed |= consumed_query;
        let observation_digest = digest(
            &serde_json::json!({"control":target.as_str(),"estimated_dispatched":estimated_dispatched,
            "query_consumed":consumed_query,"reserved_result_bytes_settled":result_bytes}),
        )?;
        payload.physical_outcome = Some(match kind {
            ContextControlKind::Cancel => {
                ContextPhysicalOutcomeEvidence::Cancelled { observation_digest }
            }
            ContextControlKind::Timeout => {
                ContextPhysicalOutcomeEvidence::TimedOut { observation_digest }
            }
        });
    }
    query.validate(limits)?;
    if let (Some(job), Some(payload)) = (&job, &payload) {
        payload.validate_for(&query, job, limits)?;
    }
    Ok(ContextControlDecision {
        query,
        job,
        job_payload: payload,
        estimated_dispatched,
        consumed_query,
        result_bytes,
    })
}
...
mod tests {
    ...
    #[test]
    fn local_read_control_closes_fence_and_reports_only_conservative_usage() {
        let fixture = fixture();
        let query = decide_context_query_admission(&fixture.command, fixture.facts, fixture.limits)
            .unwrap();
        for state in [
            ContextQueryState::Created,
            ContextQueryState::AwaitingAuthorization,
            ContextQueryState::Ready,
        ] {
            let mut current = query.clone();
            current.state = state;
            for kind in [ContextControlKind::Cancel, ContextControlKind::Timeout] {
                let at = if kind == ContextControlKind::Timeout {
                    current.deadline
                } else {
                    current.created_at
                };
                let decision =
                    decide_context_control(&current, None, None, kind, at, fixture.limits).unwrap();
                assert!(state.can_transition_to(decision.query.state));
                assert!(!decision.estimated_dispatched);
                assert!(!decision.consumed_query);
                assert_eq!(decision.result_bytes, 0);
                assert!(decide_context_control(
                    &decision.query,
                    None,
                    None,
                    kind,
        ...
        let prepared = decide_prepare_context_dispatch(
            ...
            &PrepareContextDispatch {
                ...
                job_id: id(ResourceKind::Job, 194),
        ...
        let worker = ContextLeaseIdentity {
            worker_process_generation_id: id(ResourceKind::WorkerProcessGeneration, 195),
            lease_token_digest: named_digest("control-lease"),
        };
        ...
        .unwrap();
        let running = decide_start_context_dispatch(
            &prepared.query,
            &leased,
            &payload,
            &fence(&leased),
        ...
        .unwrap();
        for (current, job, payload, dispatched) in [
            (
            ...
            (prepared.query, leased, payload, false),
            (running.0, running.1, running.2, true),
        ] {
            let decision = decide_context_control(
                &current,
                Some(&job),
                Some(&payload),
                ContextControlKind::Cancel,
            ...
            .unwrap();
            assert_eq!(decision.query.state, ContextQueryState::Cancelled);
            assert_eq!(decision.job.as_ref().unwrap().state, JobState::Cancelled);
            assert!(decision.job.as_ref().unwrap().lease.is_none());
            assert_eq!(decision.estimated_dispatched, dispatched);
            assert_eq!(decision.consumed_query, dispatched);
            assert_eq!(
                decision.result_bytes,
                if dispatched {
                    current.payload.admission.quota_ceiling.result_bytes
                } else {
                    0
                }
            );
