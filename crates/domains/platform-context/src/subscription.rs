...
impl ContextSubscriptionRefreshRequest {
    ...
    pub fn validate_at(&self, now: DateTime<Utc>) -> Result<(), ContextSubscriptionAdmissionError> {
        ...
        if self.deadline <= now || self.deadline > now + Duration::hours(24) {
            return Err(ContextSubscriptionAdmissionError::InvalidRequest);
        }
        Ok(())
    }

    pub fn validate_frozen(&self) -> Result<(), ContextSubscriptionAdmissionError> {
...
mod tests {
    ...
    #[test]
    fn frozen_refresh_metadata_is_valid_at_its_deadline() {
        let admitted_at = Utc::now();
        let request = request(admitted_at);
        assert!(request.validate_frozen().is_ok());
        assert!(request.validate_at(request.deadline).is_err());
        let payload = ContextSubscriptionRefreshJobPayload::from_request(request.clone());
        assert!(payload.validate_frozen().is_ok());
        assert!(payload.validate_at(request.deadline).is_err());
        let mut invalid = payload;
        invalid.schema_version += 1;
        assert!(invalid.validate_frozen().is_err());
    }
}
