...
#[test]
fn undispatched_and_retrying_model_control_matches_the_closed_machine() {
    use insight_platform_contracts::{JobState, ModelTurnState};
    let fixture = fixture();
    let admitted =
        decide_model_turn_admission(&fixture.command, fixture.facts.clone(), fixture.limits)
            .unwrap();
    let ready = decide_prepare_model_dispatch(
        &admitted,
        &PrepareModelDispatch {
            audit: audit(
                &admitted.tenant_id,
                &admitted.payload.admission.principal.principal_id,
                70,
                fixture.now,
            ),
            model_turn_id: admitted.model_turn_id.clone(),
    ...
    .unwrap();
    let mut awaiting_budget = admitted.clone();
    awaiting_budget.state = ModelTurnState::AwaitingBudget;
    for (turn, job, payload) in [
        (&admitted, None, None),
        (&awaiting_budget, None, None),
        (&ready.turn, Some(&ready.job), Some(&ready.job_payload)),
    ] {
        assert_eq!(
            decide_model_control(
                turn,
                job,
                payload,
                ModelControlKind::Timeout,
                turn.deadline - Duration::milliseconds(1),
                fixture.limits
            ),
            Err(ModelTurnError::FirstWinnerLost)
        );
        for (kind, now, expected) in [
            (
                ModelControlKind::Cancel,
                fixture.now,
            ...
            let decision =
                decide_model_control(turn, job, payload, kind, now, fixture.limits).unwrap();
            assert_eq!(decision.turn.state, expected);
            assert!(turn.state.can_transition_to(expected));
            assert!(decision.settlement.is_none());
            assert!(decision.turn.retry_at.is_none());
        }
    }
    let (fixture, in_flight, reservation, fence) = started(fixture);
    let retry = decide_model_outcome(
        &in_flight.turn,
        &in_flight.job,
        &in_flight.job_payload,
        &fence,
        &reservation,
        &fixture.request,
        &ModelDispatchOutcome::RetryableFailure {
            ...
            measurement: ModelAttemptMeasurement::conservative_dispatched(
                &in_flight.turn.payload.admission,
                fixture.limits,
            )
            .unwrap(),
        },
    ...
    .unwrap();
    for kind in [ModelControlKind::Cancel, ModelControlKind::Timeout] {
        let now = if kind == ModelControlKind::Timeout {
            retry.turn.deadline
        } else {
            fixture.now + Duration::seconds(1)
        };
        let decision = decide_model_control(
            &retry.turn,
            Some(&retry.job),
            Some(&retry.job_payload),
            kind,
            now,
            fixture.limits,
        )
        .unwrap();
        let expected = if kind == ModelControlKind::Cancel {
            ModelTurnState::Cancelled
        } else {
            ModelTurnState::TimedOut
        };
        assert_eq!(decision.turn.state, expected);
        assert!(retry.turn.state.can_transition_to(expected));
        assert!(decision.turn.retry_at.is_none());
        assert!(
            decision.settlement.is_none(),
            "the preceding attempt already settled its reservation"
        );
        assert_eq!(decision.turn.payload.attempts, retry.turn.payload.attempts);
        assert_eq!(
            decision.job.unwrap().state,
            if kind == ModelControlKind::Cancel {
                JobState::Cancelled
            } else {
                JobState::TimedOut
            }
