...
pub struct ModelTurnAdmissionSnapshot {
    ...
    pub adapter_execution_requirement: insight_platform_contracts::ExecutionRequirement,
...
) -> Result<insight_platform_contracts::ExecutionRequirement, ModelTurnError> {
    let capability =
        insight_platform_contracts::model_adapter_execution_capability(&provider.installed_adapter)
            .map_err(|_| ModelTurnError::InvalidDeployment)?;
    let policy = digest(
        &serde_json::json!({"model_closure":model_closure,"provider_closure":provider_closure,"profile":profile,"provider":provider,"policies":policies}),
    )?;
...
impl ModelTurnAdmissionSnapshot {
    ...
    pub fn validate(&self, limits: ModelTurnLimits) -> Result<(), ModelTurnError> {
        ...
        if self.schema_version != 2
            || self.adapter_execution_requirement != self.required_adapter_execution()?
...
pub fn decide_model_turn_admission(
    ...
    let adapter_execution_requirement = model_adapter_requirement(
        &facts.model_closure,
        &facts.provider_closure,
        &facts.profile,
        &facts.provider,
...
impl ClaimModelJobs {
    pub fn validate(&self, limits: ModelTurnLimits) -> Result<(), ModelTurnError> {
        ...
        if self.worker_manifest.work_class != insight_platform_contracts::WorkClass::Model
            || self
                .worker_manifest
                .canonical_digest()
                .map_err(|_| ModelTurnError::InvalidCommand)?
...
pub fn decide_model_control(
    ...
    if !current_turn.state.can_transition_to(turn.state) {
        return Err(ModelTurnError::InvalidControl);
    }
