...
pub struct SandboxCleanupStateV1 {
    ...
    pub owner_build_digest: Option<Sha256Digest>,
...
impl SandboxCleanupStateV1 {
    ...
    fn claim(
        ...
        Ok(Self {
            ...
            owner_build_digest: Some(worker_build_digest),
    ...
    fn validate(
        ...
        if self.schema_version != SANDBOX_CONTRACT_SCHEMA_VERSION
            ...
            || self.owner_process_generation_id.is_some() != self.owner_build_digest.is_some()
            || self
                .owner_process_generation_id
                .as_ref()
...
impl SandboxDispatcherJobPayloadV1 {
    ...
    pub fn record_cleanup_observation(
        ...
        if completed {
            ...
            next.cleanup.owner_build_digest = None;
...
impl SandboxCleanupClaimV1 {
    pub fn validate(
        ...
        if let Some(after) = &self.after {
            after
                .validate(ResourceKind::Job)
                .map_err(|_| SandboxContractError::InvalidCleanup)?;
        }
        self.worker_manifest
            .validate()
            .map_err(|_| SandboxContractError::InvalidCleanup)?;
        if self.worker_manifest.work_class != insight_platform_contracts::WorkClass::Sandbox
            || !self
                .worker_manifest
                .execution_capabilities
                .capabilities
                .contains(&sandbox_cleanup_execution_capability())
        {
            return Err(SandboxContractError::InvalidCleanup);
        }
...
impl SandboxClaimV1 {
    pub fn validate(
        ...
        if self.worker_manifest.work_class != WorkClass::Sandbox {
            return Err(SandboxContractError::InvalidJob);
        }
        let unique = self
            .lease_token_digests
...
impl ReconcileSandboxControlsV1 {
    pub fn validate(&self, maximum_batch: u16) -> Result<(), SandboxContractError> {
        if let Some(after) = &self.after {
            after
                .validate(ResourceKind::Job)
                .map_err(|_| SandboxContractError::InvalidControl)?;
        }
...
pub trait SandboxJobRepository: Send + Sync {
    ...
    async fn reconcile_controls(
        ...
        insight_platform_jobs::store::SafetyScanPage<SandboxRepositoryDecisionV1>,
        Self::Error,
    >;
    ...
    ) -> Result<insight_platform_jobs::store::SafetyScanPage<ClaimedSandboxCleanupV1>, Self::Error>;
...
mod tests {
    ...
    fn durable_payload_tracks_job_authority_and_cleanup_without_reactivation() {
        ...
        assert_eq!(cleanup_job.state, terminal_job.state);
        assert_eq!(
            claimed_cleanup_payload.cleanup.owner_build_digest,
            Some(digest('c'))
        );
        // A compatible replacement binary may resume the same terminal obligation
        // after expiry; it cannot use the previous owner's fence or reopen work.
        let replacement_worker = id(ResourceKind::WorkerProcessGeneration, 99);
        assert!(claimed_cleanup_payload
            .claim_cleanup(
                &cleanup_job,
            ...
            .is_err());
        let (replacement, replacement_fence) = claimed_cleanup_payload
            .claim_cleanup(
                &cleanup_job,
                replacement_worker,
                digest('d'),
        ...
        replacement.validate_for(&replacement_job).unwrap();
        assert_eq!(replacement_job.state, terminal_job.state);
        assert!(replacement_fence.cleanup_generation > cleanup_fence.cleanup_generation);
        assert_eq!(replacement.cleanup.owner_build_digest, Some(digest('d')));
        assert!(replacement
    ...
    #[test]
    fn cleanup_claim_requires_exact_operation_capability() {
        let mut command = SandboxCleanupClaimV1 {
            after: None,
            process_generation_id: id(ResourceKind::WorkerProcessGeneration, 99),
            limit: 1,
            ...
            worker_manifest: insight_platform_contracts::WorkerManifest {
                ...
                worker_role: "sandbox-cleanup-fixture".into(),
                ...
                execution_capabilities: insight_platform_contracts::WorkerExecutionCapabilities {
                    schema_version: 1,
                    capabilities: vec![sandbox_cleanup_execution_capability()],
                },
        ...
        command.validate(16, 120_000).unwrap();
        command.worker_manifest.worker_build_digest = digest('c');
        command.validate(16, 120_000).unwrap();
        command.worker_manifest.execution_capabilities.capabilities = vec![
            insight_platform_contracts::WorkerExecutionCapability::DomainOperation {
                operation_abi_identity: digest('d'),
                adapter: None,
            },
        ];
        assert_eq!(
            command.validate(16, 120_000),
            Err(SandboxContractError::InvalidCleanup)
        );
    }
...
/// Exact terminal instance/candidate deletion protocol implemented by this
/// dispatcher. Tenant policy and a particular physical instance are not capabilities.
pub fn sandbox_cleanup_execution_capability(
) -> insight_platform_contracts::WorkerExecutionCapability {
    insight_platform_contracts::WorkerExecutionCapability::DomainOperation {
        operation_abi_identity:insight_platform_contracts::canonical_digest(&serde_json::json!({"contract":"opensandbox.terminal_exact_generation_cleanup","abi":1,"candidate_delete":1,"instance_delete":1,"cleanup_fence":1}))
            .expect("closed Sandbox cleanup ABI").parse().expect("canonical SHA256"),
        adapter:None,
    }
}
