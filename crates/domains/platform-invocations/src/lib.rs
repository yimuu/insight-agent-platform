...
impl CapabilityExecutionContract {
    /// Derives the installed adapter requirement from this exact execution closure.
    pub fn required_adapter_execution(
        &self,
        admission: &CapabilityAdmissionSnapshot,
    ) -> Result<Option<insight_platform_contracts::ExecutionRequirement>, InvocationError> {
        let capability = insight_platform_contracts::capability_adapter_execution_capability(
            &self.deployment_closure.backend,
            &self.implementation.backend_contract,
        ...
        let mut policy =
            serde_json::to_value(admission).map_err(|_| InvocationError::Canonicalization)?;
        let object = policy
            .as_object_mut()
            .ok_or(InvocationError::Canonicalization)?;
        object.remove("canonical_digest");
        object.remove("adapter_execution_requirement");
        let policy = digest(
            &serde_json::json!({"admission":policy,"execution_contract":self.canonical_digest}),
        )?;
        insight_platform_contracts::adapter_execution_requirement(&capability, policy)
            .map(Some)
            .map_err(|_| InvocationError::InvalidDeployment)
    }
    ...
    pub fn validate_for(
        ...
        self.validate_shape()?;
        if digest_without_field(self, "canonical_digest")? != self.canonical_digest
            || admission.adapter_execution_requirement
                != self.required_adapter_execution(admission)?
...
pub struct CapabilityAdmissionSnapshot {
    pub schema_version: u32,
    pub adapter_execution_requirement: Option<insight_platform_contracts::ExecutionRequirement>,
...
impl CapabilityAdmissionSnapshot {
    pub fn validate(&self) -> Result<(), InvocationError> {
        ...
        if self.schema_version != 2
            || (self.backend_kind != CapabilityBackendKind::Sandbox) != self.adapter_execution_requirement.is_some()
            || self.adapter_execution_requirement.as_ref().is_some_and(|requirement| !matches!(requirement,insight_platform_contracts::ExecutionRequirement::DomainOperation{requirements:insight_platform_contracts::DomainOperationRequirements::Adapter{..},..}))
...
pub fn decide_capability_admission(
    ...
    let execution_contract = CapabilityExecutionContract::build(
        facts.deployment.clone(),
        facts.deployment_closure.clone(),
        facts.implementation.clone(),
    )?;
    ...
    admission.adapter_execution_requirement =
        execution_contract.required_adapter_execution(&admission)?;
...
mod tests {
    ...
    fn approval_transition_is_version_and_deadline_first_winner() {
        ...
        let mut admission = CapabilityAdmissionSnapshot {
            schema_version: 2,
            adapter_execution_requirement: Some(
                insight_platform_contracts::adapter_execution_requirement(
