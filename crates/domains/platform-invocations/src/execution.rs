...
impl ClaimCapabilityJobs {
    pub fn validate(&self) -> Result<(), InvocationError> {
        ...
        if self.worker_manifest.work_class != self.work_class
            || self
                .worker_manifest
                .canonical_digest()
                .map_err(|_| InvocationError::InvalidCommand)?
...
impl ResolveCapabilityInput {
    pub fn validate_at(&self, now: DateTime<Utc>) -> Result<(), InvocationError> {
        ...
        if self.invocation_id.kind() != ResourceKind::CapabilityInvocation
            ...
            || self.response.is_some() != self.validated_input.is_some()
        {
            return Err(InvocationError::InvalidCommand);
        }
        Ok(())
    ...
    let job = if uncertain_write {
        match fence {
            ...
            None if current_job
                .lease
                .as_ref()
                .is_none_or(|lease| lease.expires_at <= database_now) =>
            {
                decide_owner_reconciliation(current_job)
            }
...
mod tests {
    ...
    fn ready_invocation(
        ...
        let mut admission = CapabilityAdmissionSnapshot {
            schema_version: 2,
        adapter_execution_requirement: Some(insight_platform_contracts::adapter_execution_requirement(
                &insight_platform_contracts::remote_capability_adapter_execution_capability(&insight_platform_contracts::InstalledCapabilityCodecRef {
                    schema_version: insight_platform_contracts::INSTALLED_CAPABILITY_CODEC_MANIFEST_VERSION,
                    backend_kind: CapabilityBackendKind::Http, codec_id: "fixture.http".to_owned(), codec_version: "1".to_owned(),
                    module_digest: hash('1'), worker_protocol_version: insight_platform_contracts::WORKER_PROTOCOL_VERSION, descriptor_digest: hash('2'),
                }).unwrap(), hash('3')).unwrap()),
    ...
    fn sandbox_uses_one_detached_shared_job_and_generic_dispatch_is_rejected() {
        ...
        invocation.payload.admission.adapter_execution_requirement = None;
        invocation.payload.admission.canonical_digest =
            crate::digest_without_field(&invocation.payload.admission, "canonical_digest").unwrap();
        invocation.validate().unwrap();
    ...
    #[test]
    fn expired_cancellation_owner_never_converts_unknown_writes_to_success() {
        let now = DateTime::parse_from_rfc3339("2026-08-10T03:30:00Z")
            .unwrap()
            .with_timezone(&Utc);
        for (effect, idempotency, target) in [
            (
                Effect::ReadOnly,
            ...
            let (invocation, job, payload, fence) = prepared_and_started(now, effect, idempotency);
            let controlled = decide_control(
                &invocation,
                Some(&job),
                Some(&payload),
            ...
            .unwrap();
            let job = controlled.job.unwrap();
            let payload = controlled.job_payload.unwrap();
            let observation = CapabilityCancellationObservation {
            ...
            .is_err());
            let settled = decide_cancellation_outcome(
                &controlled.invocation,
                &job,
                &payload,
                None,
            ...
            .unwrap();
            assert_eq!(settled.invocation.state, target);
            assert!(settled.job.as_ref().unwrap().lease.is_none());
            assert_eq!(
                settled.job.as_ref().unwrap().attempt_count,
                job.attempt_count
            );
            assert!(decide_cancellation_outcome(
                &settled.invocation,
                settled.job.as_ref().unwrap(),
                settled.job_payload.as_ref().unwrap(),
                None,
                &observation,
                expired_at
            )
