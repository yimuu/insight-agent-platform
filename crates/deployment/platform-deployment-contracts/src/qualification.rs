...
use insight_platform_contracts::{
    canonical_digest, ComponentRole, HardLimitProfile, Sha256Digest, UtcTimestamp, WorkClass,
    WorkerManifest,
};
...
impl QualificationEvidenceManifest {
    ...
    pub fn validate_against(
        ...
        if self.seed > insight_platform_contracts::MAX_SAFE_JSON_INTEGER {
            return Err(qualification_invalid(
                "seed",
        ...
        if self.artifact_links.is_empty()
            ...
            || self.artifact_links.iter().any(|artifact| {
                !valid_qualification_name(&artifact.name)
                    ...
                    || artifact.byte_length > insight_platform_contracts::MAX_SAFE_JSON_INTEGER
...
mod tests {
    ...
    fn candidate_rejects_old_or_drifted_hard_limit_contracts() {
        ...
        old.profile_version = insight_platform_contracts::HARD_LIMIT_PROFILE_VERSION - 1;
        assert_eq!(
        ...
        drifted
            ...
            .overflow_outcome = insight_platform_contracts::OverflowOutcome::QuotaExceeded;
        assert_eq!(
    ...
    fn release_evidence_references_the_exact_production_capacity_profile() {
        ...
        candidate.component_images = ComponentRole::ALL
            ...
            .copied()
            .map(|role| {
                let digest = canonical_digest(&serde_json::json!({ "role": role })).unwrap();
                (role, digest.parse::<Sha256Digest>().unwrap())
            })
            .collect();
