...
use insight_storage::{
    repository::migration_manifest::DURABLE_MIGRATIONS, PostgresDurableRepository,
};
...
fn postgres_test_url() -> Option<String> {
    match std::env::var("TEST_POSTGRES_URL") {
        ...
        Err(error) if std::env::var_os("CI").is_some() => {
            panic!("CI must set TEST_POSTGRES_URL for migration coordinator tests: {error}")
        }
        Err(_) => None,
    }
}
...
async fn isolated_schema(label: &str) -> Option<IsolatedPostgresSchema> {
    ...
    let schema = format!("migration_{label}_{}", Uuid::new_v4().simple());
...
async fn postgres_migration_coordinator_is_concurrent_idempotent_and_exactly_once() {
    ...
    .await
    .unwrap();
    assert_eq!(rows.len(), DURABLE_MIGRATIONS.len());
    for (row, migration) in rows.iter().zip(DURABLE_MIGRATIONS.iter()) {
...
async fn postgres_migration_coordinator_rejects_checksum_name_hole_and_unknown_rows() {
    ...
    .bind(i64::try_from(DURABLE_MIGRATIONS[0].version).unwrap())
    ...
    let missing_version = DURABLE_MIGRATIONS[5].version;
    ...
    let unknown_version = DURABLE_MIGRATIONS.last().unwrap().version + 1;
    sqlx::query(
...
async fn postgres_migration_coordinator_rejects_unledgered_schema_without_adoption() {
    ...
    sqlx::raw_sql(DURABLE_MIGRATIONS[0].postgres_sql)
