...
use insight_storage::repository::migration_manifest::DURABLE_MIGRATIONS;
use workspace_assets::workspace_path;

const POSTGRES_INITIAL_SCHEMA: &str = DURABLE_MIGRATIONS[0].postgres_sql;
const SQLITE_INITIAL_SCHEMA: &str = DURABLE_MIGRATIONS[0].sqlite_sql;
const POSTGRES_PUBLIC_EVENT_IDENTITY: &str = DURABLE_MIGRATIONS[3].postgres_sql;
const SQLITE_PUBLIC_EVENT_IDENTITY: &str = DURABLE_MIGRATIONS[3].sqlite_sql;
const POSTGRES_ARTIFACT_RETENTION: &str = DURABLE_MIGRATIONS[4].postgres_sql;
const SQLITE_ARTIFACT_RETENTION: &str = DURABLE_MIGRATIONS[4].sqlite_sql;
const POSTGRES_GRAPH_VIEW_DOCUMENTS: &str = DURABLE_MIGRATIONS[5].postgres_sql;
const SQLITE_GRAPH_VIEW_DOCUMENTS: &str = DURABLE_MIGRATIONS[5].sqlite_sql;
const POSTGRES_RECOVERY_DEADLINE: &str = DURABLE_MIGRATIONS[6].postgres_sql;
const SQLITE_RECOVERY_DEADLINE: &str = DURABLE_MIGRATIONS[6].sqlite_sql;
const POSTGRES_EVENT_PROJECTION_LEDGER: &str = DURABLE_MIGRATIONS[7].postgres_sql;
const SQLITE_EVENT_PROJECTION_LEDGER: &str = DURABLE_MIGRATIONS[7].sqlite_sql;
const POSTGRES_HUMAN_WORK_ITEMS: &str = DURABLE_MIGRATIONS[8].postgres_sql;
const SQLITE_HUMAN_WORK_ITEMS: &str = DURABLE_MIGRATIONS[8].sqlite_sql;
const POSTGRES_PUBLICATION_HEADS: &str = DURABLE_MIGRATIONS[9].postgres_sql;
const SQLITE_PUBLICATION_HEADS: &str = DURABLE_MIGRATIONS[9].sqlite_sql;
const POSTGRES_EXECUTION_EVENT_AUTHORITY: &str = DURABLE_MIGRATIONS[10].postgres_sql;
const SQLITE_EXECUTION_EVENT_AUTHORITY: &str = DURABLE_MIGRATIONS[10].sqlite_sql;
const POSTGRES_PUBLIC_EVENT_AUTHORITY: &str = DURABLE_MIGRATIONS[11].postgres_sql;
const SQLITE_PUBLIC_EVENT_AUTHORITY: &str = DURABLE_MIGRATIONS[11].sqlite_sql;
const POSTGRES_PUBLIC_EVENT_RECEIPTS: &str = DURABLE_MIGRATIONS[12].postgres_sql;
const SQLITE_PUBLIC_EVENT_RECEIPTS: &str = DURABLE_MIGRATIONS[12].sqlite_sql;
const POSTGRES_PUBLIC_PROJECTION_DECISIONS: &str = DURABLE_MIGRATIONS[13].postgres_sql;
const SQLITE_PUBLIC_PROJECTION_DECISIONS: &str = DURABLE_MIGRATIONS[13].sqlite_sql;
const POSTGRES_SCHEDULER_CLAIM_AUTHORITY: &str = DURABLE_MIGRATIONS[14].postgres_sql;
const SQLITE_SCHEDULER_CLAIM_AUTHORITY: &str = DURABLE_MIGRATIONS[14].sqlite_sql;
const POSTGRES_PUBLIC_DELIVERY_HEADS: &str = DURABLE_MIGRATIONS[15].postgres_sql;
const SQLITE_PUBLIC_DELIVERY_HEADS: &str = DURABLE_MIGRATIONS[15].sqlite_sql;
const POSTGRES_ARTIFACT_STORE_AUTHORITY: &str = DURABLE_MIGRATIONS[16].postgres_sql;
const SQLITE_ARTIFACT_STORE_AUTHORITY: &str = DURABLE_MIGRATIONS[16].sqlite_sql;
const POSTGRES_LLM_TOOL_CALL_CHECKPOINTS: &str = DURABLE_MIGRATIONS[18].postgres_sql;
const SQLITE_LLM_TOOL_CALL_CHECKPOINTS: &str = DURABLE_MIGRATIONS[18].sqlite_sql;
const POSTGRES_MODEL_TOOL_PARENT_DEADLINE: &str = DURABLE_MIGRATIONS[19].postgres_sql;
const SQLITE_MODEL_TOOL_PARENT_DEADLINE: &str = DURABLE_MIGRATIONS[19].sqlite_sql;
const POSTGRES_ATOMIC_ARTIFACT_RETENTION: &str = DURABLE_MIGRATIONS[21].postgres_sql;
const SQLITE_ATOMIC_ARTIFACT_RETENTION: &str = DURABLE_MIGRATIONS[21].sqlite_sql;
const POSTGRES_RETRIEVAL_PUBLICATIONS: &str = DURABLE_MIGRATIONS[22].postgres_sql;
const SQLITE_RETRIEVAL_PUBLICATIONS: &str = DURABLE_MIGRATIONS[22].sqlite_sql;
...
#[test]
fn durable_manifest_is_contiguous_and_exactly_matches_both_backend_directories() {
    assert_eq!(DURABLE_MIGRATIONS.len(), 23);
    for (index, migration) in DURABLE_MIGRATIONS.iter().enumerate() {
        assert_eq!(
            migration.version,
            DURABLE_MIGRATIONS[0].version + u64::try_from(index).unwrap(),
            "durable migration versions must be contiguous"
        );
        assert!(
            migration
                .name
...
#[test]
fn durable_has_isolated_forward_only_migrations_for_both_backends() {
    let root = workspace_path("migrations/durable");
    assert!(root.join("postgres").is_dir());
    assert!(root.join("sqlite").is_dir());

    for migration in [POSTGRES_INITIAL_SCHEMA, SQLITE_INITIAL_SCHEMA] {
        let sql = normalize(migration);
        assert!(!sql.contains("drop table"));
...
fn public_event_notification_identity_has_a_forward_only_global_authority() {
    for migration in [POSTGRES_PUBLIC_EVENT_IDENTITY, SQLITE_PUBLIC_EVENT_IDENTITY] {
        let sql = normalize(migration);
        assert!(sql.contains(
            "create unique index if not exists uq_public_outbox_public_event_id on public_event_outbox(public_event_id)"
        ));
        assert!(!sql.contains("drop "));
        assert!(!sql.contains("truncate "));
    }
...
fn public_projection_decision_is_per_execution_one_way_and_run_by_both_backends() {
    for migration in [
        ...
        assert!(sql.contains("idx_public_projection_order"));
...
fn public_receipt_provenance_and_bounded_delivery_heads_run_on_both_backends() {
    for migration in [POSTGRES_PUBLIC_DELIVERY_HEADS, SQLITE_PUBLIC_DELIVERY_HEADS] {
        let sql = normalize(migration);
        assert!(sql.contains("uq_public_receipt_causation"));
        ...
        assert!(sql.contains("idx_public_delivery_heads_due"));
        assert!(sql.contains(
        ...
        assert!(
            sql.contains("existing public event authority lacks provenance")
                || sql.contains("public_016_validation")
        );
...
fn graph_view_is_a_forward_only_presentation_authority_separate_from_plan_semantics() {
    for migration in [POSTGRES_GRAPH_VIEW_DOCUMENTS, SQLITE_GRAPH_VIEW_DOCUMENTS] {
        ...
        assert!(sql.contains(
            "create index if not exists idx_graph_views_document on graph_view_documents(graph_document_id)"
        ));
...
fn immutable_revisions_and_runs_pin_both_execution_hashes() {
    for migration in [POSTGRES_INITIAL_SCHEMA, SQLITE_INITIAL_SCHEMA] {
        ...
        assert!(sql.contains("trg_deployment_revision_immutable"));
        assert!(sql.contains("plan_hash"));
...
fn ledger_has_per_run_sequence_and_canonical_transition_idempotency() {
    for migration in [POSTGRES_INITIAL_SCHEMA, SQLITE_INITIAL_SCHEMA] {
        ...
        assert!(sql.contains("idx_execution_events_rebuild"));
    }

    let postgres = normalize(POSTGRES_INITIAL_SCHEMA);
...
fn projection_checkpoints_close_each_event_manifest_and_subject_version() {
    for migration in [POSTGRES_INITIAL_SCHEMA, SQLITE_INITIAL_SCHEMA] {
        ...
        for authority in ["'signal'", "'task_outbox'", "'human_work_item'"] {
            assert!(
                !sql[sql.find("create table projection_checkpoints (").unwrap()
                    ..sql
                        .find("create index idx_projection_checkpoint_rebuild")
                        .unwrap()]
                    .contains(authority),
        ...
        assert!(sql.contains("idx_projection_checkpoint_rebuild"));
    }
}
...
fn human_tasks_have_a_forward_only_durable_assignment_and_claim_authority() {
    for migration in [POSTGRES_HUMAN_WORK_ITEMS, SQLITE_HUMAN_WORK_ITEMS] {
        ...
        assert!(sql.contains("idx_human_work_items_queue"));
...
fn publication_heads_are_explicit_and_public_metadata_is_immutable() {
    for base in [POSTGRES_INITIAL_SCHEMA, SQLITE_INITIAL_SCHEMA] {
...
fn scopes_forks_tokens_and_joins_are_first_class_projections() {
    for migration in [POSTGRES_INITIAL_SCHEMA, SQLITE_INITIAL_SCHEMA] {
        let sql = normalize(migration);
        assert!(sql.contains("uq_root_scope_per_run"));
        assert!(sql.contains(
...
fn attempts_and_tasks_carry_complete_monotonic_fences() {
    for migration in [POSTGRES_INITIAL_SCHEMA, SQLITE_INITIAL_SCHEMA] {
        ...
        assert!(sql.contains("idx_attempt_lease_expiry"));
        assert!(sql.contains("idx_task_outbox_dispatch"));
...
fn effect_evidence_retry_and_timer_proofs_have_durable_inputs() {
    for migration in [POSTGRES_INITIAL_SCHEMA, SQLITE_INITIAL_SCHEMA] {
        ...
        assert!(sql.contains("idx_timers_due"));
    }
}
...
fn terminal_rows_require_closed_admission_and_durable_terminal_outbox() {
    for migration in [POSTGRES_INITIAL_SCHEMA, SQLITE_INITIAL_SCHEMA] {
        ...
        assert!(sql.contains("uq_public_terminal_per_run"));
        assert!(sql.contains("uq_public_outbox_public_event_id"));
        ...
        assert!(sql.contains("uq_public_outbox_published_claim_token"));
        assert!(sql.contains(
...
fn backend_specific_types_make_sqlite_limits_explicit() {
    let postgres = normalize(POSTGRES_INITIAL_SCHEMA);
    let sqlite = normalize(SQLITE_INITIAL_SCHEMA);
    ...
    assert!(postgres.contains("idx_task_outbox_reclaim"));
    assert!(postgres.contains("idx_public_outbox_reclaim"));
