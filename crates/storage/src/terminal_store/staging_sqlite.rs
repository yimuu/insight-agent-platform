...
fn decode_stage(row: &sqlx::sqlite::SqliteRow) -> Result<TerminalArtifactStage, RepositoryError> {
...
pub(crate) async fn consume_terminal_artifact_stage(
    ...
    let consumed = sqlx::query(
        "DELETE FROM terminal_artifact_staging
         WHERE staging_id=? AND tenant_id=? AND content_ref=?
           AND source_kind=? AND source_id=? AND staging_state='pending'",
    )
    .bind(staging_id)
    .bind(tenant_id)
    .bind(content_ref)
...
async fn referenced_authority_exists(
    ...
    sqlx::query_scalar::<_, bool>(
        "SELECT
           EXISTS(SELECT 1 FROM terminal_run_admissions WHERE input_ref=?)
           OR EXISTS(SELECT 1 FROM terminal_run_results WHERE output_ref=?)
...
#[async_trait]
impl TerminalArtifactStagingStore for SqliteDurableRepository {
    async fn stage_terminal_artifact(
        ...
        let mut transaction = super::sqlite::begin_immediate(&self.pool).await?;
        ...
        let rows = sqlx::query(
            "SELECT staging_id,tenant_id,content_ref,content_hash,source_kind,source_id,
             ...
             WHERE staging_id=? OR content_ref=?
                OR (tenant_id=? AND source_kind=? AND source_id=?)",
        )
        .bind(&staging_id)
        .bind(&command.content_ref)
        .bind(&command.tenant_id)
    ...
    async fn claim_terminal_artifact_stages(
        ...
        let mut ids = sqlx::query_scalar::<_, String>(
            "SELECT staging_id
             FROM terminal_artifact_staging
             WHERE staging_state='pending' AND available_at<=?
        ...
        if remaining > 0 {
            ids.extend(
                sqlx::query_scalar::<_, String>(
                    "SELECT staging_id
                     FROM terminal_artifact_staging
                     WHERE staging_state='claimed' AND claim_expires_at<=?
        ...
        for staging_id in ids {
            ...
            let row = sqlx::query(
                "UPDATE terminal_artifact_staging
                 SET staging_state='claimed',claim_token=?,claimed_by=?,
    ...
    async fn resolve_terminal_artifact_stage(
        ...
        let row = sqlx::query(
            "SELECT staging_id,tenant_id,content_ref,content_hash,source_kind,source_id,
                    attempts,available_at,created_at
             FROM terminal_artifact_staging
             WHERE staging_id=? AND staging_state='claimed' AND claim_token=?",
        )
