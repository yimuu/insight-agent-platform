...
use insight_engine::worker::{
    ...
    ModelToolCallBatch, ModelToolResult, ResponseItemAuthority, TaskExecutionRequest,
};
use insight_engine::{
...
async fn upsert_occurrence_value(
    ...
    let stored_occurrence: insight_engine::LogicalOccurrence = serde_json::from_value(
...
async fn settle_subflow_postgres(
    ...
    .map_err(RepositoryError::storage)?
    .ok_or_else(RepositoryError::invalid_data)?;
    let stored_occurrence = serde_json::from_value::<insight_engine::LogicalOccurrence>(
        row.try_get::<Value, _>("occurrence_key")
            .map_err(|_| RepositoryError::invalid_data())?,
    )
    .map_err(|_| RepositoryError::invalid_data())?;
...
async fn apply_scheduler_action(
    ...
    match action.intent().action() {
        ...
        SchedulerAction::DispatchTask {
            ...
            let request = TaskExecutionRequest::from_scheduler_intent(action.intent())
                .map_err(|_| RepositoryError::invalid_data())?;
        ...
        SchedulerAction::CompleteRun {
            ...
            settle_dynamic_scope(tx, run_id, &insight_engine::ScopeInstanceId::root(), false)
                .await?;
...
async fn load_facts(
    ...
    for row in terminal_subflows {
        ...
        let outcome = match lifecycle.as_str() {
            "succeeded" => {
                let contracts =
                    serde_json::from_value::<Vec<insight_engine::scheduler::TaskOutputContract>>(
                        row.try_get::<Value, _>("output_contracts")
                            .map_err(|_| RepositoryError::invalid_data())?,
                    )
                    .map_err(|_| RepositoryError::invalid_data())?;
    ...
    for row in occurrence_values {
        let occurrence = serde_json::from_value::<insight_engine::LogicalOccurrence>(
            row.try_get::<Value, _>("occurrence_key")
                .map_err(|_| RepositoryError::invalid_data())?,
        )
        .map_err(|_| RepositoryError::invalid_data())?;
...
async fn validate_task_completion_projection_postgres(
    ...
    let origin_request = TaskExecutionRequest::from_scheduler_intent(&origin.intent)
        .map_err(|_| RepositoryError::invalid_data())?;
...
async fn validate_task_request_origin_postgres(
    ...
    if row
        ...
        .as_str()
            != stored_content_hash
        || TaskExecutionRequest::from_scheduler_intent(&intent)
            .map_err(|_| RepositoryError::invalid_data())?
            != *envelope.request()
    {
        return Err(RepositoryError::invalid_data());
    ...
    if let Some(completion) = completion.as_ref() {
        ...
        for row in occurrence_values {
            ...
            let submitted_ref = submitted_refs
                ...
                .ok_or_else(RepositoryError::invalid_data)?;
            let occurrence: insight_engine::LogicalOccurrence = serde_json::from_value(
                row.try_get::<Value, _>("occurrence_key")
                    .map_err(|_| RepositoryError::invalid_data())?,
            )
            .map_err(|_| RepositoryError::invalid_data())?;
