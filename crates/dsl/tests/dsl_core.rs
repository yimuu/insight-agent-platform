...
use insight_dsl::{compile_source, validate, CompileOptions, INVALID_STEP};
use insight_engine::{
    plan::{
...
fn lowers_ordered_if_elif_else_to_branch_merge_and_typed_phi() {
    let source = workspace_asset_str!("tests/fixtures/dsl/if.yaml");
    let plan = compile_source(source, options(source)).unwrap();
    let branch = plan
        .nodes()
        .iter()
...
fn lowers_parallel_to_flat_fork_join_collect() {
    let source = workspace_asset_str!("tests/fixtures/dsl/parallel.yaml");
    let plan = compile_source(source, options(source)).unwrap();
    assert_eq!(
        plan.nodes()
            .iter()
...
fn literal_match_folds_and_dynamic_match_lowers_to_a_typed_pure_expression() {
    let literal = r#"api_version: insight.agent/v1
kind: agent
inputs: {}
output: string
workflow:
...
fn wait_signal_timer_and_declared_raise_lower_to_lifecycle_nodes() {
    let wait = r#"api_version: insight.agent/v1
kind: agent
inputs: {}
output: string
workflow:
...
"#;
    ...
    let raised = r#"api_version: insight.agent/v1
kind: agent
errors:
  rejected:
    category: workflow
...
workflow:
  steps:
    ...
    - return: $answer
"#;
    let document = validate(insight_dsl::parse(source).unwrap()).unwrap();
    let insight_dsl::ast::Step::Leaf(leaf) = &document.steps[0] else {
        panic!("expected llm leaf");
    };
    let llm = leaf.llm.as_ref().unwrap();
    assert!(matches!(
        llm.messages[0],
        insight_dsl::ast::MessageExpr::Message { .. }
    ));
    assert!(matches!(
        llm.messages[1],
        insight_dsl::ast::MessageExpr::Splice(_)
    ));
    let plan = insight_dsl::compile(document, options(source)).unwrap();
...
"#;
    ...
    assert_eq!(
        llm.tool_choice,
        insight_dsl::ast::LlmToolChoice::Tool("summarize".to_owned())
    );
    assert_eq!(
...
"#;
    ...
    assert_eq!(error.code(), insight_dsl::INVALID_TYPE);
...
"#;
    let document = validate(insight_dsl::parse(source).unwrap()).unwrap();
    let insight_dsl::ast::Step::Return(value) = &document.steps[0] else {
        panic!("expected return");
    };
    let insight_dsl::ast::ValueExpr::Object(fields) = value else {
        panic!("natural mapping must remain a typed Object AST");
    };
    assert!(matches!(
        fields["answer"],
        insight_dsl::ast::ValueExpr::Reference(_)
    ));
    assert!(matches!(
        fields["lines"],
        insight_dsl::ast::ValueExpr::Array(_)
    ));
    ...
    let document = validate(insight_dsl::parse(&raised).unwrap()).unwrap();
    assert!(matches!(
        document.steps[0],
        insight_dsl::ast::Step::Raise(insight_dsl::ast::ValueExpr::ErrorRef(_))
    ));
}
...
fn legacy_control_and_child_result_are_not_in_the_positive_surface() {
    for source in [
        workspace_asset_str!("tests/fixtures/dsl/negative-switch.yaml"),
        workspace_asset_str!("tests/fixtures/dsl/negative-core.yaml"),
        workspace_asset_str!("tests/fixtures/dsl/negative-child-result.yaml"),
    ] {
        let raw = insight_dsl::parse(source).unwrap();
        assert_eq!(validate(raw).unwrap_err().code(), INVALID_STEP);
    }
}

#[test]
fn checked_in_agents_and_markdown_prompts_compile_through() {
    ...
    for agent in [
        ...
        compile_source(&source, options).unwrap_or_else(|error| {
            panic!("checked-in agent '{agent}' failed DSL compile: {error}")
        });
    }
}
