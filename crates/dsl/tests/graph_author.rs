...
fn layout_only_changes_cannot_change_graph_semantic_hash() {
    ...
    view.set_viewport(Viewport {
        origin: insight_dsl::graph::CanvasPoint::new(45.0, 80.0),
        zoom: 1.75,
    })
    .unwrap();
...
fn structured_graph_conversion_preserves_structured_parse_diagnostics() {
    ...
    assert_eq!(
        error.code(),
        insight_dsl::graph::GRAPH_STRUCTURED_COMPILE_FAILED
    );
...
fn structured_core_sources_round_trip_with_identical_plan_hashes() {
    const WAIT: &str = r#"api_version: insight.agent/v1
kind: agent
inputs: {}
output: string
workflow:
...
"#;
    let sources = [
        (
            "linear.yaml",
            workspace_asset_str!("tests/fixtures/dsl/linear.yaml"),
        ),
        (
            "if.yaml",
            workspace_asset_str!("tests/fixtures/dsl/if.yaml"),
        ),
        (
            "parallel.yaml",
            workspace_asset_str!("tests/fixtures/dsl/parallel.yaml"),
        ),
        ("wait.yaml", WAIT),
    ];
...
fn native_linear_if_parallel_map_and_loop_graphs_reduce_without_a_source_certificate() {
    const MAP: &str = r#"api_version: insight.agent/v1
kind: agent
types:
  Item:
    fields:
...
workflow:
  steps:
    ...
    - return: $copied
"#;
    const LOOP: &str = r#"api_version: insight.agent/v1
kind: agent
inputs: {seed: string}
output: string
workflow:
...
"#;
    ...
    let reduced = graph.to_structured().unwrap();
    let insight_dsl::ast::Step::Leaf(leaf) = &reduced.document().steps[0] else {
    ...
    assert_eq!(llm.tool_choice, insight_dsl::ast::LlmToolChoice::Required);
    assert_eq!(
...
workflow:
  steps:
    ...
    - return: $result
"#;
    const SUBFLOW: &str = r#"api_version: insight.agent/v1
kind: agent
...
workflow:
  steps:
    ...
    - return: $child
"#;
    const TIMER: &str = r#"api_version: insight.agent/v1
kind: agent
inputs: {}
output: string
workflow:
...
fn semantic_edit_wire_is_closed_and_grouped_topology_upsert_delete_is_atomic() {
    const DIRECT: &str = r#"api_version: insight.agent/v1
kind: agent
...
workflow:
  steps:
    ...
    - return: $route
"#;
    const BRANCHED: &str = workspace_asset_str!("tests/fixtures/dsl/if.yaml");
    ...
    let direct = graph_for_revision(DIRECT, "semantic_edit_revision_3");
    let remove_branch = GraphSemanticEditBatch::new(
        graph.semantic_hash().clone(),
        DefinitionRevisionId::new("semantic_edit_revision_3").unwrap(),
        topology_edits(&graph, &direct),
    )
    .unwrap();
    graph.apply_semantic_edit_batch(remove_branch).unwrap();
    assert_eq!(graph.semantic_hash(), direct.semantic_hash());
    assert_eq!(graph.nodes(), direct.nodes());
...
fn parallel_map_and_loop_canvas_edit_surfaces_reverify_the_complete_plan() {
    let mut parallel = structured_graph(workspace_asset_str!("tests/fixtures/dsl/parallel.yaml"));
