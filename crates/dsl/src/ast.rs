...
pub fn validate(raw: RawDocument) -> Result<StructuredAuthorDocument, CompileError> {
    if raw.api_version != API_VERSION || raw.kind != DOCUMENT_KIND {
        return Err(error(
            INVALID_DOCUMENT,
            "document must declare api_version insight.agent/v1 and kind agent",
            DslPath::root(),
        ));
    }
    ...
    let errors = raw
        ...
        .map(|(id, value)| {
            ...
            if string(required(declaration, "category")?, "error category")? != "workflow" {
                return Err(CompileError::new(
                    INVALID_DOCUMENT,
                    "the DSL Core error category must be workflow",
                ));
            }
...
fn parse_step(
        ...
        || object.contains_key("result")
    {
        return Err(CompileError::new(
            INVALID_STEP,
            "legacy switch, kind, child result, and core.* syntax is not part of the current DSL",
        ));
    }
    ...
    Err(CompileError::new(
        INVALID_STEP,
        "step must be a leaf, structured control, durable wait/call, or explicit terminator",
    ))
}
...
fn parse_leaf(
    ...
    let (kind, implementation, allowed) = match kind {
        ...
        value if value.starts_with("core.") => {
            return Err(CompileError::new(
                INVALID_STEP,
                "core.* node tags are forbidden in the current author surface",
            ));
        }
        _ => {
            return Err(CompileError::new(
...
pub fn parse_value_expr(value: &Value) -> Result<ValueExpr, CompileError> {
    ...
    if let Some(object) = value.as_object() {
            ...
            })
        {
            return Err(CompileError::new(
                INVALID_STEP,
                "legacy value wrappers are forbidden in the current natural YAML DSL",
            ));
        }
    ...
    match value {
        ...
        Value::Object(values) => {
                ...
                })
            {
                return Err(CompileError::new(
                    INVALID_TYPE,
                    "legacy value wrappers are forbidden in DSL defaults",
                ));
            }
...
impl<'a> TypeResolver<'a> {
    ...
    fn resolve_named(
        ...
        if stack.iter().any(|value| value == name) {
            return Err(CompileError::new(
                INVALID_TYPE,
                "recursive named types are not part of the DSL Core type profile",
            ));
        }
        if name == "Message" {
