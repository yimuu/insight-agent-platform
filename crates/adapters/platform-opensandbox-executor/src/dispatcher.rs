...
#[cfg(test)]
use insight_platform_sandbox::contracts::OpaqueActivationToken;
use insight_platform_sandbox::contracts::{
    ...
    LeasedSandboxJobV1, OpenSandboxCreateV1, OpenSandboxProvider, PhysicalDecision,
    RecordProvisioningIntentV1, RecordSandboxCleanupObservationV1, RecordSandboxObservationV1,
    SandboxCandidatePurposeV1, SandboxCandidateV1, SandboxCleanupObservationV1,
    ...
    P: OpenSandboxProvider,
{
    ...
    pub async fn heartbeat(
        ...
        let decision = self
            .repository
            .heartbeat(insight_platform_sandbox::contracts::HeartbeatSandboxJobV1 {
            ...
            let Some(physical) = leased.payload.physical.as_deref() else {
                let decision = self
                    ...
                    .record_provisioning_intent(RecordProvisioningIntentV1 {
                        identity: leased_identity(&leased),
                        activation_token: crate::generate_activation_token()
            ...
            match physical.phase {
                ...
                SandboxPhysicalPhaseV1::ActivationAuthorized => {
                    ...
                    let state = if state.phase == SandboxRunnerPhaseV1::Armed {
                        let frame = insight_platform_sandbox::contracts::SandboxActivationFrameV1::from_authorized(
                            &leased.request,
...
fn state_proof(
    leased: &LeasedSandboxJobV1,
    sandbox_id: &insight_platform_sandbox::contracts::OpenSandboxId,
...
fn selected_physical_evidence<'a>(
    ...
    sandbox_id: &insight_platform_sandbox::contracts::OpenSandboxId,
) -> Result<&'a insight_platform_sandbox::contracts::SandboxPhysicalEvidenceV1, SandboxContractError>
{
    let physical = leased
        .payload
        .physical
...
fn selected_sandbox_id<E>(
    leased: &LeasedSandboxJobV1,
) -> Result<insight_platform_sandbox::contracts::OpenSandboxId, SandboxDispatchError<E>> {
    leased
        .payload
        .physical
        .as_deref()
...
fn runner_boot_changed<E>(
    leased: &LeasedSandboxJobV1,
    state: &insight_platform_sandbox::contracts::SandboxRunnerStateFrameV1,
...
mod tests {
    ...
    impl SandboxJobRepository for AuthorizationRepository {
        ...
        async fn reconcile_controls(
            &self,
            _request: insight_platform_sandbox::contracts::ReconcileSandboxControlsV1,
        ) -> Result<
        ...
        async fn claim_cleanup(
            ...
            insight_platform_jobs::store::SafetyScanPage<ClaimedSandboxCleanupV1>,
            Self::Error,
        > {
    ...
    impl OpenSandboxProvider for CountingProvider {
        ...
        async fn activate(
            &self,
            _sandbox_id: &OpenSandboxId,
            _frame: insight_platform_sandbox::contracts::SandboxActivationFrameV1,
            _proof: SandboxRunnerRequestProofV1,
        ) -> Result<SandboxRunnerStateFrameV1, SandboxProviderError> {
            panic!("activation is not reached")
        }
    ...
    fn rollover_fixture(
        ...
        let candidate = SandboxCandidateV1 {
            ...
            metadata: insight_platform_sandbox::contracts::SandboxCandidateMetadataV1 {
                schema_version: SANDBOX_CONTRACT_SCHEMA_VERSION,
                purpose: insight_platform_sandbox::contracts::SandboxCandidatePurposeV1::Job,
        ...
        let mut payload = payload
            .authorize_activation(
                ...
                insight_platform_sandbox::contracts::RunnerBootId::parse("boot-original").unwrap(),
            )
            .unwrap()
            .into_inner();
        job = decide_observation_update(&job, &job_fence(&job), now).unwrap();
        if started {
            let original_state = SandboxRunnerStateFrameV1 {
                ...
                boot_id: insight_platform_sandbox::contracts::RunnerBootId::parse("boot-original")
                    .unwrap(),
                execution_request_digest: request.request_digest.clone(),
        ...
        let state = SandboxRunnerStateFrameV1 {
            ...
            boot_id: insight_platform_sandbox::contracts::RunnerBootId::parse("boot-restarted")
                .unwrap(),
            execution_request_digest: request.request_digest.clone(),
