...
#[cfg(test)]
use insight_platform_invocations::CapabilityExecutionContract;
#[cfg(test)]
use insight_platform_invocations::CapabilityExecutionInput;
#[cfg(test)]
use insight_platform_invocations::CapabilityExecutionInputMaterial;
#[cfg(test)]
use insight_platform_invocations::CapabilityInputAction;
#[cfg(test)]
use insight_platform_invocations::EncryptedRemoteState;
#[cfg(test)]
use insight_platform_invocations::InvocationValueStorage;
use insight_platform_invocations::{CapabilityUncertainty, DispatchOutcome, SafeBackendFailure};
...
impl CapabilityDispatcher {
    ...
    pub async fn dispatch(
        ...
        request
            .validate_at(now)
            .map_err(|error| contract_failure(error.into()))?;
...
impl From<insight_platform_invocations::InvocationError> for CapabilityDispatchError {
    fn from(_: insight_platform_invocations::InvocationError) -> Self {
        Self::InvalidRequest
    }
}
...
mod tests {
    ...
    fn mcp_fixture(tasks: bool) -> McpFixture {
        ...
        .unwrap();
        let authorization = McpAuthorizationContext::build(
            chrono::Utc::now(),
            NewMcpAuthorizationContext {
        ...
        .unwrap();
        let host = McpHostExecutionContract::build(
            chrono::Utc::now(),
    ...
    #[tokio::test]
    async fn native_dispatch_uses_exact_adapter_across_worker_rebuilds() {
    ...
    #[tokio::test]
    async fn remote_rebuild_preserves_compatibility_and_codec_drift_fails_before_io() {
        let execution = http_execution(1_000);
        let CapabilityBackendContract::Http(contract) = &execution.implementation.backend_contract
        else {
            unreachable!();
        ...
        let rebuilt_worker = request(execution.clone(), digest('f'));
        assert!(dispatcher.dispatch(&rebuilt_worker).await.is_ok());
        assert!(transport.observed.lock().unwrap().take().is_some());
