#[cfg(test)]
use super::CapabilityAdapterRequest;
use super::{
    adapter_failure_outcome, CapabilityAdapterFailure, CapabilityAdapterFailureClass,
    CapabilityDispatchError, CapabilityDispatcher, CapabilityTransportCancelOutcome,
    CapabilityTransportRequestIdentity,
};
use chrono::Utc;
use insight_platform_contracts::CommandOutcome;
#[cfg(test)]
use insight_platform_contracts::ExternalLeafFailureMutationIds;
#[cfg(test)]
use insight_platform_contracts::ExternalLeafResumeMutationIds;
use insight_platform_invocations::execution_port::{
    CancelCapabilityAdapterJob, CapabilityExecutionAuthority, ExecuteCapabilityAdapterJob,
};
#[cfg(test)]
use insight_platform_invocations::CapabilityWorkerAudit;
use insight_platform_invocations::{
    CommitCapabilityCancellationOutcome, CommitCapabilityOutcome, DispatchOutcome,
};
    ...
    A: CapabilityExecutionAuthority,
{
    ...
    pub async fn prepare(
        ...
        command
            .validate_at(Utc::now())
            .map_err(|error| CapabilityAdapterWorkerError::Contract(error.into()))?;
...
impl From<insight_platform_invocations::InvocationError> for CapabilityAdapterWorkerContractError {
...
mod tests {
    ...
    struct CapturingAuthority {
        commands: Arc<Mutex<Vec<CommitCapabilityOutcome>>>,
        cancellations: Arc<Mutex<Vec<CommitCapabilityCancellationOutcome>>>,
    }

    #[async_trait]
    impl CapabilityExecutionAuthority for CapturingAuthority {
    ...
    fn worker(
        ...
        CapabilityAdapterWorker<CapturingAuthority>,
        CapturingAuthority,
    ) {
        ...
        registry
            ...
            .unwrap();
        let authority = CapturingAuthority::default();
        (
