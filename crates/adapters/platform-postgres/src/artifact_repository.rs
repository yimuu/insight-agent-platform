...
use insight_platform_artifacts::store::{DriveExpiredArtifactJobs, RecoveredArtifactJob};
use insight_platform_artifacts::{
    ...
    ArtifactScanWorkRecord, ArtifactStore, ArtifactTransaction, ArtifactUploadCompletionReceiptV1,
    ArtifactUploadReplayIdentity, ArtifactWorkAuthority, ArtifactWorkError, ArtifactWorkerAudit,
    ArtifactWorkerOperationRecord, AuthorizedArtifactDeleteObject,
    AuthorizedArtifactScanObjectRead, AuthorizedWorkloadArtifactStage,
    CommitArtifactAttemptFailure, CommitArtifactBlobCleanup, CommitArtifactScanOutcome,
    CompleteArtifactDeletion, CompleteArtifactUpload, CompletedArtifactBlobCleanup,
    CompletedArtifactDeletion, CompletedArtifactUpload, CreateArtifactProvenance,
    DeleteArtifactBlobGeneration, EncryptedArtifactObjectReference, FinalizeArtifact,
    FinalizedArtifact, GatewayArtifactReadRequest, MarkArtifactDeletion, MarkedArtifactDeletion,
    PlaceArtifactHold, PrepareArtifact, PreparedArtifact, ReleaseArtifactHold,
    ReleaseArtifactReference, ScheduleArtifactRescan, ScheduleInitialArtifactScan,
    SchedulerRunValueLease, SchedulerRunValueReadRequest, SchedulerRunValueRequestResolver,
    SchedulerSkillPackageLease, SchedulerSkillPackageReadRequest,
...
impl ArtifactObjectReadAuthority<GatewayArtifactReadRequest> for PgRepository {
    ...
    > {
        ...
        let registry_authority_digest = match &request.authority {
            insight_platform_artifacts::GatewayArtifactReadAuthority::PublicContent => {
            ...
            insight_platform_artifacts::GatewayArtifactReadAuthority::RegistryValidation {
                authority,
                selector,
            } => {
                let (draft, digest) =
                    crate::registry_validation_repository::registry_validation_document(
                        &mut transaction,
                        &request.tenant_id,
                        &request.principal_id,
                        request.principal_kind,
                    ...
                    .map_err(classify_gateway_artifact_read_error)?;
                let (expected, _) =
                    crate::registry_validation_repository::registry_selected_artifact(
                        &mut transaction,
                        &request.tenant_id,
                        &draft.document,
                        *selector,
        ...
        let authorization_digest: Sha256Digest = insight_platform_contracts::canonical_digest(
            &serde_json::json!({
                ...
                "read_authority": request.authority,
...
impl PgRepository {
    ...
    pub async fn resolve_artifact_deletion_approval(
        ...
        let next_task_payload = TypedPayload::new(2, &next_task.payload)?;
        ensure_one(
            sqlx::query(
                r#"
    ...
    pub async fn drive_expired_artifact_jobs(
        ...
        for (row, slot) in candidates.into_iter().zip(command.slots.iter()) {
            ...
            let object_result: Result<(), RepositoryError> = async {
                ...
                let payload: ArtifactJobPayload = crate::recovery_isolation::job(
                ...
                .await;
                recovered.push(crate::recovery_isolation::produced(produced)?);
                Ok(())
            }
            .await;
            match object_result {
...
impl ArtifactTransaction for PgArtifactTransaction {
    ...
    async fn prepare_artifact(
        ...
        command.validate_shape(self.limits)?;
        ...
        require_tenant_permission(&mut transaction, &command.audit, Permission::ArtifactWrite)
            .await?;
        let identity = ArtifactUploadReplayIdentity::from_audit(&command.audit);
        if let Some(prepared) = read_prepare_replay(&mut transaction, &identity).await? {
            transaction.commit().await?;
        ...
        {
            let prepared = load_live_prepare_replay(&mut transaction, &identity, &result).await?;
            transaction.commit().await?;
            return Ok(CommandOutcome::Replayed(prepared));
        }
        ...
        sqlx::query(
            ...
            INSERT INTO insight_platform.jobs (
                tenant_id, job_id, job_kind, work_class, owner_kind, owner_id, state, attempt_limit, scheduled_at, deadline, request_digest, payload_schema_version, payload, payload_digest, trace_id, scheduler_partition_id, execution_requirement_version, execution_requirement, execution_requirement_digest
            ) VALUES (
                $1, $2, 'artifact_scan', 'artifact', 'artifact', $3, 'waiting', 1, $4, $5, $6, $7, $8, $9, $10, (SELECT scheduler_partition_id FROM insight_platform.tenants WHERE tenant_id=$1), $11, $12, $13
            )
            "#,
        )
        .bind(command.audit.tenant_id.to_string())
    ...
    async fn complete_upload(
        ...
        let identity = ArtifactUploadReplayIdentity::from_audit(&command.audit);
        if let Some(result) =
            read_completion_receipt(&mut transaction, &identity, &command.artifact_id, true).await?
        {
            if result != ArtifactUploadCompletionReceiptV1::from_command(&command) {
                return Err(RepositoryError::IdempotencyConflict);
            }
            let completed =
                load_completed_upload_replay_locked(&mut transaction, &identity, &result).await?;
            transaction.commit().await?;
        ...
        {
            let result =
                read_completion_receipt(&mut transaction, &identity, &command.artifact_id, true)
                    .await?
                    .ok_or_else(|| {
                        RepositoryError::CorruptRow(
                            "Artifact complete Receipt disappeared".to_owned(),
                        )
                    })?;
            if result != ArtifactUploadCompletionReceiptV1::from_command(&command) {
                return Err(RepositoryError::IdempotencyConflict);
        ...
        .await?;
        terminalize_upload_completion_receipt(&mut transaction, &command).await?;
    ...
    async fn mark_deletion(
        ...
        if create_approval {
            ...
            INSERT INTO insight_platform.jobs (
                tenant_id, job_id, job_kind, work_class, owner_kind, owner_id, state, attempt_limit, scheduled_at, deadline, request_digest, payload_schema_version, payload, payload_digest, created_at, updated_at, trace_id, scheduler_partition_id, execution_requirement_version, execution_requirement, execution_requirement_digest
            ) VALUES (
                $1, $2, 'artifact_delete', 'artifact', 'artifact', $3, 'waiting', $4, $10, $5, $6, $7, $8, $9, (SELECT observed_at FROM job_creation_time), GREATEST($10::timestamptz, (SELECT observed_at FROM job_creation_time)), $11, (SELECT scheduler_partition_id FROM insight_platform.tenants WHERE tenant_id=$1), $12, $13, $14
            )
                "#,
            )
            .bind(command.audit.tenant_id.to_string())
            ...
            task.validate()
                .map_err(|failure| RepositoryError::InvalidInput(failure.to_string()))?;
            let task_payload = TypedPayload::new(2, &task)?;
            sqlx::query(
                r#"
        ...
        sqlx::query(
            ...
            INSERT INTO insight_platform.jobs (
                ...
                $1, $2, 'artifact_delete', 'artifact', 'artifact', $3, 'ready', $4, $5, $6, $7, $8, $9, $10, (SELECT observed_at FROM job_creation_time), GREATEST($5::timestamptz, (SELECT observed_at FROM job_creation_time)), $11, (SELECT scheduler_partition_id FROM insight_platform.tenants WHERE tenant_id=$1), $12, $13, $14
            )
            "#,
        )
        .bind(command.audit.tenant_id.to_string())
...
async fn insert_artifact_scan_job(
    ...
    let execution_requirement_3 = crate::execution_requirements::StoredExecutionRequirement::new(
        &insert
            .job
            .execution_requirement()
            .map_err(|failure| RepositoryError::InvalidInput(failure.to_string()))?,
    ...
    sqlx::query(
        ...
        INSERT INTO insight_platform.jobs (
                ...
                $1, $2, $12, 'artifact', 'artifact', $3, 'ready', $4, $5, $6, $7, $8, $9, $10, $11, (SELECT scheduler_partition_id FROM insight_platform.tenants WHERE tenant_id=$1), $13, $14, $15
            )
        "#,
    )
...
async fn insert_scan_duplicate_blob_cleanup_job(
    ...
    let execution_requirement_4 = crate::execution_requirements::StoredExecutionRequirement::new(
        &artifact_job
            .execution_requirement()
            .map_err(|failure| RepositoryError::InvalidInput(failure.to_string()))?,
    )?;
    sqlx::query(
        ...
        INSERT INTO insight_platform.jobs (
                ...
                $1, $2, 'artifact_blob_cleanup', 'artifact', 'internal_blob', $3, 'ready', $4, $5, $6, $7, $8, $9, $10, (SELECT trace_id FROM insight_platform.jobs
                   WHERE tenant_id = $1 AND job_id = $11), (SELECT scheduler_partition_id FROM insight_platform.tenants WHERE tenant_id=$1), $12, $13, $14
            )
        "#,
    )
    .bind(command.audit.tenant_id.to_string())
    ...
    .bind(execution_requirement_4.version)
    .bind(&execution_requirement_4.value)
    .bind(&execution_requirement_4.digest)
    .execute(&mut **transaction)
    .await?;
    Ok(())
}
...
/// Provisioning-only SQL owned with Artifact persistence; never called by runtime composition.
pub fn artifact_role_grants_sql() -> &'static str {
    include_str!("../artifact-role-grants.sql")
}
