...
use crate::{
    ...
    repository::{
        ...
        settle_context_leaf_success_in_transaction, terminalize_command_receipt, DeploymentRecord,
...
pub(crate) async fn load_context_query(
    ...
    persisted_context_query_from_row(row, limits)
}

fn persisted_context_query_from_row(
    row: PgRow,
    limits: ContextQueryLimits,
) -> Result<ContextQueryRecord, RepositoryError> {
    ...
    crate::recovery_isolation::persisted(context_query_from_row(row, limits), &diagnostic)
}
...
impl PgRepository {
    ...
    async fn recover_expired_context_job(
        ...
        crate::recovery_isolation::job(
            decode_versioned_payload::<ContextJobPayload>(&observed.payload, "Context Job"),
            observed,
            insight_platform_jobs::store::SafetyScanPhase::OwnerDecode,
        )?;
        ...
        let payload: ContextJobPayload = crate::recovery_isolation::job(
            decode_versioned_payload(&current_job.payload, "Context Job"),
            &current_job,
            insight_platform_jobs::store::SafetyScanPhase::OwnerDecode,
        )?;
        crate::recovery_isolation::job(
            payload
                .validate_for(&current_query, &projection, self.context_query_limits())
    ...
    pub async fn claim_context_jobs(
        ...
        let rows = sqlx::query(
            ...
            WHERE work_class='context' AND job_id=ANY($2) AND owner_kind='context_query'
              AND state IN ('ready','retry_scheduled') AND terminal_at IS NULL AND worker_id IS NULL
            ...
            ORDER BY tenant_id,run_id,node_id,owner_id,job_id
        "#,
        )
        .bind(database_now)
        ...
        for row in rows {
            let result = async {
                ...
                let payload: ContextJobPayload = crate::recovery_isolation::job(
                    decode_versioned_payload(&job.payload, "Context Job"),
                    &job,
                    insight_platform_jobs::store::SafetyScanPhase::OwnerDecode,
                )?;
                let tenant = parse_id(&job.tenant_id, "Context tenant")?;
                let owner = parse_id(&job.owner_id, "Context owner")?;
                let query = load_context_query(
                    &mut transaction,
                    &tenant,
                    &owner,
                ...
                if command
                    ...
                    && command
                        ...
                        .execution_capabilities
                        .supports(&leaf)
                    && (payload.resume_same_attempt || job.attempt_no < job.attempt_limit)
                {
                    Ok(Some((job, payload, &command.slots[0])))
                } else {
                    Ok(None)
                }
            }
        ...
        for (job, payload, _) in &observed {
            let costs = accounts
                .values()
                .filter(|account| {
                    context_quota_account_matches(
                        account,
                        job,
                        &payload.binding.context_deployment.deployment_id,
                    )
                })
                .map(|account| {
                    Ok(insight_platform_scheduler::partitioned::QuotaCost {
                        account_id: account
                            ...
                            .parse::<ResourceId>()
                            .map_err(|error| RepositoryError::CorruptRow(error.to_string()))?,
                        amount: context_reserved_amount(account, payload.binding.quota_ceiling)?
                            as u64,
                    })
                })
                .collect::<Result<Vec<_>, RepositoryError>>()?;
            if costs.len() == CONTEXT_QUOTA_LINES {
                eligible.insert(
                    job.job_id.clone(),
                    crate::claim_admission::EligibleClaim {
                        lane: insight_platform_contracts::SchedulingLane::Business,
                        mode: if payload.resume_same_attempt {
        ...
        let observed = observed
            .into_iter()
            .filter(|(job, _, _)| admitted.contains(&job.job_id))
            .zip(&command.slots)
            .map(|((job, payload, _), slot)| (job, payload, slot))
            .collect::<Vec<_>>();
        ...
        for (observed_job, observed_payload, slot) in observed {
            ...
            let mut object_accounts = accounts.clone();
            let result: Result<ClaimedContextExecution, RepositoryError> = async {
                ...
                let node =
                    load_context_node_for_update(&mut object_transaction, &tenant_id, &node_id)
                        .await?;
                let query = load_context_query(
                ...
                {
                    return Err(RepositoryError::Conflict(
                        "Context execution requirement changed",
                    ));
                }
                ...
                let current_job = load_context_job(
                    &mut object_transaction,
                    &tenant_id,
                    &parse_id(&observed_job.job_id, "Context Job")?,
                    true,
                )
                .await?;
                require_same_context_job(&observed_job, &current_job)?;
                require_context_claim_parents(
                    &mut object_transaction,
                    &query,
                    &run,
                    &node,
...
/// Only SELECT and old owner validation belong to the recoverable object boundary.
async fn load_context_run_control_job(
    transaction: &mut Transaction<'_, Postgres>,
    query: &ContextQueryRecord,
    for_update: bool,
    limits: ContextQueryLimits,
    ...
    let Some(job_id) = query.payload.current_job_id.as_ref() else {
        return Ok(None);
    };
    let job = crate::recovery_isolation::addressed(
        load_context_job(transaction, &query.tenant_id, job_id, for_update).await,
        &query.tenant_id,
        job_id,
    ...
    let payload: ContextJobPayload = crate::recovery_isolation::job(
        decode_versioned_payload(&job.payload, "Context control Job"),
        &job,
        SafetyScanPhase::OwnerDecode,
    )?;
    crate::recovery_isolation::job(
        payload
            .validate_for(query, &projection, limits)
            .map_err(RepositoryError::from),
        &job,
        SafetyScanPhase::OwnerValidation,
    )?;
...
/// Restricted system completion: the committed Run control authorizes closing its existing read.
/// Called before any business lock, and never invokes a physical backend or public command.
pub(crate) async fn converge_context_for_run(
    transaction: &mut Transaction<'_, Postgres>,
    tenant_id: &ResourceId,
    run_id: &ResourceId,
    query_id: &ResourceId,
    ...
    let observed = load_context_query(transaction, tenant_id, query_id, false, limits).await?;
    if observed.terminal_at.is_some() {
        return Ok(None);
    }
    let observed_job = load_context_run_control_job(transaction, &observed, false, limits).await?;
    let quota = match observed_job
        .as_ref()
        .filter(|job| job.quota_reservation_id.is_some())
    {
        Some(job) => Some(lock_context_quota_bundle(transaction, job).await?),
        None => None,
    };
    ...
    if observed.run_id != *run_id || observed.tenant_id != *tenant_id {
        return Err(RepositoryError::CorruptRow(
            "Context Run control owner".into(),
        ));
    }
    // Node linkage is checked before the domain owner and its Job; no result body is read.
    let node=sqlx::query("SELECT record_kind,node_kind,state FROM insight_platform.run_nodes WHERE tenant_id=$1 AND run_id=$2 AND node_id=$3 FOR UPDATE")
        .bind(tenant_id.to_string()).bind(run_id.to_string()).bind(observed.node_execution_id.to_string())
        .fetch_optional(&mut **transaction).await?.ok_or(RepositoryError::NotFound("Context control Node"))?;
    let node_state = crate::recovery_isolation::addressed(
        (|| {
            let node_kind = node
                .try_get::<String, _>("node_kind")?
                .parse::<PlanNodeKind>()
                .map_err(|failure| RepositoryError::CorruptRow(failure.to_string()))?;
            if node.try_get::<String, _>("record_kind")? != "node_execution"
                || node_kind != PlanNodeKind::ContextQuery
            {
                return Err(RepositoryError::CorruptRow(
                    "Context control Node has the wrong owning kind".into(),
                ));
            }
    ...
    let current = load_context_query(transaction, tenant_id, query_id, true, limits).await?;
    if current.version != observed.version {
        return Ok(None);
    }
    let current_job = load_context_run_control_job(transaction, &current, true, limits).await?;
    if let (Some(observed), Some(current)) = (&observed_job, &current_job) {
        require_same_context_job(observed, current)?;
    }
    let projection = current_job.as_ref().map(job_projection).transpose()?;
    let payload = current_job
        .as_ref()
        .map(|job| {
            decode_versioned_payload::<ContextJobPayload>(&job.payload, "Context control Job")
        })
        .transpose()?;
    ...
    {
        insight_platform_context::ContextControlKind::Timeout
    } else {
        insight_platform_context::ContextControlKind::Cancel
    };
    let decision = insight_platform_context::decide_context_control(
        &current,
        projection.as_ref(),
        payload.as_ref(),
        kind,
    ...
    let evidence = TypedPayload::new(
        ...
        "cancel_generation":run.cancel_generation,"timeout_generation":run.timeout_generation,
        "estimated_dispatched":decision.estimated_dispatched,"accounting_basis":"fenced_read_control_reservation_ceiling",
        "consumed_query":decision.consumed_query,"reserved_result_bytes_settled":decision.result_bytes,
        "state":decision.query.state}),
    )?;
    if let (Some(lines), Some(job)) = (&quota, &current_job) {
        settle_context_quota(
            transaction,
            job,
            lines,
            &slot.quota_entry_ids[..CONTEXT_QUOTA_LINES],
            decision.consumed_query,
            decision.result_bytes,
            &evidence
                .digest
                .parse::<Sha256Digest>()
    ...
    if let (Some(current), Some(next), Some(payload)) =
        (&current_job, &decision.job, &decision.job_payload)
    {
        update_context_job(transaction, current, next, payload, None, now).await?;
        append_scheduler_event(
            transaction,
            &run.tenant_id,
            &slot.job_event_id,
            ...
            "job",
            &current.job_id,
            as_i64(next.version, "Context controlled Job version")?,
            Some(&run.run_id),
            "job.context_read_closed",
            &evidence,
        )
        .await?;
    }
    update_context_query(transaction, &current, &decision.query, limits).await?;
    let release = quota.is_some() && node_state == NodeExecutionState::Waiting;
    let run = if release {
        let row=sqlx::query("UPDATE insight_platform.runs SET active_work_count=active_work_count-1,version=version+1,updated_at=$4 WHERE tenant_id=$1 AND run_id=$2 AND version=$3 AND state='cancelling' AND active_work_count>0 RETURNING *")
            .bind(&run.tenant_id).bind(&run.run_id).bind(run.version).bind(now).fetch_optional(&mut **transaction).await?
            .ok_or(RepositoryError::Conflict("Context controlled Run permit"))?;
        crate::repository::run_from_row(row)?
    } else {
        run
    };
    append_scheduler_event(
        ...
        "context_query",
        &query_id.to_string(),
        as_i64(decision.query.version, "Context control version")?,
        Some(&run.run_id),
        "context.read_controlled",
        &evidence,
    )
    .await?;
    if release {
        append_scheduler_event(
            ...
            run.version,
            Some(&run.run_id),
            "run.context_permit_settled",
            &evidence,
        )
        .await?;
    }
    Ok(Some((run, decision.query.state.as_str().to_owned())))
}
