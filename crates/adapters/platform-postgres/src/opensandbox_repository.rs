use crate::{
    ...
    repository::{
        append_scheduler_event, job_from_row, job_projection, PgRepository, RepositoryError,
    },
};
...
impl SandboxJobRepository for PgRepository {
    ...
    async fn claim(&self, request: SandboxClaimV1) -> Result<Vec<LeasedSandboxJobV1>, Self::Error> {
        ...
        let Some(admission) = crate::claim_admission::PreparedClaimAdmission::prepare(
            &mut transaction,
            insight_platform_contracts::WorkClass::Sandbox,
        ...
        let rows = sqlx::query(
            ...
            WHERE job_kind = $1 AND job_id=ANY($3) AND work_class = 'sandbox' AND owner_kind = 'job'
        ...
        for row in rows {
            ...
            let Some(observed) = crate::recovery_isolation::collect(
                crate::recovery_isolation::persisted(locked_from_row(row), &identity),
                &mut diagnostics,
            )?
            else {
                continue;
            ...
            if !request
                ...
                .supports(&observed.record.execution_requirement)
            {
                continue;
            }
            let continuing = observed.payload.physical.is_some();
            eligible.insert(
                observed.record.job_id.clone(),
                crate::claim_admission::EligibleClaim {
                    ...
                    // Sandbox owner reserved the exact operation bundle at admission.
                    quota_costs: Vec::new(),
                },
            );
            candidates.insert(observed.record.job_id.clone(), observed);
        }
        let scheduling = admission.select(&eligible, &BTreeMap::new(), request.limit)?;
        let mut selected = scheduling
            .admitted_job_ids
            .iter()
            .zip(&request.lease_token_digests)
            .map(|(id, token)| {
                (
                    candidates
                        .remove(&id.to_string())
                        .expect("selected known Job"),
                    token.clone(),
                )
            })
            .collect::<Vec<_>>();
        selected.sort_by(|(left, _), (right, _)| {
            (&left.record.tenant_id, &left.record.job_id)
                .cmp(&(&right.record.tenant_id, &right.record.job_id))
        });
        let mut claimed = Vec::new();
        for (observed, lease_token_digest) in selected {
            ...
            .bind(&observed.record.tenant_id)
            .bind(&observed.record.job_id)
            ...
            .await?;
                ...
                if current.record.version != observed.record.version
                    || current.record.payload.digest != observed.record.payload.digest
                {
                    return Err(RepositoryError::Conflict("Sandbox partition claim changed"));
                }
                ...
                let input =
                    load_request_input(&mut object_transaction, &current.payload, false).await?;
                ...
                let produced: Result<LeasedSandboxJobV1, RepositoryError> = async {
                    ...
                    .await?;
                    let updated = crate::execution_requirements::record_attempt_build(
                        &mut object_transaction,
                        &updated,
                        &request.worker_manifest.worker_build_digest,
                    )
                    .await?;
        ...
        let scheduling = admission.settle_actual(
            ...
            &BTreeMap::new(),
            request.limit,
            claimed.iter().map(|claim| claim.job.job_id.to_string()),
        )?;
    ...
    async fn reconcile_controls(
    ...
    ) -> Result<SafetyScanPage<SandboxRepositoryDecisionV1>, Self::Error> {
        request.validate(self.recovery_batch_limit())?;
        let mut page_transaction = self.pool().begin().await?;
        let database_now = database_now(&mut page_transaction).await?;
        // Structural columns select the bounded cohort. Owning JSON is decoded per object,
        // so a malformed control cannot disappear behind a JSON presence filter.
        let candidates = sqlx::query(
            r#"
            SELECT tenant_id, job_id, created_at
            FROM insight_platform.jobs
            WHERE job_kind = $1 AND work_class = 'sandbox' AND owner_kind = 'job'
              AND terminal_at IS NULL
              AND (state = 'cancelling' OR (
                deadline <= $2 AND state IN ('ready', 'leased', 'running', 'waiting', 'retry_scheduled')
              ))
              AND ($4::timestamptz IS NULL OR (created_at,tenant_id,job_id) > ($4,$5,$6))
            ORDER BY created_at, tenant_id, job_id
            LIMIT $3
            "#,
        )
        .bind(SANDBOX_JOB_KIND).bind(database_now).bind(i64::from(request.limit))
        .bind(request.after.as_ref().map(|cursor| cursor.sort_at))
        .bind(request.after.as_ref().map(|cursor| cursor.tenant_id.to_string()))
        .bind(request.after.as_ref().map(|cursor| cursor.item_id.to_string()))
        .fetch_all(&mut *page_transaction).await?;
        ...
        for candidate in candidates {
            ...
            next_cursor = Some(SafetyScanCursor {
                sort_at: candidate.try_get("created_at")?,
                tenant_id: diagnostic.tenant_id.clone(),
                item_id: diagnostic.item_id.clone(),
            });
            let tenant_id = diagnostic.tenant_id.clone();
            let job_id = diagnostic.item_id.clone();
            let mut transaction = page_transaction.begin().await?;
            let result: Result<Option<SandboxRepositoryDecisionV1>, RepositoryError> = async {
                let snapshot = crate::recovery_isolation::persisted(
                    load_job(&mut transaction, &tenant_id, &job_id, false).await,
                    &diagnostic,
                )?;
                if snapshot.record.terminal_at.is_some()
                    || !matches!(
                        snapshot.job.state,
                        JobState::Ready
                            | JobState::Leased
                    ...
                    || (snapshot.job.state != JobState::Cancelling
                        && database_now < snapshot.job.deadline)
                {
                    // Another owner may have completed after the unlocked candidate selection.
                    return Ok(None);
                }
                let validation_diagnostic = insight_platform_jobs::store::SafeScanDiagnostic {
                    phase: SafetyScanPhase::OwnerValidation,
                    ..diagnostic.clone()
                };
                let reservation_id = crate::recovery_isolation::persisted(
                    snapshot
                        .record
                ...
                .await?;
                let invocation = crate::recovery_isolation::persisted(
                ...
                let current = crate::recovery_isolation::persisted(
                    load_job(&mut transaction, &tenant_id, &job_id, true).await,
                    &diagnostic,
                )?;
                ...
                let (controlled_invocation, cancelling_job, controlled_payload) =
                    if let Some(control) = current.payload.control.as_deref() {
                        ...
                        {
                            return Err(RepositoryError::InvalidPersistedObject(
                                validation_diagnostic,
                            ));
                        }
                        (
                ...
                .await?;
                crate::recovery_isolation::produced(decision_from(updated, terminal_payload))
                    .map(Some)
            }
            .await;
            match result {
                ...
                Ok(None) => transaction.rollback().await?,
                Err(RepositoryError::InvalidPersistedObject(diagnostic)) => {
    ...
    async fn claim_cleanup(
    ...
    ) -> Result<SafetyScanPage<ClaimedSandboxCleanupV1>, Self::Error> {
        request.validate(
            self.recovery_batch_limit(),
            u64::try_from(MAX_JOB_LEASE_MILLISECONDS).expect("positive lease maximum"),
        )?;
        let mut page_transaction = self.pool().begin().await?;
        let database_now = database_now(&mut page_transaction).await?;
        // Never cast persisted JSON while selecting a page. Decode the cleanup requirement,
        // owner/build and lease only after capturing the immutable creation cursor.
        let rows = sqlx::query(
            r#"
            SELECT * FROM insight_platform.jobs
            WHERE job_kind=$1 AND work_class='sandbox' AND owner_kind='job'
              AND state IN ('succeeded','failed','cancelled','timed_out','reconciliation_required')
              AND ($3::timestamptz IS NULL OR (created_at,tenant_id,job_id) > ($3,$4,$5))
            ORDER BY created_at,tenant_id,job_id LIMIT $2 FOR UPDATE SKIP LOCKED
        "#,
        )
        .bind(SANDBOX_JOB_KIND)
        ...
        .await?;
        let exhausted = rows.len() < usize::from(request.limit);
        let mut next_cursor = None;
        let mut records = Vec::new();
        let mut diagnostics = Vec::new();
        for row in rows {
            ...
            next_cursor = Some(SafetyScanCursor {
                sort_at: row.try_get("created_at")?,
                tenant_id: diagnostic.tenant_id.clone(),
                item_id: diagnostic.item_id.clone(),
            });
            let mut transaction = page_transaction.begin().await?;
            let result: Result<Option<ClaimedSandboxCleanupV1>, RepositoryError> = async {
                let current =
                    crate::recovery_isolation::persisted(locked_from_row(row), &diagnostic)?;
                ...
                if current.payload.cleanup.owner_process_generation_id.as_ref()
                    ...
                    && current.payload.cleanup.owner_build_digest.as_ref()
                        == Some(&request.worker_manifest.worker_build_digest)
                    && current
                        .payload
                        .cleanup
                        .expires_at
                        .is_some_and(|expires_at| expires_at > database_now)
                {
                    ...
                    let claim = ClaimedSandboxCleanupV1 {
                        fence: insight_platform_sandbox::contracts::SandboxCleanupFenceV1 {
                            schema_version: 1,
                            ...
                            expires_at: current.payload.cleanup.expires_at.ok_or_else(|| {
                                RepositoryError::CorruptRow(
                                    "claimed cleanup has no expiry".to_owned(),
                                )
                            })?,
                        },
                        job: current.job,
                ...
                if current
                    ...
                    .is_some_and(|expires_at| expires_at > database_now)
                {
                    // Another process or another build still owns the physical cleanup lease.
                    return Ok(None);
                }
                ...
                let (payload, fence) = current.payload.claim_cleanup(
                    ...
                    request.worker_manifest.worker_build_digest.clone(),
                    database_now,
                ...
                append_job_event(
                    &mut transaction,
                    &updated,
                    "sandbox.cleanup.claimed",
                    serde_json::json!({
                        "cleanup_generation": payload.cleanup.generation,
                        "process_generation_id": request.process_generation_id,
                        "worker_build_digest": request.worker_manifest.worker_build_digest,
                        "physical_attempt": fence.physical_attempt,
                        "terminal_state": next_job.state,
                    }),
                )
                .await?;
    ...
    async fn record_cleanup_observation(
        ...
        append_job_event(
            &mut transaction,
            &updated,
            "sandbox.cleanup.observed",
            serde_json::json!({
                "cleanup_generation": command.fence.cleanup_generation,
                "process_generation_id": command.fence.process_generation_id,
                "worker_build_digest": current.payload.cleanup.owner_build_digest,
                "physical_attempt": command.fence.physical_attempt,
                "cleanup_required": payload.cleanup.required,
                "terminal_state": next_job.state,
            }),
        )
        .await?;
        let fence = next_fence.unwrap_or({
            insight_platform_sandbox::contracts::SandboxCleanupFenceV1 {
