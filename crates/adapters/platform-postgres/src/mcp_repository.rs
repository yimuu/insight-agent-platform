...
use crate::repository::{
    ...
    decode_versioned_payload, job_from_row, job_lease_projection, job_projection, load_deployment,
    ...
    RepositoryError, ResourceRecord,
};
...
use insight_platform_jobs::{
    ...
    JobOwnerRef, JobProjection, WakeContract, WakeKind, WakeSource,
};
use insight_platform_mcp_host::{
    AuthenticatedMcpOAuthState, BeginMcpOAuthAuthorization, CancelMcpDiscoveryOperation,
    CompleteMcpOAuthCallback, CompleteMcpSubscriptionReconcile, CompleteMcpSubscriptionRefresh,
    ...
    ExpiredMcpDiscoveryJobObservation, FinalizeMcpDiscovery, McpAuthorizationBindingRecord,
    McpAuthorizationContext, McpAuthorizationReplacement, McpDiscoveryAdmission,
    McpDiscoveryAttemptResolution, McpDiscoveryContractQuery, McpDiscoveryExecutionContract,
    ...
    McpOAuthExchangeContract, McpOAuthPkceCleanupHint, McpResourceSubscriptionBinding,
...
use insight_platform_context::{
    ClaimContextSubscriptionRefreshJobs, ContextSubscriptionRefreshClaimSlot,
    CONTEXT_SUBSCRIPTION_REFRESH_MAX_BATCH,
};

#[derive(Debug, Clone, PartialEq)]
pub struct ClaimedContextSubscriptionRefresh {
    pub job: insight_platform_jobs::store::JobRecord,
...
pub struct DriveExpiredContextSubscriptionRefreshJobs {
    pub after: Option<insight_platform_jobs::store::SafetyScanCursor>,
    pub shard_index: u16,
    pub shard_count: u16,
    pub limit: u16,
...
impl DriveExpiredContextSubscriptionRefreshJobs {
    fn validate(&self) -> Result<(), RepositoryError> {
        if let Some(after) = &self.after {
            after.validate(ResourceKind::Job)?;
        }
...
impl PgRegistryTransaction {
    ...
    pub async fn complete_mcp_oauth_callback(
        ...
        let cleanup_job_id =
            ResourceId::from_uuid_v7(ResourceKind::Job, command.audit.event_id.uuid())
                .map_err(|error| RepositoryError::InvalidInput(error.to_string()))?;
        crate::mcp_oauth_cleanup_repository::create_cleanup_job(
            &mut transaction,
            &next_task,
            crate::mcp_oauth_cleanup_repository::CleanupJobCreation {
                event_id: &command.audit.event_id,
                trace: command.audit.trace,
                job_id: &cleanup_job_id,
                predecessor: None,
                attempt_limit: insight_platform_mcp_host::MCP_OAUTH_CLEANUP_ATTEMPT_LIMIT,
                now: database_now,
            },
        )
        .await?;
...
impl PgRepository {
    ...
    pub async fn drive_expired_mcp_oauth_tasks(
    ...
    ) -> Result<insight_platform_jobs::store::SafetyScanPage<TaskRecord>, RepositoryError> {
        command.validate().map_err(invalid_authorization)?;
        let mut transaction = self.pool().begin().await?;
        let database_now = database_now(&mut transaction).await?;
        let rows = sqlx::query(
            r#"
            SELECT tenant_id, task_id, deadline AS scan_sort_at
            FROM insight_platform.tasks
            WHERE task_kind = 'external_authorization'
              AND state = 'pending' AND responded_at IS NULL AND deadline <= $1
              AND payload->'definition'->>'kind' = 'mcp_oauth_authorization'
              AND ($3::timestamptz IS NULL OR (deadline,tenant_id,task_id)>($3,$4::text,$5::text))
            ORDER BY deadline, tenant_id, task_id
            LIMIT $2
            "#,
        )
        .bind(database_now)
        ...
        let last_cursor = rows
            .last()
            .map(|row| {
                crate::repository::safety_scan_cursor_from_row(
                    row,
                    "task_id",
        ...
        for (row, slot) in rows.into_iter().zip(&command.slots) {
            ...
            let result: Result<(), RepositoryError> = async {
                let identity = crate::recovery_isolation::identity(
                    &row,
                    "task_id",
                    ResourceKind::Interaction,
                ...
                if current.state != TaskState::Pending
                    || current.responded_at.is_some()
                    || current.deadline > database_now
                {
                    return Ok(());
                }
                let projection =
                    crate::recovery_isolation::persisted(task_projection(&current), &identity)?;
                let TaskDefinition::McpOAuthAuthorization { binding, .. } =
                ...
                let produced: Result<TaskRecord, RepositoryError> = async {
                    update_mcp_oauth_task(&mut object_transaction, &current, &next, database_now)
                        .await?;
                    let cleanup_job_id =
                        ResourceId::from_uuid_v7(ResourceKind::Job, slot.event_id.uuid())
                            .map_err(|error| RepositoryError::InvalidInput(error.to_string()))?;
                    crate::mcp_oauth_cleanup_repository::create_cleanup_job(
                        ...
                        crate::mcp_oauth_cleanup_repository::CleanupJobCreation {
                            ...
                            predecessor: None,
                            attempt_limit:
                                insight_platform_mcp_host::MCP_OAUTH_CLEANUP_ATTEMPT_LIMIT,
                            now: database_now,
                        },
                    )
                    .await?;
        ...
        transaction.commit().await?;
        Ok(
            crate::repository::safety_scan_page(expired, scanned_count, command.limit, last_cursor)
    ...
    pub async fn create_mcp_discovery_operation(
        ...
        .execute(&mut *transaction)
        .await?;
        let execution_requirement_0 =
            crate::execution_requirements::StoredExecutionRequirement::new(
                &admission
                    .execution_requirement_for_protocol(
                        &discovery_execution.server,
                        &discovery_execution.protocol_profile,
                    )
                    .map_err(|failure| RepositoryError::InvalidInput(failure.to_string()))?,
            )?;
        sqlx::query(
            ...
            INSERT INTO insight_platform.jobs (
                tenant_id, job_id, job_kind, work_class, owner_kind, owner_id, invocation_id, state, attempt_limit, scheduled_at, deadline, priority, request_digest, payload_schema_version, payload, payload_digest, created_at, updated_at, trace_id, scheduler_partition_id, execution_requirement_version, execution_requirement, execution_requirement_digest
            ) VALUES (
                $1, $2, 'mcp_discovery', 'mcp', 'mcp_operation', $3, $3, 'ready', $4, $5, $6, 0, $7, $8, $9, $10, (SELECT observed_at FROM job_creation_time), GREATEST($5::timestamptz, (SELECT observed_at FROM job_creation_time)), $11, (SELECT scheduler_partition_id FROM insight_platform.tenants WHERE tenant_id=$1), $12, $13, $14
            )
            "#,
        )
        .bind(command.audit.tenant_id.to_string())
        ...
        .bind(command.audit.trace.trace_id.to_string())
        .bind(execution_requirement_0.version)
        .bind(&execution_requirement_0.value)
        .bind(&execution_requirement_0.digest)
        .execute(&mut *transaction)
        .await?;
        ...
        sqlx::query(
            ...
            WITH job_creation_time AS MATERIALIZED (SELECT clock_timestamp() AS observed_at)
            INSERT INTO insight_platform.jobs (
                tenant_id, job_id, job_kind, work_class, owner_kind, owner_id, state, attempt_limit, scheduled_at, deadline, priority, request_digest, payload_schema_version, payload, payload_digest, created_at, updated_at, trace_id, scheduler_partition_id, execution_requirement_version, execution_requirement, execution_requirement_digest
            ) VALUES (
                $1, $2, 'artifact_scan', 'artifact', 'artifact', $3, 'waiting', $4, $5, $6, 0, $7, $8, $9, $10, (SELECT observed_at FROM job_creation_time), GREATEST($5::timestamptz, (SELECT observed_at FROM job_creation_time)), $11, (SELECT scheduler_partition_id FROM insight_platform.tenants WHERE tenant_id=$1), $12, $13, $14
            )
            "#,
        )
        .bind(command.audit.tenant_id.to_string())
        ...
        .bind(command.audit.trace.trace_id.to_string())
        .bind(execution_requirement_1.version)
        .bind(&execution_requirement_1.value)
        .bind(&execution_requirement_1.digest)
        .execute(&mut *transaction)
        .await?;
        append_command_event(
            &mut transaction,
...
impl PgRepository {
    pub async fn admit_context_subscription_refresh(
        ...
        sqlx::query(
            ...
            WITH job_creation_time AS MATERIALIZED (SELECT clock_timestamp() AS observed_at)
            INSERT INTO insight_platform.jobs (
                tenant_id, job_id, job_kind, work_class, owner_kind, owner_id, invocation_id, state, version, attempt_no, attempt_limit, lease_epoch, scheduled_at, deadline, priority, request_digest, payload_schema_version, payload, payload_digest, created_at, updated_at, trace_id, scheduler_partition_id, execution_requirement_version, execution_requirement, execution_requirement_digest
            ) VALUES (
                $1, $2, 'context_subscription_refresh', 'context', 'mcp_operation', $3, $3, 'ready', 1, 0, $4, 0, $5, $6, 0, $7, $8, $9, $10, (SELECT observed_at FROM job_creation_time), GREATEST($5::timestamptz, (SELECT observed_at FROM job_creation_time)), $11, (SELECT scheduler_partition_id FROM insight_platform.tenants WHERE tenant_id=$1), $12, $13, $14
            )
            "#,
        )
        ...
        .bind(command.audit.trace.trace_id.to_string())
        .bind(execution_requirement_2.version)
        .bind(&execution_requirement_2.value)
        .bind(&execution_requirement_2.digest)
        .execute(&mut *transaction)
        .await?;
    ...
    pub async fn claim_context_subscription_refresh_jobs(
        ...
        let rows=sqlx::query("SELECT * FROM insight_platform.jobs WHERE job_id=ANY($1) AND job_kind='context_subscription_refresh' AND work_class='context' AND owner_kind='mcp_operation' AND state IN ('ready','retry_scheduled') AND terminal_at IS NULL AND worker_id IS NULL AND scheduled_at<=$2 AND COALESCE(retry_at,scheduled_at)<=$2 AND deadline>$2 AND attempt_no<attempt_limit ORDER BY tenant_id,owner_id,job_id")
            .bind(admission.candidate_ids()).bind(database_now).fetch_all(&mut *transaction).await?;
            ...
            let result: Result<(), RepositoryError> = async {
                ...
                let validation = async {
                    let payload: ContextSubscriptionRefreshJobPayload =
                        decode_versioned_payload(&job.payload, "Context refresh Job")?;
                    payload
                        .validate_at(database_now)
                        .map_err(invalid_context_subscription_admission)?;
                    ...
                    if job.execution_requirement != requirement {
                        return Err(RepositoryError::CorruptRow(
                            "Context refresh routing differs from owner".into(),
                        ));
                    }
                    Ok(requirement)
                }
                .await;
                let requirement = crate::recovery_isolation::job(
                    validation,
                    &job,
                    insight_platform_jobs::store::SafetyScanPhase::OwnerValidation,
                )?;
        ...
        let tenants = candidates
            ...
            .collect::<Vec<_>>();
        let rows=sqlx::query("SELECT tenant_id,quota_account_id,limit_value,reserved_value,used_value FROM insight_platform.quota_accounts WHERE tenant_id=ANY($1) AND scope_kind='tenant' AND scope_id=tenant_id AND work_class='context' AND metric=$2 ORDER BY tenant_id,quota_account_id FOR UPDATE")
            .bind(tenants).bind(QuotaDimension::WorkClassConcurrentOperations.as_str()).fetch_all(&mut *transaction).await?;
        let mut by_tenant = BTreeMap::new();
        let mut available = BTreeMap::new();
        for row in rows {
            let tenant: String = row.try_get("tenant_id")?;
            let id: ResourceId = row
            ...
            let balance = row
                .try_get::<i64, _>("limit_value")?
                .saturating_sub(row.try_get("reserved_value")?)
                .saturating_sub(row.try_get("used_value")?)
                .max(0) as u64;
            if by_tenant.insert(tenant, id.clone()).is_some() {
                return Err(RepositoryError::CorruptRow(
                    "duplicate Context tenant quota account".into(),
                ));
            }
            available.insert(id, balance);
        }
        let eligible = candidates
            .iter()
            .filter_map(|job| {
                by_tenant.get(&job.tenant_id).map(|account| {
                    (
                        job.job_id.clone(),
                        crate::claim_admission::EligibleClaim {
                            ...
                            quota_costs: vec![insight_platform_scheduler::partitioned::QuotaCost {
        ...
        let scheduling = admission.select(&eligible, &available, command.slots.len() as u16)?;
        ...
        let selected = candidates
            .into_iter()
            .filter(|job| admitted.contains(&job.job_id))
            .zip(&command.slots);
        let mut claimed = Vec::new();
        for (observed, slot) in selected {
            let mut object_transaction = transaction.begin().await?;
            let result: Result<(), RepositoryError> = async {
                ...
                let payload: ContextSubscriptionRefreshJobPayload = crate::recovery_isolation::job(
                ...
                let running = decide_job_start(&leased, &lease_fence, database_now)
                    .map_err(map_job_decision_error)?;
                let produced: Result<ClaimedContextSubscriptionRefresh, RepositoryError> = async {
                    ...
                    .await?;
                    let job = crate::execution_requirements::record_attempt_build(
                        &mut object_transaction,
                        &job,
            ...
            .await?;
                    Ok(ClaimedContextSubscriptionRefresh {
                        job,
                        attempt,
                        quota_account_id,
                    })
                ...
                .await;
                claimed.push(crate::recovery_isolation::produced(produced)?);
                Ok(())
            }
            .await;
            match result {
        ...
        let scheduling = admission.settle_actual(
            &eligible,
            &available,
            command.slots.len() as u16,
            claimed.iter().map(|claim| claim.job.job_id.clone()),
        )?;
    ...
    pub async fn drive_expired_context_subscription_refresh_jobs(
        ...
        insight_platform_jobs::store::SafetyScanPage<insight_platform_jobs::store::JobRecord>,
        RepositoryError,
    > {
        command.validate()?;
        let mut transaction = self.pool().begin().await?;
        ...
        let rows = sqlx::query(
            ...
            WHERE job.work_class = 'context'
              ...
              AND ($5::timestamptz IS NULL OR (job.lease_expires_at,job.tenant_id,job.job_id) > ($5,$6,$7))
            ORDER BY job.lease_expires_at, job.tenant_id, job.job_id
            LIMIT $2
            "#,
        )
        ...
        .bind(command.after.as_ref().map(|cursor| cursor.tenant_id.to_string()))
        .bind(command.after.as_ref().map(|cursor| cursor.item_id.to_string()))
        .fetch_all(&mut *transaction)
        .await?;
        let scanned_count = rows.len();
        let last_cursor = rows
            .last()
            .map(|row| {
                Ok::<_, RepositoryError>(insight_platform_jobs::store::SafetyScanCursor {
                    sort_at: row.try_get("lease_expires_at")?,
                    tenant_id: parse_resource_id_column(row, "tenant_id")?,
                    item_id: parse_resource_id_column(row, "job_id")?,
                })
            })
            .transpose()?;
        let mut diagnostics = Vec::new();
        ...
        for (row, slot) in rows.into_iter().zip(&command.slots) {
            let mut object_tx = transaction.begin().await?;
            let result: Result<_, RepositoryError> = async {
                let transaction = &mut object_tx;
                ...
                crate::recovery_isolation::job(
                    (|| {
                        payload
                            ...
                            .validate_frozen()
                            .map_err(invalid_context_subscription_admission)?;
                        if payload.request.deadline != observed.deadline {
                            return Err(RepositoryError::CorruptRow(
                                "Context refresh deadline drift".into(),
                            ));
                        }
                        if observed.execution_requirement
                            != payload.execution_requirement().map_err(|_| {
                                RepositoryError::CorruptRow(
                                    "invalid Context refresh execution requirement".into(),
                                )
                            })?
                        {
                            return Err(RepositoryError::CorruptRow(
                                "Context refresh execution requirement drift".into(),
                            ));
                        }
                        Ok(())
                    })(),
                ...
                let subscription = crate::recovery_isolation::addressed(
                ...
                .await?;
                let current = crate::recovery_isolation::job(
                    load_context_subscription_refresh_job(
                        transaction,
                        &payload.request.tenant_id,
                        &observed_job_id,
                        true,
                ...
                .await;
                crate::recovery_isolation::produced(produced)
            }
            .await;
            match result {
                Ok(job) => {
                    object_tx.commit().await?;
                    recovered.push(job);
                }
                Err(RepositoryError::InvalidPersistedObject(diagnostic)) => {
                    object_tx.rollback().await?;
                    diagnostics.push(diagnostic);
                }
        ...
        transaction.commit().await?;
        Ok(crate::repository::safety_scan_page(
            recovered,
            scanned_count,
            command.limit,
            last_cursor,
...
fn require_same_context_subscription_refresh_job(
    observed: &insight_platform_jobs::store::JobRecord,
    current: &insight_platform_jobs::store::JobRecord,
) -> Result<(), RepositoryError> {
...
fn map_context_subscription_admission_error(
    ...
    match failure {
        ...
        | RepositoryError::CapacityUnavailable => ContextSubscriptionAdmissionError::Unavailable,
...
impl ContextSubscriptionRefreshResolver for PgRepository {
    async fn resolve_context_subscription_refresh(
        ...
        resolved.validate_for(database_now, attempt)?;
        transaction
            .commit()
            .await
            .map_err(|_| ContextSubscriptionExecutionError::Unavailable)?;
...
fn map_mcp_subscription_persistence_error(
    ...
    match failure {
        ...
        RepositoryError::CorruptRow(_)
        | RepositoryError::InvalidPersistedObject(_)
        | RepositoryError::CapacityUnavailable => {
            McpSubscriptionPersistenceError::AuthorityUnavailable
        }
        RepositoryError::InvalidInput(_) => McpSubscriptionPersistenceError::InvalidCommand,
        RepositoryError::NotFound(_)
...
fn map_mcp_oauth_start_commit_error(
    ...
    match error {
        ...
        RepositoryError::CorruptRow(_) | RepositoryError::InvalidPersistedObject(_) => {
            McpOAuthAuthorizationStartAuthorityError::Unavailable
        }
        _ => McpOAuthAuthorizationStartAuthorityError::NotFoundOrChanged,
    }
...
fn map_mcp_oauth_resolve_error(error: RepositoryError) -> McpOAuthCallbackAuthorityError {
    match error {
        ...
        | RepositoryError::PublicHistoryGap { .. }
        | RepositoryError::CorruptRow(_)
        | RepositoryError::InvalidPersistedObject(_) => McpOAuthCallbackAuthorityError::Unavailable,
...
impl McpDiscoveryExecutionContractResolver for PgRepository {
    async fn resolve_mcp_discovery_execution(
        ...
        let contract = McpDiscoveryExecutionContract::build(
            database_now,
            NewMcpDiscoveryExecutionContract {
...
pub(crate) async fn resolve_mcp_execution_contract(
    ...
    McpHostExecutionContract::build(
        database_now,
...
fn map_execution_resolution_error(failure: RepositoryError) -> McpExecutionContractResolutionError {
    match failure {
        RepositoryError::Database(_) | RepositoryError::CapacityUnavailable => {
            McpExecutionContractResolutionError::AuthorityUnavailable
        }
        RepositoryError::InvalidInput(_)
        | RepositoryError::NotFound(_)
        | RepositoryError::Conflict(_)
...
fn map_context_subscription_resolution_error(
    ...
    match failure {
        ...
        | RepositoryError::PublicHistoryGap { .. }
        | RepositoryError::CorruptRow(_)
        | RepositoryError::InvalidPersistedObject(_) => ContextSubscriptionExecutionError::Rejected,
    }
}
...
fn map_mcp_discovery_persistence_error(failure: RepositoryError) -> McpDiscoveryPersistenceError {
    match failure {
        RepositoryError::Database(_) | RepositoryError::CapacityUnavailable => {
            McpDiscoveryPersistenceError::AuthorityUnavailable
        }
        ...
        | RepositoryError::PublicHistoryGap { .. }
        | RepositoryError::CorruptRow(_)
        | RepositoryError::InvalidPersistedObject(_) => McpDiscoveryPersistenceError::Conflict,
    }
}
...
async fn insert_mcp_oauth_task(
    ...
    let payload = TypedPayload::with_limit(2, task_payload, 262_144)?;
    sqlx::query(
        r#"
...
async fn update_mcp_oauth_task(
    ...
    let payload = TypedPayload::with_limit(2, &next.payload, 262_144)?;
    let affected = sqlx::query(
        r#"
...
impl PgRegistryTransaction {
    ...
    pub async fn create_mcp_resource_subscription(
        ...
        let session = McpSessionRecord::disconnected(database_now, session_key)
            .map_err(invalid_mcp_subscription)?;
        ...
        .map_err(invalid_mcp_subscription)?;
        let payload = McpSubscriptionPayload::pending(database_now, binding.clone(), session)
            .map_err(invalid_mcp_subscription)?;
        let job_payload =
        ...
        .execute(&mut *transaction)
        .await?;
        let execution_requirement_3 =
            crate::execution_requirements::StoredExecutionRequirement::new(
                &binding
                    .execution_requirement(&contract)
                    .map_err(|failure| RepositoryError::InvalidInput(failure.to_string()))?,
            )?;
        sqlx::query(
            r#"
            ...
            INSERT INTO insight_platform.jobs (
                ...
                $1, $2, 'mcp_subscription', 'mcp', 'mcp_operation', $3, $3, 'ready', $4, $5, $6, 0, $7, $8, $9, $10, (SELECT observed_at FROM job_creation_time), GREATEST($5::timestamptz, (SELECT observed_at FROM job_creation_time)), $11, (SELECT scheduler_partition_id FROM insight_platform.tenants WHERE tenant_id=$1), $12, $13, $14
            )
            "#,
        )
        .bind(command.audit.tenant_id.to_string())
        ...
        .bind(&job_typed.digest)
        .bind(command.audit.trace.trace_id.to_string())
        .bind(execution_requirement_3.version)
        .bind(&execution_requirement_3.value)
        .bind(&execution_requirement_3.digest)
        .execute(&mut *transaction)
        .await?;
        append_command_event(
            &mut transaction,
...
impl PgRepository {
    ...
    pub async fn list_due_mcp_subscription_reconciliations_global(
        ...
        RepositoryError,
    > {
        self.scan_mcp_subscription_reconciliations(None, limit, minimum_idle_milliseconds, after)
            .await
    }
    ...
    > {
        ...
        self.scan_mcp_subscription_reconciliations(
            Some(&scan.tenant_id),
            scan.limit,
            scan.minimum_idle_milliseconds,
            scan.after,
        )
        .await
    }
    ...
    pub async fn wake_mcp_subscription_reconcile(
        ...
        let job = crate::recovery_isolation::addressed(
            load_mcp_subscription_job(
                &mut transaction,
                &command.candidate.tenant_id,
                &command.candidate.job_id,
                true,
    ...
    async fn scan_mcp_subscription_records(
        &self,
        ...
        insight_platform_jobs::store::SafetyScanPage<(
            ...
            LockedMcpSubscriptionJob,
            insight_platform_contracts::TraceIdentityV1,
            DateTime<Utc>,
        )>,
        RepositoryError,
    > {
        if limit == 0 || limit > MAX_MCP_SUBSCRIPTION_RECONCILE_SCAN {
            return Err(RepositoryError::InvalidInput(
                "invalid MCP subscription scan bound".into(),
            ));
        }
        if let Some(cursor) = &after {
            cursor.validate(ResourceKind::McpOperation)?;
        }
        let mut tx = self.pool().begin().await?;
        let now = database_now(&mut tx).await?;
        // Safe structural candidates only. Expiry strings belong to the typed owner and are
        // decoded per object; one malformed timestamp cannot turn this page into a SQL error.
        let rows=sqlx::query(r#"
            SELECT subscription.tenant_id,subscription.invocation_id,subscription.created_at AS scan_sort_at,
                   subscription.trace_id,job.job_id
            FROM insight_platform.invocations subscription JOIN insight_platform.jobs job
              ON job.tenant_id=subscription.tenant_id AND job.invocation_id=subscription.invocation_id
            WHERE ($1::text IS NULL OR subscription.tenant_id=$1)
              AND subscription.invocation_kind='mcp_subscription' AND subscription.owner_kind='mcp_operation'
              AND subscription.owner_id=subscription.invocation_id AND subscription.terminal_at IS NULL
              AND subscription.state IN ('pending','active') AND subscription.deadline>$2
              AND job.work_class='mcp' AND job.job_kind='mcp_subscription' AND job.owner_kind='mcp_operation'
              AND job.owner_id=subscription.invocation_id AND job.terminal_at IS NULL
              AND job.state IN ('leased','running','waiting')
              AND ($3::timestamptz IS NULL OR (subscription.created_at,subscription.tenant_id,subscription.invocation_id)>($3,$4,$5))
            ORDER BY subscription.created_at,subscription.tenant_id,subscription.invocation_id LIMIT $6
        "#).bind(tenant_id.map(ToString::to_string)).bind(now)
            .bind(after.as_ref().map(|c|c.sort_at)).bind(after.as_ref().map(|c|c.tenant_id.to_string()))
            .bind(after.as_ref().map(|c|c.item_id.to_string())).bind(i64::from(limit)).fetch_all(&mut *tx).await?;
        ...
        for row in rows {
            ...
            let result = async {
                ...
                .await?;
                let observed_job_id = parse_resource_id_column(&row, "job_id")?;
                let job = load_mcp_subscription_job(
                    &mut tx,
                    &diagnostic.tenant_id,
                    &observed_job_id,
                ...
                .await?;
                if record.job_id != observed_job_id
                    || job.owner_id != record.subscription_id
                    || job.payload.binding_digest != record.payload.binding.canonical_digest
                {
                    return Err(RepositoryError::CorruptRow(
                        "MCP subscription binding and Job disagree".into(),
                    ));
                }
                let trace = insight_platform_contracts::TraceIdentityV1::new(
                    row.try_get::<String, _>("trace_id")?.parse().map_err(|_| {
                        RepositoryError::CorruptRow("invalid subscription trace identity".into())
                    })?,
                );
                Ok((record, job, trace, now))
            }
            .await;
            if let Some(record) = crate::recovery_isolation::collect(
                crate::recovery_isolation::persisted(result, &diagnostic),
                &mut diagnostics,
            )? {
                records.push(record);
            }
        ...
        tx.commit().await?;
        Ok(
            crate::repository::safety_scan_page(records, scanned, limit, cursor)
                .with_diagnostics(diagnostics),
        )
    }
    async fn scan_mcp_subscription_recoveries(
        &self,
    ...
    > {
        let page = self
            .scan_mcp_subscription_records(tenant, limit, after)
            .await?;
        let mut records = Vec::new();
        let mut diagnostics = page.diagnostics;
        for (record, job, trace, now) in page.records {
            let cause = if matches!(job.state, JobState::Leased | JobState::Running)
            ...
            let candidate = DueMcpSubscriptionRecovery {
                ...
                observed_lease_generation: (cause == McpSubscriptionRecoveryCause::ExpiredLease)
                    .then_some(job.lease_generation),
    ...
    async fn scan_mcp_subscription_reconciliations(
        &self,
        tenant: Option<&ResourceId>,
        limit: u16,
        minimum_idle_milliseconds: u64,
        ...
        RepositoryError,
    > {
        if !(MIN_MCP_SUBSCRIPTION_RECONCILE_IDLE_MILLISECONDS
            ..=insight_platform_mcp_host::MAX_MCP_SUBSCRIPTION_RECONCILE_IDLE_MILLISECONDS)
            .contains(&minimum_idle_milliseconds)
        {
            return Err(RepositoryError::InvalidInput(
                "invalid MCP reconcile interval".into(),
            ));
        }
        let page = self
            .scan_mcp_subscription_records(tenant, limit, after)
        ...
        let mut diagnostics = page.diagnostics;
        for (record, job, trace, now) in page.records {
            let not_updated_after = now
                .checked_sub_signed(chrono::Duration::milliseconds(
                    i64::try_from(minimum_idle_milliseconds).map_err(|_| {
                        RepositoryError::InvalidInput("invalid MCP reconcile interval".into())
                    })?,
                ))
                .ok_or_else(|| {
                    RepositoryError::InvalidInput("invalid MCP reconcile interval".into())
                })?;
            if record.state != McpSubscriptionState::Active
                || record.updated_at > not_updated_after
    ...
    pub async fn list_due_mcp_subscription_recoveries_global(
        ...
        RepositoryError,
    > {
        self.scan_mcp_subscription_recoveries(None, limit, after)
            .await
    }
    ...
    > {
        scan.validate().map_err(invalid_mcp_subscription)?;
        self.scan_mcp_subscription_recoveries(Some(&scan.tenant_id), scan.limit, scan.after)
            .await
    }
...
async fn load_mcp_subscription_job(
    ...
    payload
        ...
        .map_err(|failure| RepositoryError::CorruptRow(failure.to_string()))?;
    let locked = LockedMcpSubscriptionJob {
        job_id: parse_resource_id_column(&row, "job_id")?,
        owner_id,
        state: row
            .try_get::<String, _>("state")?
    ...
    if (lease.is_some()
        ...
        || (lease.is_none() && matches!(locked.state, JobState::Leased | JobState::Running))
    {
        return Err(RepositoryError::CorruptRow(
            "MCP subscription Job state and lease disagree".into(),
        ));
    }
    // Subscription sessions own their wake identity; their compact Job payload does not
    // contain the generic WakeContract used by orchestration and other domains.
    let valid_wake = if locked.state == JobState::Waiting {
        record.wake_kind.as_deref() == Some("remote_invocation")
            && record.wake_state.as_deref() == Some("pending")
            && record.wake_generation > 0
    } else {
        record.wake_kind.is_none() && record.wake_state.is_none() && record.wake_generation == 0
    };
    if !valid_wake {
        return Err(RepositoryError::CorruptRow(
            "MCP subscription Job wake columns disagree".into(),
        ));
    }
    Ok(locked)
}
