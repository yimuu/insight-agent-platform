...
use insight_platform_jobs::store::{SafetyScanCursor, SafetyScanPage, SafetyScanPhase};
    ...
    pub async fn recover_expired_context_dataset_build_jobs_for_sources(
        ...
        let rows = sqlx::query(
            ...
            WHERE work_class = 'context' AND job_kind = 'context_dataset_build'
              ...
              AND ($4::timestamptz IS NULL OR (lease_expires_at,tenant_id,job_id) > ($4,$5,$6))
        ...
        for row in rows {
            ...
            let outcome: Result<Option<ResourceId>, RepositoryError> = async {
                ...
                let projection = crate::recovery_isolation::job(
                    (|| {
                        let owner = ResourceId::parse_expected(
                            &current.owner_id,
                        ...
                        let payload: ContextDatasetBuildJobPayload = decode_versioned_payload(
                            &current.payload,
                            "Context Dataset build Job",
                        )?;
                        payload.validate_for_owner(&owner)?;
                        if payload.job_id.to_string() != current.job_id
                            || payload.execution_requirement()? != current.execution_requirement
                            || !supported.iter().any(|digest| {
                                digest == payload.source_binding.canonical_digest.as_str()
                            })
                        {
                ...
                let next = decide_expired_lease(
                    ...
                    u64::try_from(current.version).map_err(|_| {
                        RepositoryError::CorruptRow(
                            "negative Context Dataset Job version".to_owned(),
                        )
                    })?,
