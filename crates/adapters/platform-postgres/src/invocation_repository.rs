...
impl InvocationTransaction for PgInvocationTransaction {
    ...
    type JobRecord = insight_platform_jobs::store::JobRecord;
    ...
    async fn resolve_capability_approval(
        ...
        let observed_task = crate::repository::load_task_by_text(
            ...
            &command.approval_task_id.to_string(),
        )
        .await?;
        ...
        if !insight_platform_tasks::is_eligible_responder(&task_projection, &resolver)? {
            return Err(RepositoryError::PermissionDenied);
        }
        ...
        let task_payload = TypedPayload::new(2, &next_task.payload)?;
        let updated_task =
            sqlx::query(
                r#"
...
pub(crate) async fn load_capability_invocation(
    ...
    persisted_capability_invocation_from_row(row)
}

fn persisted_capability_invocation_from_row(
    row: PgRow,
) -> Result<CapabilityInvocationRecord, RepositoryError> {
    ...
    crate::recovery_isolation::persisted(capability_invocation_from_row(row), &diagnostic)
}
...
fn require_exact_approval_task(
    ...
    task: &insight_platform_tasks::store::TaskRecord,
