...
use insight_platform_models::execution::ModelExecutionAuthority;
use insight_platform_orchestrator::store::{OrchestrationConvergenceSlot, RunRecord};
...
use insight_platform_models::{
    ...
    ExpiredModelLeaseObservation, ModelAdmissionFacts, ModelAttemptMeasurement, ModelControlKind,
    ModelDispatchOutcome, ModelExecutionInput, ModelExecutionInputMaterial, ModelJobPayload,
    ModelOutputValue, ModelQuotaCeiling, ModelQuotaSettlement, ModelRequestValue,
    ModelToolProjection, ModelTurnLimits, ModelTurnPayload, ModelTurnRecord, ModelTurnStore,
    ModelTurnTransaction, ModelWorkerAudit, PrepareModelDispatch, PreparedModelDispatch,
    MODEL_QUOTA_LINES,
};
...
impl ClaimedModelExecution {
    ...
    pub fn adapter_job(
        ...
        limits: ModelTurnLimits,
    ) -> Result<insight_platform_models::execution::ExecuteModelAdapterJob, RepositoryError> {
        ...
        let command = insight_platform_models::execution::ExecuteModelAdapterJob {
            audit,
...
async fn load_model_turn(
    ...
    persisted_model_turn_from_row(row, limits)
}

fn persisted_model_turn_from_row(
    row: PgRow,
    limits: ModelTurnLimits,
) -> Result<ModelTurnRecord, RepositoryError> {
    ...
    crate::recovery_isolation::persisted(model_turn_from_row(row, limits), &diagnostic)
}
...
impl PgRepository {
    ...
    pub async fn scan_cancelling_model_executions(
        ...
        limit: u16,
    ) -> Result<SafetyScanPage<ControlledModelExecution>, RepositoryError> {
        ...
        let rows = sqlx::query(
            r#"
            SELECT *, updated_at AS scan_sort_at
            ...
            WHERE work_class = 'model' AND owner_kind = 'model_turn'
              ...
              AND ($3::timestamptz IS NULL OR (updated_at, tenant_id, job_id) >
        ...
        let last_cursor = rows
            ...
            .transpose()?;
        let mut executions = Vec::with_capacity(scanned_count);
        let mut diagnostics = Vec::new();
        for row in rows {
            let result = async {
                let job = crate::repository::persisted_job_from_row(row)?;
                let (tenant_id, model_turn_id) = crate::recovery_isolation::job(
                    (|| {
                ...
                crate::recovery_isolation::job(
                    (|| {
                        ...
                        let payload: ModelJobPayload =
                            decode_versioned_payload(&job.payload, "Model Job")?;
                        payload
                ...
                Ok(ControlledModelExecution {
                    turn,
                    job: Some(job),
                })
            }
            .await;
            if let Some(execution) = crate::recovery_isolation::collect(result, &mut diagnostics)? {
                executions.push(execution);
            }
        }
        transaction.commit().await?;
        Ok(
            safety_scan_page(executions, scanned_count, limit, last_cursor)
    ...
    pub async fn drive_expired_model_jobs(
        ...
        let rows = sqlx::query(
            r#"
            SELECT job.*, LEAST(job.lease_expires_at, job.deadline) AS scan_sort_at
            ...
            WHERE job.work_class = 'model' AND job.owner_kind = 'model_turn'
              AND job.terminal_at IS NULL AND turn.terminal_at IS NULL
              AND ((job.state = 'running' AND turn.state = 'in_flight'
                    AND job.lease_expires_at <= $1 AND job.deadline > $1)
                   OR (job.state = 'cancelling' AND turn.state = 'cancelling'
                       AND LEAST(job.lease_expires_at, job.deadline) <= $1))
              AND mod(('x' || right(job.job_id, 8))::bit(32)::bigint, $4) = $3
              AND ($5::timestamptz IS NULL OR
                   (LEAST(job.lease_expires_at, job.deadline), job.tenant_id, job.job_id) >
                   ($5::timestamptz, $6::text, $7::text))
            ORDER BY LEAST(job.lease_expires_at, job.deadline), job.tenant_id, job.job_id
            LIMIT $2
            "#,
        )
        .bind(scan_now)
    ...
    async fn recover_expired_model_job(
        ...
        crate::recovery_isolation::job(
            decode_versioned_payload::<ModelJobPayload>(&observed.payload, "Model Job"),
            observed,
            insight_platform_jobs::store::SafetyScanPhase::OwnerDecode,
        )?;
        let mut transaction = self.pool().begin().await?;
        ...
        if database_now < scan_now
            ...
            || current_job.state != observed.state
            || current_job.lease_expires_at.is_none_or(|expires_at| {
                if current_turn.state == ModelTurnState::Cancelling {
                    expires_at > database_now && current_job.deadline > database_now
                } else {
        ...
        let payload: ModelJobPayload = crate::recovery_isolation::job(
        ...
        crate::recovery_isolation::job(
            payload
                .validate_for(&current_turn, &projection, self.model_turn_limits())
                .map_err(RepositoryError::from),
            &current_job,
            insight_platform_jobs::store::SafetyScanPhase::OwnerValidation,
        )?;
        let recovering_cancellation = current_turn.state == ModelTurnState::Cancelling
            && projection.state == JobState::Cancelling;
        let retry_at =
            if !recovering_cancellation && projection.attempt_count < projection.attempt_limit {
                Some(
                    database_now
        ...
        let decision = if recovering_cancellation {
            let usage_reservation_id =
                payload
                    .active_usage_reservation_id
                    .as_ref()
                    .ok_or_else(|| {
                        RepositoryError::CorruptRow(
                            "cancelling Model has no active reservation".to_owned(),
                        )
                    })?;
            let measurement = ModelAttemptMeasurement::conservative_dispatched(
                &current_turn.payload.admission,
                self.model_turn_limits(),
            )?;
            // Only an expired lease or immutable deadline permits this unfenced owner path.
            // No request is dispatched again; the existing attempt is charged conservatively.
            decide_model_cancellation_outcome(
                &current_turn,
                &projection,
                &payload,
                None,
        ...
        crate::recovery_isolation::produced(
            async {
                ...
                append_scheduler_event(
                    ...
                    if recovering_cancellation {
                        "model.cancellation_recovered"
                    } else {
    ...
    pub async fn claim_model_jobs(
        ...
        let rows = sqlx::query(
            r#"
            SELECT * FROM insight_platform.jobs candidate
            WHERE work_class='model' AND job_id=ANY($2) AND state IN ('ready','retry_scheduled')
              AND terminal_at IS NULL AND worker_id IS NULL AND attempt_no<attempt_limit
              AND scheduled_at<=$1 AND (retry_at IS NULL OR retry_at<=$1) AND deadline>$1
              AND EXISTS(SELECT 1 FROM jsonb_array_elements($3::jsonb) capability
        ...
        .bind(
            serde_json::to_value(&command.worker_manifest.execution_capabilities.capabilities)
        ...
        for row in rows {
            let result = async {
                ...
                let candidate = crate::recovery_isolation::job(
                    model_claim_candidate(job.clone(), &command.slots[0]),
                    &job,
                    insight_platform_jobs::store::SafetyScanPhase::OwnerValidation,
                )?;
                ...
                let payload: ModelJobPayload = crate::recovery_isolation::job(
                    decode_versioned_payload(&candidate.job.payload, "Model Job"),
                    &candidate.job,
                    insight_platform_jobs::store::SafetyScanPhase::OwnerDecode,
                )?;
                crate::recovery_isolation::job(
                    payload
                        .validate_for(&observed, &projection, self.model_turn_limits())
                        .map_err(RepositoryError::from),
                    &candidate.job,
                    insight_platform_jobs::store::SafetyScanPhase::OwnerValidation,
                )?;
                ...
                if command
                    ...
                    && observed
                        ...
                        .current_job_id
                        .as_ref()
                        .is_some_and(|id| id.to_string() == candidate.job.job_id)
                    && matches!(
                        observed.state,
        ...
        // Partition/fairness precedes sorted quota accounts, then Run/Node/Turn/Job.
        ...
        let available = quota_accounts
            ...
            .map(|account| {
                Ok((
                    ...
                    .map_err(|_| {
                        RepositoryError::CorruptRow("Model quota availability invalid".into())
                    })?,
                ))
            })
        ...
        for candidate in &candidates {
            let costs = quota_accounts
                .values()
                .filter(|account| {
                    model_quota_account_matches(
                        account,
                        &candidate.job,
                        &candidate.model_deployment_id,
                    )
                })
                .map(|account| {
                    Ok(insight_platform_scheduler::partitioned::QuotaCost {
                        account_id: account
                            ...
                            .parse::<ResourceId>()
                            .map_err(|error| RepositoryError::CorruptRow(error.to_string()))?,
                        amount: model_reserved_amount(account, candidate.quota_ceiling)? as u64,
                    })
                })
                .collect::<Result<Vec<_>, RepositoryError>>()?;
            if costs.len() == MODEL_QUOTA_LINES {
                eligible.insert(
        ...
        for candidate in candidates {
            ...
            let result: Result<ClaimedModelExecution, RepositoryError> = async {
                ...
                let node =
                    load_model_node_for_update(&mut object_transaction, &tenant_id, &node_id)
                        .await?;
                ...
                .await?;
                if run.execution_requirement != observed_job.execution_requirement
                    || !command.worker_manifest.execution_capabilities.supports(
                        &current_turn
                            .payload
                            .admission
                            .required_adapter_execution()
                ...
                {
                    return Err(RepositoryError::Conflict(
                        "Model execution requirement changed",
                    ));
                }
                require_model_claim_parents(
                    &mut object_transaction,
                    &current_turn,
                    &run,
                    &node,
                ...
                .await?;
                let request_input =
                    load_model_execution_input(&mut object_transaction, &current_turn).await?;
                let current_job =
                    load_model_job(&mut object_transaction, &tenant_id, &job_id, true).await?;
                ...
                {
                    return Err(RepositoryError::Conflict("Model claim discovery changed"));
                }
                let projection = crate::recovery_isolation::job(
                    job_projection(&current_job),
                    &current_job,
            ...
            .await?;
                        ...
                        Ok(ClaimedModelExecution {
                            ...
                            resume_mutations: plan_leaf
                                .then(|| slot.resume_mutations.clone())
                                .flatten(),
                            failure_mutations: plan_leaf
...
async fn release_model_plan_leaf_permit(
    ...
    if job.quota_reservation_id.is_none() {
        return Ok(());
    }
    if !matches!(job.state.as_str(), "running" | "cancelling") {
        return Err(RepositoryError::CorruptRow(
            "Model permit release requires an active reserved attempt".to_owned(),
        ));
    }
    ...
    let updated = sqlx::query(
        ...
        WHERE tenant_id = $1 AND run_id = $2 AND terminal_at IS NULL
          AND state IN ('running', 'waiting', 'cancelling') AND active_work_count > 0
        "#,
    )
    .bind(&job.tenant_id)
    .bind(turn.run_id.to_string())
...
impl<'a> PgModelTurnTransaction<'a> {
    async fn commit_model_outcome_inner(
        ...
        .await?;
        lock_model_control_parents(&mut transaction, &observed_turn).await?;
        let current_turn = load_model_turn(
            &mut transaction,
            &command.audit.tenant_id,
            &command.model_turn_id,
    ...
    async fn commit_model_cancellation_outcome_inner(
        ...
        if command.fence.is_none()
            ...
            && current_job.deadline > database_now
        {
    ...
    async fn control_model_turn_inner(
        ...
        let controlled = apply_model_control(
            &mut transaction,
            &current_turn,
            current_job.as_ref(),
            quota_lines.as_deref(),
            &command.quota_entry_ids,
            command.kind,
            &command.audit.request_digest,
            database_now,
        ...
        .await?;
        let job = controlled.job;
        let next_turn = controlled.turn;
        let (event_type, disposition) = match next_turn.state {
...
/// Applies one domain-owned control step after the caller observes the Run generation.
/// The caller owns the transaction and must not hold business locks before this call.
#[allow(clippy::too_many_arguments)]
pub(crate) async fn converge_model_for_run(
    transaction: &mut Transaction<'_, Postgres>,
    tenant_id: &ResourceId,
    run_id: &ResourceId,
    model_turn_id: &ResourceId,
    ...
    limits: ModelTurnLimits,
) -> Result<Option<(RunRecord, String)>, RepositoryError> {
    slot.validate()?;
    if tenant_id.kind() != ResourceKind::Tenant
    ...
    {
        return Err(RepositoryError::InvalidInput(
            "Model Run convergence identity is invalid".to_owned(),
        ));
    }
    let observed_turn =
        match load_model_turn(transaction, tenant_id, model_turn_id, false, limits).await {
            Ok(turn) => turn,
            Err(RepositoryError::NotFound(_)) => return Ok(None),
            Err(failure) => return Err(failure),
        };
    if observed_turn.run_id != *run_id
        || observed_turn.terminal_at.is_some()
        || observed_turn.state == ModelTurnState::Cancelling
    {
        return Ok(None);
    }
    let observed_job =
        load_model_run_control_job(transaction, &observed_turn, false, limits).await?;
    let quota_lines = match observed_job
        .as_ref()
        .filter(|job| job.quota_reservation_id.is_some())
    {
        Some(job) => Some(lock_model_quota_bundle(transaction, job).await?),
        None => None,
    };
    let run = match lock_model_run_control_parents(transaction, &observed_turn).await {
        Ok(run) => run,
        Err(RepositoryError::Conflict(_) | RepositoryError::NotFound(_)) => return Ok(None),
        Err(failure) => return Err(failure),
    };
    if run.version != observed_run_version
        || run.state != RunState::Cancelling.as_str()
        || run.terminal_at.is_some()
    ...
    let current_turn = load_model_turn(transaction, tenant_id, model_turn_id, true, limits).await?;
    if current_turn.version != observed_turn.version
        || current_turn.run_id != *run_id
        || current_turn.terminal_at.is_some()
        || current_turn.state == ModelTurnState::Cancelling
    {
        return Ok(None);
    }
    let current_job = load_model_run_control_job(transaction, &current_turn, true, limits).await?;
    match (&observed_job, &current_job) {
        (Some(observed), Some(current)) => {
            if require_same_observed_model_job(observed, current).is_err() {
                return Ok(None);
            }
        }
        (None, None) => {}
    ...
    // An in-flight request first loses new business authority. Its actual cancellation or
    // expired-fence recovery owns the physical outcome and conservative accounting.
    let kind = if current_turn.state != ModelTurnState::InFlight
        && database_now >= current_turn.deadline
    {
        ModelControlKind::Timeout
    } else {
        ModelControlKind::Cancel
    };
    let settlement_ids = if quota_lines.is_some() && current_turn.state != ModelTurnState::InFlight
    {
        slot.quota_entry_ids.as_slice()
    } else {
        &[]
    };
    let controlled = apply_model_control(
    ...
    .await?;
    let event_type = match controlled.turn.state {
        ModelTurnState::Cancelling => "model.cancelling",
        ModelTurnState::Cancelled => "model.cancelled",
        ModelTurnState::TimedOut => "model.timed_out",
        _ => {
            return Err(RepositoryError::CorruptRow(
                "invalid Model convergence state".to_owned(),
            ))
        }
    };
    append_scheduler_event(
        ...
        "model_turn",
        &model_turn_id.to_string(),
        as_i64(controlled.turn.version, "ModelTurn version")?,
        Some(&run.run_id),
        event_type,
        &TypedPayload::new(
            1,
            &serde_json::json!({
                "job_id": controlled.job.as_ref().map(|job| &job.job_id),
                "run_control_version": observed_run_version,
                "state": controlled.turn.state,
            }),
        )?,
    )
    .await?;
    if let Some(job) = &controlled.job {
        append_scheduler_event(
            transaction,
            &run.tenant_id,
            &slot.job_event_id,
    ...
    let current_run = crate::recovery_isolation::produced(
        load_run_for_update(transaction, tenant_id, run_id).await,
    )?;
    if current_run.version != run.version {
        append_scheduler_event(
            transaction,
            &run.tenant_id,
            &slot.run_event_id,
            ...
            current_run.version,
            Some(&run.run_id),
            "run.model_permit_released",
            &TypedPayload::new(
                1,
                &serde_json::json!({
                    "model_turn_id": model_turn_id,
                    "active_work_count": current_run.active_work_count,
                }),
            )?,
        )
        .await?;
    }
    // Appending a Run event advances its public sequence; return the committed projection.
    let current_run = crate::recovery_isolation::produced(
        load_run_for_update(transaction, tenant_id, run_id).await,
    )?;
    Ok(Some((
        current_run,
        controlled.turn.state.as_str().to_owned(),
    )))
}

/// Only reads and validates the existing owner. Keep quota and generated decisions outside.
async fn load_model_run_control_job(
    transaction: &mut Transaction<'_, Postgres>,
    turn: &ModelTurnRecord,
    for_update: bool,
    limits: ModelTurnLimits,
    ...
    let Some(job_id) = turn.payload.current_job_id.as_ref() else {
        return Ok(None);
    };
    let job = crate::recovery_isolation::addressed(
        load_model_job(transaction, &turn.tenant_id, job_id, for_update).await,
        &turn.tenant_id,
        job_id,
    ...
    let payload: ModelJobPayload = crate::recovery_isolation::job(
        decode_versioned_payload(&job.payload, "Model control Job"),
        &job,
        SafetyScanPhase::OwnerDecode,
    )?;
    crate::recovery_isolation::job(
        payload
            .validate_for(turn, &projection, limits)
            .map_err(RepositoryError::from),
        &job,
        SafetyScanPhase::OwnerValidation,
    )?;
...
async fn lock_model_run_control_parents(
    transaction: &mut Transaction<'_, Postgres>,
    observed_turn: &ModelTurnRecord,
) -> Result<RunRecord, RepositoryError> {
    let run =
        load_run_for_update(transaction, &observed_turn.tenant_id, &observed_turn.run_id).await?;
    let node = crate::recovery_isolation::addressed(
        load_model_node_for_update(
            transaction,
            &observed_turn.tenant_id,
            &observed_turn.node_execution_id,
        )
        .await,
        &observed_turn.tenant_id,
        &observed_turn.node_execution_id,
    ...
    require_model_control_parents(run, node, observed_turn)
}

async fn lock_model_control_parents(
    transaction: &mut Transaction<'_, Postgres>,
    observed_turn: &ModelTurnRecord,
) -> Result<RunRecord, RepositoryError> {
    let run =
...
fn require_model_control_parents(
    run: RunRecord,
    node: LockedModelNode,
    observed_turn: &ModelTurnRecord,
) -> Result<RunRecord, RepositoryError> {
    if run.terminal_at.is_some()
        || node.run_id != run.run_id
        || node.scope_instance_id != observed_turn.scope_instance_id
        || node.node_kind != PlanNodeKind::ModelLoop
        || run.bindings.canonical_digest != observed_turn.payload.admission.run_bindings_digest
        || !matches!(
            node.state,
    ...
    {
        return Err(RepositoryError::Conflict("Model control parent"));
    }
    Ok(run)
}

#[allow(clippy::too_many_arguments)]
async fn apply_model_control(
    transaction: &mut Transaction<'_, Postgres>,
    current_turn: &ModelTurnRecord,
    current_job: Option<&JobRecord>,
    quota_lines: Option<&[ModelQuotaReservationLine]>,
    quota_entry_ids: &[ResourceId],
    kind: ModelControlKind,
    request_digest: &Sha256Digest,
    database_now: DateTime<Utc>,
    limits: ModelTurnLimits,
) -> Result<ControlledModelExecution, RepositoryError> {
    let projection = current_job.map(job_projection).transpose()?;
    let payload = current_job
    ...
    match (quota_lines, decision.settlement) {
        ...
        (None, None) | (Some(_), None) if quota_entry_ids.is_empty() => {}
        _ => {
            return Err(RepositoryError::InvalidInput(
    ...
    let job = match (
        ...
        (Some(current), Some(next), Some(payload)) => {
            Some(update_model_job(transaction, current, next, payload, database_now).await?)
        }
        (None, None, None) => None,
        _ => {
            return Err(RepositoryError::CorruptRow(
    ...
    update_model_turn(transaction, current_turn, &decision.turn, limits).await?;
    if decision.settlement.is_some() {
        release_model_plan_leaf_permit(
            transaction,
            current_job.ok_or_else(|| {
                RepositoryError::CorruptRow("Model settlement Job missing".to_owned())
            })?,
            current_turn,
            database_now,
        )
