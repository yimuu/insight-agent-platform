...
use insight_platform_mcp_host::{
    ...
    McpSubscriptionRecoveryScan, McpSubscriptionWorkerAudit, McpWorkerAudit,
    NewMcpAuthorizationBinding, NewMcpDiscoveryAdmission, NewMcpDiscoverySnapshotRecord,
    ParkMcpDiscoveryVerification, RecoverDueMcpSubscription, RecoverExpiredMcpDiscoveryJob,
...
use insight_platform_postgres::{
    ...
    mcp_repository::{
        CommitContextSubscriptionRefresh, ContextSubscriptionRefreshRecoverySlot,
        DriveExpiredContextSubscriptionRefreshJobs,
    },
    repository::{
        ArtifactWorkerRole, ClaimArtifactJobs, ClaimJobs, NewPrincipal, NewQuotaAccount,
...
fn mcp_fixture_manifest() -> WorkerManifest {
    WorkerManifest {
        manifest_version: WORKER_MANIFEST_VERSION,
        worker_role: "mcp-subscription-worker".into(),
        work_class: WorkClass::Mcp,
        adapter_runtime_digest: named_digest("fixture-mcp-runtime"),
        worker_build_digest: insight_platform_worker::execution::executable_digest(
            &std::env::current_exe().unwrap(),
        )
        .unwrap(),
        execution_capabilities: insight_platform_mcp_host::execution::execution_capabilities(),
        protocol_version: WORKER_PROTOCOL_VERSION,
        max_concurrency: 16,
        critical_control_reserved_slots: 1,
    }
}
fn artifact_fixture_manifest() -> WorkerManifest {
    WorkerManifest {
        manifest_version: WORKER_MANIFEST_VERSION,
        worker_role: "artifact-data-worker".into(),
        work_class: WorkClass::Artifact,
        adapter_runtime_digest: named_digest("fixture-scanner-runtime"),
        worker_build_digest: insight_platform_worker::execution::executable_digest(
            &std::env::current_exe().unwrap(),
        )
        .unwrap(),
        execution_capabilities: insight_platform_artifacts::execution::execution_capabilities(
            &insight_platform_artifacts::execution::integrity_scanner_contract_digest(),
        ),
        protocol_version: WORKER_PROTOCOL_VERSION,
        max_concurrency: 16,
...
#[async_trait]
trait FixtureClaimRounds {
    async fn claim_jobs_fixture(
        &self,
        command: ClaimJobs,
    ) -> Result<Vec<insight_platform_jobs::store::JobRecord>, RepositoryError>;
    async fn claim_discovery_fixture(
        &self,
        command: ClaimJobs,
    ) -> Result<Vec<insight_platform_jobs::store::JobRecord>, RepositoryError>;
    async fn claim_artifact_fixture(
        &self,
        command: ClaimArtifactJobs,
    ) -> Result<Vec<insight_platform_jobs::store::JobRecord>, RepositoryError>;
    async fn claim_refresh_fixture(
        &self,
        command: ClaimContextSubscriptionRefreshJobs,
    ) -> Result<
        Vec<insight_platform_postgres::mcp_repository::ClaimedContextSubscriptionRefresh>,
        RepositoryError,
    >;
}
#[async_trait]
impl FixtureClaimRounds for PgRepository {
    async fn claim_jobs_fixture(
        &self,
        command: ClaimJobs,
    ) -> Result<Vec<insight_platform_jobs::store::JobRecord>, RepositoryError> {
    ...
    async fn claim_discovery_fixture(
        ...
        for _ in 0..(usize::from(insight_platform_contracts::SCHEDULER_PARTITION_COUNT) * 8) {
            let batch = self.claim_mcp_discovery_jobs(command.clone()).await?;
            if !batch.is_empty() {
                return Ok(batch);
            }
            tokio::task::yield_now().await;
    ...
    async fn claim_artifact_fixture(
        ...
        for _ in 0..(usize::from(insight_platform_contracts::SCHEDULER_PARTITION_COUNT) * 8) {
            let batch = self.claim_artifact_jobs(command.clone()).await?;
            if !batch.is_empty() {
                return Ok(batch);
            }
            tokio::task::yield_now().await;
    ...
    > {
        for _ in 0..(usize::from(insight_platform_contracts::SCHEDULER_PARTITION_COUNT) * 8) {
            let batch = self
                .claim_context_subscription_refresh_jobs(command.clone())
                .await?;
            if !batch.is_empty() {
                return Ok(batch);
            }
...
fn physical_manifest(
    ...
    manifest.worker_role = role.to_owned();
    if manifest.work_class == WorkClass::Mcp {
        manifest.max_concurrency = 1;
    }
    manifest.worker_build_digest =
        insight_platform_worker::execution::executable_digest(std::path::Path::new(binary))
            .unwrap();
    manifest.execution_capabilities = capabilities;
    manifest
}
...
fn map_discovery_artifact_fixture_error(error: RepositoryError) -> ArtifactWorkloadStageError {
    match error {
        ...
        | RepositoryError::InvalidPersistedObject(_) => ArtifactWorkloadStageError::Integrity,
    }
}
...
async fn seed(
    ...
    let scheduling = insight_platform_contracts::SchedulingPolicyDocument {
        version: 1,
        weight: 1,
        burst: 16,
        aging_rounds: 4,
    };
    let scheduling_digest = scheduling.canonical_digest().unwrap();
    let mut scheduling_document = policy_document(
        0xef01,
        PolicyKind::Scheduling,
        scheduling_digest.clone(),
        None,
        None,
    );
    if let ResourceDocument::Policy(policy) = &mut scheduling_document {
        policy.scheduling = Some(scheduling);
    }
    let scheduling_resource = id(ResourceKind::Policy, 0xef02);
    let scheduling_revision =
        ExactVersionRef::new(id(ResourceKind::PolicyRevision, 0xef03), scheduling_digest).unwrap();
    insert_resource(pool, &tenant_id, &scheduling_resource, "policy").await;
    insert_version(
        pool,
        &tenant_id,
        &principal_id,
    ...
    .await;
    let scheduling_deployment = insert_deployment(
        pool,
        &tenant_id,
        &principal_id,
        &id(ResourceKind::PolicyDeployment, 0xef04),
        &scheduling_resource,
        &scheduling_revision.revision_id,
        &DeploymentClosure::Policy(PolicyDeploymentClosure {
            policy_revision: scheduling_revision.clone(),
            applicability_digest: named_digest("fixture-scheduler-applicability"),
            qualification_evidence: authoring(0xef05, "scheduler-evidence").artifact,
        }),
    )
    .await;
    activate_deployment(
        ...
        &tenant_id,
        &scheduling_resource,
        &scheduling_deployment.deployment_id,
    )
    .await;
    ...
    security
        .bind_tenant_scheduling_policy(insight_platform_security::BindTenantSchedulingPolicy {
            audit: command_audit(&tenant_id, &principal_id, 0xef06, now),
            expected_tenant_version: tenant_version,
            policy: scheduling_deployment,
        })
        .await
        .unwrap();
...
async fn claim_and_start(
    ...
    let claimed = repository
        .claim_jobs_fixture(ClaimJobs {
            worker_manifest: mcp_fixture_manifest(),
...
impl ProductionArtifactProcessFixture {
    ...
    fn config(
        ...
        serde_json::json!({
            ...
            "scan_worker": {
                "worker_manifest": physical_manifest(artifact_fixture_manifest(), &self.binary, "artifact-data-worker", insight_platform_artifacts::execution::data_worker_execution_capabilities(&insight_platform_artifacts::execution::integrity_scanner_contract_digest())),
                "scanner_contract_digest": insight_platform_artifacts::execution::integrity_scanner_contract_digest(),
...
fn spawn_resource_host(input: ResourceHostSpawn<'_>) -> FixtureChild {
...
fn spawn_subscription_context_worker(input: ContextWorkerSpawn<'_>) -> FixtureChild {
    std::process::Command::new(input.binary)
        .env(
...
fn spawn_mcp_subscription_worker(input: SubscriptionWorkerSpawn<'_>) -> FixtureChild {
    std::process::Command::new(input.binary)
...
fn spawn_mcp_discovery_worker(input: DiscoveryWorkerSpawn<'_>) -> FixtureChild {
    std::process::Command::new(input.binary)
...
fn spawn_artifact_data_worker(input: ArtifactDataWorkerSpawn<'_>) -> FixtureChild {
...
async fn run_logical_subscription_worker_process_l3(
    ...
    let config = serde_json::json!({
        "schema_version": 1,
        "worker_manifest": physical_manifest(mcp_fixture_manifest(), subscription_worker_binary, "mcp-subscription-worker", insight_platform_mcp_host::execution::subscription_execution_capabilities()),
...
fn spawn_protocol_egress_process(config_path: &std::path::Path) -> FixtureChild {
...
fn spawn_discovery_artifact_process(
...
async fn run_discovery_worker_process_l3(
    ...
    let worker_config = serde_json::json!({
        "schema_version": 1,
        "worker_manifest": physical_manifest(mcp_fixture_manifest(), discovery_worker_binary, "mcp-discovery-worker", insight_platform_mcp_host::execution::discovery_execution_capabilities()),
        "observability_listen_address": observability_address.to_string(),
        "database_max_connections": 8,
        "database_acquire_timeout_milliseconds": 5000,
        "claim_batch_size": 1,
...
async fn run_subscription_protocol_process_l3(
    ...
    let context_worker_config = serde_json::json!({
        "schema_version": 1,
        "observability_listen_address": available_address().to_string(),
        "worker_manifest": physical_manifest(context_worker_manifest(), context_worker_binary, "context-worker", insight_platform_contracts::WorkerExecutionCapabilities { schema_version: 1, capabilities: vec![insight_platform_context::execution::subscription_execution_capability()] }),
        "database_max_connections": 4,
        "database_acquire_timeout_milliseconds": 5000,
    ...
    std::thread::Builder::new()
        ...
        .spawn(|| {
            tokio::runtime::Builder::new_current_thread()
                ...
                .build()
                .unwrap()
                .block_on(mcp_subscription_fixture(false));
        })
        .unwrap()
        .join()
        .unwrap();
...
#[test]
#[ignore = "requires the explicitly provisioned MCP, Context, Artifact and AWS process fixtures"]
fn mcp_subscription_current_process_recovery() {
    std::thread::Builder::new()
        .stack_size(32 * 1024 * 1024)
        .spawn(|| {
            tokio::runtime::Builder::new_current_thread()
                ...
                .build()
                .unwrap()
                .block_on(mcp_subscription_fixture(true))
        })
        .unwrap()
        .join()
        .unwrap();
...
async fn mcp_subscription_fixture(run_processes: bool) {
    ...
    let now = Utc::now();
    let production_artifact = if run_processes {
        Some(
            ProductionArtifactProcessFixture::from_environment()
                .expect("PLATFORM_ARTIFACT_DATA_WORKER_BIN is required for process qualification"),
        )
    } else {
        None
    };
    ...
    let left_claim = repository.claim_discovery_fixture(ClaimJobs {
        worker_manifest: mcp_fixture_manifest(),
    ...
    let right_claim = repository.claim_discovery_fixture(ClaimJobs {
        worker_manifest: mcp_fixture_manifest(),
        work_class: WorkClass::Mcp.as_str().to_owned(),
    ...
    let mut resumed_claims = repository
        .claim_discovery_fixture(ClaimJobs {
            worker_manifest: mcp_fixture_manifest(),
            work_class: WorkClass::Mcp.as_str().to_owned(),
    ...
    if run_processes {
        let discovery_worker_binary = std::env::var("PLATFORM_MCP_DISCOVERY_WORKER_BIN")
            .expect("process fixture requires PLATFORM_MCP_DISCOVERY_WORKER_BIN");
    ...
    let context_worker_manifest_digest_value = context_worker_manifest_digest();
    let claims = repository
        .claim_refresh_fixture(ClaimContextSubscriptionRefreshJobs {
            ...
            worker_manifest_digest: context_worker_manifest_digest_value,
    ...
    let isolation_case = if run_processes {
        None
    } else {
        Some(
            Box::pin(mcp_subscription_isolation::seed_invalid_refresh(
                &pool,
                &repository,
                &fixture,
            ))
    ...
    .await
    .unwrap();
    let recovery_command = DriveExpiredContextSubscriptionRefreshJobs {
        after: None,
        shard_index: 0,
        shard_count: 1,
        limit: 1,
    ...
    let recovered = if let Some(case) = &isolation_case {
        Box::pin(mcp_subscription_isolation::recover_context_after_bad_head(
            &pool,
            &repository,
            case,
            &first_reconcile_claim[0].job,
            recovery_command,
        ))
        .await
    } else {
        repository
            .drive_expired_context_subscription_refresh_jobs(recovery_command)
            .await
            .unwrap()
    };
    ...
    if run_processes {
        let resource_host_binary = std::env::var("PLATFORM_MCP_RESOURCE_HOST_BIN")
            .expect("process fixture requires PLATFORM_MCP_RESOURCE_HOST_BIN");
        let context_worker_binary = std::env::var("PLATFORM_SUBSCRIPTION_CONTEXT_WORKER_BIN")
            .expect("process fixture requires PLATFORM_SUBSCRIPTION_CONTEXT_WORKER_BIN");
    ...
    .await
    .unwrap();
    let recovered = Box::pin(mcp_subscription_isolation::recover_subscription(
        &pool,
        &repository,
        &fixture,
        &connecting,
    ...
    if run_processes {
        let subscription_worker_binary = std::env::var("PLATFORM_MCP_SUBSCRIPTION_WORKER_BIN")
            .expect("process fixture requires PLATFORM_MCP_SUBSCRIPTION_WORKER_BIN");
