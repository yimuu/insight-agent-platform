...
mod convergence;
use convergence as control_convergence;
...
use insight_platform_capability_adapters::{
    CapabilityAdapterFailure, CapabilityAdapterResponse, CapabilityAdapterWorker,
    CapabilityDispatcher, CapabilityTransportCancelOutcome, CapabilityTransportCancelRequest,
    GrpcNetworkTransport, GrpcTransportRequest, GrpcTransportResponse, HttpNetworkTransport,
    HttpTransportRequest, HttpTransportResponse, InstalledNativeAdapter, InstalledNativeRegistry,
...
use insight_platform_invocations::{
    ...
    DispatchOutcome, EncryptedRemoteState, InvocationApprovalRequirement, InvocationOrigin,
    InvocationPolicyDecision, InvocationPolicyDecisionBundle, InvocationPolicyDisposition,
    InvocationTransaction, McpCapabilityRuntimeRequest, PrepareCapabilityDispatch,
    ReconciliationResolution, RecordCapabilityProgress, RemoteWait, ResolveCapabilityApproval,
...
use insight_platform_orchestrator::store::DeferOrchestrationCapabilityMutationIds;
...
use insight_platform_orchestrator::{
    derive_candidate_selection, ExactRunValueRef, OrchestrationJobPayload, RequestRunCancel,
    RunCurrentSnapshot, ScopeDataEnvironmentSnapshot, ScopeEnvironmentLimits,
};
...
use insight_platform_postgres::capability_execution_repository::{
...
fn native_worker_manifest() -> insight_platform_contracts::WorkerManifest {
    ...
    manifest.adapter_runtime_digest = native_adapter_runtime_digest();
    manifest.worker_build_digest =
        support::test_actor_build("PLATFORM_CAPABILITY_NATIVE_WORKER_BIN");
    manifest.max_concurrency = 2;
    manifest
}
fn native_worker_manifest_digest() -> Sha256Digest {
    native_worker_manifest().canonical_digest().unwrap()
}
...
async fn claim_one(repository: &PgRepository, tenant_id: &ResourceId, base: u16) -> ClaimEvidence {
    ...
    let mut claims = repository
        .claim_capability_jobs_current_rounds(ClaimCapabilityJobs {
            worker_manifest: native_worker_manifest(),
    ...
    std::thread::Builder::new()
        ...
        .spawn(|| {
            tokio::runtime::Builder::new_current_thread()
                ...
                .build()
                .unwrap()
                .block_on(run_capability_phase3_fixture(false));
        })
        .unwrap()
        .join()
        .unwrap();
...
#[test]
#[ignore = "requires real worker binaries, TLS providers and process recovery harness"]
fn capability_workers_process_recovery() {
    std::thread::Builder::new()
        .name("phase3-invocation-fixture".to_owned())
        .stack_size(32 * 1024 * 1024)
        .spawn(|| {
            tokio::runtime::Builder::new_current_thread()
                ...
                .build()
                .unwrap()
                .block_on(run_capability_phase3_fixture(true));
        })
        .unwrap()
        .join()
        .unwrap();
...
async fn run_capability_phase3_fixture(exercise_processes: bool) {
    ...
    let replay_prepare = PrepareCapabilityDispatch {
        audit: audit(
            &fixture.tenant_id,
            &fixture.principal_id,
            PrincipalKind::AgentRunner,
    ...
    let prepared = match execute_prepare(&repository, replay_prepare.clone())
        .await
        .unwrap()
    {
        CommandOutcome::Applied(record) => record,
    ...
    assert!(matches!(
        execute_approval(&repository, winner_command.clone()).await.unwrap(),
    ...
    let counts: (i64, i64, i64, i64, i64, i64) = sqlx::query_as(
        ...
        SELECT
          ...
          (SELECT count(*) FROM insight_platform.outbox_events AS outbox JOIN insight_platform.events AS event ON event.tenant_id=outbox.tenant_id AND event.event_id=outbox.event_id WHERE event.tenant_id = $1 AND event.aggregate_kind = 'capability_invocation')
        "#,
    )
    .bind(fixture.tenant_id.to_string())
    ...
    let response_json = json!({"approved": true});
    ...
    let mut wrong_input = ResolveCapabilityInput {
        validated_input: Some(
            insight_platform_tasks::ValidatedTaskInput::validate_interaction_inline(
                &response_schema,
                &response_json,
            )
            .unwrap(),
    ...
    let input_resolved = match execute_input(&repository, wrong_input.clone())
        .await
        .unwrap()
    {
        CommandOutcome::Applied(record) => record,
    ...
    let replay_reconciliation = ResolveCapabilityReconciliation {
        audit: audit(
            &fixture.tenant_id,
            &fixture.principal_id,
            PrincipalKind::AgentRunner,
    ...
    let reconciled = match execute_reconciliation(&repository, replay_reconciliation.clone())
        .await
        .unwrap()
    {
        CommandOutcome::Applied(record) => record,
    ...
    let replay_control = ControlCapabilityInvocation {
        audit: audit(
            &fixture.tenant_id,
            &fixture.principal_id,
            PrincipalKind::AgentRunner,
    ...
    let controlled = match execute_control(&repository, replay_control.clone())
        .await
        .unwrap()
    {
        CommandOutcome::Applied(record) => record,
    ...
    if !exercise_processes {
        assert_run_cancel_preserves_capability_effect_uncertainty(&pool, &repository, &fixture)
            .await;
        ...
        assert!(matches!(
            execute_admit(&repository, inline).await,
            Err(RepositoryError::PermissionDenied)
        ));
        assert!(matches!(
            execute_prepare(&repository, replay_prepare).await,
            Err(RepositoryError::PermissionDenied)
        ));
        assert!(matches!(
            execute_approval(&repository, winner_command).await,
            Err(RepositoryError::PermissionDenied)
        ));
        assert!(matches!(
            execute_input(&repository, wrong_input).await,
            Err(RepositoryError::PermissionDenied)
        ));
        assert!(matches!(
            execute_control(&repository, replay_control).await,
            Err(RepositoryError::PermissionDenied)
        ));
        assert!(matches!(
            execute_reconciliation(&repository, replay_reconciliation).await,
            Err(RepositoryError::PermissionDenied)
        ));
        assert_eq!(
...
async fn run_native_worker_process_recovery(
    ...
    let Ok(binary) = std::env::var("PLATFORM_CAPABILITY_NATIVE_WORKER_BIN") else {
        panic!(
            "PLATFORM_CAPABILITY_NATIVE_WORKER_BIN is unset; process recovery fixture requires its declared fixture environment"
        );
    };
    ...
    let spawn_worker = || {
        std::process::Command::new(&binary)
            ...
            .stderr(Stdio::piped())
            .spawn()
            .map(process_guard::ChildGuard::new)
            .unwrap()
    };
...
async fn prepare_remote_http_process_fixture(
    ...
    let mut execution_capabilities =
        insight_platform_plan::execution::program_execution_capabilities();
    for (backend_kind, descriptor_digest) in [
        (CapabilityBackendKind::Http, descriptor_digest.clone()),
        (CapabilityBackendKind::Grpc, grpc_descriptor_digest.clone()),
        (CapabilityBackendKind::Mcp, mcp_descriptor_digest.clone()),
    ] {
        ...
        execution_capabilities.capabilities.push(
            insight_platform_contracts::remote_capability_adapter_execution_capability(&codec)
                .unwrap(),
        );
    }
    let mut manifest = support::manifest(
    ...
    manifest.adapter_runtime_digest = adapter_runtime_digest;
    manifest.worker_build_digest =
        support::test_actor_build("PLATFORM_CAPABILITY_REMOTE_WORKER_BIN");
    manifest.max_concurrency = 2;
    let worker_manifest = serde_json::to_value(manifest).unwrap();
...
async fn run_remote_http_worker_process_recovery(
    ...
    let Ok(binary) = std::env::var("PLATFORM_CAPABILITY_REMOTE_WORKER_BIN") else {
        panic!(
            "PLATFORM_CAPABILITY_REMOTE_WORKER_BIN is unset; remote process recovery fixture requires its declared fixture environment"
        );
    };
    let mcp_host_binary = std::env::var("PLATFORM_MCP_HOST_BIN")
        .expect("PLATFORM_MCP_HOST_BIN is required for complete remote recovery");
    let database_url = std::env::var("PLATFORM_TEST_DATABASE_URL").unwrap();
    ...
    let mut remote = prepare_remote_http_process_fixture(
        pool,
        repository,
        fixture,
    ...
    let host_egress_cert_path = PathBuf::from(format!("{temp_prefix}-mcp-host-egress-client.pem"));
    ...
    remote.config["mcp_host"]["endpoint"] =
    ...
    // This fixture installs a valid, different codec identity; its manifest must
    // truthfully name that installed capability before testing claim isolation.
    replace_fixture_codec_capability(
        &remote.config,
        &mut wrong_config,
...
fn replace_fixture_codec_capability(
    original: &serde_json::Value,
    changed: &mut serde_json::Value,
    field: &str,
    backend_kind: CapabilityBackendKind,
) {
    let mut manifest: insight_platform_contracts::WorkerManifest =
        serde_json::from_value(changed["worker_manifest"].clone()).unwrap();
    let installed = &original[field][0];
    let mut codec = InstalledCapabilityCodecRef {
        schema_version: INSTALLED_CAPABILITY_CODEC_MANIFEST_VERSION,
        backend_kind,
        codec_id: installed["codec_id"].as_str().unwrap().to_owned(),
        codec_version: installed["codec_version"].as_str().unwrap().to_owned(),
        module_digest: serde_json::from_value(installed["module_digest"].clone()).unwrap(),
        worker_protocol_version: WORKER_PROTOCOL_VERSION,
        descriptor_digest: serde_json::from_value(installed["descriptor_digest"].clone()).unwrap(),
    };
    let previous =
        insight_platform_contracts::remote_capability_adapter_execution_capability(&codec).unwrap();
    codec.descriptor_digest =
        serde_json::from_value(changed[field][0]["descriptor_digest"].clone()).unwrap();
    *manifest
        .execution_capabilities
        .capabilities
        .iter_mut()
        ...
        .unwrap() =
        insight_platform_contracts::remote_capability_adapter_execution_capability(&codec).unwrap();
    changed["worker_manifest"] = serde_json::to_value(manifest).unwrap();
}
...
async fn seed_plan_capability_command(
    pool: &PgPool,
    repository: &PgRepository,
    fixture: &Fixture,
    base: u16,
) -> DeferOrchestrationToCapabilityInvocation {
    let scope_id: String = sqlx::query_scalar("SELECT node_id FROM insight_platform.run_nodes WHERE tenant_id=$1 AND run_id=$2 AND record_kind='scope_instance' AND parent_node_id IS NULL")
        .bind(fixture.tenant_id.to_string()).bind(fixture.run_id.to_string()).fetch_one(pool).await.unwrap();
    let scope_id: ResourceId = scope_id.parse().unwrap();
    let now: DateTime<Utc> = sqlx::query_scalar("SELECT clock_timestamp()")
        .fetch_one(pool)
        .await
        .unwrap();
    ...
    sqlx::query(
        ...
        INSERT INTO insight_platform.run_nodes (
            ...
            $1, $2, $3, $4, 'node_execution', $4,
            'capability-plan', (20 + $10), ('capability-plan-owner-' || $2), 'capability_call', 'running',
            1, 1, $5, $6, $7, $8, $9, $9, $9
        )
        "#,
    )
    ...
    .bind(node_id.to_string())
    .bind(fixture.run_id.to_string())
    .bind(scope_id.clone().to_string())
    .bind(node_payload.schema_version)
    .bind(&node_payload.value)
    .bind(&node_payload.digest)
    .bind(now + Duration::minutes(30))
    .bind(now)
    .bind(i32::from(base / 256) - 128)
    .execute(pool)
    .await
    .unwrap();

    let quota_account_id = id(ResourceKind::QuotaAccount, 0x8010);
    let quota_exists: bool = sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM insight_platform.quota_accounts WHERE tenant_id=$1 AND quota_account_id=$2)")
        .bind(fixture.tenant_id.to_string()).bind(quota_account_id.to_string()).fetch_one(pool).await.unwrap();
    if !quota_exists {
        repository
            .create_quota_account(NewQuotaAccount {
                tenant_id: fixture.tenant_id.to_string(),
                ...
                limit_value: 4,
                payload: TypedPayload::new(1, &json!({"fixture": "capability-plan-owner"}))
                    .unwrap(),
            })
            .await
            .unwrap();
    ...
    .await
    .unwrap();
    let reservation_id = id(ResourceKind::UsageReservation, base + 17);
    sqlx::query(
        r#"
    ...
    .bind(fixture.tenant_id.to_string())
    .bind(id(ResourceKind::QuotaLedgerEntry, base + 18).to_string())
    .bind(quota_account_id.to_string())
    ...
    .await
    .unwrap();
    let source_job_id = id(ResourceKind::Job, base + 32);
    let worker_id = id(ResourceKind::WorkerProcessGeneration, base + 33);
    ...
    sqlx::query(
        ...
        INSERT INTO insight_platform.jobs (
            ...
            payload_digest, started_at, created_at, updated_at, trace_id, scheduler_partition_id, execution_requirement_version, execution_requirement, execution_requirement_digest, attempt_build_digest
        ) VALUES (
            $1, $2, 'orchestration_node', 'orchestration', 'node_execution', $3, $4, $3,
            'running', 2, 1, 3, 1, $5, $6, $7, $8, $8, $9,
            $10, $11, $12, $13, $14, $8, $8, $8,
            '0123456789abcdef0123456789abcdef', (SELECT scheduler_partition_id FROM insight_platform.tenants WHERE tenant_id=$1), (SELECT execution_requirement_version FROM insight_platform.runs WHERE tenant_id=$1 AND run_id=$4), (SELECT execution_requirement FROM insight_platform.runs WHERE tenant_id=$1 AND run_id=$4), (SELECT execution_requirement_digest FROM insight_platform.runs WHERE tenant_id=$1 AND run_id=$4), 'sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb'
        )
        "#,
    )
    .bind(fixture.tenant_id.to_string())
    ...
    .unwrap();
    let invocation_id = id(ResourceKind::CapabilityInvocation, base + 48);
    let capability_job_id = id(ResourceKind::Job, base + 49);
    DeferOrchestrationToCapabilityInvocation {
        fence: fence.clone(),
        ...
        approval_task_id: Some(id(ResourceKind::ApprovalTask, base + 50)),
        input_artifact_link_id: None,
        mcp_runtime: None,
...
async fn seed_read_capability_run(
    pool: &PgPool,
    repository: &PgRepository,
    fixture: &Fixture,
) -> Fixture {
    let interface_resource = id(ResourceKind::CapabilityInterface, 0xa100);
    let implementation_resource = id(ResourceKind::CapabilityImplementation, 0xa102);
    ...
    let mut interface: PublishedVersionPayload = serde_json::from_value(sqlx::query_scalar::<_, serde_json::Value>(
        "SELECT payload - 'schema_version' FROM insight_platform.resource_versions WHERE tenant_id=$1 AND resource_version_id=$2"
    ).bind(fixture.tenant_id.to_string()).bind(id(ResourceKind::CapabilityInterfaceRevision, 0x17).to_string()).fetch_one(pool).await.unwrap()).unwrap();
    let ResourceDocument::CapabilityInterface(document) = &mut interface.document else {
        panic!("fixture interface owner")
    };
    document.effect = Effect::ReadOnly;
    document.qualified_name = "fixture.reader".parse().unwrap();
    let interface_exact = ExactVersionRef::new(
        id(ResourceKind::CapabilityInterfaceRevision, 0xa101),
        canonical_digest(&serde_json::to_value(&interface.document).unwrap())
            .unwrap()
            .parse()
            .unwrap(),
    )
    ...
    .await;
    let mut implementation: PublishedVersionPayload = serde_json::from_value(sqlx::query_scalar::<_, serde_json::Value>(
        "SELECT payload - 'schema_version' FROM insight_platform.resource_versions WHERE tenant_id=$1 AND resource_version_id=$2"
    ).bind(fixture.tenant_id.to_string()).bind(id(ResourceKind::CapabilityImplementationRevision, 0x19).to_string()).fetch_one(pool).await.unwrap()).unwrap();
    let ResourceDocument::CapabilityImplementation(document) = &mut implementation.document else {
        panic!("fixture implementation owner")
    };
    document.interface_revision = interface_exact.clone();
    let implementation_exact = ExactVersionRef::new(
        id(ResourceKind::CapabilityImplementationRevision, 0xa103),
        canonical_digest(&serde_json::to_value(&implementation.document).unwrap())
            .unwrap()
            .parse()
            .unwrap(),
    )
    ...
    .await;
    let DeploymentClosure::CapabilityInterface(mut closure) = serde_json::from_value(sqlx::query_scalar::<_, serde_json::Value>(
        "SELECT bindings - 'schema_version' FROM insight_platform.deployments WHERE tenant_id=$1 AND deployment_id=$2"
    ).bind(fixture.tenant_id.to_string()).bind(fixture.capability_deployment.deployment_id.to_string()).fetch_one(pool).await.unwrap()).unwrap() else { panic!("fixture Capability closure") };
    closure.interface = interface_exact.clone();
    closure.implementation = implementation_exact;
    let payload = TypedPayload::new(1, &DeploymentClosure::CapabilityInterface(closure)).unwrap();
    let deployment_id = id(ResourceKind::CapabilityDeployment, 0xa104);
    insert_deployment(
        pool,
        &fixture.tenant_id,
        &deployment_id,
    ...
    repository
        .create_quota_account(NewQuotaAccount {
            tenant_id: fixture.tenant_id.to_string(),
            quota_account_id: id(ResourceKind::QuotaAccount, 0xa105).to_string(),
            scope_kind: "capability_deployment".to_owned(),
            scope_id: deployment_id.to_string(),
            work_class: WorkClass::CapabilityNative.as_str().to_owned(),
            metric: QuotaDimension::CapabilityConcurrentInvocations
                .as_str()
                .to_owned(),
            limit_value: 8,
            payload: TypedPayload::new(1, &json!({"fixture":"read-control"})).unwrap(),
        })
        .await
        .unwrap();
    let DeploymentClosure::Agent(mut agent) = serde_json::from_value(sqlx::query_scalar::<_, serde_json::Value>(
        "SELECT bindings - 'schema_version' FROM insight_platform.deployments WHERE tenant_id=$1 AND deployment_id=$2"
    ).bind(fixture.tenant_id.to_string()).bind(id(ResourceKind::AgentDeployment, 0x15).to_string()).fetch_one(pool).await.unwrap()).unwrap() else { panic!("fixture Agent closure") };
    let FrozenSlotTarget::Capability { candidates, .. } = &mut agent.slots[0].target else {
        panic!("fixture Capability slot")
    };
    *candidates = vec![read_deployment.clone()];
    agent.slots[0].binding_digest = agent.slots[0].expected_binding_digest().unwrap();
    let agent_payload = TypedPayload::new(1, &DeploymentClosure::Agent(agent.clone())).unwrap();
    let agent_id = id(ResourceKind::AgentDeployment, 0xa106);
    insert_deployment(
        pool,
        &fixture.tenant_id,
        &agent_id,
        &id(ResourceKind::Agent, 0x12),
        &id(ResourceKind::AgentPlanRevision, 0x14),
        &fixture.principal_id,
        &agent_payload,
    )
    .await;
    let old_bindings: RunBindingsSnapshot = serde_json::from_value(
    ...
    let bindings = RunBindingsSnapshot::build(
        ExactDeploymentRef::new(agent_id.clone(), agent_payload.digest.parse().unwrap()).unwrap(),
        old_bindings.principal,
        &agent,
    )
    .unwrap();
    let bindings_payload = TypedPayload::from_versioned(1, &bindings, 1_048_576).unwrap();
    let run_id = id(ResourceKind::Run, 0xa130);
    let scope_id = id(ResourceKind::ScopeInstance, 0xa131);
    let input_id = id(ResourceKind::RunValue, 0xa134);
    let current = TypedPayload::from_versioned(
        1,
        &RunCurrentSnapshot::initial(run_id.clone(), agent_id.clone(), input_id.clone()),
        1_048_576,
    )
    .unwrap();
    sqlx::query(r#"
        INSERT INTO insight_platform.runs (tenant_id,run_id,root_run_id,agent_deployment_id,principal_id,state,version,
            bindings_schema_version,bindings,bindings_digest,current_schema_version,current_payload,current_payload_digest,
            deadline,started_at,created_at,updated_at,trace_id,execution_requirement_version,execution_requirement,execution_requirement_digest)
        SELECT tenant_id,$3,$3,$4,principal_id,'running',1,$5,$6,$7,$8,$9,$10,deadline,statement_timestamp(),statement_timestamp(),statement_timestamp(),
            trace_id,execution_requirement_version,execution_requirement,execution_requirement_digest
        FROM insight_platform.runs WHERE tenant_id=$1 AND run_id=$2
    "#).bind(fixture.tenant_id.to_string()).bind(fixture.run_id.to_string()).bind(run_id.to_string()).bind(agent_id.to_string())
        .bind(bindings_payload.schema_version).bind(&bindings_payload.value).bind(bindings.canonical_digest.to_string())
        .bind(current.schema_version).bind(&current.value).bind(&current.digest).execute(pool).await.unwrap();
    sqlx::query("INSERT INTO insight_platform.run_values(tenant_id,value_id,run_id,value_kind,classification,schema_digest,content_digest,inline_value) SELECT tenant_id,$3,$4,value_kind,classification,schema_digest,content_digest,inline_value FROM insight_platform.run_values WHERE tenant_id=$1 AND value_id=$2")
        .bind(fixture.tenant_id.to_string()).bind(fixture.inline_value_id.to_string()).bind(input_id.to_string()).bind(run_id.to_string()).execute(pool).await.unwrap();
    sqlx::query(
    ...
    let environment = ScopeDataEnvironmentSnapshot::build(
        BTreeMap::from([(
            ...
            ExactRunValueRef {
                ...
                schema_digest: fixture.input_schema_digest.clone(),
                content_digest: canonical_digest(&json!({"amount":7}))
                    .unwrap()
                    .parse()
                    .unwrap(),
            },
    ...
    .unwrap();
    let root =
        TypedPayload::new(1, &json!({"root_run_id":run_id,"environment":environment})).unwrap();
    sqlx::query(r#"INSERT INTO insight_platform.run_nodes(tenant_id,node_id,run_id,record_kind,scope_id,logical_key,node_kind,state,generation,version,payload_schema_version,payload,payload_digest,deadline,started_at,created_at,updated_at)
        SELECT tenant_id,$3,$4,'scope_instance',$3,'root','root','open',1,1,$5,$6,$7,deadline,statement_timestamp(),statement_timestamp(),statement_timestamp()
        FROM insight_platform.runs WHERE tenant_id=$1 AND run_id=$2"#)
        .bind(fixture.tenant_id.to_string()).bind(fixture.run_id.to_string()).bind(scope_id.to_string()).bind(run_id.to_string())
        .bind(root.schema_version).bind(&root.value).bind(&root.digest).execute(pool).await.unwrap();
    let mut read = fixture.clone();
    read.run_id = run_id;
    read.inline_value_id = input_id;
    read.capability_deployment = read_deployment;
    read
}

async fn assert_run_cancel_preserves_capability_effect_uncertainty(
    pool: &PgPool,
    repository: &PgRepository,
    fixture: &Fixture,
) {
    let read = seed_read_capability_run(pool, repository, fixture).await;
    let mut leaves = Vec::new();
    for (owner, base) in [(fixture, 0xb000), (&read, 0xb200), (fixture, 0xb400)] {
        let mut command = seed_plan_capability_command(pool, repository, owner, base).await;
        command.policy_decisions = allowed_policy(&fixture.policy);
        command.approval_task_id = None;
        let mut tx = repository.begin_scheduler_transaction().await.unwrap();
        let CommandOutcome::Applied(record) = tx
            .defer_orchestration_to_capability_invocation(command)
            .await
            .unwrap()
        else {
            panic!("fresh cancellation leaf replayed")
        };
        tx.commit().await.unwrap();
        leaves.push(record);
    }
    let write_claim = claim_one(repository, &fixture.tenant_id, 0xb700).await;
    let read_claim = claim_one(repository, &fixture.tenant_id, 0xb800).await;
    assert_eq!(
        write_claim.claimed.job.job_id,
        leaves[0].capability_job.as_ref().unwrap().job_id
    );
    assert_eq!(
        read_claim.claimed.job.job_id,
        leaves[1].capability_job.as_ref().unwrap().job_id
    );
    assert_eq!(
        write_claim.claimed.invocation.payload.admission.effect,
        Effect::NonIdempotentWrite
    );
    assert_eq!(
        read_claim.claimed.invocation.payload.admission.effect,
        Effect::ReadOnly
    );
    assert!(read_claim.claimed.job.attempt_limit > 1);
    for (owner, base) in [(fixture, 0xba00), (&read, 0xba10)] {
        let (version, generation, permits): (i64, i64, i32) = sqlx::query_as("SELECT version,cancel_generation,active_work_count FROM insight_platform.runs WHERE tenant_id=$1 AND run_id=$2")
            .bind(owner.tenant_id.to_string()).bind(owner.run_id.to_string()).fetch_one(pool).await.unwrap();
        assert_eq!(permits, 1);
        ...
        tx.request_run_cancel(RequestRunCancel {
            ...
            reason_code: "fixture_capability_cancel".to_owned(),
        })
        .await
        .unwrap();
        tx.commit().await.unwrap();
    ...
    leaf_convergence_isolation::verify(
        ...
        [
            leaf_convergence_isolation::Leaf {
                ...
                job: write_claim.claimed.job.job_id.parse().unwrap(),
                expected_state: "cancelling",
                expected_settlements: 0,
            },
            leaf_convergence_isolation::Leaf {
                tenant: read.tenant_id.clone(),
                run: read.run_id.clone(),
                job: read_claim.claimed.job.job_id.parse().unwrap(),
                expected_state: "cancelling",
                expected_settlements: 0,
            },
        ],
    ...
    for _ in 0..64 {
        repository
            .drive_orchestration_convergence(convergence::command())
            .await
            .unwrap();
        let states: Vec<(String,String)> = sqlx::query_as("SELECT invocation_id,state FROM insight_platform.invocations WHERE tenant_id=$1 AND invocation_id=ANY($2)")
            .bind(fixture.tenant_id.to_string()).bind(leaves.iter().map(|leaf|leaf.invocation.invocation_id.to_string()).collect::<Vec<_>>()).fetch_all(pool).await.unwrap();
        if states.iter().all(|(owner, state)| {
            if owner == &leaves[2].invocation.invocation_id.to_string() {
                state == "cancelled"
            } else {
                state == "cancelling"
            }
    ...
    assert!(
        controlled,
        "Run control reaches both physical effects and an undispatched leaf"
    );
    let ready: (String,i32,Option<String>) = sqlx::query_as("SELECT state,attempt_no,quota_reservation_id FROM insight_platform.jobs WHERE tenant_id=$1 AND job_id=$2")
        .bind(fixture.tenant_id.to_string()).bind(&leaves[2].capability_job.as_ref().unwrap().job_id).fetch_one(pool).await.unwrap();
    assert_eq!(ready, ("cancelled".to_owned(), 0, None));
    for (claim, base) in [(&write_claim, 0xc000), (&read_claim, 0xc020)] {
        let version:i64=sqlx::query_scalar("SELECT version FROM insight_platform.invocations WHERE tenant_id=$1 AND invocation_id=$2")
            .bind(fixture.tenant_id.to_string()).bind(claim.claimed.invocation.invocation_id.to_string()).fetch_one(pool).await.unwrap();
        let mut tx = repository.begin_invocation_transaction().await.unwrap();
        let result = tx
            .commit_capability_cancellation_outcome(CommitCapabilityCancellationOutcome {
                audit: worker_audit(&fixture.tenant_id, &claim.worker_id, base, '4', '5'),
                invocation_id: claim.claimed.invocation.invocation_id.clone(),
                job_id: claim.claimed.job.job_id.parse().unwrap(),
                expected_invocation_version: u64::try_from(version).unwrap(),
                fence: None,
                quota_entry_ids: vec![
                    id(ResourceKind::QuotaLedgerEntry, base + 3),
                    id(ResourceKind::QuotaLedgerEntry, base + 4),
                ],
                cancellation_observation_digest: digest('6'),
                external_identity_digest: None,
                no_effect_proof_digest: None,
                cleanup_deadline: None,
            })
            .await;
        assert!(matches!(result, Err(RepositoryError::StaleFence)));
        tx.rollback().await.unwrap();
    }
    let recovery = |base: u16| DriveExpiredCapabilityJobs {
        work_class: WorkClass::CapabilityNative,
        shard: SafetyScanShard::whole(),
        after: None,
        limit: 2,
        slots: (0_u16..2)
            .map(|offset| {
                let base = base + offset * 32;
                ExpiredCapabilityRecoverySlot {
                    quota_entry_ids: vec![
                        id(ResourceKind::QuotaLedgerEntry, base),
                        id(ResourceKind::QuotaLedgerEntry, base + 1),
                    ],
                    event_id: id(ResourceKind::Event, base + 2),
                    outbox_id: id(ResourceKind::OutboxEvent, base + 3),
                    failure_mutations: failure_mutations(base + 8),
                }
            })
            .collect(),
    };
    let live = repository
        .drive_expired_capability_jobs(recovery(0xc100))
        .await
        .unwrap();
    assert!(live
        .records
        .iter()
        .all(|record| record.job.job_id != write_claim.claimed.job.job_id
            && record.job.job_id != read_claim.claimed.job.job_id));
    for claim in [&write_claim, &read_claim] {
        sqlx::query("UPDATE insight_platform.jobs SET heartbeat_at=clock_timestamp()-interval '2 seconds',lease_expires_at=clock_timestamp()-interval '1 second' WHERE tenant_id=$1 AND job_id=$2")
            .bind(fixture.tenant_id.to_string()).bind(&claim.claimed.job.job_id).execute(pool).await.unwrap();
    }
    let mut recovered = BTreeMap::new();
    for step in 0..8 {
        for record in repository
            .drive_expired_capability_jobs(recovery(0xc200 + step * 64))
            .await
            .unwrap()
            .records
        {
            recovered.insert(record.job.job_id.clone(), record);
        }
        if recovered.contains_key(&write_claim.claimed.job.job_id)
            && recovered.contains_key(&read_claim.claimed.job.job_id)
        {
            break;
        }
    }
    let write = &recovered[&write_claim.claimed.job.job_id];
    let read_result = &recovered[&read_claim.claimed.job.job_id];
    assert_eq!(
        write.invocation.state,
        insight_platform_contracts::InvocationState::ReconciliationRequired
    );
    assert_eq!(write.job.state, "reconciliation_required");
    assert!(write.invocation.terminal_at.is_none());
    let reconciliation = write.invocation.payload.reconciliation.as_ref().unwrap();
    assert!(reconciliation.manual);
    assert_eq!(reconciliation.effect, Effect::NonIdempotentWrite);
    assert_eq!(
        write.job.payload.value["physical_outcome"]["kind"],
        "uncertain"
    );
    assert_eq!(
        read_result.invocation.state,
        insight_platform_contracts::InvocationState::Cancelled
    );
    assert_eq!(read_result.job.state, "cancelled");
    assert!(read_result.invocation.terminal_at.is_some());
    assert!(read_result.invocation.payload.reconciliation.is_none());
    assert!(read_result.job.payload.value["physical_outcome"]["no_effect_proof_digest"].is_null());
    for (claim, result, base) in [
        (&write_claim, write, 0xc500),
        (&read_claim, read_result, 0xc520),
    ] {
        assert_eq!(result.job.attempt_no, claim.claimed.job.attempt_no);
        assert!(result.job.lease_token_digest.is_none());
        assert!(result.job.quota_reservation_id.is_none());
        assert!(result.job.retry_at.is_none());
        let (permits,settlements):(i32,i64)=sqlx::query_as("SELECT active_work_count,(SELECT count(*) FROM insight_platform.quota_ledger WHERE tenant_id=$1 AND correlation_id=$3 AND entry_kind='settle') FROM insight_platform.runs WHERE tenant_id=$1 AND run_id=$2")
            .bind(fixture.tenant_id.to_string()).bind(result.invocation.run_id.to_string()).bind(claim.claimed.job.quota_reservation_id.as_ref().unwrap()).fetch_one(pool).await.unwrap();
        assert_eq!((permits, settlements), (0, 2));
        let late = execute_outcome(
            repository,
            CommitCapabilityOutcome {
                audit: worker_audit(&fixture.tenant_id, &claim.worker_id, base, '7', '8'),
                invocation_id: claim.claimed.invocation.invocation_id.clone(),
                job_id: claim.claimed.job.job_id.parse().unwrap(),
                expected_invocation_version: claim.claimed.invocation.version,
                fence: claim.fence(),
                ...
                outcome: DispatchOutcome::Completed(CapabilityOutputValue {
                    value_id: id(ResourceKind::RunValue, base + 5),
                    classification: DataClassification::Internal,
                    schema_digest: fixture.output_schema_digest.clone(),
                    content_digest: canonical_digest(&json!({"accepted":true}))
                        .unwrap()
                        .parse()
                        .unwrap(),
                    value: ValueRef::Inline {
                    ...
                    artifact_link_id: None,
                    validation_evidence_digest: digest('9'),
                }),
                resume_mutations: claim.claimed.resume_mutations.clone(),
                failure_mutations: None,
            },
        )
        .await;
        assert!(matches!(late, Err(RepositoryError::StaleFence)));
    }
    let repeated = repository
        .drive_expired_capability_jobs(recovery(0xc600))
        .await
        .unwrap();
    assert!(repeated
        .records
        .iter()
        .all(|record| record.job.job_id != write.job.job_id
            && record.job.job_id != read_result.job.job_id));
    for _ in 0..4 {
        repository
            .drive_orchestration_convergence(convergence::command())
            .await
    ...
    for claim in [&write_claim, &read_claim] {
        ...
        .bind(fixture.tenant_id.to_string())
        .bind(claim.claimed.job.quota_reservation_id.as_ref().unwrap())
        .fetch_one(pool)
        .await
        .unwrap();
        assert_eq!(
            settlements, 2,
            "late callbacks and repeated scans cannot settle quota twice"
        );
    }
    let protected:(String,i32,bool)=sqlx::query_as("SELECT state,active_work_count,terminal_at IS NULL FROM insight_platform.runs WHERE tenant_id=$1 AND run_id=$2")
        .bind(fixture.tenant_id.to_string()).bind(fixture.run_id.to_string()).fetch_one(pool).await.unwrap();
    assert_eq!(
        protected,
        ("cancelling".to_owned(), 0, true),
        "uncertain write remains an unresolved external obligation"
    );
}
...
async fn seed_fixture(pool: &PgPool, repository: &PgRepository) -> Fixture {
    ...
    support::bind_fixture_scheduling_policy(pool, repository, &tenant_id).await;
    support::bind_fixture_scheduling_policy(pool, repository, &other_tenant_id).await;
    ...
    let plan = RuntimePlan {
        ...
        .into_iter()
        .map(|schema| (schema.canonical_digest.clone(), schema.try_into().unwrap()))
        .collect(),
        plan_version: 6,
    ...
    let current_execution = insight_platform_plan::execution::program_execution_requirement(
        plan.canonical_digest(PlanLimits::from_profile(&checked_in_hard_limit_profile()).unwrap())
            .unwrap(),
        6,
    )
    .unwrap();
    sqlx::query(
        ...
        INSERT INTO insight_platform.runs (
            ...
            $8, $9, $10, $11, $12, $12, $12,
            '0123456789abcdef0123456789abcdef', 1, $13, $14
        )
        "#,
    )
    ...
    .bind(now + Duration::hours(1))
    .bind(now)
    .bind(serde_json::to_value(&current_execution).unwrap())
    .bind(current_execution.canonical_digest().unwrap().to_string())
    .execute(pool)
    .await
    .unwrap();
...
async fn insert_version(
    ...
    revision_no: i64,
    principal_id: &ResourceId,
    mut published: PublishedVersionPayload,
) {
    if let ResourceDocument::Agent(agent) = &published.document {
        published.validation.program_requirement = Some(
            insight_platform_plan::execution::program_execution_requirement(
                agent.typed_plan_digest.clone(),
                6,
            )
            .unwrap(),
        );
