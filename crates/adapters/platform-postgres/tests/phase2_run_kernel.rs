...
use insight_platform_orchestrator::store::ChildRunCancellationSlot;
use insight_platform_orchestrator::store::ClaimOrchestrationJobs;
...
use insight_platform_orchestrator::store::ControllerActivationSlot;
use insight_platform_orchestrator::store::ControllerFacts;
use insight_platform_orchestrator::store::ControllerLoopRolloverSlot;
use insight_platform_orchestrator::store::ControllerPendingNodeSlot;
use insight_platform_orchestrator::store::ControllerPendingWakeSlot;
use insight_platform_orchestrator::store::ControllerRemainderCancellationSlot;
use insight_platform_orchestrator::store::ControllerScopeSlot;
use insight_platform_orchestrator::store::ControllerStepMutationIds;
use insight_platform_orchestrator::store::ControllerStructuralExitSlot;
use insight_platform_orchestrator::store::DeferOrchestrationChildMutationIds;
use insight_platform_orchestrator::store::DeferOrchestrationTaskMutationIds;
...
use insight_platform_orchestrator::store::DriveChildRunCancellations;
use insight_platform_orchestrator::store::DriveDueOrchestrationRetries;
use insight_platform_orchestrator::store::DriveDueOrchestrationWaits;
use insight_platform_orchestrator::store::DriveExpiredOrchestrationJobs;
use insight_platform_orchestrator::store::DriveExpiredOrchestrationTasks;
use insight_platform_orchestrator::store::DriveOrchestrationConvergence;
use insight_platform_orchestrator::store::DriveTerminalChildRuns;
use insight_platform_orchestrator::store::DueOrchestrationRetrySlot;
use insight_platform_orchestrator::store::DueOrchestrationWaitSlot;
use insight_platform_orchestrator::store::ExpiredOrchestrationRecoverySlot;
use insight_platform_orchestrator::store::ExpiredOrchestrationTaskSlot;
...
use insight_platform_orchestrator::store::OrchestrationClaimSlot;
use insight_platform_orchestrator::store::OrchestrationConvergenceSlot;
...
use insight_platform_orchestrator::store::OrchestrationTerminalMutationIds;
use insight_platform_orchestrator::store::OrchestrationWakeMutationIds;
...
use insight_platform_orchestrator::store::ResolveOrchestrationSignalTarget;
use insight_platform_orchestrator::store::ResolveOrchestrationTask;
use insight_platform_orchestrator::store::ResolveOrchestrationTaskMutationIds;
use insight_platform_orchestrator::store::RunRecord;
use insight_platform_orchestrator::store::StartOrchestrationJob;
use insight_platform_orchestrator::store::TerminalChildRunSlot;
use insight_platform_orchestrator::store::WakeOrchestrationJob;
use insight_platform_orchestrator::store::YieldOrchestrationJob;
use insight_platform_orchestrator::{
    derive_candidate_selection, derive_expression_controller, AdmitRun, ChildOutcome,
    CommittedExpressionInput, ControllerObservation, ObserveRunTimeout, RequestRunCancel,
    RunInputValue, SetRunPause,
};
use insight_platform_plan::{
    ChildCancellationPolicy, DataPortKey, ExpressionLimits, HumanTaskDefinition, JoinPolicy,
    JoinRemainderPolicy, LoopCarriedPort, MapFailurePolicy, PlanLimits, PlanNodeKey,
    PortAssignment, RuntimeDependencyKind, RuntimeDependencySlot, RuntimeNode, RuntimePlan,
    TypedExpressionProgram, TypedInstruction,
};
use insight_platform_postgres::{
    product_repository::RunProductListQuery,
    repository::{
        NewPrincipal, NewQuotaAccount, NewTenant, NewTenantPrincipal, PgRepository, RepositoryError,
    },
    verify_schema,
};
use insight_platform_security::BindTenantSchedulingPolicy;
use insight_platform_tasks::{TaskDefinition, TaskState};
use orchestration_claim_rounds::begin_orchestration_claim_fixture;
...
fn fixture_value_schema(kind: &str) -> insight_platform_contracts::ClosedValueSchema {
    let integer = json!({"type":"integer","minimum":0,"maximum":1000});
    let schema = match kind {
        "boolean" => json!({"type":"boolean"}),
        "loop" => {
            json!({"type":"object","properties":{"iteration":integer},"required":["iteration"],"additionalProperties":false})
        }
        "item" => {
            json!({"type":"object","properties":{"item":integer},"required":["item"],"additionalProperties":false})
        }
        "items" => {
            json!({"type":"array","minItems":0,"maxItems":3,"items":{"type":"object","properties":{"item":integer},"required":["item"],"additionalProperties":false}})
        }
        "response" => {
            json!({"type":"object","properties":{"answer":{"type":"string","minLength":0,"maxLength":1024,"x-platform-max-bytes":4096}},"required":["answer"],"additionalProperties":false})
        }
        "signal" => {
            json!({"type":"object","properties":{"released":{"type":"boolean"}},"required":["released"],"additionalProperties":false})
        }
        _ => panic!("unknown fixture value schema"),
    };
    ...
    insight_platform_contracts::ClosedValueSchema::build(schema).unwrap()
}
fn fixture_schema_documents(
) -> BTreeMap<Sha256Digest, insight_platform_contracts::ClosedValueSchema> {
    let mut schemas = vec![
        insight_platform_contracts::ClosedValueSchema::try_from(agent_schema()).unwrap(),
        insight_platform_contracts::ClosedValueSchema::try_from(agent_error_schema()).unwrap(),
    ];
    schemas.extend(
        ["boolean", "loop", "item", "items", "response", "signal"]
            .into_iter()
            .map(fixture_value_schema),
    );
    schemas
        .into_iter()
        .map(|schema| (schema.canonical_digest.clone(), schema))
        .collect()
}

fn literal_program(value: Value) -> TypedExpressionProgram {
    let schema = fixture_value_schema(if value.is_boolean() {
        "boolean"
        ...
        panic!("unexpected fixture literal")
    });
...
fn run_input_predicate() -> TypedExpressionProgram {
    TypedExpressionProgram::build(
        vec![insight_platform_plan::ExactDataPortRef::RunInput {
            schema_digest: agent_schema().canonical_digest,
        }],
        vec![TypedInstruction::Literal {
            value: ClosedJsonValue::build(
                fixture_value_schema("boolean").canonical_digest,
                json!(true),
            )
            .unwrap(),
        }],
...
fn input_compute_output_port() -> insight_platform_plan::ExactDataPortRef {
    insight_platform_plan::ExactDataPortRef::NodeOutput {
...
fn return_run_input() -> RuntimeNode {
    RuntimeNode::Return {
        value: insight_platform_plan::ExactDataPortRef::RunInput {
            schema_digest: agent_schema().canonical_digest,
        },
    }
}
...
fn echo_run_input_program() -> TypedExpressionProgram {
    let input = insight_platform_plan::ExactDataPortRef::RunInput {
        schema_digest: agent_schema().canonical_digest,
    };
    TypedExpressionProgram::build(
        vec![input.clone()],
...
fn item_port(node: &str) -> insight_platform_plan::ExactDataPortRef {
    insight_platform_plan::ExactDataPortRef::NodeOutput {
        producer_node_id: PlanNodeKey::new(node.to_owned()).unwrap(),
        port_id: DataPortKey::new("item".to_owned()).unwrap(),
        schema_digest: fixture_value_schema("item").canonical_digest,
    }
}

fn loop_next_port() -> insight_platform_plan::ExactDataPortRef {
    insight_platform_plan::ExactDataPortRef::NodeOutput {
        producer_node_id: PlanNodeKey::new("loop".to_owned()).unwrap(),
        port_id: DataPortKey::new("carried".to_owned()).unwrap(),
        schema_digest: fixture_value_schema("loop").canonical_digest,
    }
}

fn loop_body_output_port() -> insight_platform_plan::ExactDataPortRef {
    insight_platform_plan::ExactDataPortRef::NodeOutput {
...
fn runtime_plan() -> RuntimePlan {
    ...
    RuntimePlan {
        schema_documents: fixture_schema_documents(),
        plan_version: 6,
        ...
        nodes: BTreeMap::from([
            ...
            (
                input_branch,
                RuntimeNode::Branch {
                    ordered_arms: vec![insight_platform_plan::BranchArm {
            ...
            (
                ...
                RuntimeNode::HumanTask {
                    ...
                    response: insight_platform_plan::ExactDataPortRef::NodeOutput {
                        producer_node_id: human_task,
                        port_id: DataPortKey::new("response".to_owned()).unwrap(),
                        schema_digest: fixture_value_schema("response").canonical_digest,
                    },
            ...
            (
                ...
                RuntimeNode::SignalWait {
                    ...
                    payload: Some(insight_platform_plan::ExactDataPortRef::NodeOutput {
                        producer_node_id: signal_wait,
                        port_id: DataPortKey::new("payload".to_owned()).unwrap(),
                        schema_digest: fixture_value_schema("signal").canonical_digest,
                    }),
                    timeout_milliseconds: 60_000,
                    resume: finish.clone(),
                },
            ...
            (
                raise,
                RuntimeNode::Raise {
                    failure: insight_platform_plan::ExactDataPortRef::RunInput {
...
fn run_admission_and_controls_are_atomic_exact_and_first_winner() {
    ...
    runtime.block_on(async {
        tokio::spawn(async {
    let database_url = std::env::var("PLATFORM_TEST_RUN_KERNEL_DATABASE_URL").expect(
        "PLATFORM_TEST_RUN_KERNEL_DATABASE_URL requires an isolated current-schema database for the Run kernel transaction fixture",
    );
    ...
    let (scheduler, rolled_back) = begin_orchestration_claim_fixture(&repository, claim.clone()).await.unwrap();
    ...
    let (scheduler, claimed) = begin_orchestration_claim_fixture(&repository, claim).await.unwrap();
    ...
    assert_eq!(
        ...
        .await
        .unwrap(),
        i64::from(insight_platform_contracts::SCHEDULER_PARTITION_COUNT)
    );
    ...
    let (scheduler, leased_for_recovery) = begin_orchestration_claim_fixture(&repository, short_claim).await.unwrap();
    scheduler.commit().await.unwrap();
    ...
    let (scheduler, claimed_for_running_recovery) = begin_orchestration_claim_fixture(&repository, running_claim).await.unwrap();
    scheduler.commit().await.unwrap();
    ...
    isolate_ready_orchestration_job(&pool, &terminal_command.orchestration_job_id.to_string()).await;
    ...
    let (scheduler, claimed_terminal) = begin_orchestration_claim_fixture(&repository, terminal_claim).await.unwrap();
    scheduler.commit().await.unwrap();
    ...
    .await
    .unwrap();
    let (scheduler, wait_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            'd',
            "7906",
            ["7907", "7908", "7909", "790a"],
    ...
    *delay_milliseconds += 1;
    ...
    let (scheduler, retry_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            'e',
            "791d",
            ["791e", "791f", "7920", "7921"],
    ...
    .await
    .unwrap();
    isolate_ready_orchestration_job(&pool, &signal_command.orchestration_job_id.to_string()).await;
    let (scheduler, signal_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            '6',
            "8106",
            ["8107", "8108", "8109", "810a"],
    ...
    .await
    .unwrap();
    let (scheduler, timeout_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            '7',
            "8206",
            ["8207", "8208", "8209", "820a"],
    ...
    assert_eq!(
        repository
            .read_public_run_events_for_principal(
                ...
                PrincipalKind::AgentRunner,
                &command.run_id,
                PublicRunReadPosition::AfterSequence { sequence: 2 },
                1,
            )
            .await
            .unwrap().events[0]
    ...
    let timed_out = converge_next_run(&repository, timeout_convergence)
        .await
        .unwrap();
    assert_eq!(timed_out.len(), 1);
    assert_eq!(timed_out[0].run.state, "timed_out");
    assert_eq!(timed_out[0].jobs[0].state, "timed_out");
    assert_eq!(timed_out[0].run.active_work_count, 0);
    assert!(timed_out[0].settled_quota_account_ids.is_empty());
    let ready_cancel_convergence = orchestration_convergence(
        ["7b09", "7b0a", "7b0b", "7b0c"],
        ["7b0d", "7b0e", "7b0f", "7b10", "7b11", "7bf1"],
    );
    let ready_cancelled = converge_next_run(&repository, ready_cancel_convergence)
        .await
        .unwrap();
    assert_eq!(ready_cancelled.len(), 1);
    assert_eq!(ready_cancelled[0].run.state, "cancelled");
    assert_eq!(ready_cancelled[0].jobs[0].state, "cancelled");
    assert!(ready_cancelled[0]
        .settled_quota_account_ids
        .is_empty());

    isolate_ready_orchestration_job(&pool, &wait_command.orchestration_job_id.to_string()).await;
    let (scheduler, active_cancel_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            '3',
            "7b12",
            ["7b13", "7b14", "7b15", "7b16"],
    ...
    let converged_cancel = converge_next_run(&repository, orchestration_convergence(
            ["7b28", "7b29", "7b2a", "7b2b"],
    ...
    assert_eq!(converged_cancel[0].run.state, "cancelled");
    assert_eq!(converged_cancel[0].jobs[0].state, "cancelled");
    assert_eq!(
        converged_cancel[0].settled_quota_account_ids,
        vec![QUOTA_ACCOUNT_ID]
    );
    ...
    let (scheduler, exhausted_claimed) = begin_orchestration_claim_fixture(&repository, exhausted_claim).await.unwrap();
    scheduler.commit().await.unwrap();
    ...
    let exhausted = converge_next_run(&repository, orchestration_convergence(
            ["7c0c", "7c0d", "7c0e", "7c0f"],
    ...
    assert_eq!(exhausted[0].run.state, "failed");
    assert_eq!(exhausted[0].jobs[0].state, "failed");
    assert_eq!(exhausted[0].jobs[0].attempt_no, 1);
    assert_eq!(
        exhausted[0].settled_quota_account_ids,
        vec![QUOTA_ACCOUNT_ID]
    );
    ...
    isolate_ready_orchestration_job(&pool, &$fixture_job_id).await;
    ...
    .await
    .unwrap();
    let (scheduler, task_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            '2',
            "7d06",
            ["7d07", "7d08", "7d09", "7d0a"],
    ...
    let response_schema_digest = fixture_value_schema("response").canonical_digest;
    ...
    let public_task = repository
        ...
        .await
        .unwrap();
    assert_eq!(public_task.task.task_id, task_id.to_string());
    assert_eq!(public_task.task.state, TaskState::Pending);
    ...
    let resolve_task = ResolveOrchestrationTask {
        validated_input: Some(insight_platform_tasks::ValidatedTaskInput::validate_inline(&ClosedJsonSchema::try_from(fixture_value_schema("response")).unwrap(), match &response.value { ValueRef::Inline { value } => value, _ => unreachable!() }).unwrap()),
    ...
    // Exercise the actual cancellation writer and Receipt replay against the same pending
    // authority. Roll back this alternative outcome so the committed response chain below
    // retains its independent continuation coverage.
    Box::pin(assert_cancelled_task_replay(&repository, &resolve_task)).await;
    ...
    let mut retried_task = resolve_task.clone();
    let new_id = |kind| ResourceId::from_uuid_v7(kind, uuid::Uuid::now_v7()).unwrap();
    retried_task.resume_job_id = new_id(ResourceKind::Job);
    retried_task.response.as_mut().unwrap().value_id = new_id(ResourceKind::RunValue);
    retried_task.audit.receipt_id = new_id(ResourceKind::Receipt);
    retried_task.audit.event_id = new_id(ResourceKind::Event);
    retried_task.audit.outbox_id = new_id(ResourceKind::OutboxEvent);
    let mut scheduler = repository.begin_scheduler_transaction().await.unwrap();
    assert!(matches!(scheduler.resolve_orchestration_task(retried_task).await.unwrap(),
        CommandOutcome::Replayed(record) if record.task.state == TaskState::Responded
            && record.job.job_id == resolved.job.job_id && record.task.response_value_id == resolved.task.response_value_id
    ));
    scheduler.commit().await.unwrap();
    let mut scheduler = repository.begin_scheduler_transaction().await.unwrap();
    assert!(matches!(
    ...
    isolate_ready_orchestration_job(&pool, &resolved.job.job_id).await;
    ...
    .await
    .unwrap();
    let (scheduler, expiry_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            'f',
            "7d26",
            ["7d27", "7d28", "7d29", "7d2a"],
    ...
    let expiring_task = DeferOrchestrationToTask {
        ...
        response_schema_digest: Some(fixture_value_schema("response").canonical_digest),
    ...
    .await
    .unwrap();
    let (scheduler, child_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            'a',
            "7e30",
            ["7e31", "7e32", "7e33", "7e34"],
    ...
    assert_eq!(
        deferred_child.child_run.deadline,
        (deferred_child.child_run.created_at + Duration::milliseconds(60_000))
            .min(deferred_child.parent_run.deadline)
            .min(deferred_child.parent_job.deadline)
    );
    let admission_timestamps: Vec<(DateTime<Utc>, DateTime<Utc>)> = sqlx::query_as(
    ...
    .bind(TENANT_ID)
    .bind(&deferred_child.child_run.run_id)
    .bind(vec![
        defer_child.child_root_scope_id.to_string(),
        defer_child.child_entry_node_execution_id.to_string(),
        defer_child.child_link_id.to_string(),
    ])
    .bind(&deferred_child.child_job.job_id)
    .bind(defer_child.input.value_id.to_string())
    .fetch_all(&pool)
    .await
    .unwrap();
    assert_eq!(admission_timestamps.len(), 6);
    assert!(admission_timestamps.iter().all(|(created, updated)| {
        *created == deferred_child.child_run.created_at && created == updated
    }));
    assert_eq!(
    ...
    let mut altered_budget = defer_child.clone();
    altered_budget.budget.maximum_duration_milliseconds -= 1;
    let mut scheduler = repository.begin_scheduler_transaction().await.unwrap();
    assert!(matches!(
        scheduler.defer_orchestration_to_child_run(altered_budget).await,
        Err(RepositoryError::IdempotencyConflict)
    ));
    scheduler.rollback().await.unwrap();
    let mut scheduler = repository.begin_scheduler_transaction().await.unwrap();
    assert!(matches!(
        ...
        CommandOutcome::Replayed(record)
            if record.child_link.child_link_id == deferred_child.child_link.child_link_id
                && record.child_run.deadline == deferred_child.child_run.deadline
    ));
    scheduler.commit().await.unwrap();
    ...
    let mut altered_logical_budget = logical_replay.clone();
    altered_logical_budget.budget.maximum_duration_milliseconds -= 1;
    let mut scheduler = repository.begin_scheduler_transaction().await.unwrap();
    assert!(matches!(
        scheduler.defer_orchestration_to_child_run(altered_logical_budget).await,
        Err(RepositoryError::Conflict("orchestration child exact Plan contract"))
    ));
    scheduler.rollback().await.unwrap();
    let mut scheduler = repository.begin_scheduler_transaction().await.unwrap();
    assert!(matches!(
    ...
    let (scheduler, claimed_child_job) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_C_ID,
            '5',
            "7f00",
            ["7f01", "7f02", "7f03", "7f04"],
    ...
    assert_eq!(terminal_child[0].resume_job.node_id.as_deref(), Some(terminal_child[0].parent_node_id.as_str()));
    let continued_child_job = settle_child_completion_fixture(&repository, &pool, &terminal_child[0].resume_job, &runtime_plan()).await;
    ...
    let (scheduler, cancel_parent_claim) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            'd',
            "8000",
            ["8001", "8002", "8003", "8004"],
    ...
    let cancelled_parent = match run_command!(repository, request_run_cancel, cancel_parent.clone()).unwrap()
    {
        CommandOutcome::Applied(record) => record,
    ...
    let child_cancelled = converge_next_run(&repository, orchestration_convergence(
            ["8025", "8026", "8027", "8028"],
    ...
    assert_eq!(child_cancelled[0].run.state, "cancelled");
    assert_eq!(
        child_cancelled[0].run.run_id,
    ...
    let parent_cancelled = converge_next_run(&repository, orchestration_convergence(
            ["8034", "8035", "8036", "8037"],
    ...
    assert_eq!(parent_cancelled[0].run.state, "cancelled");
    assert_eq!(
        parent_cancelled[0].run.run_id,
    ...
    .unwrap());
    let (scheduler, controller_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            '7',
            "9010",
            ["9011", "9012", "9013", "9014"],
    ...
    let controller_cleanup = converge_next_run(&repository, orchestration_convergence(
            ["9031", "9032", "9033", "9034"],
    ...
    assert_eq!(controller_cleanup[0].run.state, "cancelled");
    ...
    let (scheduler, fork_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            '1',
            "9110",
            ["9111", "9112", "9113", "9114"],
    ...
    let (scheduler, left_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            '2',
            "9160",
            ["9161", "9162", "9163", "9164"],
    ...
    let (scheduler, right_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            '8',
            "9180",
            ["9181", "9182", "9183", "9184"],
    ...
    let (scheduler, join_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_C_ID,
            'e',
            "91a0",
            ["91a1", "91a2", "91a3", "91a4"],
    ...
    let fork_cleanup = converge_next_run(&repository, orchestration_convergence(
            ["91c1", "91c2", "91c3", "91c4"],
    ...
    assert_eq!(fork_cleanup[0].run.state, "cancelled");
    ...
    for (quota_suffixes, event_suffixes) in [
        ...
        let cleaned = converge_next_run(&repository, orchestration_convergence(
                quota_suffixes,
                event_suffixes,
            ))
            .await
        ...
        assert_eq!(cleaned[0].run.state, "cancelled");
    }
    assert_eq!(
        sqlx::query_scalar::<_, i64>(
            "SELECT reserved_value FROM insight_platform.quota_accounts WHERE tenant_id = $1 AND quota_account_id = $2",
    ...
    let (scheduler, quorum_fork_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            '7',
            "9210",
            ["9211", "9212", "9213", "9214"],
    ...
    let (scheduler, first_quorum_leg) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            'c',
            "9260",
            ["9261", "9262", "9263", "9264"],
            ["9265", "9266", "9267"],
        )).await.expect("first Quorum leg claim must succeed");
    scheduler.commit().await.unwrap();
    let (scheduler, second_quorum_leg) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            'd',
            "9270",
            ["9271", "9272", "9273", "9274"],
            ["9275", "9276", "9277"],
        )).await.expect("second Quorum leg claim must succeed");
    scheduler.commit().await.unwrap();
    ...
    let (scheduler, quorum_join_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_C_ID,
            '5',
            "92a0",
            ["92a1", "92a2", "92a3", "92a4"],
            ["92a5", "92a6", "92a7"],
        )).await.expect("Quorum Join claim must succeed");
    scheduler.commit().await.unwrap();
    ...
    let quorum_cleanup = converge_next_run(&repository, orchestration_convergence(
            ["92d1", "92d2", "92d3", "92d4"],
    ...
    assert_eq!(quorum_cleanup[0].run.state, "cancelled");
    ...
    let (scheduler, cancel_fork_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            '7',
            "9410",
            ["9411", "9412", "9413", "9414"],
    ...
    let (scheduler, first_cancel_leg) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            'c',
            "9460",
            ["9461", "9462", "9463", "9464"],
            ["9465", "9466", "9467"],
        )).await.expect("first Cancel leg claim must succeed");
    scheduler.commit().await.unwrap();
    let (scheduler, second_cancel_leg) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            'd',
            "9470",
            ["9471", "9472", "9473", "9474"],
            ["9475", "9476", "9477"],
        )).await.expect("second Cancel leg claim must succeed");
    scheduler.commit().await.unwrap();
    ...
    let (scheduler, cancel_join_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_C_ID,
            '5',
            "94b0",
            ["94b1", "94b2", "94b3", "94b4"],
            ["94b5", "94b6", "94b7"],
        )).await.expect("Cancel Join claim must succeed");
    scheduler.commit().await.unwrap();
    ...
    let cancel_cleanup = converge_next_run(&repository, orchestration_convergence(
            ["94d1", "94d2", "94d3", "94d4"],
    ...
    assert_eq!(cancel_cleanup[0].run.state, "cancelled");
    ...
    let (scheduler, failed_fork_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            '7',
            "9510",
            ["9511", "9512", "9513", "9514"],
    ...
    let (scheduler, first_failed_leg) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            'c',
            "9560",
            ["9561", "9562", "9563", "9564"],
    ...
    scheduler.commit().await.unwrap();
    let (scheduler, second_failed_leg) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            'd',
            "9570",
            ["9571", "9572", "9573", "9574"],
    ...
    let (scheduler, failed_join_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_C_ID,
            '5',
            "95b0",
            ["95b1", "95b2", "95b3", "95b4"],
    ...
    assert!(matches!(
        ...
        insight_platform_orchestrator::store::ControllerStructuralRequirement::Close
    ));
    ...
    let (scheduler, boundary_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            'c',
            "9610",
            ["9611", "9612", "9613", "9614"],
    ...
    let (scheduler, boundary_body_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            '1',
            "9630",
            ["9631", "9632", "9633", "9634"],
    ...
    let (scheduler, boundary_handler_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            '6',
            "9650",
            ["9651", "9652", "9653", "9654"],
    ...
    let (scheduler, expression_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            '3',
            "aa06",
            ["aa07", "aa08", "aa09", "aa0a"],
    ...
    let expression_cleanup = converge_next_run(&repository, orchestration_convergence(
            ["aa12", "aa13", "aa14", "aa15"],
    ...
    assert_eq!(expression_cleanup[0].run.state, "cancelled");
    ...
    let (scheduler, compute_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            'a',
            "ab06",
            ["ab07", "ab08", "ab09", "ab0a"],
    ...
    let (scheduler, terminal_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            'f',
            "ab30",
            ["ab31", "ab32", "ab33", "ab34"],
    ...
    let (scheduler, raise_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            '6',
            "ab56",
            ["ab57", "ab58", "ab59", "ab5a"],
    ...
    .unwrap());
    let (scheduler, unsafe_raise_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            'd',
            "af06",
            ["af07", "af08", "af09", "af0a"],
    ...
    let unsafe_converged = converge_next_run(&repository, orchestration_convergence(
            ["af1c", "af1d", "af1e", "af1f"],
    ...
    assert_eq!(unsafe_converged[0].run.state, "cancelled");
    assert_eq!(unsafe_converged[0].jobs[0].state, "cancelled");
    ...
    let (scheduler, loop_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            'd',
            "9710",
            ["9711", "9712", "9713", "9714"],
    ...
    let (scheduler, loop_body_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            '2',
            "9730",
            ["9731", "9732", "9733", "9734"],
    ...
    let (scheduler, loop_continuation_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            '8',
            "9750",
            ["9751", "9752", "9753", "9754"],
    ...
    let (scheduler, second_body_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            '3',
            "ac10",
            ["ac11", "ac12", "ac13", "ac14"],
    ...
    let (scheduler, final_continuation_claimed) = begin_orchestration_claim_fixture(&repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            '9',
            "ac31",
            ["ac32", "ac33", "ac34", "ac35"],
    ...
    let loop_cleanup = converge_next_run(&repository, orchestration_convergence(
            ["9771", "9772", "9773", "9774"],
    ...
    assert_eq!(loop_cleanup[0].run.state, "cancelled");
    ...
    let (scheduler, map_claimed) = begin_orchestration_claim_fixture(&map_repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            '9',
            "9810",
            ["9811", "9812", "9813", "9814"],
    ...
    isolate_ready_orchestration_job(&pool, &first_map_batch.woken_nodes[0].job.job_id).await;
    let (scheduler, map_admission_continuation) = begin_orchestration_claim_fixture(&map_repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            'f',
            "9844",
            ["9845", "9846", "9847", "9848"],
    ...
    {
        let item_job = first_map_batch.activations.iter().chain(final_map_batch.activations.iter()).nth(index).unwrap();
        isolate_ready_orchestration_job(&pool, &item_job.job.job_id).await;
        let (scheduler, claimed) = begin_orchestration_claim_fixture(&map_repository, orchestration_claim_with_ids(
                WORKER_D_ID,
                token,
                reservation,
                claim_quota,
    ...
    isolate_ready_orchestration_job(&pool, map_settlement_job_id.as_ref().unwrap()).await;
    let (scheduler, map_settlement_claimed) = begin_orchestration_claim_fixture(&map_repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            'd',
            "98c0",
            ["98c1", "98c2", "98c3", "98c4"],
    ...
    let map_cleanup = converge_next_run(&map_repository, orchestration_convergence(
            ["98e1", "98e2", "98e3", "98e4"],
    ...
    assert_eq!(map_cleanup[0].run.state, "cancelled");
    ...
    let (scheduler, fail_fast_root_claimed) = begin_orchestration_claim_fixture(&map_repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            '0',
            "9906",
            ["9907", "9908", "9909", "990a"],
    ...
    let (scheduler, fail_fast_item_a) = begin_orchestration_claim_fixture(&map_repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            '5',
            "9930",
            ["9931", "9932", "9933", "9934"],
    ...
    scheduler.commit().await.unwrap();
    let (scheduler, fail_fast_item_b) = begin_orchestration_claim_fixture(&map_repository, orchestration_claim_with_ids(
            WORKER_C_ID,
            '6',
            "9940",
            ["9941", "9942", "9943", "9944"],
    ...
    let (scheduler, fail_fast_settlement_claimed) = begin_orchestration_claim_fixture(&map_repository, orchestration_claim_with_ids(
            WORKER_D_ID,
            'e',
            "9970",
            ["9971", "9972", "9973", "9974"],
    ...
    repository
        .bind_tenant_principal(NewTenantPrincipal {
            ...
            payload: TenantPrincipalPayload {
                permissions: PermissionSet::new(vec![Permission::RuntimeRead, Permission::ArtifactRead]).unwrap(),
            },
        })
        .await
        .unwrap();
    ...
    child_budget_admission::verify(&repository, &pool, &bindings).await;

    // Receipt replay never grants current Run control or Task responder authority.
    assert!(matches!(run_command!(repository, request_run_cancel, cancel_parent.clone()).unwrap(), CommandOutcome::Replayed(_)));
    support::revoke_fixture_principal(&pool, &repository, &id(TENANT_ID), &id(PRINCIPAL_ID), PrincipalKind::AgentRunner).await;
    let counts = || async {
        sqlx::query_as::<_, (i64,i64,i64,i64)>("SELECT (SELECT count(*) FROM insight_platform.receipts), (SELECT count(*) FROM insight_platform.events), (SELECT count(*) FROM insight_platform.outbox_events), (SELECT count(*) FROM insight_platform.jobs)").fetch_one(&pool).await.unwrap()
    };
    let before = counts().await;
    assert!(matches!(run_command!(repository, request_run_cancel, cancel_parent), Err(RepositoryError::PermissionDenied)));
    let mut transaction = repository.begin_scheduler_transaction().await.unwrap();
    assert!(matches!(transaction.resolve_orchestration_task(resolve_task).await, Err(RepositoryError::PermissionDenied)));
    transaction.rollback().await.unwrap();
...
// Aggregate assertions across actual per-Run transactions. Each Job appears once;
// the terminal observation is accepted only after the owning RunTerminal step.
struct ConvergenceEvidence {
    run: RunRecord,
    jobs: Vec<insight_platform_jobs::store::JobRecord>,
    settled_quota_account_ids: Vec<String>,
}

async fn assert_cancelled_task_replay(
    repository: &PgRepository,
    resolve_task: &ResolveOrchestrationTask,
) {
    let mut cancel = resolve_task.clone();
    cancel.target = TaskState::Cancelled;
    cancel.response = None;
    cancel.validated_input = None;
    let mut cancellation = repository.begin_scheduler_transaction().await.unwrap();
    let cancelled = match cancellation
        .resolve_orchestration_task(cancel.clone())
        .await
        .unwrap()
    {
        CommandOutcome::Applied(record) => record,
        _ => panic!("first cancellation must apply"),
    };
    assert_eq!(cancelled.task.state, TaskState::Cancelled);
    assert!(cancelled.task.payload.value["resolution"]["principal"].is_null());
    assert!(
        matches!(cancellation.resolve_orchestration_task(cancel).await.unwrap(), CommandOutcome::Replayed(record) if record.task.state == TaskState::Cancelled && record.job.job_id == cancelled.job.job_id)
    );
    cancellation.rollback().await.unwrap();
}

async fn converge_next_run(
    repository: &PgRepository,
    mut command: DriveOrchestrationConvergence,
) -> Result<Vec<ConvergenceEvidence>, RepositoryError> {
    use insight_platform_orchestrator::store::OrchestrationConvergenceStep;
    let mut evidence = BTreeMap::<String, ConvergenceEvidence>::new();
    for _ in 0..512 {
        let steps = repository
            .drive_orchestration_convergence(command.clone())
            .await?;
        if steps.is_empty() {
            command.after = None;
        }
        for step in steps {
            command.after = if matches!(step.step, OrchestrationConvergenceStep::NotReady) {
                Some(insight_platform_jobs::store::SafetyScanCursor {
                    sort_at: step.run.deadline,
                    tenant_id: id(&step.run.tenant_id),
                    item_id: id(&step.run.run_id),
                })
            } else {
                // Complete the oldest eligible Run before collecting the next fixture outcome.
                None
            };
            let record =
                evidence
                    .entry(step.run.run_id.clone())
                    .or_insert_with(|| ConvergenceEvidence {
                        run: step.run.clone(),
                        jobs: Vec::new(),
                ...
                OrchestrationConvergenceStep::JobNode {
                    ...
                    assert!(
                        !record
                            .jobs
                            .iter()
                            .any(|earlier| earlier.job_id == job.job_id),
                        "one Job settles once"
                    );
                    assert!(matches!(
                        job.state.as_str(),
                    ...
                    record
                        .settled_quota_account_ids
                        .extend(settled_quota_account_ids);
                }
                OrchestrationConvergenceStep::RunTerminal => {
                    assert_eq!(record.run.active_work_count, 0);
                    let run_id = record.run.run_id.clone();
                    return Ok(vec![evidence.remove(&run_id).unwrap()]);
                }
        ...
        for slot in &mut command.slots {
            ...
            for identity in &mut slot.quota_entry_ids {
                *identity = fresh(ResourceKind::QuotaLedgerEntry);
            }
            for identity in [
                &mut slot.run_event_id,
                &mut slot.node_event_id,
                &mut slot.node_cancelling_event_id,
                &mut slot.scope_closing_event_id,
                &mut slot.scope_terminal_event_id,
                &mut slot.job_event_id,
            ] {
                *identity = fresh(ResourceKind::Event);
            }
            for identity in [
                &mut slot.run_outbox_id,
                &mut slot.node_outbox_id,
                &mut slot.node_cancelling_outbox_id,
                &mut slot.scope_closing_outbox_id,
                &mut slot.scope_terminal_outbox_id,
                &mut slot.job_outbox_id,
            ] {
                *identity = fresh(ResourceKind::OutboxEvent);
            }
        }
    }
    panic!(
        "bounded convergence did not reach a Run terminal; observed {} Runs",
        evidence.len()
    );
}
...
// This monolithic transition fixture retains other ready Runs for later assertions. Give only
// the scenario under test a due time; production scheduling promises finite-cohort progress,
// not global priority order. Keep Job/Node states and all execution ownership facts intact.
async fn isolate_ready_orchestration_job(pool: &PgPool, job_id: &str) {
    let mut transaction = pool.begin().await.unwrap();
    sqlx::query("UPDATE insight_platform.jobs SET scheduled_at = clock_timestamp() + interval '1 hour' WHERE tenant_id = $1 AND work_class = 'orchestration' AND state = 'ready' AND job_id <> $2")
        .bind(TENANT_ID).bind(job_id).execute(&mut *transaction).await.unwrap();
    let selected = sqlx::query("UPDATE insight_platform.jobs SET scheduled_at = clock_timestamp() WHERE tenant_id = $1 AND work_class = 'orchestration' AND state = 'ready' AND job_id = $2")
        .bind(TENANT_ID).bind(job_id).execute(&mut *transaction).await.unwrap();
...
async fn claim_with_serializable_retry(
    ...
    for retry in 0..4 {
        match begin_orchestration_claim_fixture(repository, command.clone()).await {
            Ok((transaction, claimed)) => match transaction.commit().await {
    ...
    let agent_plan_payload = TypedPayload::new(
        ...
        &PublishedVersionPayload {
            ...
            validation: ValidationSummary {
                program_requirement: Some(
                    insight_platform_plan::execution::program_execution_requirement(
    ...
    let child_interface_payload = TypedPayload::new(
        ...
        &PublishedVersionPayload {
            document: ResourceDocument::Agent(AgentResourceSpec {
                ...
                typed_plan_digest: child_typed_plan_digest.clone(),
            }),
            validation: ValidationSummary {
                program_requirement: Some(
                    insight_platform_plan::execution::program_execution_requirement(
                        child_typed_plan_digest.clone(),
                        6,
                    )
                    .unwrap(),
                ),
...
/// Exercise the genuine completed Child owner before the later cancellation scenario. Fault
/// injection changes only this fixture's rows and restores exact values before ordinary advance.
async fn settle_child_completion_fixture(
    repository: &PgRepository,
    pool: &PgPool,
    completion_job: &insight_platform_jobs::store::JobRecord,
    plan: &RuntimePlan,
) -> insight_platform_jobs::store::JobRecord {
    let fresh = |kind| ResourceId::from_uuid_v7(kind, uuid::Uuid::now_v7()).unwrap();
    isolate_ready_orchestration_job(pool, &completion_job.job_id).await;
    let config: Value =
    ...
    unbound["scheduling_policy"] = Value::Null;
    sqlx::query(
        "UPDATE insight_platform.tenants SET config=$2,config_digest=$3 WHERE tenant_id=$1",
    )
    .bind(TENANT_ID)
    .bind(&unbound)
    .bind(canonical_digest(&unbound).unwrap())
    .execute(pool)
    .await
    .unwrap();
    let fairness: (Option<String>,Option<String>,Option<String>,i64) = sqlx::query_as("SELECT policy_version_id,policy_version_digest,rules_digest,deficit FROM insight_platform.scheduler_tenant_state WHERE tenant_id=$1 AND work_class='orchestration'").bind(TENANT_ID).fetch_one(pool).await.unwrap();
    sqlx::query("UPDATE insight_platform.scheduler_tenant_state SET policy_version_id=NULL,policy_version_digest=NULL,rules_digest=NULL,deficit=0 WHERE tenant_id=$1 AND work_class='orchestration'").bind(TENANT_ID).execute(pool).await.unwrap();
    ...
    // Removing the typed settlement marker makes the same ready work business-only and Unbound.
    let mut ordinary = insight_platform_orchestrator::OrchestrationJobPayload::from_payload(
        &completion_job.payload,
    )
    .unwrap();
    ordinary.external_leaf_completion = None;
    let ordinary = ordinary.to_payload().unwrap();
    sqlx::query("UPDATE insight_platform.jobs SET payload=$3,payload_digest=$4 WHERE tenant_id=$1 AND job_id=$2").bind(TENANT_ID).bind(&completion_job.job_id).bind(&ordinary.value).bind(&ordinary.digest).execute(pool).await.unwrap();
    let (tx, claimed) = begin_orchestration_claim_fixture(repository, claim.clone())
        .await
        .unwrap();
    assert!(claimed.is_empty());
    tx.commit().await.unwrap();
    sqlx::query("UPDATE insight_platform.jobs SET payload=$3,payload_digest=$4 WHERE tenant_id=$1 AND job_id=$2").bind(TENANT_ID).bind(&completion_job.job_id).bind(&completion_job.payload.value).bind(&completion_job.payload.digest).execute(pool).await.unwrap();
    // Control settlement remains subject to the same concurrent-work quota.
    let limit:i64=sqlx::query_scalar("SELECT limit_value FROM insight_platform.quota_accounts WHERE tenant_id=$1 AND quota_account_id=$2").bind(TENANT_ID).bind(QUOTA_ACCOUNT_ID).fetch_one(pool).await.unwrap();
    sqlx::query("UPDATE insight_platform.quota_accounts SET limit_value=used_value+reserved_value WHERE tenant_id=$1 AND quota_account_id=$2").bind(TENANT_ID).bind(QUOTA_ACCOUNT_ID).execute(pool).await.unwrap();
    let (tx, claimed) = begin_orchestration_claim_fixture(repository, claim.clone())
        .await
        .unwrap();
    assert!(claimed.is_empty());
    tx.commit().await.unwrap();
    sqlx::query("UPDATE insight_platform.quota_accounts SET limit_value=$3 WHERE tenant_id=$1 AND quota_account_id=$2").bind(TENANT_ID).bind(QUOTA_ACCOUNT_ID).bind(limit).execute(pool).await.unwrap();
    let (tx, claimed) = begin_orchestration_claim_fixture(repository, claim)
        .await
        .unwrap();
    tx.commit().await.unwrap();
    assert_eq!(claimed.len(), 1);
    assert_eq!(claimed[0].job.job_id, completion_job.job_id);
    sqlx::query(
        "UPDATE insight_platform.tenants SET config=$2,config_digest=$3 WHERE tenant_id=$1",
    )
    .bind(TENANT_ID)
    .bind(&config)
    .bind(canonical_digest(&config).unwrap())
    .execute(pool)
    .await
    .unwrap();
    sqlx::query("UPDATE insight_platform.scheduler_tenant_state SET policy_version_id=$2,policy_version_digest=$3,rules_digest=$4,deficit=$5 WHERE tenant_id=$1 AND work_class='orchestration'").bind(TENANT_ID).bind(fairness.0).bind(fairness.1).bind(fairness.2).bind(fairness.3).execute(pool).await.unwrap();
    let start = StartOrchestrationJob {
        fence: JobFence {
            tenant_id: TENANT_ID.into(),
            job_id: completion_job.job_id.clone(),
            worker_id: id(WORKER_D_ID),
    ...
    let started = match tx.start_orchestration_job(start.clone()).await.unwrap() {
        CommandOutcome::Applied(job) => job,
        _ => panic!("fresh completion"),
    };
    tx.commit().await.unwrap();
    let fence = JobFence {
        expected_job_version: started.version,
    ...
    let completion = insight_platform_orchestrator::OrchestrationJobPayload::from_payload(
        &completion_job.payload,
    )
    .unwrap();
    ...
    let classification: String = sqlx::query_scalar(
        "SELECT classification FROM insight_platform.run_values WHERE tenant_id=$1 AND value_id=$2",
    )
    .bind(TENANT_ID)
    ...
    .await
    .unwrap();
    sqlx::query("UPDATE insight_platform.run_values SET classification='public' WHERE tenant_id=$1 AND value_id=$2").bind(TENANT_ID).bind(output.to_string()).execute(pool).await.unwrap();
    assert!(
        repository
            .load_controller_facts(&fence, plan)
            .await
            .is_err(),
        "completion cannot lower copied output classification"
    );
    sqlx::query("UPDATE insight_platform.run_values SET classification=$3 WHERE tenant_id=$1 AND value_id=$2").bind(TENANT_ID).bind(output.to_string()).bind(classification).execute(pool).await.unwrap();
    let mut forged = completion.clone();
    forged
        .external_leaf_completion
        .as_mut()
        .unwrap()
        .source_orchestration_job_id = fresh(ResourceKind::Job);
    let forged = forged.to_payload().unwrap();
    sqlx::query("UPDATE insight_platform.jobs SET payload=$3,payload_digest=$4 WHERE tenant_id=$1 AND job_id=$2").bind(TENANT_ID).bind(&completion_job.job_id).bind(&forged.value).bind(&forged.digest).execute(pool).await.unwrap();
    assert!(
        repository
            .load_controller_facts(&fence, plan)
            .await
            .is_err(),
        "missing source cannot become success evidence"
    );
    sqlx::query("UPDATE insight_platform.jobs SET payload=$3,payload_digest=$4 WHERE tenant_id=$1 AND job_id=$2").bind(TENANT_ID).bind(&completion_job.job_id).bind(&completion_job.payload.value).bind(&completion_job.payload.digest).execute(pool).await.unwrap();
    let mut command = ApplyOrchestrationControllerStep {
        fence,
        plan: plan.clone(),
        observation: ControllerObservation::None,
        idempotency_key_digest: digest('a'),
        ...
        mutations: ControllerStepMutationIds {
            ...
            job_outbox_id: fresh(ResourceKind::OutboxEvent),
            activations: vec![ControllerActivationSlot {
                node_execution_id: id("nod_0198f1c3-9a00-7c3e-b1f3-773c28366f17"),
    ...
    let mut tx = repository.begin_scheduler_transaction().await.unwrap();
    assert!(
        tx.apply_orchestration_controller_step(command.clone())
            .await
            .is_err(),
        "ordinary observation cannot redispatch a completed leaf"
    );
    tx.rollback().await.unwrap();
    command.observation = ControllerObservation::ExternalLeafCompleted;
    let mut tx = repository.begin_scheduler_transaction().await.unwrap();
    ...
    {
        CommandOutcome::Applied(record) => record,
        _ => panic!("completion applies once"),
    };
    tx.commit().await.unwrap();
    assert_eq!(applied.activations.len(), 1);
    assert_eq!(
        applied.activations[0].plan_node_key.as_str(),
        "child_call_2"
    );
