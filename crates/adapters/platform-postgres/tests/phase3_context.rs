...
use insight_platform_artifacts::{
    ...
    StageWorkloadArtifact, StageWorkloadArtifactRequest, WorkloadArtifactStagePreflight,
};
...
use insight_platform_orchestrator::store::DeferOrchestrationContextMutationIds;
...
use insight_platform_orchestrator::{
    ExactRunValueRef, OrchestrationJobPayload, RunCurrentSnapshot, ScopeDataEnvironmentSnapshot,
...
use insight_platform_postgres::{
    ...
    repository::{
        ArtifactWorkerRole, ClaimArtifactJobs, ClaimJobs, NewPrincipal, NewQuotaAccount, NewTenant,
...
async fn scan_context_dataset_artifacts_with_dispositions(
    ...
    let scanner_contract: String = sqlx::query_scalar("SELECT payload #>> '{scan,scanner_contract_digest}' FROM insight_platform.jobs WHERE tenant_id=$1 AND job_id=$2")
        .bind(tenant_id.to_string()).bind(requests[0].verification_job_id.to_string()).fetch_one(pool).await.unwrap();
    let worker_id = id(ResourceKind::WorkerProcessGeneration, suffix);
    let command = ClaimArtifactJobs {
        worker_manifest: support::manifest(
            "artifact-data-worker",
            WorkClass::Artifact,
            insight_platform_artifacts::execution::data_worker_execution_capabilities(
                &scanner_contract.parse().unwrap(),
            ),
        ),
    ...
    // Allow enrollment plus one fairness credit round per expected scan.
    let poll_budget =
        usize::from(insight_platform_contracts::SCHEDULER_PARTITION_COUNT) * (requests.len() + 4);
    for _ in 0..poll_budget {
        let mut round = command.clone();
        round.limit = u16::try_from(requests.len() - claims.len()).unwrap();
        round.lease_token_digests = command.lease_token_digests[claims.len()..].to_vec();
        claims.extend(repository.claim_artifact_jobs(round).await.unwrap());
        if claims.len() == requests.len() {
            break;
        }
...
/// The fixture's public observation shape is frozen alongside the Plan, including
/// the row content and citation structure exercised below.
fn observation_schema() -> ClosedJsonSchema {
    ...
    fn text() -> serde_json::Value {
        json!({"type":"string","minLength":0,"maxLength":4096,"x-platform-max-bytes":16384})
    }
    fn integer() -> serde_json::Value {
        json!({"type":"integer","minimum":0,"maximum":1000000000})
    }
    fn nominal(name: &str) -> serde_json::Value {
        json!({"$ref":insight_platform_contracts::nominal::pinned_nominal_reference(name).unwrap()})
    }
    fn nullable(schema: serde_json::Value) -> serde_json::Value {
        json!({"oneOf":[schema,{"type":"null"}]})
    }
    let digest = nominal("Digest");
    let resource_id = nominal("UuidV7Id");
    let dataset = json!({"oneOf":[
        object(json!({"kind":{"const":"external_observation"},"source_identity_digest":digest})),
        object(json!({"kind":{"const":"generation"},"exact":object(json!({"dataset_id":resource_id,"generation_id":resource_id,"generation_digest":digest}))}))
    ]});
    let citation = object(
        json!({"context_deployment":object(json!({"deployment_id":resource_id,"resource_kind":text(),"deployment_digest":digest})),"interface_revision":object(json!({"revision_id":resource_id,"resource_kind":text(),"semantic_digest":digest})),"dataset_view":dataset,"locator":object(json!({"kind":{"const":"remote_opaque"},"locator_digest":digest})),"strength":text(),"content_digest":digest,"observed_at":nominal("UtcTimestamp"),"display_label":text()}),
    );
    let item = object(
        json!({"item_id":resource_id,"source_item_identity_digest":digest,"content":object(json!({"kind":{"const":"inline"},"value":text()})),"structured_fields":object(json!({"schema_digest":digest,"canonical_digest":digest,"value":object(json!({"customer_id":integer()}))})),"score":nullable(object(json!({"millionths":integer(),"score_domain_digest":digest}))),"classification":text(),"citation":citation,"authorization_evidence_digest":digest}),
    );
    let evidence = object(
        json!({"backend_request_digest":digest,"backend_response_digest":digest,"authorization_evidence_digest":digest,"ranking_evidence_digest":digest,"candidate_count":integer(),"rejected_count":integer(),"truncated":{"type":"boolean"}}),
    );
    let mut schema = object(
        json!({"schema_version":integer(),"observation_id":resource_id,"context_query_id":resource_id,"dataset_view":dataset,"normalized_query_digest":digest,"items":{"type":"array","minItems":0,"maxItems":100,"items":item},"next_cursor_digest":nullable(digest),"evidence":evidence,"observed_at":nominal("UtcTimestamp"),"total_bytes":integer()}),
    );
    schema["$schema"] = json!("https://json-schema.org/draft/2020-12/schema");
    ClosedJsonSchema::build(schema).unwrap()
}
...
struct Fixture {
    ...
    context_worker_manifest: WorkerManifest,
...
fn context_runtime_plan(interface_contract_digest: Sha256Digest) -> RuntimePlan {
    ...
    let result = ExactDataPortRef::NodeOutput {
        ...
        schema_digest: observation_schema().canonical_digest,
    };
    let plan = RuntimePlan {
        schema_documents: [agent_schema(), observation_schema()]
            .into_iter()
...
async fn seed_fixture_with_backend(
    ...
    let interface = ContextInterfaceResourceSpec {
        ...
        item_schema_digest,
        observation_schema_digest: observation_schema().canonical_digest,
    ...
    let implementation = ContextImplementationResourceSpec {
        ...
        contract: ContextImplementationContract {
            backend: backend_contract.clone(),
    ...
    let remote_adapter_digest = match &backend_fixture {
        ContextFixtureBackend::RemoteSearch {
            installed_adapter_digest,
            ..
        } => Some(installed_adapter_digest.clone()),
        _ => None,
    };
    ...
    let mut context_worker_manifest = support::manifest(
        "context-worker",
        WorkClass::Context,
        insight_platform_plan::execution::program_execution_capabilities(),
    );
    context_worker_manifest
        .execution_capabilities
        .capabilities
        .push(
            insight_platform_contracts::context_adapter_execution_capability(
                &backend_binding,
                &backend_contract,
            )
            .unwrap(),
    ...
    match &backend_binding {
        ContextBackendBinding::NativeCatalog {
            ...
            context_worker_manifest.adapter_runtime_digest = installed_adapter_digest.clone();
            if std::env::var_os("PLATFORM_CONTEXT_WORKER_BIN").is_some() {
                context_worker_manifest.worker_build_digest =
                    support::binary_digest("PLATFORM_CONTEXT_WORKER_BIN");
            }
        }
        ContextBackendBinding::RemoteSearch { .. } => {
            context_worker_manifest.adapter_runtime_digest =
                remote_adapter_digest.expect("remote fixture binary identity");
            if std::env::var_os("PLATFORM_REMOTE_CONTEXT_WORKER_BIN").is_some() {
                context_worker_manifest.worker_build_digest =
                    support::binary_digest("PLATFORM_REMOTE_CONTEXT_WORKER_BIN");
            }
        }
        _ => {}
    }
    let context_worker_manifest_digest = context_worker_manifest.canonical_digest().unwrap();
...
async fn park_direct_context_leaf(
    ...
    sqlx::query(
        ...
        INSERT INTO insight_platform.jobs (
            ...
            trace_id, scheduler_partition_id, execution_requirement_version, execution_requirement, execution_requirement_digest, attempt_build_digest
        ) VALUES (
            $1, $2, 'orchestration_node', 'orchestration', 'node_execution', $3, $4, $3,
...
async fn claim_with_manifest(
    ...
    let mut claimed = repository
        .claim_context_jobs_current_rounds(ClaimContextJobs {
            worker_process_generation_id: worker_id.clone(),
            worker_manifest_digest,
            worker_manifest: fixture.context_worker_manifest.clone(),
    ...
    assert_eq!(claimed[0].job.job_id, job_id.to_string());
    Ok(ClaimEvidence {
...
fn context_dataset_build_is_fenced_recoverable_and_publicly_discoverable() {
    ...
    Box::pin(recovery_scanner_isolation::verify(&repository, &pool, &original_job)).await;
    ...
    let wrong_source_claim = repository
        .claim_dataset_sources_current_rounds(
            ClaimJobs {
                worker_manifest: support::manifest("context-dataset-worker", WorkClass::Context, insight_platform_contracts::WorkerExecutionCapabilities { schema_version: 1, capabilities: vec![insight_platform_context::execution::dataset_execution_capability(&original_payload.source_binding.adapter_contract_digest)] }),
    ...
    let resumed_claim = repository
        .claim_dataset_jobs_current_rounds(ClaimJobs {
                worker_manifest: support::manifest("context-dataset-worker", WorkClass::Context, insight_platform_contracts::WorkerExecutionCapabilities { schema_version: 1, capabilities: vec![insight_platform_context::execution::dataset_execution_capability(&original_payload.source_binding.adapter_contract_digest)] }),
            work_class: WorkClass::Context.to_string(),
...
fn context_query_is_atomic_quota_accounted_deferred_and_tenant_scoped() {
    ...
    let replay_prepare = PrepareContextDispatch {
    ...
    let terminal_atomic: (String, String, String, String, String, i64) = sqlx::query_as(
        r#"
        SELECT run.state, leaf.state, job.state, leaf.plan_node_key,
               job.payload->'external_leaf_completion'->'owner'->>'kind',
               (SELECT count(*) FROM insight_platform.run_values
                WHERE tenant_id = $1 AND value_id = $6
                  AND schema_digest = $7 AND node_id = $3)
        ...
        JOIN insight_platform.jobs AS job
          ON job.tenant_id = run.tenant_id AND job.job_id = $4 AND job.node_id = leaf.node_id
        WHERE run.tenant_id = $1 AND run.run_id = $2 AND leaf.scope_id = $5
        "#,
    )
    .bind(fixture.tenant_id.to_string())
    .bind(fixture.run_id.to_string())
    ...
    .bind(observation_schema().canonical_digest.to_string())
    .fetch_one(&pool)
    .await
    .unwrap();
    assert_eq!(
    ...
    support::revoke_fixture_principal(&pool, &repository, &fixture.tenant_id, &fixture.principal_id, PrincipalKind::AgentRunner).await;
    let before = support::fixture_durable_counts(&pool, &fixture.tenant_id).await;
    assert!(matches!(execute_create(&repository, command).await, Err(RepositoryError::PermissionDenied)));
    assert!(matches!(execute_prepare(&repository, replay_prepare).await, Err(RepositoryError::PermissionDenied)));
    assert_eq!(support::fixture_durable_counts(&pool, &fixture.tenant_id).await, before);
        }))
        .unwrap();
}
...
fn production_native_context_worker_recovers_commit_window_process_loss() {
    let (Ok(database_url), Ok(binary), Ok(orchestration_binary)) = (
        ...
        panic!("Context Worker binary or dedicated PostgreSQL fixture is unset; process recovery requires its declared fixture environment");
    };
    let _fixture_namespace = select_fixture_namespace(0xa0f7);
    let runtime = tokio::runtime::Builder::new_multi_thread()
        .worker_threads(2)
    ...
    runtime.block_on(async move {
        ...
        wrong_config["worker_manifest"] = serde_json::to_value(native_context_worker_manifest(
            serde_json::from_value(wrong_config["native_catalog"]["installed_adapter_digest"].clone()).unwrap(),
            serde_json::from_value(config["native_catalog"]["adapter_contract_digest"].clone()).unwrap(),
        )).unwrap();
        ...
        context_process_completion::complete_with_orchestration(
            &pool, &fixture, &database_url, &orchestration_binary, &prefix,
            &context_process_mtls_fixture(), &created.context_query_id, &job_id,
        ).await;
        second.kill().unwrap();
        second.wait().unwrap();
...
fn production_remote_context_worker_recovers_mtls_response_commit_window() {
    let (Ok(database_url), Ok(binary), Ok(orchestration_binary)) = (
        ...
        panic!(
            "Remote Context/Orchestration Worker binary or dedicated PostgreSQL fixture is unset; process recovery requires its declared fixture environment"
        );
    };
    ...
    runtime.block_on(async move {
        ...
        let mut wrong_config = config.clone();
        wrong_config["protocol_contract_digest"] =
            serde_json::to_value(named_digest("unsupported-remote-context-protocol")).unwrap();
        ...
        std::io::BufReader::new(wrong.stderr.take().unwrap()).read_line(&mut startup).unwrap();
        assert!(startup.contains("configuration is invalid"), "{startup}");
        assert!(!wrong.wait().unwrap().success());
        let unclaimed: (String, i32, bool, bool) = sqlx::query_as(
            "SELECT state, attempt_no, worker_id IS NULL, quota_reservation_id IS NULL FROM insight_platform.jobs WHERE tenant_id = $1 AND job_id = $2",
        ).bind(fixture.tenant_id.to_string()).bind(job_id.to_string()).fetch_one(&pool).await.unwrap();
        assert_eq!(unclaimed, ("ready".to_owned(), 0, true, true));
        ...
        context_process_completion::complete_with_orchestration(
            &pool, &fixture, &database_url, &orchestration_binary, &prefix,
            &tls, &created.context_query_id, &job_id,
        ).await;
        assert_eq!(remote.calls.load(Ordering::SeqCst), 2, "structural completion does not dispatch Context again");
        second.kill().unwrap();
        second.wait().unwrap();
        second_log.join().unwrap();
...
fn native_context_process_config(
    ...
    let manifest = native_context_worker_manifest(
        installed_adapter_digest.clone(),
        adapter_contract_digest.clone(),
    );
    json!({
...
fn remote_context_process_config(
    ...
    manifest.adapter_runtime_digest = installed_adapter_digest.clone();
    manifest.worker_build_digest = support::binary_digest("PLATFORM_REMOTE_CONTEXT_WORKER_BIN");
...
fn native_context_worker_manifest(
    installed_adapter_digest: Sha256Digest,
    adapter_contract_digest: Sha256Digest,
) -> WorkerManifest {
    ...
    WorkerManifest {
        worker_build_digest: support::binary_digest("PLATFORM_CONTEXT_WORKER_BIN"),
        execution_capabilities,
        manifest_version: WORKER_MANIFEST_VERSION,
...
#[test]
fn run_control_fences_context_reads_and_settles_reservations_once() {
    let _namespace = select_fixture_namespace(0xb0f7);
    let runtime = tokio::runtime::Builder::new_multi_thread()
        .worker_threads(2)
        .thread_stack_size(16 * 1024 * 1024)
        .enable_all()
    ...
    runtime.block_on(runtime.spawn(async {
        let url=std::env::var("PLATFORM_TEST_DATABASE_URL").expect("real Context control PostgreSQL fixture required");
        let pool=PgPoolOptions::new().max_connections(16).connect(&url).await.unwrap();
        verify_schema(&pool).await.unwrap();
        let repository=PgRepository::new(pool.clone());
        for running in [false,true] {
            CONTEXT_FIXTURE_NAMESPACE.store(if running {0xb1f7}else{0xb0f7},Ordering::SeqCst);
            let fixture=seed_fixture_with_backend(&pool,&repository,ContextFixtureBackend::SqlCatalog).await;
            let created=match execute_create(&repository,create_command(&fixture,0x100)).await.unwrap(){CommandOutcome::Applied(x)|CommandOutcome::Replayed(x)=>x};
            let job_id=id(ResourceKind::Job,0x120);
            execute_prepare(&repository,PrepareContextDispatch{audit:audit(&fixture.tenant_id,&fixture.principal_id,0x121,"control-prepare"),
                context_query_id:created.context_query_id.clone(),expected_query_version:created.version,job_id:job_id.clone(),scheduled_at:created.created_at}).await.unwrap();
            park_direct_context_leaf(&pool,&fixture,&created.context_query_id,&job_id,0).await;
            let claim=if running {Some(claim(&repository,&fixture,job_id.clone(),0x130).await)}else{None};
            let before:(i64,i32)=sqlx::query_as("SELECT version,active_work_count FROM insight_platform.runs WHERE tenant_id=$1 AND run_id=$2")
                .bind(fixture.tenant_id.to_string()).bind(fixture.run_id.to_string()).fetch_one(&pool).await.unwrap();
            assert_eq!(before.1,i32::from(running));
            let mut tx=repository.begin_run_transaction().await.unwrap();
            tx.request_run_cancel(insight_platform_orchestrator::RequestRunCancel{audit:audit(&fixture.tenant_id,&fixture.principal_id,0x170,"control-run-cancel"),run_id:fixture.run_id.clone(),
                expected_run_version:before.0,expected_cancel_generation:0,reason_code:"cancel_read".into()}).await.unwrap();
            tx.commit().await.unwrap();
            context_convergence_isolation::advance_until_domain(&repository,&created.context_query_id).await;
            let actual:(String,i32,bool)=sqlx::query_as("SELECT state,attempt_no,lease_expires_at IS NULL FROM insight_platform.jobs WHERE tenant_id=$1 AND job_id=$2")
                .bind(fixture.tenant_id.to_string()).bind(job_id.to_string()).fetch_one(&pool).await.unwrap();
            assert_eq!(actual,("cancelled".into(),i32::from(running),true));
            let permits:i32=sqlx::query_scalar("SELECT active_work_count FROM insight_platform.runs WHERE tenant_id=$1 AND run_id=$2")
                .bind(fixture.tenant_id.to_string()).bind(fixture.run_id.to_string()).fetch_one(&pool).await.unwrap();
            assert_eq!(permits,0);
            let accounts:Vec<(String,i64,i64)>=sqlx::query_as("SELECT metric,reserved_value,used_value FROM insight_platform.quota_accounts WHERE tenant_id=$1 AND work_class='context' ORDER BY metric")
                .bind(fixture.tenant_id.to_string()).fetch_all(&pool).await.unwrap();
            assert!(accounts.iter().all(|(_,reserved,_)|*reserved==0));
            assert_eq!(accounts.iter().find(|(metric,_,_)|metric==QuotaDimension::ContextQueries.as_str()).unwrap().2,i64::from(running));
            assert_eq!(accounts.iter().find(|(metric,_,_)|metric==QuotaDimension::ContextResultBytes.as_str()).unwrap().2,
                if running {i64::try_from(created.payload.admission.quota_ceiling.result_bytes).unwrap()}else{0});
            if let Some(claim)=claim {
                let before:i64=sqlx::query_scalar("SELECT count(*) FROM insight_platform.receipts WHERE tenant_id=$1").bind(fixture.tenant_id.to_string()).fetch_one(&pool).await.unwrap();
                let late=execute_outcome(&repository,CommitContextOutcome{audit:worker_audit(&fixture.tenant_id,&claim.worker_id,0x181,"late-cancelled-read"),
                    context_query_id:created.context_query_id.clone(),expected_query_version:claim.claimed.query.version,job_id:job_id.clone(),fence:claim.fence(),
                    outcome:ContextBackendOutcome::Completed(Box::new(output(&fixture,&claim.claimed.query,Utc::now(),id(ResourceKind::RunValue,0x180)))),
                    quota_entry_ids:[id(ResourceKind::QuotaLedgerEntry,0x184),id(ResourceKind::QuotaLedgerEntry,0x185),id(ResourceKind::QuotaLedgerEntry,0x186)],
                    resume_mutations:Some(resume_mutations(0x190)),failure_mutations:None}).await;
                assert!(late.is_err(),"late read must not commit against closed fence");
                let after:i64=sqlx::query_scalar("SELECT count(*) FROM insight_platform.receipts WHERE tenant_id=$1").bind(fixture.tenant_id.to_string()).fetch_one(&pool).await.unwrap();assert_eq!(before,after);
                let outputs:i64=sqlx::query_scalar("SELECT count(*) FROM insight_platform.run_values WHERE tenant_id=$1 AND value_id=$2")
                    .bind(fixture.tenant_id.to_string()).bind(id(ResourceKind::RunValue,0x180).to_string()).fetch_one(&pool).await.unwrap();assert_eq!(outputs,0);
            }
            context_convergence_isolation::finish_run(&pool,&repository,&fixture).await;
            let after:Vec<(String,i64,i64)>=sqlx::query_as("SELECT metric,reserved_value,used_value FROM insight_platform.quota_accounts WHERE tenant_id=$1 AND work_class='context' ORDER BY metric")
                .bind(fixture.tenant_id.to_string()).fetch_all(&pool).await.unwrap();assert_eq!(accounts,after);
            let state:String=sqlx::query_scalar("SELECT state FROM insight_platform.runs WHERE tenant_id=$1 AND run_id=$2")
                .bind(fixture.tenant_id.to_string()).bind(fixture.run_id.to_string()).fetch_one(&pool).await.unwrap();assert_eq!(state,"cancelled");
        }
        Box::pin(context_convergence_isolation::verify(&pool,&repository)).await;
    })).unwrap();
}
