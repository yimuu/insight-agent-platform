...
use support::FixtureOrchestrationClaimRounds;
...
use insight_platform_orchestrator::store::DeferOrchestrationModelMutationIds;
...
use insight_platform_orchestrator::store::ModelToolCapabilityMutationIds;
...
use insight_platform_orchestrator::{
    derive_candidate_selection, AdmitRun, ExactRunValueRef, OrchestrationJobPayload,
    RequestRunCancel, RunCurrentSnapshot, RunInputValue, ScopeDataEnvironmentSnapshot,
...
use process_guard::ChildGuard as Child;
use rcgen::{
...
impl ProductionArtifactFixture {
    ...
    fn process_config(
        ...
        manifest.worker_build_digest = support::binary_digest("PLATFORM_ARTIFACT_DATA_WORKER_BIN");
        json!({
            "schema_version": 1,
...
fn production_model_worker_manifest() -> WorkerManifest {
    ...
    WorkerManifest {
        worker_build_digest: support::test_actor_build("PLATFORM_MODEL_WORKER_BIN"),
...
fn production_capability_worker_manifest() -> WorkerManifest {
    ...
    WorkerManifest {
        worker_build_digest: support::test_actor_build("PLATFORM_CAPABILITY_NATIVE_WORKER_BIN"),
        execution_capabilities,
        manifest_version: WORKER_MANIFEST_VERSION,
...
fn model_orchestration_process_config(artifact_endpoint: String) -> serde_json::Value {
    json!({
        ...
        "worker_manifest": WorkerManifest {
            worker_build_digest: support::binary_digest("PLATFORM_ORCHESTRATION_WORKER_BIN"),
...
fn model_runtime_plan(
    ...
    let plan = RuntimePlan {
        schema_documents: [run_input_schema(), output_schema.clone()]
            .into_iter()
...
fn return_run_input_runtime_plan(interface_contract_digest: Sha256Digest) -> RuntimePlan {
    ...
    let plan = RuntimePlan {
        schema_documents: [(
            run_input_schema().canonical_digest,
            run_input_schema().try_into().unwrap(),
        )]
        .into_iter()
        .collect(),
        plan_version: 6,
...
async fn seed_fixture(pool: &PgPool, repository: &PgRepository) -> Fixture {
    ...
    let agent_document = ResourceDocument::Agent(AgentResourceSpec {
        ...
        dependency_versions: vec![],
        policy_versions: vec![agent_policy.clone()],
        author_instructions: Some("Answer using only the current user input.".to_owned()),
        input_schema: run_input_schema(),
...
async fn seed_running_model_orchestration(
    ...
    let input_value_id: ResourceId = sqlx::query_scalar::<_, String>(
        "SELECT input_value_id FROM insight_platform.runs WHERE tenant_id=$1 AND run_id=$2",
    )
    .bind(fixture.tenant_id.to_string())
    ...
    let command = DeferOrchestrationToModelTurn {
        ...
        input: ResolvedExpressionInput {
            run_value_id: input_value_id.clone(),
...
fn model_run_convergence_slot(base: u16) -> OrchestrationConvergenceSlot {
    OrchestrationConvergenceSlot {
        quota_entry_ids: (base..base + 4)
            .map(|n| id(ResourceKind::QuotaLedgerEntry, n))
            .collect(),
        run_event_id: id(ResourceKind::Event, base + 4),
        run_outbox_id: id(ResourceKind::OutboxEvent, base + 5),
        node_event_id: id(ResourceKind::Event, base + 6),
        node_outbox_id: id(ResourceKind::OutboxEvent, base + 7),
        node_cancelling_event_id: id(ResourceKind::Event, base + 8),
        node_cancelling_outbox_id: id(ResourceKind::OutboxEvent, base + 9),
        scope_closing_event_id: id(ResourceKind::Event, base + 10),
        scope_closing_outbox_id: id(ResourceKind::OutboxEvent, base + 11),
        scope_terminal_event_id: id(ResourceKind::Event, base + 12),
        scope_terminal_outbox_id: id(ResourceKind::OutboxEvent, base + 13),
        job_event_id: id(ResourceKind::Event, base + 14),
        job_outbox_id: id(ResourceKind::OutboxEvent, base + 15),
    }
}

async fn assert_model_run_cancellation_is_owned_and_accounted(
    pool: &PgPool,
    repository: &PgRepository,
    fixture: &Fixture,
) {
    // Admit independent Plan leaves before acquiring their physical permits.
    let mut deferred = Vec::new();
    for base in [0x2000, 0x2200, 0x2400, 0x2600] {
        sqlx::query(
        ...
        .await
        .unwrap();
        let (_, command) = seed_running_model_orchestration(pool, repository, fixture, base).await;
        let mut tx = repository.begin_scheduler_transaction().await.unwrap();
        let CommandOutcome::Applied(record) =
            tx.defer_orchestration_to_model_turn(command).await.unwrap()
        else {
            panic!("fresh cancellation Model admission replayed")
        };
        tx.commit().await.unwrap();
        deferred.push(record);
    }
    let worker_id = id(ResourceKind::WorkerProcessGeneration, 0x2800);
    let slots = [0x2810, 0x2850, 0x2890].map(|base| ModelClaimSlot {
        lease_token_digest: named_digest(&format!("run-cancel-model-lease-{base}")),
        usage_reservation_id: id(ResourceKind::UsageReservation, base),
        quota_entry_ids: (base + 1..base + 5)
            .map(|n| id(ResourceKind::QuotaLedgerEntry, n))
            .collect(),
    ...
    assert!(claims.iter().all(|claim| deferred
        .iter()
        .any(|leaf| leaf.model_job.job_id == claim.job.job_id)));
    let unclaimed = deferred
        .iter()
        .find(|leaf| {
            claims
                .iter()
                .all(|claim| claim.job.job_id != leaf.model_job.job_id)
        })
        .unwrap();
    let retry_claim = &claims[0];
    let ModelExecutionInputMaterial::Inline { value } = &retry_claim.request_input.material;
    let retry_request: CanonicalModelRequest = serde_json::from_value(value.clone()).unwrap();
    let retry_result = execute_outcome(
        repository,
        CommitModelOutcome {
            audit: worker_audit(&fixture.tenant_id, &worker_id, 0x2d00, 'c', 'd'),
            model_turn_id: retry_claim.turn.model_turn_id.clone(),
            job_id: retry_claim.job.job_id.parse().unwrap(),
            expected_turn_version: retry_claim.turn.version,
            fence: retry_claim.fence.clone(),
            usage_reservation_id: retry_claim.usage_reservation_id.clone(),
            quota_entry_ids: (0x2d10..0x2d14)
                .map(|n| id(ResourceKind::QuotaLedgerEntry, n))
                .collect(),
            ...
            outcome: ModelDispatchOutcome::RetryableFailure {
                ...
                retry_at: Utc::now() + Duration::milliseconds(100),
    ...
    .unwrap();
    assert!(
        matches!(retry_result, CommandOutcome::Applied(record) if record.turn.state == ModelTurnState::RetryScheduled && record.job.quota_reservation_id.is_none())
    );
    let retry_permits: i32 = sqlx::query_scalar(
    ...
    assert_eq!(
        retry_permits, 2,
        "a retryable outcome releases its physical attempt permit"
    );
    // Isolate the retried attempt from the intentionally undispatched fourth leaf.
    sqlx::query("UPDATE insight_platform.jobs SET scheduled_at=clock_timestamp()+interval '1 minute' WHERE tenant_id=$1 AND job_id=$2")
        .bind(fixture.tenant_id.to_string()).bind(&unclaimed.model_job.job_id).execute(pool).await.unwrap();
    tokio::time::sleep(StdDuration::from_millis(150)).await;
    let mut retried_claims = repository
        .claim_model_jobs_current_rounds(ClaimModelJobs {
            worker_manifest: production_model_worker_manifest(),
            worker_process_generation_id: worker_id.clone(),
            worker_manifest_digest: production_model_worker_manifest_digest(),
            ...
            lease_milliseconds: 30_000,
            slots: vec![ModelClaimSlot {
                lease_token_digest: named_digest("run-cancel-model-retry-lease"),
                usage_reservation_id: id(ResourceKind::UsageReservation, 0x2e00),
                quota_entry_ids: (0x2e01..0x2e05)
                    .map(|n| id(ResourceKind::QuotaLedgerEntry, n))
                    .collect(),
                event_id: id(ResourceKind::Event, 0x2e05),
                outbox_id: id(ResourceKind::OutboxEvent, 0x2e06),
                resume_mutations: None,
                failure_mutations: None,
                tool_continuation_mutations: None,
            }],
    ...
    let retried_claim = retried_claims.pop().unwrap();
    assert_eq!(retried_claim.job.job_id, retry_claim.job.job_id);
    assert_eq!(retried_claim.job.attempt_no, 2);
    claims[0] = retried_claim;
    sqlx::query(
        "UPDATE insight_platform.jobs SET scheduled_at=$3 WHERE tenant_id=$1 AND job_id=$2",
    )
    .bind(fixture.tenant_id.to_string())
    .bind(&unclaimed.model_job.job_id)
    .bind(unclaimed.model_job.scheduled_at)
    .execute(pool)
    .await
    .unwrap();
    let (run_version, cancel_generation, permits): (i64, i64, i32) = sqlx::query_as(
        "SELECT version,cancel_generation,active_work_count FROM insight_platform.runs WHERE tenant_id=$1 AND run_id=$2"
    ...
    tx.request_run_cancel(RequestRunCancel {
        audit: audit(&fixture.tenant_id, &fixture.principal_id, 0x2900, '8', '9'),
        run_id: fixture.run_id.clone(),
        expected_run_version: run_version,
        expected_cancel_generation: u64::try_from(cancel_generation).unwrap(),
        reason_code: "fixture_model_cancellation".to_owned(),
    })
    .await
    .unwrap();
    tx.commit().await.unwrap();

    let tool_claim = &claims[2];
    let tool_node = tool_claim.turn.node_execution_id.to_string();
    let original_wait: String = sqlx::query_scalar(
        "SELECT payload_digest FROM insight_platform.run_nodes WHERE tenant_id=$1 AND node_id=$2",
    )
    .bind(fixture.tenant_id.to_string())
    .bind(&tool_node)
    .fetch_one(pool)
    .await
    .unwrap();
    let ModelExecutionInputMaterial::Inline { value } = &tool_claim.request_input.material;
    let tool_request: CanonicalModelRequest = serde_json::from_value(value.clone()).unwrap();
    let tool_output_id = id(ResourceKind::RunValue, 0x2c20);
    let suppressed_tool_job = id(ResourceKind::Job, 0x2b00);
    let tool_result = execute_outcome(
        repository,
        CommitModelOutcome {
            audit: worker_audit(&fixture.tenant_id, &worker_id, 0x2c00, 'e', 'f'),
            ...
            fence: tool_claim.fence.clone(),
            usage_reservation_id: tool_claim.usage_reservation_id.clone(),
            quota_entry_ids: (0x2c10..0x2c14)
                .map(|n| id(ResourceKind::QuotaLedgerEntry, n))
                .collect(),
            request: tool_request,
            outcome: ModelDispatchOutcome::Succeeded(Box::new(output(
                fixture,
                tool_response(fixture, json!({"query":"cancelled parent tool result"})),
                tool_output_id.clone(),
            ))),
            resume_mutations: None,
            failure_mutations: None,
            tool_continuation_mutations: Some(
                insight_platform_models::ModelToolContinuationMutationIds {
                    continuation_job_id: suppressed_tool_job.clone(),
                    run_event_id: id(ResourceKind::Event, 0x2b01),
                    run_outbox_id: id(ResourceKind::OutboxEvent, 0x2b02),
                    node_event_id: id(ResourceKind::Event, 0x2b03),
                    node_outbox_id: id(ResourceKind::OutboxEvent, 0x2b04),
                    continuation_job_event_id: id(ResourceKind::Event, 0x2b05),
                    continuation_job_outbox_id: id(ResourceKind::OutboxEvent, 0x2b06),
                },
            ),
        },
    )
    .await
    .unwrap();
    assert!(matches!(tool_result, CommandOutcome::Applied(record)
        if record.turn.state == ModelTurnState::Succeeded && record.job.state == "succeeded"
            && record.turn.payload.result.as_ref().unwrap().tool_intent_count == 1));
    let suppressed: (String, i32, String, String, i64, i64) = sqlx::query_as(
        "SELECT run.state,run.active_work_count,node.state,node.payload_digest,(SELECT count(*) FROM insight_platform.jobs WHERE tenant_id=$1 AND job_id=$4),(SELECT count(*) FROM insight_platform.run_values WHERE tenant_id=$1 AND value_id=$5) FROM insight_platform.runs run JOIN insight_platform.run_nodes node ON node.tenant_id=run.tenant_id AND node.run_id=run.run_id WHERE run.tenant_id=$1 AND run.run_id=$2 AND node.node_id=$3"
    ).bind(fixture.tenant_id.to_string()).bind(fixture.run_id.to_string()).bind(&tool_node)
        .bind(suppressed_tool_job.to_string()).bind(tool_output_id.to_string()).fetch_one(pool).await.unwrap();
    assert_eq!(
        suppressed,
        (
            "cancelling".to_owned(),
    ...
    for step in 0..64 {
        repository
            .drive_orchestration_convergence(DriveOrchestrationConvergence {
                ...
                after: None,
                limit: 1,
                slots: vec![model_run_convergence_slot(0x3000 + step * 16)],
            })
            .await
            .unwrap();
        let states: Vec<(String, String)> = sqlx::query_as(
            "SELECT invocation_id,state FROM insight_platform.invocations WHERE tenant_id=$1 AND invocation_id=ANY($2)"
        ).bind(fixture.tenant_id.to_string())
            .bind(deferred.iter().map(|leaf| leaf.turn.model_turn_id.to_string()).collect::<Vec<_>>())
            .fetch_all(pool).await.unwrap();
        if states.iter().all(|(turn_id, state)| {
            if turn_id == &unclaimed.turn.model_turn_id.to_string() {
                state == "cancelled"
            } else if turn_id == &tool_claim.turn.model_turn_id.to_string() {
                state == "succeeded"
            } else {
                state == "cancelling"
            }
    ...
    assert!(
        converged,
        "Run control must reach Ready and InFlight Model owners"
    );
    let ready_job: (String, i32, Option<String>) = sqlx::query_as(
        "SELECT state,attempt_no,quota_reservation_id FROM insight_platform.jobs WHERE tenant_id=$1 AND job_id=$2"
    ).bind(fixture.tenant_id.to_string()).bind(&unclaimed.model_job.job_id).fetch_one(pool).await.unwrap();
    assert_eq!(ready_job, ("cancelled".to_owned(), 0, None));
    let active: (String, i32) = sqlx::query_as(
        "SELECT state,active_work_count FROM insight_platform.runs WHERE tenant_id=$1 AND run_id=$2"
    ).bind(fixture.tenant_id.to_string()).bind(fixture.run_id.to_string()).fetch_one(pool).await.unwrap();
    assert_eq!(active, ("cancelling".to_owned(), 2));
    let cancellations = repository
        .scan_cancelling_model_executions(&worker_id, None, 16)
        .await
        .unwrap();
    assert_eq!(cancellations.len(), 2);
    let callback = cancellations
        .iter()
        .find(|record| record.turn.model_turn_id == claims[0].turn.model_turn_id)
        .unwrap();
    let callback_job = callback.job.as_ref().unwrap();
    let callback_projection = callback.job_projection().unwrap().unwrap();
    let callback_lease = callback_projection.lease.as_ref().unwrap();
    let callback_command = CommitModelCancellationOutcome {
        audit: worker_audit(&fixture.tenant_id, &worker_id, 0x4000, 'a', 'b'),
        model_turn_id: callback.turn.model_turn_id.clone(),
        job_id: callback_projection.job_id.clone(),
        expected_turn_version: callback.turn.version,
        fence: Some(JobFence {
            expected_version: callback_projection.version,
            worker_process_generation_id: worker_id.clone(),
            lease_generation: callback_projection.lease_generation,
            token_digest: callback_lease.token_digest.clone(),
        }),
        usage_reservation_id: callback_job
            .quota_reservation_id
            .as_ref()
            .unwrap()
            .parse()
            .unwrap(),
        quota_entry_ids: (0x4010..0x4014)
            .map(|n| id(ResourceKind::QuotaLedgerEntry, n))
            .collect(),
        measurement: measurement(5, 0, 5),
    };
    let mut unfenced = callback_command.clone();
    unfenced.fence = None;
    assert!(matches!(
        execute_cancellation_outcome(repository, unfenced).await,
        Err(RepositoryError::StaleFence)
    ));
    assert!(
        matches!(execute_cancellation_outcome(repository, callback_command.clone()).await.unwrap(),
        CommandOutcome::Applied(record) if record.turn.state == ModelTurnState::Cancelled)
    );
    assert!(matches!(
        execute_cancellation_outcome(repository, callback_command)
            .await
            .unwrap(),
        CommandOutcome::Replayed(_)
    ));
    let permits: i32 = sqlx::query_scalar(
        "SELECT active_work_count FROM insight_platform.runs WHERE tenant_id=$1 AND run_id=$2",
    )
    .bind(fixture.tenant_id.to_string())
    .bind(fixture.run_id.to_string())
    ...
    assert_eq!(
        permits, 1,
        "callback and Receipt replay release only the completed attempt"
    );

    let abandoned = cancellations
        .iter()
        .find(|record| record.turn.model_turn_id != callback.turn.model_turn_id)
        .unwrap();
    let abandoned_job = abandoned.job.as_ref().unwrap();
    let live_version = abandoned_job.version;
    let live_attempt = abandoned_job.attempt_no;
    let abandoned_reservation = abandoned_job.quota_reservation_id.clone().unwrap();
    let recovery = |base| DriveExpiredModelJobs {
        shard: SafetyScanShard::whole(),
        after: None,
        limit: 1,
        retry_backoff_milliseconds: u64::MAX,
        slots: vec![ExpiredModelRecoverySlot {
            ...
            failure_mutations: failure_mutations(base + 16),
        }],
    };
    assert!(
        repository
            .drive_expired_model_jobs(recovery(0x4100))
            .await
            .unwrap()
            .records
            .is_empty(),
        "recovery cannot take a live cancellation fence"
    );
    sqlx::query("UPDATE insight_platform.jobs SET heartbeat_at=clock_timestamp()-interval '2 seconds', lease_expires_at=clock_timestamp()-interval '1 second' WHERE tenant_id=$1 AND job_id=$2")
        .bind(fixture.tenant_id.to_string()).bind(&abandoned_job.job_id).execute(pool).await.unwrap();
    let recovered = repository
        .drive_expired_model_jobs(recovery(0x4140))
        .await
        .unwrap();
    assert_eq!(recovered.records.len(), 1);
    let recovered = &recovered.records[0];
    assert_eq!(recovered.turn.model_turn_id, abandoned.turn.model_turn_id);
    assert_eq!(recovered.turn.state, ModelTurnState::Cancelled);
    assert_eq!(recovered.job.state, "cancelled");
    assert_eq!(recovered.job.attempt_no, live_attempt);
    assert!(recovered.job.version > live_version);
    assert!(recovered.job.lease_token_digest.is_none());
    assert!(recovered.job.quota_reservation_id.is_none());
    assert!(repository
        .drive_expired_model_jobs(recovery(0x4180))
        .await
        .unwrap()
        .records
        .is_empty());
    let permit_and_settlement: (i32, i64) = sqlx::query_as(
        "SELECT active_work_count,(SELECT count(*) FROM insight_platform.quota_ledger WHERE tenant_id=$1 AND correlation_id=$3 AND entry_kind='settle') FROM insight_platform.runs WHERE tenant_id=$1 AND run_id=$2"
    ).bind(fixture.tenant_id.to_string()).bind(fixture.run_id.to_string()).bind(&abandoned_reservation).fetch_one(pool).await.unwrap();
    assert_eq!(permit_and_settlement, (0, 4));
    assert_eq!(recovered.turn.payload.attempts.len(), 1);
    assert_eq!(
        recovered.turn.payload.attempts[0].usage.tokens,
        recovered.turn.payload.admission.quota_ceiling.tokens
    );
}
...
async fn run_model_worker_process_recovery(
    ...
    let (Ok(binary), Ok(nats_url), Ok(nats_ca), Ok(nats_cert), Ok(nats_key)) = (
        ...
        panic!("Model Worker binary or TLS NATS fixture is unset; process recovery requires its declared fixture environment");
    };
...
fn production_workers_complete_model_tool_result_return_chain() {
    let (
        ...
        panic!("Model tool-chain database, Worker binaries, TLS NATS, or S3/KMS fixture is unset; process chain requires its declared fixture environment");
    };
    ...
    std::thread::Builder::new()
        ...
        .spawn(|| {
            tokio::runtime::Builder::new_current_thread()
                ...
                .build()
                .unwrap()
                .block_on(model_turn_fixture(false));
        })
        .unwrap()
        .join()
        .unwrap();
...
fn model_worker_process_recovery() {
    std::thread::Builder::new()
        .name("phase3-model-turn-fixture".to_owned())
        .stack_size(32 * 1024 * 1024)
        .spawn(|| {
            tokio::runtime::Builder::new_current_thread()
                ...
                .build()
                .unwrap()
                .block_on(model_turn_fixture(true));
        })
        .unwrap()
        .join()
        .unwrap();
...
async fn model_turn_fixture(exercise_processes: bool) {
    let Ok(database_url) = model_test_database_url() else {
        panic!(
            "PLATFORM_MODEL_TEST_DATABASE_URL and PLATFORM_TEST_DATABASE_URL are unset; real PostgreSQL fixture requires its declared fixture environment"
        );
    };
    let pool = PgPoolOptions::new()
        .max_connections(16)
    ...
    let replay_create = primary.clone();
    let admitted_primary = match execute_create(&repository, primary.clone()).await.unwrap() {
        CommandOutcome::Applied(record) => record,
        CommandOutcome::Replayed(_) => panic!("fresh primary ModelTurn replayed"),
    };
    let replay_prepare = PrepareModelDispatch {
        audit: audit(&fixture.tenant_id, &fixture.principal_id, 0x121, '9', 'a'),
        model_turn_id: primary.model_turn_id.clone(),
        expected_turn_version: admitted_primary.version,
        job_id: id(ResourceKind::Job, 0x120),
        scheduled_at: Utc::now() - Duration::seconds(1),
    };
    assert!(matches!(
        execute_prepare(&repository, replay_prepare.clone())
            .await
            .unwrap(),
        CommandOutcome::Applied(_)
    ));
    ...
    match (completion_result, control_result) {
        ...
        (Err(_), Ok(CommandOutcome::Applied(controlled))) => {
            ...
            let discovered = repository
                .scan_cancelling_model_executions(&cancellation_claim.worker_id, None, 16)
                .await
                .unwrap();
    ...
    let owner_states: (String, String, i32) = sqlx::query_as(
        ...
        FROM insight_platform.run_nodes AS leaf
        JOIN insight_platform.jobs AS continuation
          ON continuation.tenant_id = leaf.tenant_id AND continuation.node_id = leaf.node_id AND continuation.job_id = $3
        JOIN insight_platform.runs AS run
          ON run.tenant_id = leaf.tenant_id AND run.run_id = leaf.run_id
        WHERE leaf.tenant_id = $1 AND leaf.node_id = $2
        "#,
    ...
    .await
    .unwrap();
    assert_eq!(owner_states, ("ready".to_owned(), "ready".to_owned(), 0));
    ...
    if !exercise_processes {
        let replay_control = ControlModelTurn {
            audit: audit(&fixture.tenant_id, &fixture.principal_id, 0xc00, 'e', 'f'),
            model_turn_id: continued.turn.model_turn_id.clone(),
            expected_turn_version: continued.turn.version,
            quota_entry_ids: vec![],
            kind: ModelControlKind::Cancel,
        };
        assert!(matches!(
