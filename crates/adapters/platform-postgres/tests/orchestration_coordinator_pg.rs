#[path = "support/evaluation_execution.rs"]
mod evaluation_execution;
#[path = "support/orchestration_claim_isolation.rs"]
mod orchestration_claim_isolation;
...
use insight_platform_orchestrator::{AdmitRun, RunInputValue};
use insight_platform_plan::{
    ChildBudgetLimit, ChildCancellationPolicy, DataPortKey, ExactDataPortRef, ExpressionLimits,
    HumanTaskDefinition, PlanLimits, PlanNodeKey, RuntimeDependencyKind, RuntimeDependencySlot,
    RuntimeNode, RuntimePlan,
};
use insight_platform_postgres::connections::{
    PostgresConnectionBulkheadConfig, PostgresConnectionBulkheads,
};
use insight_platform_postgres::plan_generation_store::PostgresDurablePlanGenerationStore;
use insight_platform_postgres::{
    repository::{NewPrincipal, NewQuotaAccount, NewTenant, NewTenantPrincipal, PgRepository},
    verify_schema,
};
...
fn fixture_plan() -> RuntimePlan {
    ...
    RuntimePlan {
        schema_documents: BTreeMap::from([(
            agent_schema().canonical_digest.clone(),
...
impl StartedOrchestrationJobHandler for PostgresHandoffHandler {
    ...
    async fn commit(
        ...
        _fence: insight_platform_jobs::store::JobCommandFence,
        ...
        let coordinator = WorkCoordinator::new(
            ...
            OrchestrationCoordinatorConfig::from_profile(
                ...
                CoordinatorTiming {
                    ...
                    safety_scan_interval: Duration::from_millis(5),
...
fn real_postgres_coordinator_claims_with_physical_and_connection_bulkheads() {
    ...
    runtime.block_on(async {
        ...
        .unwrap()
        .spawn();
        tokio::time::timeout(Duration::from_secs(10), handler.wait_running())
            .await
        ...
        let validated_task_input = insight_platform_tasks::ValidatedTaskInput::validate_inline(&agent_schema(), &task_input).unwrap();
        ...
        validated_input: Some(validated_task_input),
                audit: CommandAudit {
                    trace: insight_platform_contracts::TraceIdentityV1::generate(),
                    tenant_id: id(TENANT_ID),
                    principal_id: id(PRINCIPAL_ID),
...
async fn admit_run(repository: &PgRepository, bindings: RunBindingsSnapshot) -> RunRecord {
    ...
    .await;
    let command = admission_command(bindings);
...
fn admission_command(bindings: RunBindingsSnapshot) -> AdmitRun {
    let input = json!({"question":"coordinator"});
    let input_bytes = canonical_json(&input).unwrap();
    let input_digest: Sha256Digest = canonical_digest(&input).unwrap().parse().unwrap();
    let input_artifact = ArtifactRef::new(
...
struct ObservedClaimStore {
    repository: PgRepository,
    observed_error: AtomicBool,
    observed_retry_error: AtomicBool,
}

#[async_trait]
impl insight_platform_runtime::OrchestrationClaimStore for ObservedClaimStore {
    type Error = insight_platform_postgres::repository::RepositoryError;

    async fn claim_orchestration_jobs(
        &self,
        command: insight_platform_orchestrator::store::ClaimOrchestrationJobs,
    ) -> Result<Vec<insight_platform_orchestrator::store::ClaimedOrchestrationJob>, Self::Error>
    {
        let result = insight_platform_runtime::OrchestrationClaimStore::claim_orchestration_jobs(
            &self.repository,
            command,
        )
        .await;
        if let Err(error) = &result {
            let retryable = matches!(error, insight_platform_postgres::repository::RepositoryError::Database(sqlx::Error::Database(db)) if matches!(db.code().as_deref(), Some("40001" | "40P01")));
            let observed = if retryable {
                &self.observed_retry_error
            } else {
                &self.observed_error
            };
            if !observed.swap(true, Ordering::Relaxed) {
                eprintln!("first actual claim failure in isolated fixture: {error:?}");
            }
        }
        result
    }
...
fn phase2_claim_worker_process_entry() {
    ...
    runtime.block_on(async {
        ...
        let running = WorkCoordinator::new(
            Arc::new(ObservedClaimStore {
                repository: bulkheads.business_repository(),
                observed_error: AtomicBool::new(false),
                observed_retry_error: AtomicBool::new(false),
            }),
            Arc::clone(&executor),
            Arc::new(UuidCoordinatorIdentityFactory),
            pools,
...
fn q1_fifty_runs_use_multiple_processes_and_preserve_database_fairness() {
    ...
    runtime.block_on(async {
        ...
        let tenant_claim_counts: Vec<i64> = sqlx::query_scalar(
            "SELECT successful_claims FROM insight_platform.scheduler_tenant_state WHERE work_class = 'orchestration' AND tenant_id = ANY($1) ORDER BY tenant_id",
        )
        .bind(tenants.iter().collect::<Vec<_>>())
        .fetch_all(bulkheads.business_pool())
        .await
        .unwrap();
        assert_eq!(tenant_claim_counts.len(), PHASE2_TENANT_COUNT);
        assert!(tenant_claim_counts.into_iter().all(|count| count == i64::try_from(PHASE2_RUNS_PER_TENANT).unwrap()));
...
async fn seed_capacity_tenant(repository: &PgRepository) -> (ResourceId, RunBindingsSnapshot) {
    ...
    .unwrap();
    let capacity_plan = child_fixture_plan();
    let capacity_plan_digest = capacity_plan
        .canonical_digest(PlanLimits::from_profile(&checked_in_hard_limit_profile()).unwrap())
        .unwrap();
    let capacity_plan_bytes =
        canonical_json(&serde_json::to_value(&capacity_plan).unwrap()).unwrap();
    let capacity_plan_artifact = ArtifactRef::new(
        fresh_id(ResourceKind::Artifact),
        capacity_plan_digest.clone(),
        capacity_plan_bytes.len() as u64,
        "application/json",
        DataClassification::Internal,
        Some("capacity-plan.json".into()),
    )
    .unwrap();
    let capacity_agent = TypedPayload::new(
        1,
        &PublishedVersionPayload {
            document: ResourceDocument::Agent(AgentResourceSpec {
                authoring_name: "capacity-agent".into(),
                required_features: vec![],
                input_classification: DataClassification::Internal,
                default_deadline_seconds: 120,
                authoring_package: AuthoringPackage {
                    artifact: capacity_plan_artifact.clone(),
                    manifest_digest: digest('4'),
                },
                contract_digest: digest('5'),
                dependency_versions: vec![],
                ...
                output_schema: agent_schema(),
                error_schema: agent_schema(),
                typed_plan_artifact_id: capacity_plan_artifact.artifact_id().clone(),
                typed_plan_digest: capacity_plan_digest.clone(),
            }),
            validation: ValidationSummary {
                program_requirement: Some(
                    insight_platform_plan::execution::program_execution_requirement(
