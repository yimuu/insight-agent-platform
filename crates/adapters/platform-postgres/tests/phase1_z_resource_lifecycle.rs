...
use insight_platform_contracts::{
    ActiveTarget, AdministrativeGate, AgentDeploymentClosure, AgentProductState, ArtifactRef,
...
use insight_platform_postgres::{
    ...
    repository::{
        ClaimJobs, CommitRegistryValidation, NewPrincipal, NewTenant, NewTenantPrincipal,
        PgRepository, RegistryValidationCommitOutcome, RepositoryError,
    },
    verify_schema,
};
...
#[path = "support/registry_publication_revisions.rs"]
mod registry_publication_revisions;

async fn claim_registry_fixture(
    repository: &PgRepository,
    command: ClaimJobs,
    validator: &ResourceId,
) -> Result<Vec<insight_platform_jobs::store::JobRecord>, RepositoryError> {
    for _ in 0..520 {
        let batch = repository
            .claim_registry_validation_jobs(command.clone(), validator)
            .await?;
        if !batch.is_empty() {
            return Ok(batch);
        }
...
#[test]
fn resource_lifecycle_is_typed_atomic_and_not_auto_activated() {
    std::thread::Builder::new()
        .name("registry-lifecycle-fixture".into())
        ...
        .spawn(|| {
            tokio::runtime::Builder::new_current_thread()
                ...
                .build()
                .unwrap()
                .block_on(resource_lifecycle_fixture());
        })
        .unwrap()
        .join()
        .unwrap();
...
async fn resource_lifecycle_fixture() {
    ...
    for tenant in [TENANT_ID, TENANT_B_ID] {
        support::bind_fixture_scheduling_policy(&pool, &repository, &id(tenant)).await;
    }
    ...
    let mut saturated_profile = insight_platform_contracts::checked_in_hard_limit_profile();
    saturated_profile
        .control_data
        .outbox_admission_backlog
        .q1_default = 1;
    let saturated =
        PgRepository::with_hard_limit_profile(pool.clone(), &saturated_profile).unwrap();
    let before_capacity=sqlx::query_as::<_,(i64,i64,i64,i64)>("SELECT (SELECT count(*) FROM insight_platform.resources WHERE tenant_id=$1),(SELECT count(*) FROM insight_platform.receipts WHERE tenant_id=$1),(SELECT count(*) FROM insight_platform.jobs WHERE tenant_id=$1),(SELECT count(*) FROM insight_platform.outbox_events WHERE tenant_id=$1)").bind(TENANT_ID).fetch_one(&pool).await.unwrap();
    assert!(matches!(
        registry_command!(saturated, create_resource_draft, create_command.clone()).unwrap(),
        CommandOutcome::Replayed(_)
    ));
    let mut rejected_audit = audit(TENANT_ID, PRINCIPAL_ID, "6ed0", '1', '2');
    rejected_audit.idempotency_key_digest = insight_platform_contracts::canonical_digest(
        &json!({"fixture":"new-draft-capacity-rejection"}),
    )
    .unwrap()
    .parse()
    .unwrap();
    let rejected_create = CreateResourceDraft {
        audit: rejected_audit,
        resource_id: sandbox_id(ResourceKind::Policy, 0x6ed0),
        draft: draft.clone(),
    };
    let rejected_result = registry_command!(saturated, create_resource_draft, rejected_create);
    assert!(
        matches!(rejected_result, Err(RepositoryError::CapacityUnavailable)),
        "actual rejection: {rejected_result:?}"
    );
    let after_capacity=sqlx::query_as::<_,(i64,i64,i64,i64)>("SELECT (SELECT count(*) FROM insight_platform.resources WHERE tenant_id=$1),(SELECT count(*) FROM insight_platform.receipts WHERE tenant_id=$1),(SELECT count(*) FROM insight_platform.jobs WHERE tenant_id=$1),(SELECT count(*) FROM insight_platform.outbox_events WHERE tenant_id=$1)").bind(TENANT_ID).fetch_one(&pool).await.unwrap();
    assert_eq!(
        before_capacity, after_capacity,
        "replay and failed admission must not append durable facts"
    );
    ...
    let before_validation_capacity=sqlx::query_as::<_,(i64,i64)>("SELECT (SELECT count(*) FROM insight_platform.receipts WHERE tenant_id=$1),(SELECT count(*) FROM insight_platform.jobs WHERE tenant_id=$1)").bind(TENANT_ID).fetch_one(&pool).await.unwrap();
    assert!(matches!(
        registry_command!(
            saturated,
    ...
    let after_validation_capacity=sqlx::query_as::<_,(i64,i64)>("SELECT (SELECT count(*) FROM insight_platform.receipts WHERE tenant_id=$1),(SELECT count(*) FROM insight_platform.jobs WHERE tenant_id=$1)").bind(TENANT_ID).fetch_one(&pool).await.unwrap();
    assert_eq!(
        before_validation_capacity, after_validation_capacity,
        "rejected validation cannot leave a Receipt or Job"
    );
    ...
    Box::pin(recovery_scanner_isolation::verify(
        &repository,
        &pool,
        &validation_job,
    ))
    ...
    let replayed_validation =
        registry_command!(saturated, request_resource_validation, replay_request).unwrap();
    ...
    assert!(matches!(
        repository
            .claim_jobs(ClaimJobs {
                worker_manifest: support::registry(),
    ...
    // Actual current compiler bytes are the only Agent publication evidence in this fixture.
    use insight_platform_agent_compiler::{
        AgentCompileResponseV1, AgentCompilerProfile, AgentSourceBundleV1, AgentSourceFilesV1,
        ArtifactAuthority, ModelLoopCompilerLimits,
    };
    let profile = AgentCompilerProfile {
        ...
        deployment_policies: vec![policy_binding.clone()],
    ...
    let bundle=AgentSourceBundleV1{schema_version:insight_platform_agent_compiler::AGENT_SOURCE_BUNDLE_VERSION,compiler_semantic_identity:insight_platform_agent_compiler::compiler_semantic_identity(),compile_policy_inputs_digest:AgentSourceBundleV1::policy_digest(&profile),profile,bindings:Default::default(),sources:AgentSourceFilesV1{manifest_path:"agent.yaml".to_owned(),files:std::collections::BTreeMap::from([
        ("agent.yaml".to_owned(),"apiVersion: insight.platform/v1\nkind: Agent\nmetadata:\n  name: deployment-agent\n  displayName: Deployment closure agent\nspec:\n  execution:\n    kind: deterministic\n  input:\n    schema: io.json\n    classification: internal\n  output:\n    schema: io.json\n".to_owned()),
        ("io.json".to_owned(),serde_json::to_string(&agent_schema().schema).unwrap()),
    ])}};
    let AgentCompileResponseV1::Compiled { compilation } =
        insight_platform_agent_compiler::compile_source_bundle(bundle)
    else {
        panic!("current compiler rejected fixture")
    };
    let authority = |intent: &insight_platform_agent_compiler::ArtifactIntent,
                     artifact_id: ResourceId| ArtifactAuthority {
    ...
    let source_authority = authority(
        &compilation.compiled.resource_intent.authoring_artifact,
        sandbox_id(ResourceKind::Artifact, 0x7df0),
    );
    let plan_authority = authority(
        &compilation.compiled.resource_intent.typed_plan_artifact,
        id(TYPED_PLAN_ARTIFACT_ID),
    );
    // The PG fixture stores Ready metadata for the actual byte buffers above; physical broker I/O is tested separately.
    let source_blob = sandbox_id(ResourceKind::InternalBlob, 0x7df1);
    let source_blob_time = Utc::now();
    sqlx::query("INSERT INTO insight_platform.artifact_blobs (tenant_id,blob_id,backend,storage_binding_digest,security_domain_digest,object_reference_ciphertext,object_generation,key_id,encryption_domain_id,content_digest,size_bytes,state,verified_at,created_at,updated_at) SELECT tenant_id,$3,backend,storage_binding_digest,security_domain_digest,object_reference_ciphertext,'compiled-source-generation',key_id,encryption_domain_id,$4,$5,'verified',$6,$6,$6 FROM insight_platform.artifact_blobs WHERE tenant_id=$1 AND blob_id=$2")
        .bind(TENANT_ID).bind(ARTIFACT_BLOB_ID).bind(source_blob.to_string()).bind(source_authority.artifact.content_digest().to_string()).bind(source_authority.artifact.byte_length() as i64).bind(source_blob_time).execute(&pool).await.unwrap();
    sqlx::query("INSERT INTO insight_platform.artifacts (tenant_id,artifact_id,blob_id,purpose,classification,expected_size_bytes,expected_digest,declared_media_type,verified_media_type,state,metadata_schema_version,metadata,metadata_digest,retention_policy_revision_id,retain_until,created_by) SELECT tenant_id,$3,$4,'authoring_document',classification,$5,$6,$7,$7,'ready',metadata_schema_version,metadata,metadata_digest,retention_policy_revision_id,retain_until,created_by FROM insight_platform.artifacts WHERE tenant_id=$1 AND artifact_id=$2")
        .bind(TENANT_ID).bind(ARTIFACT_ID).bind(source_authority.artifact.artifact_id().to_string()).bind(source_blob.to_string()).bind(source_authority.artifact.byte_length() as i64).bind(source_authority.artifact.content_digest().to_string()).bind(source_authority.artifact.media_type()).execute(&pool).await.unwrap();
    sqlx::query("UPDATE insight_platform.artifact_blobs SET content_digest=$3,size_bytes=$4 WHERE tenant_id=$1 AND blob_id=$2").bind(TENANT_ID).bind(TYPED_PLAN_BLOB_ID).bind(plan_authority.artifact.content_digest().to_string()).bind(plan_authority.artifact.byte_length() as i64).execute(&pool).await.unwrap();
    sqlx::query("UPDATE insight_platform.artifacts SET expected_digest=$3,expected_size_bytes=$4,declared_media_type=$5,verified_media_type=$5 WHERE tenant_id=$1 AND artifact_id=$2").bind(TENANT_ID).bind(TYPED_PLAN_ARTIFACT_ID).bind(plan_authority.artifact.content_digest().to_string()).bind(plan_authority.artifact.byte_length() as i64).bind(plan_authority.artifact.media_type()).execute(&pool).await.unwrap();
    let agent_document = compilation
        .materialize(&source_authority, &plan_authority)
        .unwrap();
    let compilation_evidence = insight_platform_agent_compiler::validate_frozen_agent_artifacts(
        &agent_document,
        &source_authority,
    ...
    let job = applied(
        registry_command!(
            ...
            RequestResourceValidation {
                ...
                resource_id: id(AGENT_ID),
                expected_resource_version: 1,
                job_id: sandbox_id(ResourceKind::Job, 0x7df2),
                validator_digest: digest('5'),
                validation_profile_digest: digest('f'),
                compilation_input: Some(compilation_evidence.input.clone()),
                attempt_limit: 3,
    ...
    let permissions = TypedPayload::new(
        ...
        &TenantPrincipalPayload {
            permissions: PermissionSet::new(vec![Permission::PolicyWrite, Permission::AgentWrite])
                .unwrap(),
        },
    )
    .unwrap();
    sqlx::query("UPDATE insight_platform.tenant_principals SET permissions=$3,permissions_digest=$4 WHERE tenant_id=$1 AND principal_id=$2").bind(TENANT_ID).bind(VALIDATOR_PRINCIPAL_ID).bind(permissions.value).bind(permissions.digest).execute(&pool).await.unwrap();
    ...
    let worker_id = sandbox_id(ResourceKind::WorkerProcessGeneration, 0x7df3);
    let claimed = claim_registry_fixture(
        &repository,
        ClaimJobs {
    ...
    .expect("compiler-compatible validation claim");
    let fence = JobFence {
    ...
    let result = repository
        .commit_registry_validation(CommitRegistryValidation {
            ...
            validator_digest: digest('5'),
            validation_profile_digest: digest('f'),
            compilation: Some(compilation_evidence),
            receipt_id: sandbox_id(ResourceKind::Receipt, 0x7df4),
            resource_event_id: sandbox_id(ResourceKind::Event, 0x7df4),
            resource_outbox_id: sandbox_id(ResourceKind::OutboxEvent, 0x7df4),
            job_event_id: sandbox_id(ResourceKind::Event, 0x7df5),
            job_outbox_id: sandbox_id(ResourceKind::OutboxEvent, 0x7df5),
        ...
        .await
        .unwrap();
    let RegistryValidationCommitOutcome::Committed { resource, .. } = result else {
        panic!("actual compilation did not commit")
    };
    let agent_validation: ValidationSummary =
        serde_json::from_value(resource.payload.value["validation"].clone()).unwrap();
    let interface_ref =
        ExactVersionRef::new(id(AGENT_INTERFACE_ID), agent_contract_digest.clone()).unwrap();
    let plan_ref = ExactVersionRef::new(id(AGENT_PLAN_ID), agent_plan_digest.clone()).unwrap();
    let agent_versions = vec![
        NewPublishedVersion {
            resource_version_id: id(AGENT_INTERFACE_ID),
            revision_no: 1,
            content_digest: agent_contract_digest,
            artifact_id: None,
            payload: PublishedVersionPayload {
                document: agent_document.clone(),
        ...
        NewPublishedVersion {
            ...
            revision_no: 1,
            content_digest: agent_plan_digest,
    ...
    product_authoring_query_fixture::assert_product_authoring_queries(
        &repository,
        &id(TENANT_ID),
        &id(AGENT_ID),
    ...
    .await;
    let before_revoked_replay = support::fixture_durable_counts(&pool, &id(TENANT_ID)).await;
    assert!(
        matches!(
            registry_command!(saturated, create_resource_draft, create_command),
            Err(RepositoryError::PermissionDenied)
        ),
        "revoked principal cannot disclose the original create result even under backpressure"
    );
    assert!(
        matches!(
            registry_command!(
        ...
        "stored historical update projection requires current permission"
    );
    assert_eq!(
        before_revoked_replay,
        support::fixture_durable_counts(&pool, &id(TENANT_ID)).await
    );
}
