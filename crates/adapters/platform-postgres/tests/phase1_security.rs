...
async fn phase1_bootstrap_membership_and_secret_contract() {
    let database_url = std::env::var("PLATFORM_TEST_INSTALLATION_DATABASE_URL").expect(
        "PLATFORM_TEST_INSTALLATION_DATABASE_URL requires an isolated current-schema database with no principals or tenant bindings",
    );
