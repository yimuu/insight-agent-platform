...
use insight_platform_postgres::{
    ...
    repository::{
        ClaimJobs, CommitJob, JobCommitOutcome, JobTerminalState, NewJob, NewQuotaAccount,
        NewTenant, PgRepository, QuotaMutationOutcome, RepositoryError, ReserveQuota, SettleQuota,
    },
    verify_schema, BASELINE_TABLE_COUNT, CURRENT_SCHEMA_SQL, EXPECTED_TABLES,
};
use serde_json::json;
use sqlx::postgres::PgPoolOptions;
use support::FixtureClaimRounds;
...
fn current_schema_has_only_shared_authority_tables() {
    ...
    for rejected in [
        ...
        assert!(!CURRENT_SCHEMA_SQL.contains(rejected), "found {rejected}");
    }
}
...
async fn real_postgres_baseline_job_receipt_outbox_and_quota() {
    ...
    support::bind_fixture_scheduling_policy(&pool, &repository, &TENANT_ID.parse().unwrap()).await;
        ...
        .create_job(NewJob {
            execution_requirement:
                insight_platform_contracts::ExecutionRequirement::DomainOperation {
                    operation_abi_identity: support::digest("fixture.interaction"),
                    requirements:
                        insight_platform_contracts::DomainOperationRequirements::Control {
                            control_policy_digest: support::digest("fixture.interaction.policy"),
                        },
                },
            tenant_id: TENANT_ID.to_owned(),
