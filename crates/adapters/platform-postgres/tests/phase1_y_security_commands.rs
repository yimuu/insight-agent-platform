...
async fn security_commands_are_fenced_atomic_and_secret_safe() {
    ...
    let restricted_registration_count = {
        let configured_role = std::env::var("PLATFORM_SECURITY_AUTHORITY_TEST_ROLE").expect(
            "PLATFORM_SECURITY_AUTHORITY_TEST_ROLE is required for the least-privilege fixture",
        );
    ...
    // Prepared registration is a new SecretBind command, not a fenced effect
    // completion. Even an exact durable Receipt replay needs current authority.
    let limited = PgPoolOptions::new()
        ...
        .await
        .unwrap();
    let limited_repository = PgRepository::new(limited.clone());
    assert_eq!(
        limited_repository
            .register_prepared(prepared.clone())
            .await
    ...
    security
        .revoke_tenant_principal(RevokeTenantPrincipal {
            audit: audit(TENANT_ID, ADMIN_ID, "7d0c", 'c', 'd'),
            principal_id: id(BROKER_ID),
            principal_kind: PrincipalKind::ServiceIdentity,
            expected_generation: 1,
            expected_version: 1,
    ...
    let before:(i64,i64,i64)=sqlx::query_as("SELECT (SELECT count(*) FROM insight_platform.receipts WHERE tenant_id=$1),(SELECT count(*) FROM insight_platform.events WHERE tenant_id=$1),(SELECT count(*) FROM insight_platform.outbox_events WHERE tenant_id=$1)")
        .bind(TENANT_ID).fetch_one(&pool).await.unwrap();
    let binding_before:(i64,i64,String,String)=sqlx::query_as("SELECT generation,version,state,payload_digest FROM insight_platform.secret_bindings WHERE tenant_id=$1 AND secret_binding_id=$2")
    ...
    let after:(i64,i64,i64)=sqlx::query_as("SELECT (SELECT count(*) FROM insight_platform.receipts WHERE tenant_id=$1),(SELECT count(*) FROM insight_platform.events WHERE tenant_id=$1),(SELECT count(*) FROM insight_platform.outbox_events WHERE tenant_id=$1)")
        .bind(TENANT_ID).fetch_one(&pool).await.unwrap();
    let binding_after:(i64,i64,String,String)=sqlx::query_as("SELECT generation,version,state,payload_digest FROM insight_platform.secret_bindings WHERE tenant_id=$1 AND secret_binding_id=$2")
        .bind(TENANT_ID).bind(PREPARED_SECRET_ID).fetch_one(&pool).await.unwrap();
