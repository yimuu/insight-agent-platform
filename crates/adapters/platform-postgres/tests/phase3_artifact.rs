use insight_platform_artifacts::store::ArtifactRecoverySlot;
...
use insight_platform_artifacts::{
    ...
    ArtifactScanWorkRecord, ArtifactScanner, ArtifactTransaction, ArtifactUploadCompletionReplay,
    ArtifactUploadReplayIdentity, ArtifactWorkerAudit, ArtifactWorkerService,
    CommitArtifactBlobCleanup, CommitArtifactScanOutcome, CompleteArtifactDeletion,
    CompleteArtifactUpload, CompletedArtifactDeletion, CompletedArtifactUpload,
    CreateArtifactProvenance, DeleteArtifactBlobGeneration, FinalizeArtifact, FinalizedArtifact,
    ...
    ScheduleArtifactRescan, ScheduleInitialArtifactScan,
};
...
use insight_platform_postgres::{
    ...
    repository::{
        ArtifactWorkerRole, ClaimArtifactJobs, NewPrincipal, NewQuotaAccount, NewTenant,
...
async fn artifact_replay_facts(pool: &PgPool, tenant_id: &ResourceId) -> serde_json::Value {
    sqlx::query_scalar(
        r#"
        SELECT jsonb_build_object(
            ...
            'links', (SELECT jsonb_agg(to_jsonb(r) ORDER BY artifact_link_id)
                FROM insight_platform.artifact_links r WHERE tenant_id = $1),
...
async fn assert_retained_upload_completion_replay(
    pool: &PgPool,
    repository: &PgRepository,
    original: &CompleteArtifactUpload,
    scan: &ScheduleInitialArtifactScan,
    expected_artifact: &insight_platform_artifacts::ArtifactRecord,
    expected_blob: &insight_platform_artifacts::ArtifactBlobRecord,
    denied_principal: &ResourceId,
) {
    let tenant = &original.audit.tenant_id;
    assert!(expected_artifact.version > original.expected_artifact_version);
    assert_eq!(
        scan.audit.idempotency_key_digest,
        original.audit.idempotency_key_digest
    );
    assert_ne!(scan.audit.request_digest, original.audit.request_digest);
    let facts = artifact_replay_facts(pool, tenant).await;
    let CommandOutcome::Replayed(replayed) = execute_complete(repository, original.clone())
        .await
        .unwrap()
    else {
        panic!("retained upload completion must replay its original command");
    };
    assert_eq!(&replayed.artifact, expected_artifact);
    assert_eq!(&replayed.blob, expected_blob);
    assert_ne!(replayed.blob.blob_id, original.blob_id);
    assert_eq!(replayed.grant.upload_grant_id, original.upload_grant_id);
    assert_eq!(replayed.grant.state, ArtifactLinkState::Consumed);
    assert_eq!(replayed.operation.operation_id, original.operation_id);
    if expected_artifact.state == ArtifactState::Ready {
        assert_eq!(replayed.operation.state, JobState::Succeeded);
    }
    let operation: (String, i64) = sqlx::query_as(
        "SELECT state, version FROM insight_platform.jobs WHERE tenant_id=$1 AND job_id=$2",
    )
    .bind(tenant.to_string())
    .bind(original.operation_id.to_string())
    .fetch_one(pool)
    .await
    .unwrap();
    assert_eq!(replayed.operation.state.as_str(), operation.0);
    assert_eq!(
        replayed.operation.version,
        u64::try_from(operation.1).unwrap()
    );
    assert_eq!(artifact_replay_facts(pool, tenant).await, facts);

    let query = ArtifactUploadCompletionReplay {
        identity: ArtifactUploadReplayIdentity::from_audit(&original.audit),
        artifact_id: original.artifact_id.clone(),
        expected_artifact_version: original.expected_artifact_version,
        grant_token_digest: original.grant_token_digest.clone(),
        scan_request_digest: scan.audit.request_digest.clone(),
    };
    assert_eq!(
        repository
            .load_gateway_artifact_completion_replay(query.clone())
            .await
            .unwrap(),
        Some(replayed)
    );
    ...
    for mismatch in ["complete_digest", "scan_digest", "version", "token"] {
        let mut changed = query.clone();
        match mismatch {
            "complete_digest" => changed.identity.request_digest = digest('0'),
            "scan_digest" => changed.scan_request_digest = digest('0'),
            "version" => changed.expected_artifact_version += 1,
            "token" => changed.grant_token_digest = digest('0'),
            _ => unreachable!(),
        }
        assert!(
            matches!(
                repository
                    .load_gateway_artifact_completion_replay(changed)
                    .await,
                Err(RepositoryError::IdempotencyConflict)
            ),
            "retained completion must reject changed {mismatch}"
        );
        assert_eq!(artifact_replay_facts(pool, tenant).await, facts);
    }
    let mut denied = query;
    denied.identity.principal_id = denied_principal.clone();
    assert!(matches!(
        repository
            .load_gateway_artifact_completion_replay(denied)
            .await,
    ...
    changed_command.audit.request_digest = digest('0');
    assert!(matches!(
        execute_complete(repository, changed_command).await,
        Err(RepositoryError::IdempotencyConflict)
    ));
    assert_eq!(artifact_replay_facts(pool, tenant).await, facts);
    let mut denied_command = original.clone();
    denied_command.audit.principal_id = denied_principal.clone();
    assert!(matches!(
        execute_complete(repository, denied_command).await,
        Err(RepositoryError::PermissionDenied)
    ));
    assert_eq!(artifact_replay_facts(pool, tenant).await, facts);
}
...
async fn seed_approved_deletion_task(
    ...
    let payload = TypedPayload::new(2, &payload).unwrap();
    sqlx::query(
        r#"
...
async fn claim_and_start_artifact_job(
    ...
    let command = ClaimArtifactJobs {
        ...
        worker_manifest: insight_platform_contracts::WorkerManifest {
            ...
            worker_role: match role {
                ArtifactWorkerRole::DataWorker => "artifact-data-worker",
                ArtifactWorkerRole::Maintenance => "artifact-maintenance",
            }
            .to_owned(),
            work_class: insight_platform_contracts::WorkClass::Artifact,
            worker_build_digest: insight_platform_worker::execution::executable_digest(
                &std::env::current_exe().unwrap(),
            )
            .unwrap(),
            adapter_runtime_digest: digest('a'),
            // FixtureScanner returns exact frozen scan evidence for this test's digest('4')
            // contract. It does not impersonate the installed production integrity scanner.
            execution_capabilities: match role {
    ...
    // A claim visits one bounded partition/cohort; an empty visit is valid even
    // while another tenant has work. Drive finite scheduler rounds in this fixture.
    for _ in 0..520 {
        claimed.extend(
            repository
                .claim_artifact_jobs(command.clone())
                .await
                .unwrap(),
        );
        if claimed
            .iter()
            .any(|job| job.tenant_id == tenant_id.to_string() && job.job_id == job_id.to_string())
        {
            break;
        }
    ...
    let claimed = claimed
        ...
        .expect("requested Artifact Job must be included after bounded scheduler rounds");
...
async fn artifact_upload_lifecycle_fixture() {
    ...
    let duplicate_deadline: DateTime<Utc> =
        sqlx::query_scalar("SELECT clock_timestamp() + interval '20 seconds'")
            .fetch_one(&pool)
            .await
            .unwrap();
    duplicate_command.operation_deadline = duplicate_deadline;
    duplicate_command.grant_expires_at = duplicate_deadline;
    ...
    let mut duplicate_complete = complete_command(&duplicate_command, 0x1510, 'e', '5');
    duplicate_complete.audit.receipt_expires_at = duplicate_deadline;
    let CommandOutcome::Applied(_) = execute_complete(&repository, duplicate_complete.clone())
        .await
        .unwrap()
    else {
        panic!("duplicate-content upload must apply");
    };
    let mut duplicate_schedule =
        schedule_initial_scan_command(&duplicate_command, scan_policy_a.clone(), 0x1520);
    duplicate_schedule.deadline = duplicate_deadline;
    duplicate_schedule.audit.idempotency_key_digest =
        duplicate_complete.audit.idempotency_key_digest.clone();
    let CommandOutcome::Applied(duplicate_scan) =
        execute_schedule_initial_scan(&repository, duplicate_schedule.clone())
            .await
            .unwrap()
    else {
    ...
    let prepare_facts = artifact_replay_facts(&pool, &tenant_a).await;
    assert!(matches!(
        execute_prepare(&repository, duplicate_command.clone()).await,
        Err(RepositoryError::Conflict(_))
    ));
    assert_eq!(artifact_replay_facts(&pool, &tenant_a).await, prepare_facts);
    let prepare_identity = ArtifactUploadReplayIdentity::from_audit(&duplicate_command.audit);
    assert!(matches!(
        repository
            .load_gateway_artifact_prepare_replay(prepare_identity.clone())
            .await,
        Err(RepositoryError::Conflict(_))
    ));
    assert_eq!(artifact_replay_facts(&pool, &tenant_a).await, prepare_facts);
    let mut changed_prepare = prepare_identity.clone();
    changed_prepare.request_digest = digest('0');
    assert!(matches!(
        repository
            .load_gateway_artifact_prepare_replay(changed_prepare)
            .await,
        Err(RepositoryError::IdempotencyConflict)
    ));
    assert_eq!(artifact_replay_facts(&pool, &tenant_a).await, prepare_facts);
    let mut denied_prepare = prepare_identity.clone();
    denied_prepare.principal_id = denied_principal.clone();
    assert!(matches!(
        repository
            .load_gateway_artifact_prepare_replay(denied_prepare)
            .await,
        Err(RepositoryError::PermissionDenied)
    ));
    assert_eq!(artifact_replay_facts(&pool, &tenant_a).await, prepare_facts);
    assert_retained_upload_completion_replay(
        &pool,
        &repository,
        &duplicate_complete,
        &duplicate_schedule,
        &duplicate_verified.artifact,
        &duplicate_verified.blob,
        &denied_principal,
    )
    .await;
    ...
    assert_retained_upload_completion_replay(
        ...
        &duplicate_complete,
        &duplicate_schedule,
        &duplicate_finalized.artifact,
        &duplicate_finalized.blob,
        &denied_principal,
    )
    .await;
    let completed_facts = artifact_replay_facts(&pool, &tenant_a).await;
    assert!(matches!(
        repository
            .load_gateway_artifact_prepare_replay(prepare_identity.clone())
            .await,
    ...
    assert_eq!(
        artifact_replay_facts(&pool, &tenant_a).await,
        completed_facts
    );

    let (persisted_deadline, database_now): (DateTime<Utc>, DateTime<Utc>) = sqlx::query_as(
        "SELECT deadline, clock_timestamp() FROM insight_platform.jobs WHERE tenant_id=$1 AND job_id=$2",
    )
    .bind(tenant_a.to_string())
    .bind(duplicate_complete.operation_id.to_string())
    .fetch_one(&pool)
    .await
    .unwrap();
    assert_eq!(persisted_deadline, duplicate_deadline);
    let remaining = (persisted_deadline - database_now)
        .to_std()
        .unwrap_or_default();
    assert!(remaining <= std::time::Duration::from_secs(20));
    tokio::time::sleep(remaining + std::time::Duration::from_millis(25)).await;
    let expired_at: DateTime<Utc> = sqlx::query_scalar("SELECT clock_timestamp()")
        .fetch_one(&pool)
        .await
        .unwrap();
    assert!(persisted_deadline < expired_at);
    assert!(duplicate_complete.audit.receipt_expires_at < expired_at);
    assert_retained_upload_completion_replay(
        &pool,
        &repository,
        &duplicate_complete,
    ...
    assert!(matches!(
        repository
            .load_gateway_artifact_prepare_replay(prepare_identity)
            .await,
        Err(RepositoryError::Conflict(_))
    ));
    assert_eq!(
    ...
    assert_eq!(
        execute_release_hold(&repository, release_command.clone())
            .await
            .unwrap(),
    ...
    assert_eq!(
        execute_mark_deletion(&repository, mark_shared.clone())
            .await
            .unwrap(),
    ...
    .await;
    let before_revoked_replay = support::fixture_durable_counts(&pool, &tenant_a).await;
    assert!(matches!(
        execute_prepare(&repository, prepared_command).await,
        Err(RepositoryError::PermissionDenied)
    ));
    assert!(matches!(
        execute_finalize(&repository, finalized_command).await,
        Err(RepositoryError::PermissionDenied)
    ));
    assert!(matches!(
        execute_place_hold(&repository, hold_command).await,
        Err(RepositoryError::PermissionDenied)
    ));
    assert!(matches!(
        execute_release_hold(&repository, release_command).await,
        Err(RepositoryError::PermissionDenied)
    ));
    assert!(matches!(
        execute_mark_deletion(&repository, mark_shared).await,
        Err(RepositoryError::PermissionDenied)
    ));
    assert_eq!(
        before_revoked_replay,
        support::fixture_durable_counts(&pool, &tenant_a).await,
        "denied Receipt replay cannot return Artifact data or append durable facts"
    );
}
