...
// Both the SDK start timestamp and whole-second lifetime derive from this one observation.
// KMS latency and presigner execution can therefore never renew the original grant window.
...
impl AwsArtifactUploadProvider {
    ...
    pub async fn prepare_upload(
        ...
        if request.tenant_id.kind() != ResourceKind::Tenant
            ...
            || request.expected_size_bytes == 0
            || request.expected_size_bytes > self.maximum_object_bytes
            || upload_presigning_config(SystemTime::now(), request.expires_at).is_err()
            || request.declared_media_type.is_some_and(|value| {
                value.is_empty()
                    || value.len() > 255
                    || !value.is_ascii()
        ...
        let presigned = upload_request
            .presigned(upload_presigning_config(
                SystemTime::now(),
                request.expires_at,
            )?)
            .await
...
mod tests {
    ...
    #[test]
    fn kms_latency_does_not_extend_upload_signature_past_original_expiry() {
        let admitted = SystemTime::UNIX_EPOCH + Duration::from_secs(1_800_000_000);
        let expiry = admitted + Duration::from_millis(60_500);
        let after_kms = admitted + Duration::from_millis(17_750);
        let config = upload_presigning_config(after_kms, expiry).unwrap();
        assert_eq!(config.start_time(), after_kms);
        assert_eq!(config.expires(), Duration::from_secs(42));
        assert!(config.start_time() + config.expires() <= expiry);
        assert!(upload_presigning_config(expiry, expiry).is_err());
        assert!(upload_presigning_config(expiry + Duration::from_secs(1), expiry).is_err());
        assert!(upload_presigning_config(expiry - Duration::from_millis(999), expiry).is_err());
        assert!(upload_presigning_config(admitted, admitted + Duration::from_secs(3_601)).is_err());
    }
    ...
    #[tokio::test]
    #[ignore = "requires real HTTPS S3 and KMS; run owning qualification harness"]
    async fn real_https_s3_and_kms_round_trip_exact_generation() {
        let endpoint = std::env::var("PLATFORM_TEST_AWS_ENDPOINT")
            .expect("real HTTPS Artifact provider fixture requires PLATFORM_TEST_AWS_ENDPOINT");
        let bucket = std::env::var("PLATFORM_TEST_S3_BUCKET")
            .expect("real HTTPS Artifact provider fixture requires PLATFORM_TEST_S3_BUCKET");
        let key_id = std::env::var("PLATFORM_TEST_KMS_KEY_ID")
            .expect("real HTTPS Artifact provider fixture requires PLATFORM_TEST_KMS_KEY_ID");
