...
//! `tools/qualification/qualify-platform-sandbox-l3.sh`. It never imports OpenSandbox implementation code;
...
impl L3Context {
    fn load(expected_phase: &str) -> Option<Self> {
        let Ok(phase) = env::var(L3_PHASE_ENV) else {
            panic!("{L3_PHASE_ENV} is required for the exact Kubernetes qualification phase");
        };
        assert_eq!(
            phase, expected_phase,
            "qualification harness selected the wrong phase"
        );
...
#[ignore = "requires the explicit Kubernetes qualification phase"]
async fn opensandbox_kubernetes_l3_concurrent_response_loss_and_network_modes() {
    ...
    assert_eq!(
        ...
        Err(insight_platform_sandbox::contracts::SandboxProviderError::Unauthorized)
    );
...
async fn execute_retained(
    ...
    request: &SandboxExecutionRequestV1,
    token_character: char,
) -> (insight_platform_sandbox::contracts::OpenSandboxId, Value) {
...
async fn wait_for_runner(
    ...
    signing_seed: &OpaqueActivationToken,
    execution_request_digest: &Sha256Digest,
) -> insight_platform_sandbox::contracts::SandboxRunnerStateFrameV1 {
...
async fn wait_for_runner_phase(
    client: &OpenSandboxHttpClient,
    sandbox_id: &insight_platform_sandbox::contracts::OpenSandboxId,
    boot_id: &insight_platform_sandbox::contracts::RunnerBootId,
...
async fn wait_for_recreated_runner(
    ...
    sandbox_id: &insight_platform_sandbox::contracts::OpenSandboxId,
    old_pod_uid: &str,
    old_boot_id: &insight_platform_sandbox::contracts::RunnerBootId,
    signing_seed: &OpaqueActivationToken,
    execution_request_digest: &Sha256Digest,
) -> insight_platform_sandbox::contracts::SandboxRunnerStateFrameV1 {
...
async fn wait_for_candidates(
    ...
    expected: usize,
) -> Vec<insight_platform_sandbox::contracts::OpenSandboxId> {
    let deadline = Instant::now() + Duration::from_secs(60);
    loop {
