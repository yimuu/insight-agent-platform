...
struct SandboxControllerPostgresObserver(Arc<DependencyObservationMetrics>);

impl PostgresHealthObserver for SandboxControllerPostgresObserver {
    fn observe(&self, outcome: PostgresHealthOutcome) {
        self.0
            ...
            .expect("Sandbox Controller installs its PostgreSQL dependency metric");
    }
}
...
> {
    ...
    let observer: Arc<dyn PostgresHealthObserver> =
        Arc::new(SandboxControllerPostgresObserver(Arc::clone(&metrics)));
    Ok((metrics, observer))
}
