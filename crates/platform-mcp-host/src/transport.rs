...
/// Exact managed-runner request. It contains package/runtime/policy identities but no executable
/// path, PID, host namespace, plaintext session or credential value.
#[derive(Debug, Clone, PartialEq)]
pub struct McpManagedSubscriptionRequest {
    pub tenant_id: ResourceId,
    pub deployment: ExactDeploymentRef,
    pub package: ExactVersionRef,
    pub runtime: ExactVersionRef,
...
#[async_trait]
pub trait ManagedMcpSubscriptionBroker: Send + Sync {
    async fn establish_subscription(
        &self,
        request: McpManagedSubscriptionRequest,
    ) -> Result<PreparedMcpSubscription, McpTransportFailure>;
}

pub struct ManagedStdioMcpSubscriptionTransport {
    broker: Arc<dyn ManagedMcpSubscriptionBroker>,
}

impl ManagedStdioMcpSubscriptionTransport {
    pub fn new(broker: Arc<dyn ManagedMcpSubscriptionBroker>) -> Self {
        Self { broker }
    }
}

#[async_trait]
impl McpSubscriptionTransport for ManagedStdioMcpSubscriptionTransport {
    fn kind(&self) -> McpTransportKind {
        McpTransportKind::ManagedStdio
    }
    ...
    async fn establish(
        ...
        binding
            ...
            .map_err(|_| contract_mismatch())?;
        self.broker
            .establish_subscription(McpManagedSubscriptionRequest {
                tenant_id: binding.tenant_id.clone(),
                deployment: contract.deployment.clone(),
                package: package.clone(),
                runtime: runtime.clone(),
