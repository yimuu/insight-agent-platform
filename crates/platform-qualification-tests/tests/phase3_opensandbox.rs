...
async fn claim_expected_cleanup(
    ...
    let claimed = SandboxJobRepository::claim_cleanup(
        repository,
        SandboxCleanupClaimV1 {
    ...
    .unwrap()
    .pop()
    .expect("the expected cleanup job is present in the durable queue");
    assert_eq!(
        (&claimed.fence.tenant_id, &claimed.fence.job_id),
        (tenant_id, job_id),
        "a qualification test must only clean the job it created"
    );
    claimed
}
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn opensandbox_cleanup_claim_replays_active_owner_before_next_unowned_work() {
    let Ok(database_url) = std::env::var("PLATFORM_TEST_DATABASE_URL") else {
        eprintln!("PLATFORM_TEST_DATABASE_URL is unset; cleanup replay L2 skipped");
        return;
    };
    let pool = PgPoolOptions::new()
        .max_connections(8)
    ...
    assert!(
        ...
        "claiming the first cleanup must refresh it behind the next unowned job"
    );
    ...
    .unwrap()
    .pop()
    .expect("the active cleanup lease is replayable after response loss");
    assert_eq!(
        replay.fence, first.fence,
        "the same process generation must recover its active cleanup fence before unowned work"
    );
