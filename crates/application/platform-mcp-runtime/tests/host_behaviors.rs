...
fn fixture(tasks: bool, effect: Effect, timeout_milliseconds: u64) -> Fixture {
    ...
    .unwrap();
    let contract = McpHostExecutionContract::build(
        chrono::Utc::now(),
...
fn session_record_rejects_a_tampered_nested_binding_key() {
    let fixture = fixture(false, Effect::ReadOnly, 1_000);
    let key = McpSessionBindingKey::build(&fixture.contract).unwrap();
    let mut session = McpSessionRecord::disconnected(chrono::Utc::now(), key).unwrap();
...
fn session_state_is_closed_and_opaque_state_is_encrypted() {
    let fixture = fixture(false, Effect::ReadOnly, 1_000);
    let key = McpSessionBindingKey::build(&fixture.contract).unwrap();
    let disconnected = McpSessionRecord::disconnected(chrono::Utc::now(), key).unwrap();
...
fn subscription_record(
    ...
    .unwrap();
    let session = McpSessionRecord::disconnected(
        chrono::Utc::now(),
        McpSessionBindingKey::build(contract).unwrap(),
    )
    .unwrap();
    McpSubscriptionRecord {
        ...
        version: 1,
        payload: McpSubscriptionPayload::pending(chrono::Utc::now(), binding, session).unwrap(),
