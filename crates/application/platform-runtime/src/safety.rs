...
use insight_platform_artifacts::store::RecoveredArtifactJob;
...
use insight_platform_orchestrator::store::CancellingOrchestrationChildRun;
use insight_platform_orchestrator::store::ChildRunCancellationSlot;
use insight_platform_orchestrator::store::ConvergedOrchestrationRun;
...
use insight_platform_orchestrator::store::PromotedOrchestrationRetry;
use insight_platform_orchestrator::store::RecoveredOrchestrationJob;
use insight_platform_orchestrator::store::ResolvedOrchestrationChildRun;
use insight_platform_orchestrator::store::ResolvedOrchestrationTask;
use insight_platform_orchestrator::store::TerminalChildRunSlot;
use insight_platform_orchestrator::store::WokenOrchestrationJob;
...
impl SafetyMetrics {
    fn new() -> Self {
        Self {
            ...
            invalid_objects: AtomicU64::new(0),
...
struct SafetyCursors {
    ...
    terminal_children: Option<SafetyScanCursor>,
    child_cancellations: Option<SafetyScanCursor>,
}
    ...
    I: CoordinatorIdentityFactory + 'static,
{
    ...
    async fn drive_cycle(
        ...
        let results = [
            self.drive_child_cancellations(cursors, metrics).await,
            self.drive_terminal_children(cursors, metrics).await,
    ...
    async fn drive_child_cancellations(
        ...
        let command = DriveChildRunCancellations {
            after: cursors.child_cancellations.clone(),
            limit: self.config.batch_size,
            slots: (0..self.config.batch_size)
        ...
        observe_page(&cancelling, &mut cursors.child_cancellations, metrics);
        Ok(())
    }
    ...
    async fn drive_terminal_children(
        ...
        let command = DriveTerminalChildRuns {
            after: cursors.terminal_children.clone(),
            limit: self.config.batch_size,
            slots: (0..self.config.batch_size)
        ...
        let resolved = self
            ...
            .await
            .map_err(|_| SafetyDriveFailure::StoreUnavailable)?;
        observe_page(&resolved, &mut cursors.terminal_children, metrics);
        Ok(())
    }
}
...
fn observe_page<T>(
    ...
    metrics
        .invalid_objects
        .fetch_add(page.diagnostics.len() as u64, Ordering::Relaxed);
    for diagnostic in &page.diagnostics {
        tracing::warn!(
            phase = ?diagnostic.phase,
            code = ?diagnostic.code,
            "Invalid durable object retained; safety scan continued"
        );
    }
    if page.exhausted {
...
fn safety_snapshot(metrics: &SafetyMetrics) -> SafetyDriverSnapshot {
    SafetyDriverSnapshot {
        ...
        invalid_objects: metrics.invalid_objects.load(Ordering::Relaxed),
...
mod tests {
    ...
    struct RecordingStore {
        ...
        child_cancellation_after: Mutex<Vec<Option<SafetyScanCursor>>>,
        terminal_child_after: Mutex<Vec<Option<SafetyScanCursor>>>,
    }
    ...
    impl OrchestrationSafetyStore for RecordingStore {
        ...
        async fn drive_expired_jobs(
            ...
            Ok(SafetyScanPage {
                diagnostics: first.then(|| insight_platform_jobs::store::SafeScanDiagnostic {
                    schema_version: 1,
                    tenant_id: cursor().tenant_id,
                    item_id: cursor().item_id,
                    phase: insight_platform_jobs::store::SafetyScanPhase::JobDecode,
                    code: insight_platform_jobs::store::SafetyScanDiagnosticCode::InvalidPersistedObject,
                }).into_iter().collect(),
                records: Vec::new(),
        ...
        async fn drive_terminal_children(
            ...
            Ok(child_page(&self.terminal_child_after, command.after))
        }
        ...
        async fn drive_child_cancellations(
            ...
            self.child_cancellation_limits
                ...
                .unwrap()
                .push(command.limit);
            Ok(child_page(&self.child_cancellation_after, command.after))
        }
    }

    fn child_page<T>(
        observations: &Mutex<Vec<Option<SafetyScanCursor>>>,
        ...
        SafetyScanPage {
            diagnostics: Vec::new(),
            records: Vec::new(),
            next_cursor: first.then(child_cursor),
            exhausted: !first,
        }
    }

    fn child_cursor() -> SafetyScanCursor {
        SafetyScanCursor {
            item_id: id(ResourceKind::ChildRunLink, "0015"),
            ..cursor()
        }
    }
    ...
    async fn high_water_cursor_advances_then_wraps_without_durable_authority() {
        ...
        for observations in [&store.child_cancellation_after, &store.terminal_child_after] {
            let observations = observations.lock().unwrap();
            assert!(observations.len() >= 2);
            assert_eq!(observations[0], None);
            assert_eq!(observations[1], Some(child_cursor()));
        }
        ...
        assert_eq!(snapshot.invalid_objects, 1);
        assert_eq!(snapshot.mutations, 0);
        assert_eq!(snapshot.scan_failures, 0);
