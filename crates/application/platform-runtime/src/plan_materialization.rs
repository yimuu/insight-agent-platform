...
use insight_platform_plan::{PlanLimits, RuntimePlan};
...
mod tests {
    ...
    use insight_platform_plan::{ExactDataPortRef, PlanNodeKey, RuntimeNode};
    ...
    fn fixture_schema() -> insight_platform_contracts::ClosedValueSchema {
        insight_platform_contracts::ClosedValueSchema::build(serde_json::json!({"$schema":"https://json-schema.org/draft/2020-12/schema","type":"object","properties":{},"required":[],"additionalProperties":false})).unwrap()
    }
    ...
    fn fence_and_job() -> (JobFence, JobRecord) {
        ...
        let job = JobRecord {
            ...
            payload: crate::test_orchestration_payload(id(ResourceKind::NodeExecution, "7705")),
            started_at: Some(now),
            terminal_at: None,
            created_at: now,
            updated_at: now,
