    ...
    I: CoordinatorIdentityFactory + 'static,
{
    ...
    async fn drive_once(
        ...
        for (claimed, permit) in claimed.into_iter().zip(permits) {
            ...
            let trace = ExecutionTraceContext::start(claimed.job.trace, TraceFlags::NotSampled)
                .map_err(|_| {
...
mod tests {
    ...
    impl OrchestrationClaimStore for FakeStore {
        ...
        async fn claim_orchestration_jobs(
            ...
            let job = JobRecord {
                ...
                payload: crate::test_orchestration_payload(node_id.clone()),
                started_at: None,
                terminal_at: None,
                created_at: now,
                updated_at: now,
    ...
    impl OrchestrationJobExecutor for FakeExecutor {
        async fn execute(&self, job: ActiveOrchestrationJob) -> ExecutionDisposition {
            assert_eq!(
                insight_platform_execution_context::current_trace()
                    .unwrap()
                    .identity,
